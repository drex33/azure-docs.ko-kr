...
**--cpk-by-name** 문자열 클라이언트가 이름으로 키를 제공하므로 클라이언트는 Azure Blob에 대해 요청을 Storage 요청별로 암호화 키를 제공하는 옵션을 제공합니다. 제공된 키 이름은 Azure Key Vault 가져와 데이터를 암호화하는 데 사용됩니다.
