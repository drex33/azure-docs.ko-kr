...
## <a name="add-syxsense-from-the-gallery"></a>갤러리에서 Syxsense 추가
...
## <a name="configure-and-test-azure-ad-sso-for-syxsense"></a>Syxsense에 대한 Azure AD SSO 구성 및 테스트
...
Syxsense에서 Azure AD SSO를 구성하고 테스트하려면 다음 단계를 수행합니다.
...
1. **[Syxsense SSO 구성](#configure-syxsense-sso)** - 애플리케이션 쪽에서 Single Sign-On 설정을 구성합니다.
    1. **[Syxsense 테스트 사용자 만들기](#create-syxsense-test-user)** - B.Simon의 Azure AD 표현과 연결되는 대응 사용자를 Syxsense에 만듭니다.
...
1. Azure Portal의 **Syxsense** 애플리케이션 통합 페이지에서 **관리** 섹션을 찾은 후 **Single Sign-On** 을 선택합니다.
...
1. **IDP** 시작 모드에서 애플리케이션을 구성하려면 **기본 SAML 구성** 섹션에서 다음 단계를 수행합니다.
    ...
    > [!NOTE]
    > 이러한 값은 실제 값이 아닙니다. 실제 식별자 및 회신 URL로 해당 값을 업데이트합니다. 이러한 값을 얻으려면 [Syxsense 클라이언트 지원 팀](mailto:DevTeam@syxsense.com)에 문의하세요. Azure Portal의 **기본 SAML 구성** 섹션에 표시된 패턴을 참조할 수도 있습니다.
...
1. **설정 아이콘** 을 클릭합니다.
    ...
    ![스크린샷은 설정 아이콘을 보여줍니다.](./media/syxsense-tutorial/settings.png)

1. **외부 인증** 을 클릭하고 **앱 페더레이션 메타데이터 Url** 값을 **SAML 2.0 메타데이터** 텍스트 상자에 제공하고 **저장** 을 클릭합니다.

    ![스크린샷은 앱 페더레이션 메타데이터 URL 값을 입력할 수 있는 외부 인증 페이지를 보여줍니다.](./media/syxsense-tutorial/metadata.png)
...
1. 왼쪽 탐색 패널에서 **사용자 계정** 을 클릭합니다.
    ...
    ![스크린샷은 탐색 패널에서 선택한 사용자 계정을 보여줍니다.](./media/syxsense-tutorial/user.png)

1. **추가** 를 클릭합니다.

    ![스크린샷은 추가를 선택할 수 있는 사용자 계정 창을 보여줍니다.](./media/syxsense-tutorial/add-user.png)

1. 조직 요구 사항에 따라 사용자 세부 정보를 제공하고 **저장** 을 클릭합니다.

    ![스크린샷은 정보를 입력할 수 있는 페이지를 보여줍니다.](./media/syxsense-tutorial/user-account.png)
...
* Azure Portal에서 **이 애플리케이션 테스트** 를 클릭합니다. 그러면 로그인 흐름을 시작할 수 있는 Syxsense 로그온 URL로 리디렉션됩니다.  

* Syxsense 로그온 URL로 직접 이동하여 해당 위치에서 로그인 흐름을 시작합니다.

#### <a name="idp-initiated"></a>IDP 시작:

* Azure Portal에서 **이 애플리케이션 테스트** 를 클릭하면 SSO를 설정한 Syxsense에 자동으로 로그인됩니다. 

Microsoft 내 앱을 사용하여 모든 모드에서 애플리케이션을 테스트할 수도 있습니다. 내 앱에서 Syxsense 타일을 클릭하면 SP 모드로 구성된 경우 로그인 흐름을 시작하기 위해 애플리케이션 로그온 페이지로 리디렉션되고, IDP 모드로 구성된 경우에는 SSO를 설정한 Syxsense에 자동으로 로그인됩니다. 내 앱에 대한 자세한 내용은 [내 앱 소개](../user-help/my-apps-portal-end-user-access.md)를 참조하세요.

## <a name="next-steps"></a>다음 단계

Syxsense가 구성되면 세션 제어를 적용하여 조직에서 중요한 데이터의 반출 및 반입을 실시간으로 보호할 수 있습니다. 세션 제어는 조건부 액세스에서 확장됩니다. [Microsoft Cloud App Security를 사용하여 세션 제어를 적용하는 방법을 알아봅니다](/cloud-app-security/proxy-deployment-aad).