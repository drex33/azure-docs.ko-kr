...
구하기 및 조사가 더 복잡 해지면 Azure 센티널 노트북을 사용 하 여 기계 학습, 시각화 및 데이터 분석을 통해 작업을 향상 시킵니다.
...
전자 필기장은 사용자가 쉽게 기억할 수 없거나, 세부 정보를 보거나, 쿼리와 결과를 저장 해야 하는 경우에 유용 합니다. 전자 필기장을 만들고 공유 하는 데 도움이 되도록 Azure 센티널은 Azure 센티널 **노트북** 페이지에 직접 통합 된 오픈 소스, 대화형 개발 및 데이터 조작 환경인 [Jupyter 노트북](https://jupyter.org)을 제공 합니다.
