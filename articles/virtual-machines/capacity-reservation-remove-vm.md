...
1. 용량 예약 그룹과의 연결을 제거하도록 VM을 업데이트합니다.
    ...
    ```
    요청 본문에서 `capacityReservationGroup` 속성을 비어 있음으로 설정하여 그룹에 대한 VM 연결을 제거합니다.
...
1. `CapacityReservationGroupId` 속성을 비어 있음으로 설정하여 용량 예약 그룹과의 연결을 제거하도록 VM을 업데이트합니다.
