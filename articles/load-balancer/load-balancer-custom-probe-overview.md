...
간격 값은 상태 프로브가 백 엔드 풀 인스턴스의 응답을 검색하는 빈도를 결정합니다. 상태 프로브가 실패하면 백 엔드 풀 인스턴스가 비정상으로 즉시 표시됩니다. 마찬가지로 다음 정상 프로브에서 상태 프로브는 백 엔드 풀 인스턴스를 다시 정상으로 즉시 표시합니다.

상태 프로브 간격이 5초로 설정된 예제를 통해 동작을 더 자세히 설명할 수 있습니다. 프로브가 전송되는 시간은 애플리케이션이 상태를 변경할 수 있는 시간과 동기화되지 않으므로 상태 프로브가 애플리케이션 상태를 반영하는 데 걸리는 총 시간은 다음 두 가지 시나리오 중 하나에 속할 수 있습니다.

1. 애플리케이션이 다음 프로브가 도착하기 직전에 시간 시간 프로브 응답을 생성하기 시작하면 이러한 이벤트를 검색하는 데 5초가 더 걸리고 애플리케이션이 프로브가 도착할 때의 시간 아웃 신호를 보내기 시작하는 데 시간이 더 걸립니다.  이 검색은 5초가 약간 넘게 소요된다고 가정할 수 있습니다.
2. 애플리케이션이 다음 프로브가 도착한 직후에 시간 시간 프로브 응답을 생성하기 시작하면 프로브가 도착할 때까지(및 시간 부족) 5초가 더해질 때까지 이러한 이벤트의 검색이 시작되지 않습니다.  이 검색은 10초 이내에 완료된다고 가정할 수 있습니다.

이 예에서는 검색이 발생하면 플랫폼이 이 변경에 대응하는 데 약간의 시간이 소요됩니다.  즉, 에 따라 

1. 애플리케이션이 상태 변경을 시작하는 경우
2. 이 변경 내용이 검색되면(다음 상태 프로브가 전송되는 경우) 및
3. 플랫폼 간에 검색이 전달된 경우 

시간 초과 프로브 응답에 대한 반응에서 애플리케이션의 신호 변경에 대응하는 데 최소 5초에서 최대 10초 사이가 소요된다고 가정할 수 있습니다.  이 예는 발생하는 상황을 설명하기 위해 제공되지만, 이 예에 설명된 위의 대략적인 지침 이상으로 정확한 기간을 예측할 수는 없습니다.
