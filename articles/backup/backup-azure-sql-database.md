...
[Azure Backup](backup-overview.md)은 Azure VM에서 실행되는 SQL Server를 백업하는 스트림 기반 특수 솔루션을 제공합니다. 이 솔루션은 인프라 없는 백업, 장기 보존 및 중앙 관리라는 Azure Backup의 이점에 부합합니다. 또한 SQL Server를 위해 특별히 다음과 같은 추가 이점을 제공합니다.
...
* 데이터 전송이 완료되면 코디네이터가 백업 서비스를 사용하여 커밋을 확인합니다.
  ...
  ![SQL 백업 아키텍처](./media/backup-azure-sql-database/azure-backup-sql-overview.png)
...
2. [기능 고려 사항](sql-support-matrix.md#feature-considerations-and-limitations) 및 [시나리오 지원](sql-support-matrix.md#scenario-support)을 검토합니다.
3. 이 시나리오에 대한 [일반적인 질문을 검토](faq-backup-sql-server.yml)합니다.
...
Windows 2008 R2에서 실행되는 **SQL 2008** 및 **208 R2** 의 경우 권한을 부여하려면 [여기](#give-sql-sysadmin-permissions-for-sql-2008-and-sql-2008-r2)를 참조하세요.
...
9. 권한을 부여하고 나면 포털에서 **DB를 다시 검색합니다**(Azure VM의 Vault **->** Backup Infrastructure **->** Workload).
