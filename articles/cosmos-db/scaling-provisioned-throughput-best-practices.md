...
즉, 1 TB의 데이터에는 1000GB/40GB = 25개의 실제 파티션이 필요합니다. 25개의 실제 파티션을 얻을 수 있도록 수동 처리량을 사용하는 경우 먼저 25 * 6000 RU/s = 150,000 RU/s를 프로비전합니다. 그런 다음, 컨테이너를 만든 후에는 더 빠르게 추출할 수 있도록 RU/s를 250,000 RU/s로 늘인 후, 이 경우는 이미 25개의 실제 파티션이 있으므로 즉시 발생합니다. 이렇게 하면 각 파티션이 최대 10,000 RU/s를 얻을 수 있습니다. 
...
이론적으로 250,000 RU/s 및 1TB의 데이터를 사용하며, 1kb 문서와 10 RU가 쓰기에 필요하다고 가정하는 경우 이론적으로는 1000GB * (1,000,000kb/1GB) * (1 문서/1kb) * (10 RU/문서) * (1초/250,000 RU) * (1시간/3600초) = 11.1시간에서 완료할 수 있습니다. 
