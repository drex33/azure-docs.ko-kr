...
##  <a name="mask-urls-in-content"></a><a name="MaskURLSContent"></a> 콘텐츠의 URL 마스킹
...
> [!NOTE]
> 백엔드 엔터티는 [Azure Portal](how-to-configure-service-fabric-backend.md), 관리 [API](/rest/api/apimanagement) 및 [PowerShell](https://www.powershellgallery.com/packages?q=apimanagement)을 통해 관리할 수 있습니다.
...
|backend-id|라우팅할 백 엔드의 식별자입니다. (백엔드 엔터티는 [Azure Portal](how-to-configure-service-fabric-backend.md), [API](/rest/api/apimanagement) 및 [PowerShell](https://www.powershellgallery.com/packages?q=apimanagement)을 통해 관리됩니다.)|`base-url` 또는 `backend-id` 중 하나가 있어야 합니다.|해당 없음|
...
#### <a name="transform-json-using-a-liquid-template"></a>Liquid 템플릿을 사용하여 JSON 변환
```xml
{
"order": {
...
> [!NOTE]
> 헤더의 다중 값은 CSV 문자열에 연결됩니다. 예: `headerName: value1,value2,value3`
>
...
>
> 이러한 예외의 경우 여러 헤더 값이 하나의 문자열에 연결되지 않고 별도의 헤더로 전달됩니다. 예: `User-Agent: value1`
>`User-Agent: value2`
>`User-Agent: value3`
