---
title: Azure App Configuration REST API - HMAC 인증
description: HMAC를 사용하여 REST API 사용하여 Azure App Configuration 인증
...
ms.locfileid: "128670427"
---
# <a name="hmac-authentication---rest-api-reference"></a>HMAC 인증 - REST API 참조

HMAC-SHA256 인증 체계를 사용하여 HTTP 요청을 인증할 수 있습니다. (HMAC는 해시 기반 메시지 인증 코드를 나타냅니다.) 이러한 요청은 TLS를 통해 전송되어야 합니다.

## <a name="prerequisites"></a>필수 구성 요소

- **자격 증명** - \<Access Key ID\>
- **비밀** - base64로 디코딩된 액세스 키 값입니다. ``base64_decode(<Access Key Value>)``

자격 `id` 증명(라고도 함) 및 비밀(라고도 함)에 대한 값은 `value` Azure App Configuration 인스턴스에서 얻어야 합니다. Azure Portal 또는 [Azure CLI](https://portal.azure.com) 를 사용하여 이 작업을 수행할 수 [있습니다.](/cli/azure/)

인증에 필요한 모든 HTTP 헤더를 각 요청에 제공합니다. 필요한 최소값은 다음과 같습니다.

|  요청 헤더 | Description  |
| --------------- | ------------ |
| **Host** | 인터넷 호스트 및 포트 번호입니다. 자세한 내용은 섹션 [3.2.2를 참조하세요.](https://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html#sec3.2.2) |
| **날짜** | 요청이 시작된 날짜 및 시간입니다. 현재 협정 세계시(그리니치 표준시)에서 15분을 초과할 수 없습니다. 값은 섹션 [3.3.1에](https://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html#sec3.3.1)설명된 대로 HTTP 날짜입니다.
| **x-ms-date** | ```Date```위와 동일합니다. 에이전트가 요청 헤더에 직접 액세스할 수 없거나 프록시에서 수정하는 경우 대신 사용할 수 ```Date``` 있습니다. ```x-ms-date```및 ```Date``` 가 둘 다 제공되면 가 ```x-ms-date``` 우선적으로 적용됩니다. |
| **x-ms-content-sha256** | 요청 본문의 base64로 인코딩된 SHA256 해시입니다. 본문이 없는 경우에도 제공해야 합니다. ```base64_encode(SHA256(body))```|
| **권한 부여** | HMAC-SHA256 체계에 필요한 인증 정보입니다. 형식 및 세부 정보는 이 문서의 후반부에 설명되어 있습니다. |
...
| **자격 증명** | 서명을 계산하는 데 사용되는 액세스 키의 ID입니다. _(필수)_ |
| **SignedHeaders** | 서명에 추가된 HTTP 요청 헤더입니다. _(필수)_ |
| **서명** | string-to-Sign의 base64로 인코딩된 HMACSHA256 _(필수)_|

### <a name="credential"></a>자격 증명

서명을 계산하는 데 사용되는 액세스 키의 ID입니다.

### <a name="signed-headers"></a>서명된 헤더

요청에 서명하는 데 필요한 세미콜론으로 구분된 HTTP 요청 헤더 이름입니다. 이러한 HTTP 헤더는 요청과 함께 올바르게 제공되어야 합니다. 공백을 사용하지 마세요.

### <a name="required-http-request-headers"></a>필수 HTTP 요청 헤더

`x-ms-date`[또는 `Date` ]; `host` ;`x-ms-content-sha256`

다른 모든 HTTP 요청 헤더를 서명에 추가할 수도 있습니다. 인수에 추가하기만 ```SignedHeaders``` 합니다.
...
String-To-Sign의 Base64로 인코딩된 HMACSHA256 해시입니다. 로 식별되는 액세스 키를 `Credential` 사용합니다. `base64_encode(HMACSHA256(String-To-Sign, Secret))`

### <a name="string-to-sign"></a>문자열-서명
...
|  인수 | Description  |
| ------ | ------ |
| **HTTP_METHOD** | 요청과 함께 사용되는 대문자 HTTP 메서드 이름입니다. 자세한 내용은 [섹션 9를 참조하세요.](https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html) |
|**path_and_query** | 요청 절대 URI 경로 및 쿼리 문자열의 결합입니다. 자세한 내용은 [섹션 3.3을](https://tools.ietf.org/html/rfc3986#section-3.3)참조하세요.
| **signed_headers_values** | 에 나열된 모든 HTTP 요청 헤더의 세미콜론으로 구분된 `SignedHeaders` 값입니다. 형식은 `SignedHeaders` 의미 체계를 따릅니다. |
...
**이유:** HMAC-SHA256 스키마가 있는 권한 부여 요청 헤더는 제공되지 않습니다.
...
**이유:** ```Date``` 또는 ```x-ms-date``` 요청 헤더가 현재 협정 세계시(그리니치 표준시)에서 15분 이상 끄기 때문에

**해결 방법:** 올바른 날짜 및 시간을 제공합니다.
...
**이유:** 누락되었거나 ```Date``` 잘못되었거나 ```x-ms-date``` 요청 헤더입니다.
...
**이유:** 요청 헤더에서 필수 매개 변수가 ```Authorization``` 누락되었습니다.
...
**이유:** 제공된 [ ```Host``` ]/[액세스 키 ID]를 찾을 수 없습니다.

**해결 방법:** 요청 ```Credential``` 헤더의 매개 변수를 ```Authorization``` 확인합니다. 유효한 액세스 키 ID인지 확인하고 ```Host``` 헤더가 등록된 계정을 가리키는지 확인합니다.
...
**이유:** 제공된 가 ```Signature``` 서버에서 예상하는 것과 일치하지 않습니다.

**해결 방법:** 이 ```String-To-Sign``` 올바른지 확인합니다. ```Secret```이 올바르고 올바르게 사용되는지 확인합니다(사용하기 전에 base64 디코딩).
...
**이유:** 헤더에 매개 변수에 필요한 요청 ```SignedHeaders```  ```Authorization``` 헤더가 없습니다.

**해결 방법:** 필요한 헤더를 올바른 값으로 제공합니다.
...
**이유:** 에 매개 변수가 ```SignedHeaders``` 없습니다.

**해결 방법:** 서명된 헤더 최소 요구 사항을 확인합니다.

## <a name="code-snippets"></a>코드 조각

### <a name="javascript"></a>JavaScript

*필수 조건:* [Crypto-JS](https://code.google.com/archive/p/crypto-js/)
