...
ms.openlocfilehash: ece06cbcecc6815cf272ddbf790ac39d072473ec
ms.sourcegitcommit: e9e332a512ed615a3c8ad5a11baa21649f14116d
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/24/2021
ms.locfileid: "133096811"
---
...
[Dapr(분산](https://dapr.io/) Application Runtime)은 복원력 있고 상태 비 상태 비지정적인 마이크로서비스를 빌드하는 데 도움이 되는 런타임입니다. 이 자습서에서는 샘플 Dapr 애플리케이션이 Azure Container Apps에 배포됩니다.
...
> * 메시지를 생성 및 사용하고 상태 저장소를 사용하여 유지하는 두 개의 앱 배포
...
* `storage_container_name` 는 위에 정의된 의 `STORAGE_ACCOUNT_CONTAINER` 값입니다(예: `mycontainer` ). dapr은 Azure Storage 계정에 컨테이너가 없는 경우 이 이름으로 컨테이너를 만듭니다.
...
위의 단계에서 얻은 속성을 사용하여 *components.yaml이라는* 구성 파일을 만듭니다. 이 파일을 사용하면 Dapr 앱이 상태 저장소에 액세스할 수 있습니다. 다음 예제에서는 Azure Blob Storage 계정에 대해 구성할 때 *components.yaml* 파일이 어떻게 표시되는지 보여 줍니다.
