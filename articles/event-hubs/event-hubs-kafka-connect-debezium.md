...
>
> Apache Kafka Connect에서는 동적 구성이 무제한 보존 방식이 아닌 압축된 토픽에 유지됩니다. Event Hubs [압축을 브로커 기능으로 구현하지 않으며,](event-hubs-federation-overview.md#log-projections) Event Hubs 장기 데이터 또는 구성 저장소가 아니라 실시간 이벤트 스트리밍 엔진이라는 원칙을 기반으로 보존된 이벤트에 항상 시간 기반 보존 제한을 적용합니다.
>
> Apache Kafka 프로젝트는 이러한 역할 혼합에 적합할 수 있지만 Azure는 적절한 데이터베이스 또는 구성 스토리지에서 이러한 정보를 가장 잘 관리할 수 있다고 판단합니다.
>
> 많은 Apache Kafka 커넥트 시나리오가 작동하지만 Apache Kafka 및 Event Hubs 보존 모델 간의 이러한 개념적 차이로 인해 특정 구성이 예상대로 작동하지 않을 수 있습니다. 

이 자습서에서는 Event Hubs(Kafka용), [Azure DB for PostgreSQL](./event-hubs-about.md?WT.mc_id=devto-blog-abhishgu) 및 Debezium을 사용하여 Azure에서 변경 [](../postgresql/overview.md) 데이터 캡처 기반 시스템을 설정하는 방법을 안내합니다. [Debezium PostgreSQL 커넥터를 사용하여 의 PostgreSQL에서](https://debezium.io/documentation/reference/1.2/connectors/postgresql.html) Kafka 항목으로 데이터베이스 수정 사항을 스트리밍합니다Event Hubs
...
[Debezium 설명서의](https://debezium.io/documentation/reference/1.2/connectors/postgresql.html#postgresql-deploying-a-connector) 최신 지침에 따라 커넥터를 다운로드하고 설정합니다.
...
> [!IMPORTANT]
> - Debezium은 테이블당 토픽과 여러 메타데이터 항목을 자동으로 만듭니다. Kafka **토픽은** Event Hubs 인스턴스(이벤트 허브)에 해당합니다. Azure Event Hubs 매핑에 대한 Apache Kafka [Kafka 및 Event Hubs 개념적 매핑을 참조하세요.](event-hubs-for-kafka-ecosystem-overview.md#kafka-and-event-hub-conceptual-mapping) 
> - 계층(기본, 표준, Premium 또는 전용)에 따라 Event Hubs 네임스페이스의 이벤트 허브 수에 대한 **제한이** 다릅니다. 이러한 제한은 [할당량을 참조하세요.](compare-tiers.md#quotas)
...
변경 데이터 캡처가 작동하는 모습을 보려면 Azure PostgreSQL 데이터베이스에서 레코드를 생성/업데이트/삭제해야 합니다.
...
테이블에 추가한 행에 대한 응답으로 PostgreSQL에서 생성된 변경 데이터 이벤트를 나타내는 JSON 페이로드가 `todos` 표시됩니다. 페이로드의 조각은 다음과 같습니다.
...
이벤트는 `schema`(간단하게 나타내기 위해 생략함)와 함께 `payload`로 구성됩니다. `payload`섹션에서 만들기 작업( )이 표시되는 방식을 확인합니다. `"op": "c"` `"before": null` 즉, 새로 선택된 `INSERT` 행이고, `after` 행의 열에 대한 값을 `source` 제공하고, 이 이벤트가 선택된 위치에서 PostgreSQL 인스턴스 메타데이터를 제공합니다.
...
이제 Event Hubs `todos` 항목에서 모든 테이블 변경 내용이 캡처되기 때문에 FileStreamSink 커넥터(Kafka 커넥트 기본적으로 사용 가능)를 사용하여 이러한 이벤트를 사용합니다.
...
## <a name="cleanup"></a>정리
Kafka Connect는 Connect 클러스터가 중단된 후에도 유지되는 구성, 오프셋 및 상태를 저장하는 이벤트 허브 토픽을 만듭니다. 이 지속성을 원하는 경우가 아니면 이러한 항목을 삭제하는 것이 좋습니다. 이 안내 중에 생성된 이벤트 허브를 삭제할 수도 `my-server.public.todos` 있습니다.
