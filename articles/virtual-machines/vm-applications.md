---
title: Azure Compute 갤러리의 VM 애플리케이션 개요(미리 보기)
description: Azure Compute 갤러리의 VM 애플리케이션 패키지에 대해 자세히 알아보세요.
author: cynthn
...
ms.locfileid: "132708991"
---
# <a name="vm-applications-overview-preview"></a>VM 애플리케이션 개요(미리 보기)
...
앱이 미리 설치된 VM의 이미지를 만들 수 있지만 애플리케이션이 변경될 때마다 이미지를 업데이트해야 합니다. VM 이미지에서 애플리케이션 설치를 분리하면 코드 변경의 모든 줄에 대해 새 이미지를 게시할 필요가 없습니다.

애플리케이션 패키지는 다른 배포 및 패키징 방법에 비해 이점을 제공합니다.

- 패키지 그룹화 및 버전 관리자

- 인프라에 더 가깝게 VM 애플리케이션을 전역적으로 복제할 수 있으므로 AzCopy 또는 다른 스토리지 복사 메커니즘을 사용하여 Azure 지역 간에 비트를 복사할 필요가 없습니다.

- Azure RBAC(역할 기반 Access Control)를 통해 다른 사용자와 공유

- 가상 머신 및 유연하고 균일한 확장 집합에 대한 지원

- VM 또는 확장 집합에 적용된 NSG(네트워크 보안 그룹) 규칙이 있는 경우 인터넷 리포지토리에서 패키지를 다운로드할 수 없습니다. 스토리지 계정을 통해 잠긴 VM에 패키지를 다운로드하려면 프라이빗 링크를 설정해야 합니다.
- VM 애플리케이션은 [DeployIfNotExists](../governance/policy/concepts/effects.md) 정책과 함께 사용할 수 있습니다.


## <a name="what-are-vm-app-packages"></a>VM 앱 패키지란?

VM 애플리케이션 패키지는 여러 리소스 종류를 사용합니다.

| 리소스 | 설명|
|----------|------------|
| **Azure 컴퓨팅 갤러리** | 갤러리는 애플리케이션 패키지를 관리하고 공유하기 위한 리포지토리입니다. 사용자는 갤러리 리소스를 공유할 수 있으며 모든 자식 리소스가 자동으로 공유됩니다. 갤러리 이름은 구독별로 고유해야 합니다. 예를 들어 모든 OS 이미지를 저장할 하나의 갤러리와 모든 VM 애플리케이션을 저장할 다른 갤러리가 있을 수 있습니다.|
| **VM 애플리케이션** | 이는 VM 애플리케이션의 정의입니다. 이 리소스는 모든 버전에 대한 공통 메타데이터를 저장하는 *논리* 리소스입니다. 예를 들어 Apache Tomcat에 대한 애플리케이션 정의가 있고 그 안에 여러 버전이 있을 수 있습니다. |
...
- **실패한 설치 다시 시도:** 현재 실패한 설치를 다시 시도할 수 있는 유일한 방법은 프로필에서 애플리케이션을 제거한 다음 다시 추가하는 것입니다.

- **VM당 5개 애플리케이션만:** 언제든지 5개 이하의 애플리케이션을 VM에 배포할 수 있습니다.

- **1GB 애플리케이션 크기:** 애플리케이션 버전의 최대 파일 크기는 1GB입니다. 

- **스크립트에서 다시 부팅이 보장되지 않습니다. 스크립트를** 다시 부팅해야 하는 경우 배포 중에 해당 애플리케이션을 마지막으로 배치하는 것이 좋습니다. 코드가 다시 부팅을 처리하려고 하는 동안 실패할 수 있습니다.

- **VM 에이전트 필요: VM** 에이전트가 VM에 있어야 하며 목표 상태를 받을 수 있어야 합니다.

- **동일한 VM에 있는 동일한 애플리케이션의 여러 버전: VM에** 동일한 애플리케이션의 여러 버전을 사용할 수 없습니다.


## <a name="cost"></a>Cost

VM 애플리케이션 패키지 사용에 대한 추가 요금은 없지만 다음 리소스에 대한 요금이 청구됩니다.

- 각 패키지 및 모든 복제본을 저장하는 데 드는 비용을 Storage. 
...
## <a name="vm-applications"></a>VM 애플리케이션

VM 애플리케이션 리소스는 VM 애플리케이션에 대해 다음을 정의합니다.

- VM 애플리케이션이 저장되는 Azure Compute 갤러리
- 애플리케이션의 이름입니다.
- Linux 또는 Windows 같은 지원되는 OS 유형
- VM 애플리케이션에 대한 설명

## <a name="vm-application-versions"></a>VM 애플리케이션 버전

VM 애플리케이션 버전은 배포 가능한 리소스입니다. 버전은 다음 속성으로 정의됩니다.
- 버전 번호
- 스토리지 계정의 애플리케이션 패키지 파일에 연결
- 애플리케이션을 설치하기 위한 문자열 설치
- 문자열을 제거하여 앱을 제대로 제거하는 방법 표시
- VM에 다운로드할 때 사용할 패키지 파일 이름
- VM에서 앱을 구성하는 데 사용할 구성 파일 이름
- VM 애플리케이션의 구성 파일에 대한 링크
- VM 애플리케이션을 최신 버전으로 업데이트하는 방법에 대한 문자열 업데이트
- 수명 종료 날짜입니다. 수명 종료 날짜는 정보 제공입니다. 수명이 종료된 후에도 VM 애플리케이션 버전을 배포할 수 있습니다.
- 최신 항목에서 제외. 버전을 애플리케이션의 최신 버전으로 사용하지 않도록 유지할 수 있습니다. 
- 복제 대상 지역
- 지역당 복제본 수

## <a name="download-directory"></a>디렉터리 다운로드 
...
설치/업데이트/제거 명령은 애플리케이션 패키지 및 구성 파일이 현재 디렉터리에 있다고 가정하여 작성해야 합니다.

## <a name="file-naming"></a>파일 이름 지정

미리 보기 중에 애플리케이션 파일이 VM에 다운로드되면 파일 이름은 VM 애플리케이션을 만들 때 사용하는 이름과 동일합니다. 예를 들어 내 VM 애플리케이션의 이름을 로 지정하면 `myApp` 스토리지 계정에 사용되는 파일 이름에 관계없이 VM에 다운로드할 `myApp` 파일의 이름도 로 지정됩니다. VM 애플리케이션에 구성 파일도 있는 경우 해당 파일은 가 추가된 애플리케이션의 `_config` 이름입니다. 에 구성 파일이 있는 경우 이름이 로 `myApp` `myApp_config` 지정됩니다.

예를 들어 갤러리에서 만들 때 내 VM 애플리케이션의 이름을 `myApp` 지정하지만 스토리지 계정에 로 저장되는 경우 `myApplication.exe` VM에 다운로드될 때 파일 이름은 `myApp` 입니다. 내 설치 문자열은 파일 이름을 VM에서 실행하는 데 필요한 대로 바쳐야 합니다(예: myApp.exe).

이 점을 염두에 두고 설치, 업데이트 및 제거 명령을 작성해야 합니다. 
...
실행 파일을 호출하고 명령을 전달하여 컴퓨터에 설치되어 있는 한 다른 인터프리터를 사용할 수 있습니다. 예를 들어 cmd 대신 Windows PowerShell에서 명령을 실행하려면 를 전달할 수 있습니다.`powershell.exe -Command '<powershell commmand>'`
  

## <a name="how-updates-are-handled"></a>업데이트 처리 방법

애플리케이션 버전을 업데이트하면 배포 중에 제공한 업데이트 명령이 사용됩니다. 업데이트된 버전에 update 명령이 없으면 현재 버전이 제거되고 새 버전이 설치됩니다. 

업데이트 명령은 이전 버전의 VM 애플리케이션에서 업데이트될 수 있다는 기대로 작성해야 합니다.


## <a name="tips-for-creating-vm-applications-on-linux"></a>Linux에서 VM 애플리케이션을 만들기 위한 팁 

Linux용 타사 애플리케이션은 몇 가지 방법으로 패키지할 수 있습니다. 가장 일반적인 몇 가지에 대한 설치 명령 만들기를 처리하는 방법을 살펴보겠습니다.

### <a name="tar-and-gz-files"></a>.tar 및 .gz 파일 

압축된 보관 파일이며 원하는 위치로 추출할 수 있습니다. 특정 위치에 추출해야 하는 경우 원래 패키지에 대한 설치 지침을 확인합니다. .tar.gz 파일에 소스 코드가 포함된 경우 소스에서 설치하는 방법에 대한 패키지 지침을 참조하세요. 
...
플랫폼별 패키지 관리자에 대한 개별 패키지를 다운로드할 수 있지만 일반적으로 일부 의존성만 포함됩니다. 이러한 파일의 경우 애플리케이션 패키지에 모든 의존성도 포함하거나 시스템 패키지 관리자가 VM에서 사용할 수 있는 리포지토리를 통해 해당 애플리케이션을 다운로드하도록 해야 합니다. 인터넷 액세스가 제한된 VM으로 작업하는 경우 모든 dependencies를 직접 패키지해야 합니다.


의존성을 파악하는 것은 약간 어려울 수 있습니다. 전체 종속성 트리를 표시할 수 있는 타사 도구가 있습니다. 

Ubuntu에서 를 `apt-get install <name> --simulate` 실행하여 명령에 대해 설치할 모든 패키지를 표시할 수 `apt-get install <name>` 있습니다. 그런 다음 해당 출력을 사용하여 모든 .deb 파일을 다운로드하여 애플리케이션 패키지로 사용할 수 있는 보관 파일을 만들 수 있습니다. 이 메서드의 단점은 가 VM에 이미 설치된 dependencies를 표시하지 않는다는 것입니다.
...
또한 Cmd.exe 실행 파일의 확장명 .exe 필요하므로 확장명이 te .exe 파일의 이름을 바꿔야 합니다.  
...
설치 관리자의 명령줄 실행의 경우 `.msi` 애플리케이션을 설치하거나 제거하는 명령은 를 사용해야 `msiexec` 합니다. 일반적으로 는 `msiexec` 별도의 프로세스로 실행되며 `cmd` 완료되기를 기다리지 않으므로 VM 애플리케이션을 두 개 이상 설치할 때 문제가 발생할 수 있습니다.  `start`명령을 와 함께 사용하여 `msiexec` 명령이 반환되기 전에 설치가 완료되도록 할 수 있습니다. 예를 들어 다음과 같습니다.
...
.zip 또는 다른 압축된 파일의 경우 애플리케이션 패키지의 내용의 압축을 원하는 대상으로 압축을 풀면됩니다. 
...
| 메시지 | Description |
|--|--|
| 현재 VM 애플리케이션 버전 {name}은 {date}에서 더 이상 사용되지 않습니다. | 이미 사용되지 않는 VM 애플리케이션 버전을 배포하려고 했습니다. 특정 `latest` 버전을 지정하는 대신 을 사용해 보세요. |
| 현재 VM 애플리케이션 버전 {name}은 OS {OS}를 지원하지만 현재 OSDisk의 OS는 {OS}입니다. | Windows 인스턴스에 Linux 애플리케이션을 배포하려고 했거나 그 반대의 경우도 마찬가지입니다. |
...
| 이 이미지에 액세스할 권한이 없는 구독입니다. | 구독에 이 애플리케이션 버전에 대한 액세스 권한이 없습니다. |
...
| 다음 오류로 인해 갤러리 애플리케이션 버전 URL {url}에 액세스할 수 없습니다. 원격 이름을 찾을 수 없습니다. Blob이 존재하고 공개적으로 액세스할 수 있는지 또는 읽기 권한이 있는 SAS URL인지 확인합니다. | 읽기 권한이 있는 SAS URI가 제공되지 않은 경우가 가장 가능성이 높습니다. |
| {오류 설명} 오류로 인해 갤러리 애플리케이션 버전 URL {url}에 액세스할 수 없습니다. Blob이 존재하고 공개적으로 액세스할 수 있는지 또는 읽기 권한이 있는 SAS URL인지 확인합니다. | 제공된 스토리지 Blob에 문제가 있습니다. 오류 설명은 자세한 정보를 제공합니다. |
| {application}이(가) 삭제되도록 표시되어 있기 때문에 {operationName} 작업이 허용되지 않습니다. 삭제 작업을 다시 시도(하거나 실행 중인 작업이 완료되기를 기다림)할 수 있습니다. | 현재 삭제 중인 애플리케이션을 업데이트하려고 시도합니다. |
...
| 속성 ' galleryApplicationVersion.properties.publishingProfile.settings.packageFileName '을 변경할 수 없습니다. (또는 configFileName) | 패키지 파일 이름 또는 구성 파일 이름과 같은 설정은 변경할 수 없습니다. 새 VmApplication 버전을 만들어야 합니다. |
