...
적절한 [자격 증명](/python/api/overview/azure/identity-readme#credentials)을 생성하고 Azure Monitor 내보내기 생성자에 전달합니다. 연결 문자열이 리소스의 계측 키 및 수집 엔드포인트로 설정되어 있는지 확인합니다.
...
에이전트에서 Azure AD를 사용 하는 경우 아웃 바운드 트래픽에는 HTTP 헤더 "Authorization"이 포함 됩니다.
...
로그 파일에 다음 예외가 표시 되는 경우 `com.microsoft.aad.msal4j.MsalServiceException: Application with identifier <CLIENT_ID> was not found in the directory` 에이전트에서 액세스 토큰을 획득 하지 못했음을 나타내며, 클라이언트 암호 구성에서 잘못 된/잘못 된 "clientId"를 제공한 이유가 있을 수 있습니다.

 이 오류는 테넌트의 관리자가 애플리케이션을 설치하지 않았거나 테넌트의 사용자가 동의하지 않은 경우에 발생할 수 있습니다. 잘못된 테넌트에 인증 요청을 보냈을 수도 있습니다.

### <a name="python"></a>[Python](#tab/python)

#### <a name="error-starts-with-credential-error-with-no-status-code"></a>오류는 "자격 증명 오류" (상태 코드 없음)로 시작 합니다.
...
#### <a name="error-starts-with-authentication-error-with-no-status-code"></a>오류는 "인증 오류" (상태 코드 없음)로 시작 합니다.

클라이언트가 지정된 자격 증명을 사용하여 인증하지 못했습니다. 일반적으로 사용된 자격 증명에 올바른 역할 할당이 없는 경우에 발생합니다.

#### <a name="im-getting-a-status-code-400-in-my-error-logs"></a>내 오류 로그에서 상태 코드 400을 받고 있습니다.
...
#### <a name="im-getting-a-status-code-403-in-my-error-logs"></a>내 오류 로그에서 상태 코드 403을 받고 있습니다.
