...
Azure Cosmos 계정이 *N* Azure 지역에 분산된 경우 모든 데이터의 *복사본은 N* x 4개입니다. 데이터 배포에 대한 자세한 개요는 [전체 데이터 배포를 참조하세요.](global-dist-under-the-hood.md) 2개 이상의 지역에 Azure Cosmos 계정이 있으면 애플리케이션의 가용성이 향상되고 연결된 지역에서 짧은 대기 시간을 제공합니다.
