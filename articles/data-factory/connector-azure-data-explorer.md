...
2. 데이터 팩터리 또는 Synapse 작업 영역에 사용자 할당 관리 ID를 하나 이상 할당하고 각 사용자 할당 관리 ID에 대한 [자격 증명을 만듭니다.](credentials.md)
...
매핑 데이터 흐름에서 데이터를 변환할 때 Azure Data Explorer 테이블에서 읽고 쓸 수 있습니다. 자세한 내용은 매핑 데이터 흐름에서 [원본 변환](data-flow-source.md) 및 [싱크 변환](data-flow-sink.md)을 참조하세요. Azure Data Explorer 데이터 세트 또는 [인라인 데이터 세트를](data-flow-source.md#inline-datasets) 원본 및 싱크 형식으로 사용하도록 선택할 수 있습니다.

### <a name="source-transformation"></a>원본 변환

아래 표에는 Azure Data Explorer 원본에서 지원하는 속성이 나와 있습니다. 이러한 속성은 **원본 옵션** 탭에서 편집할 수 있습니다.

| Name | 설명 | 필수 | 허용되는 값 | 데이터 흐름 스크립트 속성 |
| ---- | ----------- | -------- | -------------- | ---------------- |
| 테이블 | 입력으로 테이블을 선택하면 데이터 흐름은 인라인 데이터 세트를 사용할 때 Azure Data Explorer 데이터 세트 또는 원본 옵션에 지정된 테이블에서 모든 데이터를 가져옵니다. | 예 | String | *(인라인 데이터 세트에만 해당)*<br>tableName |
| 쿼리 | [KQL 형식](/azure/data-explorer/kusto/query/)으로 제공되는 읽기 전용 요청입니다. 사용자 지정 KQL 쿼리를 참조로 사용합니다.  | 예 | String | Query |
| 제한 시간 | 쿼리 요청 시간이 시간 단축되기 전의 대기 시간입니다. 기본값은 '172000'(2일)입니다.  | 예 | 정수 | 시간 제한 |

#### <a name="azure-data-explorer-source-script-examples"></a>Azure Data Explorer 원본 스크립트 예제

Azure Data Explorer 데이터 세트를 원본 유형으로 사용하는 경우 연결된 데이터 흐름 스크립트는 다음과 같습니다.
...
아래 표에는 Azure Data Explorer 싱크에서 지원하는 속성이 나와 있습니다. 이러한 속성은 **설정** 탭에서 편집할 수 있습니다. 인라인 데이터 세트를 사용하는 경우 [데이터 세트 속성](#dataset-properties) 섹션에 설명된 속성과 동일한 추가 설정이 표시됩니다. 
...
| 사전 및 사후 SQL 스크립트 | 싱크 데이터베이스에 데이터를 쓰기 전(전처리) 후(후처리) 실행할 여러 [Kusto 제어 명령](/azure/data-explorer/kusto/query/#control-commands) 스크립트를 지정합니다. | 예 | String | preSQL; postSQL |
| 제한 시간 | 쿼리 요청 시간이 시간 단축되기 전의 대기 시간입니다. 기본값은 '172000'(2일)입니다. | 예 | 정수 | 시간 제한 |


#### <a name="azure-data-explorer-sink-script-examples"></a>싱크 스크립트 예제 Azure Data Explorer
