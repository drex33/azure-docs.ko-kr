...
ms.locfileid: "132705136"
---
# <a name="azure-web-pubsub-service-client-library-for-python"></a>Python 용 Azure 웹 PubSub 서비스 클라이언트 라이브러리
...
_Python 2.7에 대 한 Azure SDK Python 패키지 지원은 2022 년 1 월 01 일에 끝납니다. 자세한 내용 및 질문은 다음을 참조 하세요. https://github.com/Azure/azure-sdk-for-python/issues/20691_
...
- 이 패키지를 사용 하려면 [azure 구독][azure_sub]및 [Azure webpubsub 서비스 인스턴스가][webpubsubservice_docs] 필요 합니다.
...
`WebPubSubServiceClient` [연결 문자열][connection_string]을 사용 하 여를 인증할 수 있습니다.
...
또는 [Azure Active Directory][aad_doc]를 사용 합니다.
1. [pip][pip] 설치 [`azure-identity`][azure_identity_pip]
2. 문서에 따라 [webpubsub 리소스에서 AAD 인증을 사용 하도록 설정][aad_doc] 합니다.
3. [DefaultAzureCredential][default_azure_credential] 를 사용 하는 코드 업데이트
...
### <a name="broadcast-messages-in-json-format"></a>JSON 형식의 메시지 브로드캐스트
...
WebSocket 클라이언트는 serialize 된 JSON 텍스트를 수신 `{"from": "user1", "data": "Hello world"}` 합니다.
...
WebSocket 클라이언트는 텍스트를 수신 `Hello world` 합니다.

### <a name="broadcast-messages-in-binary-format"></a>메시지를 이진 형식으로 브로드캐스트
...
```
WebSocket 클라이언트는 이진 텍스트를 수신 `b'Hello world'` 합니다.
...
로깅을 구성 하 여 stdout 또는 원하는 모든 위치에 디버깅 정보를 출력할 수 있습니다.
...
마찬가지로는 `logging_enable` WebPubSubServiceClient에 대해 사용 하도록 설정 되지 않은 경우에도 단일 호출에 대해 자세한 로깅을 사용 하도록 설정할 수 있습니다.
...
이 프로젝트에는 [Microsoft Open Source Code of Conduct][code_of_conduct](Microsoft 오픈 소스 준수 사항)가 적용됩니다. 자세한 내용은 수행 FAQ 코드를 참조 하거나 opencode@microsoft.com 추가 질문이 나 의견을 문의 하세요.
