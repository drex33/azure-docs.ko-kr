...
Microsoft Sentinel의 기본 제공 커넥터를 사용하여 [Azure Active Directory](../active-directory/fundamentals/active-directory-whatis.md) 데이터를 수집하고 Microsoft Sentinel로 스트리밍할 수 있습니다. 커넥터를 사용하면 다음 로그 유형을 스트리밍할 수 있습니다.
...
- Microsoft Sentinel에 로그인 로그를 Azure Active Directory P1 또는 P2 라이선스가 필요합니다. 모든 Azure AD 라이선스(Free/O365/P1/P2)는 다른 로그 유형을 수집하기에 충분합니다. Azure Monitor(Log Analytics) 및 Microsoft Sentinel에는 기가바이트당 추가 요금이 적용될 수 있습니다.

- 사용자에게 작업 영역에서 Microsoft Sentinel 기여자 역할이 할당되어야 합니다.
...
1. Microsoft Sentinel로 스트리밍하려는 로그 유형 옆에 있는 확인란을 표시하고(위 참조) **커넥트** 선택합니다.
...
이 문서에서는 microsoft Sentinel에 Azure Active Directory 연결하는 방법을 배웠습니다. Microsoft Sentinel에 대한 자세한 내용은 다음 문서를 참조하세요.
