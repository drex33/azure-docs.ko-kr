...
1. 애플리케이션이 다음 프로브가 도착하기 직전에 시간 시간 프로브 응답을 생성하기 시작하면 이러한 이벤트를 검색하는 데 5초가 더 걸리고 애플리케이션이 프로브가 도착할 때 시간제한을 알리기 시작하는 데 시간이 더 걸립니다.  이 검색은 5초가 약간 넘게 소요된다고 가정할 수 있습니다.
2. 애플리케이션이 다음 프로브가 도착한 직후에 시간 시간 프로브 응답을 생성하기 시작하면 프로브가 도착할 때까지(및 시간 부족) 추가로 5초가 더해질 때까지 이러한 이벤트의 검색이 시작되지 않습니다.  이 검색은 10초 이내에 완료된다고 가정할 수 있습니다.
