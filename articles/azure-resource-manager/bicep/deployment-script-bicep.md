...
- [샘플 1](https://raw.githubusercontent.com/Azure/azure-docs-bicep-samples/master/samples/deployment-script/deploymentscript-keyvault.bicep): 주요 자격 증명 모음을 만들고 배포 스크립트를 사용하여 키 자격 증명 모음에 인증서를 할당합니다.
- [샘플 2](https://raw.githubusercontent.com/Azure/azure-docs-bicep-samples/master/samples/deployment-script/deploymentscript-keyvault-subscription.bicep): 구독 수준에서 리소스 그룹을 만들고 리소스 그룹에 키 자격 증명 모음을 만든 다음, 배포 스크립트를 사용하여 키 자격 증명 모음에 인증서를 할당합니다.
- [샘플 3](https://raw.githubusercontent.com/Azure/azure-docs-bicep-samples/master/samples/deployment-script/deploymentscript-keyvault-mi.bicep): 사용자 할당 관리 ID를 만들고, 리소스 그룹 수준에서 ID에 참가자 역할을 할당하고, 주요 자격 증명 모음을 만든 다음, 배포 스크립트를 사용하여 인증서를 주요 자격 증명 모음에 할당합니다.
...
![ARM Bicep 배포 스크립트 출력](./media/deployment-script-bicep/resource-manager-template-deployment-script-inline-script-output.png)
...
- `storageAccountKey` : 스토리지 계정 키 중 하나를 지정합니다. [listKeys()](./bicep-functions-resource.md#listkeys) 함수를 사용하여 키를 검색할 수 있습니다. 다음은 그 예입니다.
...
컨테이너 인스턴스에서 환경 변수(EnvironmentVariable)를 설정하면 컨테이너가 실행하는 애플리케이션 또는 스크립트의 동적 구성을 제공할 수 있습니다. 배포 스크립트는 Azure Container Instance와 동일한 방식으로 비보안 환경 변수 및 보안 환경 변수를 처리합니다. 자세한 내용은 [컨테이너 인스턴스에서 환경 변수 설정](../../container-instances/container-instances-environment-variables.md#secure-values)을 참조하세요. 예제는 샘플 [Bicep 파일 을](#sample-bicep-files)참조하세요.

환경 변수에 대해 허용되는 최대 크기는 64KB입니다.

## <a name="monitor-and-troubleshoot-deployment-scripts"></a>배포 스크립트 모니터링 및 문제 해결

스크립트 서비스는 기존 [스토리지 계정](../../storage/common/storage-account-overview.md) 및/또는 기존 컨테이너 [인스턴스를](../../container-instances/container-instances-overview.md) 지정하지 않는 한 스크립트 실행을 위해 스토리지 계정 및 컨테이너 인스턴스를 만듭니다. 이러한 리소스는 스크립트 서비스에서 자동으로 생성되는 경우 리소스 이름에 `azscripts` 접미사가 붙습니다.

![Resource Manager 템플릿 배포 스크립트 리소스 이름](./media/deployment-script-bicep/resource-manager-template-deployment-script-resources.png)
...
![Resource Manager 템플릿 배포 스크립트 포털 개요](./media/deployment-script-bicep/resource-manager-deployment-script-portal.png)

개요 페이지에는 **프로 비전 상태**, **스토리지 계정**, **컨테이너 인스턴스** 및 **로그** 와 같은 리소스에 대한 몇 가지 중요한 정보가 표시됩니다.

왼쪽 메뉴에서 배포 스크립트 콘텐츠, 스크립트에 전달된 인수 및 출력을 볼 수 있습니다. 배포 스크립트를 포함하여 배포 스크립트에 대한 JSON 템플릿을 내보낼 수도 있습니다.
...
![Resource Manager 템플릿 배포 스크립트, 숨겨진 유형 표시, 포털](./media/deployment-script-bicep/resource-manager-deployment-script-portal-show-hidden-types.png)
...
이러한 리소스의 수명 주기는 Bicep 파일의 다음 속성에 의해 제어됩니다.
...
배포 스크립트 실행은 idempotent 작업입니다. `deploymentScripts`리소스 속성(인라인 스크립트 포함)이 변경되지 않으면 Bicep 파일을 다시 배포할 때 스크립트가 실행되지 않습니다. 배포 스크립트 서비스는 Bicep 파일의 리소스 이름을 동일한 리소스 그룹의 기존 리소스와 비교합니다. 동일한 배포 스크립트를 여러 번 실행하려는 경우 다음 두 가지 옵션이 있습니다.

- `deploymentScripts` 리소스 그룹의 이름을 바꿉니다. 예를 들어 [utcNow](./bicep-functions-date.md#utcnow) Bicep 함수를 리소스 이름으로 사용하거나 리소스 이름의 일부로 사용합니다. 리소스 이름을 변경하면 새 `deploymentScripts` 리소스가 생성됩니다. 스크립트 실행 기록을 보관하는 것이 좋습니다.

    > [!NOTE]
    > `utcNow` 함수는 매개 변수의 기본값에만 사용할 수 있습니다.

- 속성에 다른 값을 `forceUpdateTag` 지정합니다. 예를 들어 값으로 `utcNow`를 사용합니다.
...
배포 스크립트 개발 환경으로 미리 구성된 컨테이너 이미지를 사용할 수 있습니다. 자세한 내용은 [배포 스크립트에 대한 개발 환경 구성을 참조하세요.](../templates/deployment-script-template-configure-dev.md)

스크립트가 성공적으로 테스트되면 Bicep 파일에서 배포 스크립트로 사용할 수 있습니다.

## <a name="deployment-script-error-codes"></a>배포 스크립트 오류 코드

| 오류 코드 | Description |
|------------|-------------|
| DeploymentScriptInvalidOperation | Bicep 파일의 배포 스크립트 리소스 정의에 잘못된 속성 이름이 포함되어 있습니다. |
...
이 문서에서는 배포 스크립트를 사용하는 방법을 알아보았습니다. Microsoft Learn 모듈을 진행하려면 다음을 수행합니다.

> [!div class="nextstepaction"]
> [배포 스크립트를 사용하여 ARM 템플릿 확장](/learn/modules/extend-resource-manager-template-deployment-scripts)
