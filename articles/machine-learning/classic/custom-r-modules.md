---
title: 'ML 스튜디오(클래식): 사용자 지정 R 모듈 만들기 및 배포 - Azure'
description: ML 스튜디오(클래식)에서 사용자 지정 R 모듈을 작성하고 배포하는 방법을 알아봅니다.
...
**표준 출력 포트:** 출력 포트는 R 함수의 반환 값에 매핑되어 후속 모듈에 사용됩니다. *DataTable* 은 현재 지원되는 유일한 표준 출력 포트 형식입니다. (*학습자* 및 *변환* 에 대한 지원이 곧 제공될 예정입니다.) *DataTable* 출력은 다음과 같이 정의됩니다.
...
그런 다음 ‘CustomAddRows.R’에서 올바른 순서로 목록에 있는 개체의 목록을 반환합니다.
...
입력 및 출력과 마찬가지로 각 매개 변수에 고유한 ID 값이 연결되어 있는 것이 중요합니다. 이 빠른 시작 예제에서 연결된 ID/매개 변수는 *swap* 입니다.
...
**ColumnPicker**: 열 선택 매개 변수입니다. 이 형식은 UX에서 열 선택기로 렌더링됩니다. 여기에서 **속성** 요소는 열을 선택할 포트의 ID를 지정하는 데 사용되며, 대상 포트 형식은 *DataTable* 이어야 합니다. 열 선택의 결과는 선택한 열 이름이 포함된 문자열 목록으로 R 함수에 전달됩니다. 
...
* *필수 속성*: **portId** - *DataTable* 형식의 입력 요소의 ID와 일치합니다.
* *선택적 속성*:
...
그런 다음 ‘CustomAddRows.R’, ‘CustomAddRows.xml’, ‘RemoveDupNARows.R’이 포함된 Zip 파일을 사용자 지정 R 모듈로 업로드합니다.
