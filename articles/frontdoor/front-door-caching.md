...
ms.date: 09/13/2021
ms.author: duau
ms.openlocfilehash: 8bddcb651629642adb17a7e78ea2550f338a5653
ms.sourcegitcommit: 1aeff9f012cfd868104ef0159c5204e402d75696
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/24/2021
ms.locfileid: "133030236"
---
...
## <a name="cache-behavior-and-duration"></a>캐시 동작 및 기간

캐시 동작 및 기간은 Front Door 디자이너 라우팅 규칙과 규칙 엔진 모두에서 구성할 수 있습니다. 규칙 엔진 캐싱 구성은 항상 Front Door 디자이너 라우팅 규칙 구성을 재정의합니다.

* *캐싱을* 사용하지 **않도록 설정하면** 원본 응답 지시문에 관계없이 Front Door 응답 콘텐츠를 캐시하지 않습니다.

* *캐싱을* **사용하도록 설정하면** 캐시 *기본 기간 사용* 의 값에 따라 캐시 동작이 다릅니다.
    * *캐시 기본 기간 사용* 을 **예** 로 설정하면 Front Door 항상 원본 응답 헤더 지시문을 사용합니다. origin 지시문이 없으면 Front Door 1~3일 동안 콘텐츠를 캐시합니다.
    * *캐시 기본 기간 사용이* **아니요로** 설정된 경우 Front Door 항상 *캐시 기간(필수* 필드)으로 재정의됩니다. 즉, 원본 응답 지시문의 값을 무시하고 캐시 기간 동안 콘텐츠를 캐시합니다. 

> [!NOTE]
> * Front Door 디자이너 라우팅 규칙에 설정된 *캐시 기간은* **최소 캐시 기간** 입니다. 백 엔드의 캐시 컨트롤 헤더에 재정의 값보다 더 큰 TTL이 있는 경우 이 재정의가 작동하지 않습니다.
> * Azure Front Door 콘텐츠가 캐시에 저장되는 시간을 보장하지 않습니다. 콘텐츠가 자주 사용되지 않는 경우 콘텐츠가 만료되기 전에 캐시된 콘텐츠가 에지 캐시에서 제거될 수 있습니다. Front Door 캐시된 데이터가 만료된 경우에도 캐시에서 데이터를 제공할 수 있습니다. 이 동작은 백 엔드가 오프라인 상태일 때 사이트를 부분적으로 사용할 수 있도록 하는 데 도움이 될 수 있습니다.
>
