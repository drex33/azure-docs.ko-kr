...
5. 다음 URL을 입력합니다(`{subscription-key}`를 기본 구독 키로 바꿈).
...
5. [핀 및 경로 데이터 업로드](#upload-pins-and-path-data)에서 복사한 `status URL`을 입력합니다. 요청은 다음 URL과 같습니다(`{subscription-key}`를 기본 구독 키로 바꿈).
...
5. [Render Service](/rest/api/maps/render/get-map-image)에 다음 URL을 입력합니다(`{subscription-key}`를 기본 구독 키로 바꾸고, `udid`를 업로드된 데이터의 `udid`로 바꿈).
...
5. [Render Service](/rest/api/maps/render/get-map-image)에 다음 URL을 입력합니다(`{subscription-key}`를 기본 구독 키로 바꿈).
