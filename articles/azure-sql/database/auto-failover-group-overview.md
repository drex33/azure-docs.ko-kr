...
- **데이터 손실이 있는 유예 기간**
  ...
  보조 데이터베이스는 비동기 복제를 사용하여 동기화되므로 자동 장애 조치(failover)로 인해 데이터가 손실될 수 있습니다. 애플리케이션의 데이터 손실 허용 오차를 반영하도록 자동 장애 조치 정책을 사용자 지정할 수 있습니다. 를 구성하면 `GracePeriodWithDataLossHours` 강제 장애 조치(failover)를 시작하기 전에 시스템이 대기하는 시간을 제어할 수 있으며 이로 인해 데이터가 손실됩니다.
