---
title: 이벤트 처리기를 구성 하는 방법
description: Azure 웹 PubSub 서비스를 사용 하 여 개발할 때 이벤트 처리기 개념과 통합 소개에 대 한 지침을 제공 합니다.
author: JialinXin
ms.author: jixin
...
ms.locfileid: "131576399"
---
# <a name="event-handler-in-azure-web-pubsub-service"></a>Azure 웹 PubSub 서비스의 이벤트 처리기

이벤트 처리기는 수신 클라이언트 이벤트를 처리합니다. 이벤트 처리기는 포털 또는 Azure CLI를 통해 서비스에 미리 등록되고 구성되므로, 클라이언트 이벤트가 트리거되면 서비스에서 이벤트 처리가 예상되는지 여부를 식별할 수 있습니다. 이제 이벤트 처리기를 서버 쪽으로 지원 합니다 .이는 이벤트를 트리거할 때 서비스에서 호출 하는 공용 액세스 가능 끝점을 노출 합니다. 즉, **webhook** 로 작동 합니다. 
...
:::image type="content" source="media/howto-develop-eventhandler/event-trigger.png" alt-text="웹 PubSub 서비스 이벤트 트리거의 스크린샷":::
...
Azure Portal 또는 CLI를 통해 이벤트 처리기 업스트림을 설정 하는 경우 서비스는 [CloudEvents 불건전 보호](https://github.com/cloudevents/spec/blob/v1.0/http-webhook.md#4-abuse-protection) 를 따르고 업스트림 webhook의 유효성을 검사 합니다. 이 메커니즘을 통해 등록 된 모든 업스트림 webhook URL의 유효성을 검사 합니다. `WebHook-Request-Origin`요청 헤더는 서비스 도메인 이름으로 설정 `xxx.webpubsub.azure.com` 되며 `WebHook-Allowed-Origin` 이 도메인 이름 또는를 포함 하는 헤더가 있는 응답을 예상 합니다 `*` .
...
- 익명 모드
- 를 사용한 단순 `?code=<code>` 인증은 구성 된 웹 후크 URL을 통해 쿼리 매개 변수로 제공 됩니다.
- AAD 인증. 
...
### <a name="configure-through-azure-portal"></a>Azure Portal를 통해 구성

**Azure Portal** 에서 Azure 웹 pubsub 서비스를 찾습니다. **설정** 로 이동 하 고 허브 이름을 입력 합니다. 그런 다음 **추가** 를 클릭 하 여 서버 쪽 webhook URL을 구성 합니다. 완료 되 면 **저장** 을 클릭 해야 합니다.

:::image type="content" source="media/quickstart-serverless/set-event-handler.png" alt-text="이벤트 처리기를 설정하는 스크린샷.":::

### <a name="configure-through-azure-cli"></a>Azure CLI를 통해 구성

Azure CLI [**az webpubsub hub**](/cli/azure/webpubsub/hub) group 명령을 사용 하 여 이벤트 처리기 설정을 구성 합니다.

명령 | Description
--|--
create | WebPubSub 서비스에 대 한 허브 설정을 만듭니다.
delete | WebPubSub 서비스에 대 한 허브 설정을 삭제 합니다.
list   | WebPubSub 서비스에 대 한 모든 허브 설정을 나열 합니다.
show   | WebPubSub 서비스에 대 한 허브 설정을 표시 합니다.
update | WebPubSub 서비스에 대 한 허브 설정을 업데이트 합니다.

다음은 리소스 허브에 대 한 두 개의 webhook Url을 만드는 예입니다 `MyHub` `MyWebPubSub` .
