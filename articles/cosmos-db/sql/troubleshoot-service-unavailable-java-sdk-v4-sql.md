...
SDK가 Cosmos DB 인스턴스와 통신할 수 없는 경우 다음 예외가 발생합니다. 이는 일반적으로 방화벽 규칙이 요청을 차단하는 것과 같은 일부 보안 프로토콜을 나타냅니다.
...
SDK가 Cosmos DB 계정과 통신할 수 있는지 확인하려면 애플리케이션이 호스트되는 위치에서 다음 명령을 실행합니다. 실패하면 요청을 차단하는 방화벽 규칙 또는 기타 보안 기능을 나타냅니다. 성공하면 SDK가 Cosmos DB 계정과 통신할 수 있어야 합니다.
```
