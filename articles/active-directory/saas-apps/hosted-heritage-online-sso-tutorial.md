---
title: '자습서: Hosted Heritage Online SSO와 Azure AD SSO 통합'
...
ms.locfileid: "132295727"
---
# <a name="tutorial-azure-ad-sso-integration-with-hosted-heritage-online-sso"></a>자습서: Hosted Heritage Online SSO와 Azure AD SSO 통합
...
* Hosted Heritage Online SSO에서 **SP** 시작 SSO를 지원합니다.

## <a name="add-hosted-heritage-online-sso-from-the-gallery"></a>갤러리에서 Hosted Heritage Online SSO 추가
...
## <a name="configure-and-test-azure-ad-sso-for-hosted-heritage-online-sso"></a>Hosted Heritage Online SSO용 Azure AD SSO 구성 및 테스트
...
Hosted Heritage Online SSO에서 Azure AD SSO를 구성하고 테스트하려면 다음 단계를 수행합니다.
...
1. Azure Portal의 **Hosted Heritage Online SSO** 애플리케이션 통합 페이지에서 **관리** 섹션을 찾아 **Single Sign-On** 을 선택합니다.
...
1. **기본 SAML 구성** 섹션에서 다음 단계를 수행합니다.
    ...
    a. **식별자(엔터티 ID)** 텍스트 상자에서 `https://<SUBDOMAIN>.cirqahosting.com/shibboleth` 패턴을 사용하는 URL을 입력합니다.

    b. **로그온 URL** 텍스트 상자에서 `https://<SUBDOMAIN>.cirqahosting.com/Shibboleth.sso/Login` 패턴을 사용하는 URL을 입력합니다.

    > [!NOTE]
    > 이러한 값은 실제 값이 아닙니다. 해당 값을 실제 식별자 및 로그온 URL로 업데이트합니다. 이러한 값을 얻으려면 [Hosted Heritage Online SSO 클라이언트 지원 팀](mailto:support@isoxford.com)에 문의하세요. Azure Portal의 **기본 SAML 구성** 섹션에 표시된 패턴을 참조할 수도 있습니다.
...
* Azure Portal에서 **이 애플리케이션 테스트** 를 클릭합니다. 그러면 로그인 흐름을 시작할 수 있는 Hosted Heritage Online SSO 로그온 URL로 리디렉션됩니다. 

* Hosted Heritage Online SSO 로그온 URL로 직접 이동하여 해당 위치에서 로그인 흐름을 시작합니다.

* Microsoft 내 앱을 사용할 수 있습니다. 내 앱에서 Hosted Heritage Online SSO 타일을 클릭하면 Hosted Heritage Online SSO 로그온 URL로 리디렉션됩니다. 내 앱에 대한 자세한 내용은 [내 앱 소개](../user-help/my-apps-portal-end-user-access.md)를 참조하세요.

## <a name="next-steps"></a>다음 단계

Hosted Heritage Online SSO가 구성되면 세션 제어를 적용하여 조직의 중요한 데이터의 반출 및 반입을 실시간으로 보호할 수 있습니다. 세션 제어는 조건부 액세스에서 확장됩니다. [Microsoft Defender for Cloud Apps를 사용하여 세션 제어를 적용하는 방법을 알아봅니다](/cloud-app-security/proxy-deployment-aad).
