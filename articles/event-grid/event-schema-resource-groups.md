...
리소스 그룹에 대한 이벤트를 구독하면 엔드포인트는 해당 리소스 그룹에 대한 모든 이벤트를 받습니다. 이 이벤트에는 가상 머신 업데이트 같이 확인하려는 이벤트뿐만 아니라 배포 기록에 새 항목 작성 같이 중요하지 않을 수 있는 이벤트도 포함될 수 있습니다. 엔드포인트에서 모든 이벤트를 수신하고, 처리하려는 이벤트를 처리하는 코드를 작성할 수 있습니다. 또는 이벤트 구독을 만들 때 필터를 설정할 수 있습니다.
...
| [Resource Manager 템플릿: 리소스 구독](https://github.com/Azure/azure-quickstart-templates/tree/master/quickstarts/microsoft.eventgrid/event-grid-resource-events-to-webhook) | Azure 구독 또는 리소스 그룹에 대한 이벤트를 구독합니다. WebHook에 이벤트를 보냅니다. |
