---
title: 연속 내보내기를 통해 Microsoft Defender for Cloud의 경고 및 권장 사항을 Log Analytics 작업 영역 또는 Azure Event Hubs
...
ms.locfileid: "132529331"
---
# <a name="continuously-export-microsoft-defender-for-cloud-data"></a>클라우드용 Microsoft Defender 데이터를 지속적으로 내보내기

[!INCLUDE [Banner for top of topics](./includes/banner.md)]

Microsoft Defender for Cloud는 자세한 보안 경고 및 권장 사항을 생성합니다. 관련 내용은 포털에서 또는 프로그래밍 도구를 통해 볼 수 있습니다. 사용자 환경에서 다른 모니터링 도구를 사용해 추적하기 위해서는 이 정보의 일부 또는 전부 내보내야 할 수도 있습니다. 

내보낼  항목과 **연속 내보내기** 를 사용할 *위치를* 완전히 사용자 지정합니다. 예를 들면 다음과 같이 구성할 수 있습니다.
...
이 기능을 *연속이라고* 하더라도 주간 스냅샷을 내보내는 옵션도 있습니다.

이 문서에서는 Log Analytics 작업 영역 또는 Azure Event Hubs로 연속 내보내기를 구성하는 방법을 설명합니다.

> [!NOTE]
> Defender for Cloud를 SIEM과 통합해야 하는 경우 [SIEM, SOAR 또는 IT Service Management 솔루션으로 경고 스트리밍을](export-to-siem.md)참조하세요.

> [!TIP]
> Defender for Cloud는 CSV로 일회성 수동 내보내기를 수행하는 옵션도 제공합니다. [경고 및 권장 사항을 수동으로 한 번 내보내기](#manual-one-time-export-of-alerts-and-recommendations)에서 자세히 알아보세요.
...
제공된 Azure Policy 템플릿을 사용하여 Azure Portal, REST API 통해 또는 대규모로 Microsoft Defender for Cloud 페이지에서 연속 내보내기를 구성할 수 있습니다. 각각에 대한 자세한 내용을 보려면 아래에서 해당하는 탭을 선택하세요.

### <a name="use-the-azure-portal"></a>[**Azure Portal 사용**](#tab/azure-portal)

### <a name="configure-continuous-export-from-the-defender-for-cloud-pages-in-azure-portal"></a>Azure Portal Defender for Cloud 페이지에서 연속 내보내기 구성
...
1. 해당 구독에 대한 설정 페이지의 사이드바에서 **연속 내보내기** 를 선택합니다.
    ...
    :::image type="content" source="./media/continuous-export/continuous-export-options-page.png" alt-text="Microsoft Defender for Cloud의 내보내기 옵션.":::
...
1. 적절한 내보내기 빈도를 선택합니다.
    ...
    - **스냅샷** – 선택한 데이터 형식의 현재 상태에 대한 스냅샷은 구독당 일주일에 한 번 전송됩니다. 스냅샷 데이터를 식별하려면 필드 ``IsSnapshot``을 찾습니다.
...
연속 내보내기 는 Microsoft Defender for Cloud [Automations API를](/rest/api/securitycenter/automations)통해 구성 및 관리될 수 있습니다. 이 API를 사용하여 다음의 가능한 대상으로 내보내기 위한 규칙을 만들거나 업데이트합니다.
...
* **더 큰 볼륨** - API를 사용하여 단일 구독에 여러 내보내기 구성을 만들 수 있습니다. Defender for Cloud 포털 UI의 **연속 내보내기** 페이지는 구독당 하나의 내보내기 구성만 지원합니다.

* **추가 기능** - API는 UI에 표시되지 않은 추가 매개 변수를 제공합니다. 예를 들어 Automation 리소스에 태그를 추가하고, Defender for Cloud 포털 UI의 연속 내보내기 페이지에 제공되는 것보다 광범위한 경고 및 권장 사항 속성 집합을 기반으로 **내보내기를** 정의할 수 있습니다.

* **더 집중된 범위** - API는 내보내기 구성 범위에 보다 세분화된 수준을 제공합니다. API로 내보내기를 정의할 때 리소스 그룹 수준에서 정의할 수 있습니다. Defender for Cloud 포털 UI에서 **연속 내보내기** 페이지를 사용하는 경우 구독 수준에서 정의해야 합니다.

    > [!TIP]
    > API를 사용하여 여러 내보내기 구성을 설정했거나 API 전용 매개 변수를 사용한 경우 이러한 추가 기능은 Defender for Cloud UI에 표시되지 않습니다. 대신 다른 구성이 있음을 알려 주는 배너가 표시됩니다.
...
> [!TIP]
> Azure 리소스 Graph 제한 사항으로 인해 보고서는 13K 행의 파일 크기로 제한됩니다. 너무 많은 데이터를 내보내는 것과 관련된 오류가 표시되는 경우 내보낼 더 작은 구독 집합을 선택하여 출력을 제한해 보세요.
...
예 많은 경고는 고급 보호를 사용하도록 설정한 경우에만 제공됩니다. 내보낸 데이터에서 얻을 경고를 미리 보는 좋은 방법은 Azure Portal Defender for Cloud의 페이지에 표시된 경고를 보는 것입니다.
