---
title: CLI 스크립트 - Azure Database for MySQL - 유연한 서버(미리 보기)의 서버 매개 변수 나열 및 변경
description: 이 Azure CLI 샘플 스크립트에서는 Azure Database for MySQL - 유연한 서버의 서버 매개 변수를 나열하고 변경하는 방법을 보여줍니다.
author: shreyaaithal
ms.author: shaithal
ms.service: mysql
ms.devlang: azurecli
...
ms.locfileid: "128604666"
---
# <a name="list-and-change-server-parameters-of-an-azure-database-for-mysql---flexible-server-preview-using-azure-cli"></a>Azure CLI를 사용하여 Azure Database for MySQL - 유연한 서버(미리 보기)의 서버 매개 변수 나열 및 변경

이 샘플 CLI 스크립트는 사용 가능한 모든 [서버 매개 변수](../concepts-server-parameters.md)와 Azure Database for MySQL - 유연한 서버에 허용되는 값을 나열하고 *max_connections* 및 전역 *time_zone* 매개 변수를 기본값 이외의 값으로 설정합니다.
...
|[az mysql flexible-server parameter list](/cli/azure/mysql/flexible-server/parameter#az_mysql_flexible_server_parameter_list)|유연한 서버의 매개 변수 값을 나열합니다.|
...
|[az mysql flexible-server parameter show](/cli/azure/mysql/flexible-server/parameter#az_mysql_flexible_server_parameter_show)|유연한 서버의 특정 매개 변수 값을 가져옵니다.|
