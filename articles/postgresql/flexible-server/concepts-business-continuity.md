...
- 지진으로 인해 정전이 발생하고 임시로 데이터 센터 또는 가용성 영역을 사용하지 않도록 설정합니다.
...
| **지역 중복 백업** | 유연한 서버 백업은 원격 지역에 복사됩니다. 주 서버 지역의 가동이 중단된 경우 재해 복구 상황에 도움이 되는 입니다. | 이 기능은 현재 선택한 지역에서 사용하도록 설정되어 있습니다. 복원할 데이터의 크기와 수행할 복구 양에 따라 더 긴 RTO 및 더 높은 RPO가 걸립니다.  |
