---
title: L4S5I-IOT01A to Azure IoT Central 빠른 시작 커넥트
description: Azure rtos embedded 소프트웨어를 사용 하 여 L4S5I-IOT01A 장치를 Azure IoT에 연결 하 고 원격 분석을 보냅니다.
author: timlt
ms.author: timlt
...
ms.openlocfilehash: fbfb06554ea76040a8374b36ac2c1679bd5eba63
ms.sourcegitcommit: 93c7420c00141af83ed3294923b4826dd4dc6ff2
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/02/2021
ms.locfileid: "133440594"
---
# <a name="quickstart-connect-an-stmicroelectronics-b-l4s5i-iot01a-discovery-kit-to-iot-central"></a>빠른 시작: IoT Central L4S5I-IOT01A Discovery 키트를 커넥트 합니다.

**적용 대상**: [임베디드 디바이스 개발](about-iot-develop.md#embedded-device-development)<br>
**총 완료 시간**: 30분

[![코드 찾아보기](media/common/browse-code.svg)](https://github.com/azure-rtos/getting-started/tree/master/STMicroelectronics/B-L4S5I-IOT01A)

이 빠른 시작에서는 Azure RTOS를 사용 하 여 [L4S5I IOT01A](https://www.st.com/en/evaluation-tools/b-l4S5i-iot01a.html) Discovery 키트 () Azure IoT를 연결 합니다.
...
* 하드웨어
    ...
    * [B-L4S5I-IOT01A](https://www.st.com/en/evaluation-tools/b-l4s5i-iot01a.html) (STM devkit)
    * Wi-Fi 2.4GHz
...
1. STM DevKit MCU에서 **다시 설정** 단추, **USB STLink** 라는 레이블이 지정된 마이크로 USB 포트, 보드 부품 번호를 찾습니다. 이러한 항목은 다음 단계에서 참조합니다. 다음 그림에는 이러한 구성 요소가 모두 강조 표시되어 있습니다.
    ...
    :::image type="content" source="media/quickstart-devkit-stm-b-l4s5i/stm-devkit-board.jpg" alt-text="STM DevKit 보드에서 주요 구성 요소 찾기":::

1. 마이크로 USB 케이블을 STM DevKit의 **USB STLINK** 포트에 연결한 다음, 컴퓨터에 연결합니다.

    > [!NOTE]
    > STM DevKit에 대 한 자세한 설치 정보는 패키지에 대 한 지침을 참조 하거나 [B-L4S5I-IOT01A Resources](https://www.st.com/en/evaluation-tools/b-l4s5i-iot01a.html#resource) 를 참조 하세요.

1. 파일 탐색기에서 이전 섹션에서 만든 이진 파일을 찾습니다.

1. 파일이 stm32l4s5_azure_iot 파일의 이진 파일을 복사 합니다 *.*
...
1. **직렬 포트 설정** 대화 상자에서 다음 설정을 확인하고, 필요한 경우 업데이트합니다.
    ...
    * **포트**: STM DevKit가 연결된 포트입니다. 드롭다운에 여러 포트 옵션이 있는 경우 사용할 올바른 포트를 찾을 수 있습니다. Windows **디바이스 관리자** 를 열고, **포트** 를 확인하여 사용할 포트를 식별합니다.
    ...
    :::image type="content" source="media/quickstart-devkit-stm-b-l4s5i/termite-settings.png" alt-text="Termite 앱의 직렬 포트 설정 스크린샷":::
...
1. **디바이스 템플릿** 이 **시작 가이드** 로 업데이트되었는지 확인합니다.
    ...
    :::image type="content" source="media/quickstart-devkit-stm-b-l4s5i/iot-central-device-view-status.png" alt-text="IoT Central의 디바이스 상태 스크린샷":::
...
1. **개요** 탭에서 디바이스에서 메시지를 클라우드에 보낼 때 원격 분석을 봅니다.
    ...
    :::image type="content" source="media/quickstart-devkit-stm-b-l4s5i/iot-central-device-telemetry.png" alt-text="IoT Central의 디바이스 원격 분석 스크린샷":::
...
1. **상태** 드롭다운에서 **True** 를 선택한 다음, **실행** 을 선택합니다.  LED 표시등이 켜집니다.
    ...
    :::image type="content" source="media/quickstart-devkit-stm-b-l4s5i/iot-central-invoke-method.png" alt-text="IoT Central의 디바이스에서 직접 메서드를 호출하는 스크린샷":::
...
:::image type="content" source="media/quickstart-devkit-stm-b-l4s5i/iot-central-device-about.png" alt-text="IoT Central의 디바이스 정보 스크린샷":::
