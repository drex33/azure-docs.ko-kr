---
title: Azure Arc 지원 서버를 사용하여 Azure 리소스에 대해 인증
description: 이 문서에서는 Azure Arc 지원 서버에 대한 Azure Instance Metadata Service 지원 및 비밀을 사용하여 Azure 리소스 및 로컬에 대해 인증하는 방법을 설명합니다.
ms.topic: conceptual
ms.date: 11/08/2021
...
ms.locfileid: "132400170"
---
# <a name="authenticate-against-azure-resources-with-azure-arc-enabled-servers"></a>Azure Arc 지원 서버를 사용하여 Azure 리소스에 대해 인증

Azure Arc 지원 서버에서 직접 실행되는 애플리케이션 또는 프로세스는 관리 ID를 사용하여 Azure Active Directory 기반 인증을 지원하는 다른 Azure 리소스에 액세스할 수 있습니다. 애플리케이션은 해당 ID를 나타내는 [액세스 토큰을](../../active-directory/develop/developer-glossary.md#access-token) 가져올 수 있습니다. 이 토큰은 Azure Arc 지원 서버에 대해 시스템이 할당하고 이를 'bearer' 토큰으로 사용하여 다른 서비스에 인증합니다.

[관리 ID에](../../active-directory/managed-identities-azure-resources/overview.md) 대한 자세한 설명은 관리 ID 개요 설명서를 참조하고 시스템 할당 ID와 사용자 할당 ID 간의 차이점을 이해합니다.
...
- 관리 ID에 대한 이해.
- Windows 로컬 **관리자** 그룹 또는 **하이브리드 에이전트 확장 애플리케이션** 그룹의 구성원이어야 합니다.
- Linux에서는 **himds** 그룹의 구성원이어야 합니다.
- Azure Arc 지원 서버에 연결되고 등록된 서버입니다.
- 필요한 리소스 만들기 및 역할 관리 단계를 수행하기 위해 구독 또는 리소스 그룹에서 [소유자](../../role-based-access-control/built-in-roles.md#owner) 그룹의 구성원입니다.
- 자격 증명을 저장 및 검색하고 Azure Arc ID 액세스를 KeyVault에 할당하는 Azure Key Vault.
...
Azure Arc 지원 Windows 서버의 경우 PowerShell을 사용하여 웹 요청을 호출하여 특정 포트의 로컬 호스트에서 토큰을 가져옵니다. IP 주소 또는 환경 변수 **IDENTITY_ENDPOINT** 를 사용하여 요청을 지정합니다.
...
Azure Arc 지원 Linux 서버의 경우 Bash를 사용하여 웹 요청을 호출하여 특정 포트의 로컬 호스트에서 토큰을 가져옵니다. IP 주소 또는 환경 변수 **IDENTITY_ENDPOINT** 를 사용하여 다음 요청을 지정합니다. 이 단계를 완료하려면 SSH 클라이언트가 필요합니다.
