---
title: '자습서: Power BI를 핵심 구 추출과 통합'
titleSuffix: Azure Cognitive Services
description: 핵심 구 추출 기능을 사용하여 Power BI에 저장된 텍스트를 가져오는 방법을 알아봅니다.
...
ms.locfileid: "131100849"
---
# <a name="tutorial-extract-key-phrases-from-text-stored-in-power-bi"></a>자습서: Power BI에 저장된 텍스트에서 핵심 구 추출

Microsoft Power BI Desktop은 데이터에 연결하고, 데이터를 변환 및 시각화할 수 있는 무료 애플리케이션입니다. Azure Cognitive Service for Language의 기능 중 하나인 핵심 구 추출은 자연어 처리를 제공합니다. 원시 비구조 텍스트가 주어지면 이 서비스는 가장 중요한 구를 추출하고, 감정을 분석하고, 브랜드와 같은 잘 알려진 엔터티를 식별할 수 있습니다. 이러한 도구를 함께 사용하면 고객이 말하는 제품이 무엇인지, 그 제품에 대해 어떻게 생각하는지 빨리 파악할 수 있습니다.
...
> * Power BI Desktop을 Azure Cognitive Service for Language의 핵심 구 추출 기능과 통합
...
- 언어 리소스입니다. 계정이 없으면 [만들 수 있습니다](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesTextAnalytics).
- 등록하는 동안 생성된 [언어 리소스 키](../../../cognitive-services-apis-create-account.md#get-the-keys-for-your-resource)입니다.
...
핵심 구 추출에서 처리할 준비가 되기 전에 Power BI Desktop에서 데이터를 변환해야 할 수 있습니다.
...
[핵심 구 추출](https://westus.dev.cognitive.microsoft.com/docs/services/TextAnalytics-V3-1/operations/KeyPhrases)은 HTTP 요청당 최대 1,000개의 텍스트 문서를 처리할 수 있습니다. Power BI는 레코드를 한 번에 하나씩 처리하는 것을 선호하므로 이 자습서에서 API 호출에는 각각 단일 문서만 포함됩니다. 핵심 구 API는 처리할 각 문서에 대한 다음 필드가 필요합니다.
...
| `text`  | 처리할 텍스트입니다. 이 필드의 값은 [이전 섹션](#prepare-the-data)에서 만든 `Merged` 열에서 가져오며, 결합된 제목 줄과 주석 텍스트를 포함합니다. 핵심 구 API에서는 이 데이터가 약 5,120자 이하여야 합니다.|
...
> [!NOTE]
> Power BI Desktop 사용자 지정 함수는 [파워 쿼리 M 수식 언어](/powerquery-m/power-query-m-reference) 또는 줄여서 "M"으로 작성됩니다. M은 [F#](/dotnet/fsharp/) 기반의 함수 프로그래밍 언어입니다. 하지만 프로그래머가 아니어도 이 자습서를 완료할 수 있도록 아래에 코드가 포함되어 있습니다.
...
> [!NOTE]
> 아래 예시 엔드포인트(`<your-custom-subdomain>` 포함)를 언어 리소스에 대해 생성된 엔드포인트로 바꿉니다. [Azure Portal](https://azure.microsoft.com/features/azure-portal/)에 로그인하고 리소스로 이동한 다음 **키 및 엔드포인트** 를 선택하여 이 엔드포인트를 찾을 수 있습니다.
...
`YOUR_API_KEY_HERE`을 언어 리소스 키로 바꿉니다. [Azure Portal](https://azure.microsoft.com/features/azure-portal/)에 로그인하고 언어 리소스로 이동한 다음 **키 및 엔드포인트** 페이지를 선택하여 이 키를 찾을 수도 있습니다. 키 앞뒤에 있는 따옴표를 그대로 두어야 합니다. **완료** 를 클릭합니다.
...
대화 상자에 새 필드 **텍스트(선택 사항)** 가 표시됩니다. 이 필드에는 핵심 구 API의 `text` 매개 변수 값을 제공하는 데 사용할 열을 입력해야 합니다. (`language` 및 `id` 매개 변수의 값은 이미 하드 코드했습니다.) 드롭다운 메뉴에서 `Merged`([이전에](#prepare-the-data) 제목 및 메시지 필드를 병합하여 만든 열)를 선택합니다.
...
> [!NOTE]
> 핵심 구 추출이 액세스 키를 사용하여 요청을 인증하므로 Power BI가 HTTP 요청 자체에 대한 자격 증명을 제공할 필요가 없으므로 `Anonymous`를 선택합니다.

> [!div class="mx-imgBorder"]
> ![익명으로 인증 설정](../media/tutorials/power-bi/access-web-content.png)

익명 액세스를 선택한 후에도 자격 증명 편집 배너가 표시되면 언어 리소스 키를 `KeyPhrases` [사용자 지정 함수](#create-a-custom-function)의 코드에 붙여넣는 것을 잊었을 수 있습니다.
...
## <a name="using-other-features"></a>다른 기능 사용

Azure Cognitive Service for Language는 감정 분석 및 언어 탐지도 제공합니다. 언어 감지는 고객 피드백의 일부가 영어가 아닌 경우에 특히 유용합니다.
...
Azure Cognitive Service for Language, 파워 쿼리 M 수식 언어 또는 Power BI에 대해 자세히 알아봅니다.
