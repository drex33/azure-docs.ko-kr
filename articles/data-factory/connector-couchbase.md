---
title: 에서 데이터 복사 (미리 보기)
description: Azure Data Factory 또는 Synapse Analytics 파이프라인에서 복사 작업을 사용 하 여에서 지원 되는 싱크 데이터 저장소로 데이터를 복사 하는 방법에 대해 알아봅니다.
...
이 문서에서는 Azure Data Factory 또는 Synapse Analytics 파이프라인에서 복사 작업을 사용 하 여가는에서 데이터를 복사 하는 방법을 설명 합니다. 이 문서는 복사 작업에 대한 일반적인 개요를 제공하는 [복사 작업 개요](copy-activity-overview.md) 문서를 기반으로 합니다.
...
## <a name="create-a-linked-service-to-couchbase-using-ui"></a>UI를 사용하여 Couchbase에 연결된 서비스 만들기

다음 단계를 사용하여 Azure Portal UI에서 Couchbase에 연결된 서비스를 만듭니다.
...
2. Couchbase를 검색하고 Couchbase 커넥터를 선택합니다.

   :::image type="content" source="media/connector-couchbase/couchbase-connector.png" alt-text="Couchbase 커넥터의 스크린샷.":::    


1. 서비스 세부 정보를 구성하고 연결을 테스트하고 새 연결된 서비스를 만듭니다.

   :::image type="content" source="media/connector-couchbase/configure-couchbase-linked-service.png" alt-text="Couchbase의 연결된 서비스 구성 스크린샷.":::
