---
title: Azure Site Recovery를 사용하여 Hyper-V 재해 복구 문제 해결
...
5. 게스트 VM에서 WMI가 사용되도록 설정되어 있고 액세스 가능한지 확인합니다.
   - 기본 WMI 테스트에 대해 [자세히 알아봅니다](https://techcommunity.microsoft.com/t5/ask-the-performance-team/bg-p/AskPerf).
   - WMI [문제를 해결합니다](/windows/win32/wmisdk/wmi-troubleshooting).
   - WMI 스크립트와 서비스 관련 [문제를 해결합니다](/previous-versions/tn-archive/ff406382(v=msdn.10)#H22).
6. 게스트 VM에 최신 버전의 Integration Services가 실행되고 있는지 확인합니다.
    - 최신 버전이 있는지 [확인합니다](/windows-server/virtualization/hyper-v/manage/manage-hyper-v-integration-services).
    - Integration Services를 최신 상태로 [유지합니다](/windows-server/virtualization/hyper-v/manage/manage-hyper-v-integration-services#keep-integration-services-up-to-date).

### <a name="cannot-enable-protection-as-the-virtual-machine-is-not-highly-available-error-code-70094"></a>가상 머신을 항상 사용할 수 있는 것은 아니므로 보호를 사용하도록 설정할 수 없습니다(오류 코드 70094).

머신에 대해 복제를 사용하도록 설정할 때 머신을 항상 사용할 수 있는 것은 아니므로 복제를 사용하도록 설정할 수 없다는 오류가 발생합니다. 이 문제를 해결하려면 아래 단계를 수행합니다.

- VMM 서버에서 VMM 서비스를 다시 시작합니다.
- 클러스터에서 가상 머신을 제거하고 다시 추가합니다.

### <a name="the-vss-writer-ntds-failed-with-status-11-and-writer-specific-failure-code-0x800423f4"></a>VSS 기록기 NTDS가 상태 11 및 기록기 관련 오류 코드 0x800423F4로 인해 실패했습니다.

복제를 사용하도록 설정할 때 NTDS가 실패함에 따라 복제 설정에 실패했음을 알리는 오류가 발생할 수 있습니다. 이 문제가 발생하는 원인 중 하나는 가상 머신 운영 체제가 Windows Server 2012 R2가 아닌 Windows Server 2012에 있기 때문입니다. 이 문제를 해결하려면 아래 단계를 수행합니다.

- 4072650이 적용된 Windows Server R2로 업그레이드합니다.
- Hyper-V 호스트도 Windows 2016 이상이어야 합니다.
...
4. [데이터 변동 제한 사항](hyper-v-deployment-planner-analyze-report.md#azure-site-recovery-limits)을 확인합니다. VM에서 높은 데이터 변동이 확인되면 다음을 수행합니다.
   ...
   - [이 단계](https://techcommunity.microsoft.com/t5/virtualization/bg-p/Virtualization)에 따라 변동의 원본을 조사합니다.
...
## <a name="app-consistent-snapshot-issues"></a>앱 일치 스냅샷 문제
...
8. Linux 기반 서버를 사용하는 경우 앱에서 일관성을 사용하도록 설정했는지 확인합니다. [자세한 정보](./site-recovery-faq.yml)
...
2. VM에 대한 VSS 스냅샷을 생성하려면 VM에서 Hyper-V Integration Services가 설치되어 있는지와 Backup(VSS) Integration Services가 사용되도록 설정되어 있는지 확인합니다.
    ...
    - **Get-VMIntegrationService -VMName\<VMName>-Name VSS** 명령을 사용하여 Hyper-V 호스트의 관리자 권한 PowerShell 세션에서 이를 확인할 수 있습니다. 게스트 VM에 로그인하여 이 정보를 가져올 수도 있습니다. [자세히 알아보기](/windows-server/virtualization/hyper-v/manage/manage-hyper-v-integration-services).
