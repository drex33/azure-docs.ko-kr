...
Azure NetApp Files 볼륨은 Microsoft Azure Virtual Network 내에서 [위임된 서브넷](../virtual-network/virtual-network-manage-subnet.md)이라는 특수 용도의 서브넷에 포함되도록 설계되었습니다. 따라서 VNet 피어링을 통해 Azure 내에서 직접 또는 필요에 따라 Virtual Network Gateway(ExpressRoute 또는 VPN Gateway)를 통해 온-프레미스에서 볼륨에 액세스할 수 있습니다. 서브넷은 Azure NetApp Files 전용이며 인터넷에 연결되지 않습니다. 

## <a name="configurable-network-features"></a>구성 가능한 네트워크 기능  

 Azure NetApp Files 대한 [**표준 네트워크 기능**](configure-network-features.md) 구성은 공개 미리 보기에서 사용할 수 있습니다. 구독에 이 기능을 등록한 후 지원되는 지역에서 *표준* 또는 *기본* 네트워크 기능을 선택하여 새 볼륨을 만들 수 있습니다. 표준 네트워크 기능이 지원되지 않는 지역에서 볼륨은 기본 네트워크 기능을 사용하는 것으로 기본 설정됩니다.  

* ***Standard***  
    이 설정을 선택하면 위임된 서브넷의 네트워크 보안 [그룹](../virtual-network/network-security-groups-overview.md) 및 [사용자 정의 경로와](../virtual-network/virtual-networks-udr-overview.md#user-defined) 같은 더 높은 IP 제한 및 표준 VNet 기능과 이 문서에 표시된 추가 연결 패턴을 사용할 수 있습니다.

* ***기본***  
    이 설정을 선택하면 고려 사항 섹션에 설명된 대로 선택적 연결 패턴 및 제한된 IP 크기 [조정이](#considerations) 가능합니다. 모든 [제약 조건이](#constraints) 이 설정에 적용됩니다. 
...
다음 표에서는 각 네트워크 기능 구성에 대해 지원되는 기능에 대해 설명합니다.

|      기능     |      표준 네트워크 기능     |      기본 네트워크 기능     |
|---|---|---|
|     Azure NetApp Files VNet에서 사용 중이던 IP 수(즉시 피어된 VNet 포함)    |     [VM으로 표준 제한](../azure-resource-manager/management/azure-subscription-service-limits.md#azure-resource-manager-virtual-networking-limits)    |     1000    |
|     VNet당 ANF 위임 서브넷    |     1    |     1    |
|     Azure NetApp Files 위임된 서브넷의 NSG(네트워크 [보안 그룹)](../virtual-network/network-security-groups-overview.md)    |     Yes    |     아니요    |
|     Azure NetApp Files 위임된 서브넷의 UDR(사용자 [정의 경로)](../virtual-network/virtual-networks-udr-overview.md#user-defined)    |     Yes    |     아니요    |
|     [프라이빗 엔드포인트에 연결](../private-link/private-endpoint-overview.md)    |     아니요    |     아니요    |
|     [서비스 엔드포인트에 연결](../virtual-network/virtual-network-service-endpoints-overview.md)    |     아니요    |     아니요    |
|     Azure NetApp Files 인터페이스의 Azure 정책(예: 사용자 지정 명명 정책)    |     아니요    |     아니요    |
|     Azure NetApp Files 트래픽에 대한 부하 분산    |     아니요    |     아니요    |
|     이중 스택(IPv4 및 IPv6) VNet    |     No <br> (IPv4만 지원됨)    |     No <br> (IPv4만 지원됨)    |

### <a name="supported-network-topologies"></a>지원되는 네트워크 토폴로지

다음 표에서는 각 네트워크 기능 구성에서 지원하는 네트워크 토폴로지 Azure NetApp Files 설명합니다. 

|      토폴로지     |      표준 네트워크 기능     |      기본 네트워크 기능     |
|---|---|---|
|     로컬 VNet의 볼륨에 연결    |     예    |     예    |
|     피어링된 VNet의 볼륨에 연결(동일한 지역)    |     예    |     예    |
|     피어링된 VNet의 볼륨에 연결(지역 간 또는 글로벌 피어링)    |     아니요    |     아니요    |
|     ExpressRoute 게이트웨이를 통해 볼륨에 연결    |     예    |     예    |
|     ExpressRoute(ER) FastPath    |     Yes    |     아니요    |
|     ExpressRoute 게이트웨이를 통해 온-프레미스에서 스포크 VNet의 볼륨으로 연결 및 게이트웨이 전송을 사용하여 VNet 피어링    |     예    |     예    |
|     VPN Gateway를 통해 온-프레미스에서 스포크 VNet의 볼륨으로 연결    |     예    |     예    |
|     VPN Gateway를 통해 온-프레미스에서 스포크 VNet의 볼륨으로 연결 및 게이트웨이 전송을 사용하여 VNet 피어링    |     예    |     예    |
|     활성/수동 VPN 게이트웨이를 통해 연결    |     예    |     예    |
|     활성/활성 VPN 게이트웨이를 통해 연결    |     Yes    |     아니요    |
|     활성/활성 영역 중복 게이트웨이를 통해 연결    |     아니요    |     아니요    |
|     Virtual WAN 통해 연결(VWAN)    |     아니요    |     아니요    |
...
UDR(사용자 정의 경로) 및 NSG(네트워크 보안 그룹)는 표준 네트워크 기능을 사용하여 하나 이상의 볼륨을 만든 Azure NetApp Files 위임된 서브넷에서만 지원됩니다.  

> [!NOTE]
> 네트워크 인터페이스 수준의 NSG 연결은 Azure NetApp Files 네트워크 인터페이스에서 지원되지 않습니다. 

서브넷에 표준 및 기본 네트워크 기능(또는 기능 미리 보기에 등록되지 않은 기존 볼륨의 경우)이 조합된 경우 위임된 서브넷에 적용된 UDR 및 NSG는 표준 네트워크 기능이 있는 볼륨에만 적용됩니다.

위임된 서브넷 및 다음 홉의 주소 접두사를 사용하여 원본 VM 서브넷에서 UDR(사용자 정의 경로)을 NVA로 구성하는 것은 기본 네트워크 기능을 가진 볼륨에서 지원되지 않습니다. 이러한 설정으로 인해 연결 문제가 발생합니다.
...
* 온-프레미스 리소스 VM 1 및 VM 2는 사이트간 VPN 및 지역 VNet 피어링을 통해 볼륨 2 또는 볼륨 3에 연결할 수 있습니다.
...
* [Azure NetApp Files 볼륨에 대한 네트워크 기능 구성](configure-network-features.md) 
