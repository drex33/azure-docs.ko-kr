---
title: SQL IaaS 확장에 등록(Windows)
description: SQL IaaS 에이전트 확장을 사용하여 Windows Azure VM에 SQL Server 등록하여 Azure 기능을 사용하도록 설정하고 규정 준수 및 관리 편의성을 개선하는 방법을 알아봅니다.
...
ms.locfileid: "131044022"
---
# <a name="register-windows-sql-server-vm-with-sql-iaas-extension"></a>SQL IaaS 확장에 Windows SQL Server VM 등록
...
SQL [IaaS 에이전트 확장에 SQL Server](sql-server-iaas-agent-extension-automate-management.md) VM을 등록하여 Windows Azure VM에서 SQL Server 다양한 기능 이점을 얻을 수 있습니다. 

이 문서에서는 SQL IaaS Agent 확장에 단일 SQL Server VM를 등록하는 방법을 설명합니다. 또는 스크립트를 사용하여 구독의 모든 SQL Server VM을 [자동으로](sql-agent-extension-automatic-registration-all-vms.md) 등록하거나 [여러 VM을 대량으로](sql-agent-extension-manually-register-vms-bulk.md)등록할 수 있습니다.
...
SQL IaaS Agent 확장을 활용하려면 [**Microsoft.SqlVirtualMachine** 공급자에 구독을 등록](#register-subscription-with-rp)하여 해당 구독 내에서 리소스를 만들 수 있는 권한을 SQL IaaS 확장에 부여해야 합니다. 그런 다음 SQL Server VM을 확장에 등록할 수 있습니다. 
...
Azure CLI 및 Azure PowerShell 사용하여 SQL Server VM을 전체 모드로 직접 등록하거나 Azure Portal, Azure CLI 또는 Azure PowerShell 사용하여 경량 모드에서 전체 모드로 업그레이드할 수 있습니다. OS가 Windows 2008 R2 이상으로 업그레이드될 _때까지는 에이전트 없음_ 모드에서 VM 업그레이드가 지원되지 않습니다.  

2021년 9월부터 SQL Server VM을 전체 모드로 등록하면 더 이상 SQL Server 서비스를 다시 시작할 필요가 없습니다. 

전체 모드에 대해 자세히 알아보려면 [관리 모드](sql-server-iaas-agent-extension-automate-management.md#management-modes)를 참조하세요.

### <a name="register-in-full-mode"></a>전체 모드로 등록
...
Azure CLI SQL Server VM을 전체 모드로 등록합니다.
...
Azure PowerShell SQL Server VM을 FULL 모드로 등록합니다.
...
Azure Portal 사용하여 확장을 전체 모드로 업그레이드합니다.
...
Azure CLI 사용하여 확장을 전체 모드로 업그레이드합니다.
...
Azure PowerShell 사용하여 확장을 전체 모드로 업그레이드합니다. 
...
여러 인스턴스가 있는 장애 조치(failover) 클러스터 인스턴스 및 SQL Server VM은 경량 모드에서 SQL IaaS 에이전트 확장에만 등록할 수 있습니다.
...
## <a name="noagent-mode"></a>에이전트 없음 모드
...
Azure CLI 또는 Azure PowerShell 사용하여 Windows Server 2008 `SQL2008-WS2008` VM에서 SQL Server 2008( ) 또는 `SQL2008R2-WS2008` 2008 R2( ) 인스턴스를 등록합니다. 
...
## <a name="check-management-mode"></a>관리 모드 확인

Azure PowerShell을 사용하여 SQL Server IaaS 에이전트 확장이 있는 관리 모드를 확인합니다. 

Azure PowerShell 를 통해 확장 모드를 확인합니다.  
...
Azure Portal 사용하여 확장을 복구하려면 다음을 수행합니다.  
...
SQL Server VM을 SQL IaaS Agent 확장에서 등록 취소하려면 Azure Portal 또는 Azure CLI를 사용하여 SQL 가상 머신 *리소스* 를 삭제합니다. SQL 가상 머신 *리소스* 를 삭제해도 SQL Server VM은 삭제되지 않습니다. 관리 모드를 전체 모드에서 다운그레이드하려면 SQL 가상 머신을 SQL IaaS Agent 확장에서 등록 취소해야 합니다.

>[!CAUTION]
> 확장에서 SQL Server VM의 등록을 취소할 **때는 매우 주의해야** 합니다. *리소스를* 제거하려고 할 때 **실수로 가상 머신을 삭제할 수** 있으므로 단계를 신중하게 수행합니다.


### <a name="azure-portal"></a>Azure Portal

Azure Portal 사용하여 확장에서 SQL Server VM 등록을 취소합니다.
...
Azure CLI 사용하여 확장에서 SQL Server VM의 등록을 취소하려면 [az sql vm delete](/cli/azure/sql/vm#az_sql_vm_delete) 명령을 사용합니다. 이렇게 하면 SQL Server VM *리소스가* 제거되지만 가상 머신은 삭제되지 않습니다.

Azure CLI SQL Server VM의 등록을 취소하려면 다음을 수행합니다. 
...
Azure PowerShell 사용하여 확장에서 SQL Server VM의 등록을 취소하려면 [Remove-AzSqlVM](/powershell/module/az.sqlvirtualmachine/remove-azsqlvm) 명령을 사용합니다. 이렇게 하면 SQL Server VM *리소스가* 제거되지만 가상 머신은 삭제되지 않습니다.

SQL Server VM의 등록을 취소하려면 Azure PowerShell. 
...
* [Azure VM의 SQL Server 대한 가격 책정 지침](../windows/pricing-guidance.md)
* [Azure VM의 SQL Server 새로운 내용](../windows/doc-changes-updates-release-notes-whats-new.md)
