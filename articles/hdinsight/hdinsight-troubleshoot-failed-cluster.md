---
title: Azure HDInsight 클러스터에서 속도가 느리거나 오류가 발생하는 작업 문제 해결
description: Azure HDInsight 클러스터에서 속도가 느리거나 오류가 발생하는 작업을 진단하고 문제를 해결합니다.
...
HDInsight 클러스터에서 데이터를 처리하는 애플리케이션이 느리게 실행되거나 오류 코드와 함께 실패하는 경우 여러 가지 문제 해결 옵션이 있습니다. 작업 실행 시간이 예상보다 오래 걸리거나 응답 시간이 대체적으로 느려진 경우 클러스터가 실행되는 서비스처럼 클러스터에서 위로 올라가는 업스트림 오류가 있을 수 있습니다. 그러나 이러한 성능 저하의 가장 일반적인 원인은 부족한 크기 조정입니다. 새 HDInsight 클러스터를 만들 때 적절한 [가상 머신 크기](hdinsight-supported-node-configuration.md)를 선택해야 합니다.
...
:::image type="content" source="./media/hdinsight-troubleshoot-failed-cluster/hdi-azure-portal-info.png" alt-text="HDInsight Azure Portal 정보":::

[Azure CLI](/cli/azure/)를 사용할 수도 있습니다.
...
:::image type="content" source="./media/hdinsight-troubleshoot-failed-cluster/apache-ambari-overview.png" alt-text="Apache Ambari 대시보드 개요":::
...
이 코드는 게이트웨이 노드에서 반환하는 일반적인 메시지이며 가장 일반적인 오류 상태 코드입니다. 이 코드가 표시되는 이유 중 하나는 활성 헤드 노드에서 WebHCat 서비스가 중지되는 것입니다. 이 가능성을 확인하려면 다음 CURL 명령을 사용합니다.
...
:::image type="content" source="./media/hdinsight-troubleshoot-failed-cluster/restart-webhcat-server.png" alt-text="Apache Ambari WebHCat 서버 다시 시작":::
...
1. YARN 작업 제출이 오래 걸려 시간 제한이 발생할 수 있습니다.
    ...
    :::image type="content" source="./media/hdinsight-troubleshoot-failed-cluster/hdi-job-launcher-queue.png" alt-text="HDInsight 작업 시작 관리자 큐 보기":::
...
:::image type="content" source="./media/hdinsight-troubleshoot-failed-cluster/ambari-stack-versions.png" alt-text="Apache Ambari 스택 및 버전":::
...
:::image type="content" source="./media/hdinsight-troubleshoot-failed-cluster/apache-ambari-quick-links.png" alt-text="로그 파일에 대한 Ambari 빠른 링크":::
...
* [Apache Hadoop YARN 애플리케이션에 액세스 Linux 기반 HDInsight에 로그인](hdinsight-hadoop-access-yarn-app-logs-linux.md)
