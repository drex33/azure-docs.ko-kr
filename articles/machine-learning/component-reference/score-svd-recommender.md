---
title: '.SVD 추천 점수 매기기: 구성 요소 참조'
titleSuffix: Azure Machine Learning
description: Azure Machine Learning에서 점수 .svd 추천 구성 요소를 사용 하 여 데이터 집합에 대 한 추천 예측 점수를 매기는 방법을 알아봅니다.
...
ms.locfileid: "131570107"
---
# <a name="score-svd-recommender"></a>SVD 추천 점수 매기기

이 문서에서는 Azure Machine Learning 디자이너에서 점수 .svd 추천 구성 요소를 사용 하는 방법을 설명 합니다. 이 구성 요소를 사용 하 여 .SVD (단일 값 분해) 알고리즘을 기반으로 학습 된 추천 모델을 사용 하 여 예측을 만들 수 있습니다.
...
이 구성 요소는 요구 사항이 서로 다른 두 가지 유형의 예측을 지원 합니다. 
...
1. 파이프라인에 학습된 추천 모델을 추가하고 이를 **학습된 SVD 추천** 에 연결합니다. [학습 .Svd 추천](train-SVD-recommender.md) 구성 요소를 사용 하 여 모델을 만들어야 합니다.
...
3. **권장 항목 선택을** 위해 프로덕션에서 점수 매기기 구성 요소를 사용 하 고 있는지 또는 모델 평가에 사용 하 고 있는지를 나타냅니다. 다음 값 중 하나를 선택합니다.

    - **모든 항목에서**: 웹 서비스 또는 프로덕션에서 사용할 파이프라인을 설정하는 경우 이 옵션을 선택합니다.  이 옵션은 *프로덕션 모드* 를 사용하도록 설정합니다. 구성 요소는 학습 중에 표시 되는 모든 항목에서 권장 사항을 만듭니다.

    - **등급이 지정된 항목에서(모델 평가의 경우)** : 모델을 개발하거나 테스트하는 경우 이 옵션을 선택합니다. 이 옵션은 *평가 모드* 를 사용하도록 설정합니다. 구성 요소는 입력 데이터 집합에서 등급이 지정 된 항목 에서만 권장 사항을 만듭니다.
    
    - 등급이 지정 되지 않은 **항목에서 (사용자에 게 새 항목을 제안 하기 위해)**: 구성 요소가 등급을 지정 하지 않은 학습 데이터 집합에 있는 해당 항목 에서만 권장 사항을 만들도록 하려면이 옵션을 선택 합니다. 
...
5. **사용자에게 추천할 최대 항목 수**: 각 사용자에 대해 반환할 항목의 수를 입력합니다. 기본적으로 구성 요소는 5 개 항목을 추천 합니다.
