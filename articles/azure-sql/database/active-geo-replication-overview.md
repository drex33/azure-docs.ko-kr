...
ms.reviewer: mathoma
ms.date: 10/19/2021
ms.openlocfilehash: 29cdc4d4e78cf76631fdc2c28228144694d6ad3b
ms.sourcegitcommit: 692382974e1ac868a2672b67af2d33e593c91d60
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 10/22/2021
ms.locfileid: "130260724"
---
...
활성 지역 복제는 [Always On 가용성 그룹](/sql/database-engine/availability-groups/windows/overview-of-always-on-availability-groups-sql-server) 기술을 활용하여 주 복제본에서 생성된 트랜잭션 로그를 모든 지역 복제본에 비동기적으로 복제합니다. 특정 시점에 보조 데이터베이스는 주 데이터베이스 뒤에 약간 뒤에 있을 수 있지만 보조 데이터베이스의 데이터는 트랜잭션 일관성이 보장됩니다. 즉, 커밋되지 않은 트랜잭션에서 변경한 내용은 표시되지 않습니다. 
...
- **계획된 지역 장애 조치(failover)**
  ...
  - 중단이 완화된 후 주 지역으로 데이터베이스를 반환합니다(장애 복구(failback)라고 함).

- **계획되지 않은 지역 장애 조치(failover)**

  계획되지 않거나 강제적인 지역 장애 조치(failover)는 주 데이터베이스와의 동기화 없이 지역 보조 데이터베이스를 주 역할로 즉시 전환합니다. 주 에서 커밋되었지만 보조에 아직 복제되지 않은 트랜잭션은 모두 손실됩니다. 이 작업은 주 데이터베이스에 액세스할 수 없는 가동 중단 시 복구 방법으로 설계되었지만 데이터베이스 가용성을 신속하게 복원해야 합니다. 원래 주 주 데이터가 다시 온라인 상태가 되면 자동으로 다시 연결되고, 현재 주 데이터를 사용하여 다시 연결되고, 새로운 지역 보조가 됩니다.
...
- **읽을 수 있는 여러 지역 보조**
  ...
  > [!TIP]
  > 활성 지역 복제를 사용하여 전역적으로 분산된 애플리케이션을 빌드하고 4개 이상의 지역에 있는 데이터에 대한 읽기 전용 액세스를 제공해야 하는 경우 보조 보조 복제본(연결이라고 하는 프로세스)을 만들어 추가 지역 복제본을 만들 수 있습니다. 연결된 지역 복제본의 복제 지연 시간이 주 복제본에 직접 연결된 지역 복제본보다 높을 수 있습니다. 연결된 지역 복제 토폴로지 설정은 프로그래밍 방식으로만 지원되며 Azure Portal 지원하지 않습니다.
...
불균형 지역 보조 구성의 또 다른 결과는 장애 조치(failover) 후 새 주 데이터베이스의 컴퓨팅 용량 부족으로 인해 애플리케이션 성능이 저하할 수 있다는 것입니다. 이 경우 충분한 리소스를 포함하도록 데이터베이스를 확장해야 하며, 이는 상당한 시간이 걸릴 수 있으며, 확장 프로세스가 끝날 때 [고가용성](high-availability-sla.md) 장애 조치(failover)가 필요하며, 이로 인해 애플리케이션 워크로드가 중단될 수 있습니다.

컴퓨팅 크기가 낮은 지역 보조 데이터베이스를 만들려는 경우 시간별 주 데이터베이스의 로그 IO 속도를 모니터링해야 합니다. 이렇게 하면 복제 부하를 유지하는 데 필요한 지역 보조 데이터베이스의 최소 컴퓨팅 크기를 추정할 수 있습니다. 예를 들어 주 데이터베이스가 P6(1000 DTU)이고 로그 IO가 50%에서 유지되는 경우 지역 보조 데이터베이스는 P4(500 DTU) 이상이어야 합니다. 기록 로그 IO 데이터를 검색하려면 [sys.resource_stats](/sql/relational-databases/system-catalog-views/sys-resource-stats-azure-sql-database) 뷰를 사용합니다. 단기 급증을 더 잘 반영하는 세분성이 높은 최근 로그 IO 데이터를 검색하려면 [sys.dm_db_resource_stats](/sql/relational-databases/system-dynamic-management-views/sys-dm-db-resource-stats-azure-sql-database) 보기를 사용합니다.
...
기본적으로 지역 보조 데이터베이스의 백업 스토리지 중복성은 주 데이터베이스와 동일합니다. 다른 백업 스토리지 중복성을 사용하여 지역 보조를 구성하도록 선택할 수 있습니다. 백업은 항상 주 데이터베이스에서 수행됩니다. 보조가 다른 백업 스토리지 중복으로 구성된 경우 지역 장애 조치(failover) 후 지역 보조가 주 백업으로 승격되면 새 백업이 저장되고 새 주(이전 보조)에서 선택한 스토리지 유형(RA-GRS, ZRS, LRS)에 따라 요금이 청구됩니다. 

## <a name="cross-subscription-geo-replication"></a>구독 간 지역 복제

동일한 Azure Active Directory 테넌트에서 주 구독과 다른 구독에 지역 보조를 만들려면 이 섹션의 단계를 수행합니다.
...
8. 동일한 데이터베이스에서 로그인할 사용자를 만들고 `dbmanager` 역할에 추가합니다.
...
> 
> 기본 또는 보조 논리 서버에서 Azure SQL 대한 [인증만 활성화된 경우](https://techcommunity.microsoft.com/t5/azure-sql/azure-active-directory-only-authentication-for-azure-sql/ba-p/2417673) 다른 Azure 테넌트에서 논리 서버에 지역 보조를 만드는 Azure Active Directory 지원되지 않습니다.

## <a name="keeping-credentials-and-firewall-rules-in-sync"></a>자격 증명 및 방화벽 규칙의 동기화 유지

데이터베이스에 연결하기 위해 공용 네트워크 액세스를 사용하는 경우 지역 복제 [데이터베이스에 데이터베이스 수준 IP 방화벽 규칙을](firewall-configure.md) 사용하는 것이 좋습니다. 이러한 규칙은 데이터베이스와 함께 복제되어 모든 지역 보조 데이터베이스가 주 데이터베이스와 동일한 IP 방화벽 규칙을 갖도록 합니다. 이 방법을 사용하면 고객이 주 데이터베이스와 보조 데이터베이스를 호스트하는 서버에서 방화벽 규칙을 수동으로 구성하고 유지 관리하지 않아도 될 수 있습니다. 마찬가지로 포함된 [데이터베이스 사용자를](logins-create-manage.md) 데이터 액세스에 사용하면 주 데이터베이스와 보조 데이터베이스의 인증 자격 증명이 항상 동일합니다. 이렇게 하면 지역 장애 조치(failover) 후 인증 자격 증명 불일치로 인한 중단이 없습니다.
...
> [!IMPORTANT]
> 보조 데이터베이스가 먼저 상위 계층으로 확장되지 않는 한 장애 조치(failover) 그룹의 주 데이터베이스는 더 높은 서비스 계층(버전)으로 확장할 수 없습니다. 예를 들어 주 범용 중요 비즈니스용 확장하려면 먼저 지역 보조를 중요 비즈니스용 확장해야 합니다. 이 규칙을 위반하는 방식으로 주 또는 지역 보조를 확장하려고 하면 다음 오류가 표시됩니다.
>
...
RPO 측면에서 지연 시간을 모니터링하려면 주 데이터베이스에서 [sys.dm_geo_replication_link_status](/sql/relational-databases/system-dynamic-management-views/sys-dm-geo-replication-link-status-azure-sql-database)의 *replication_lag_sec* 열을 사용합니다. 주 지역에서 커밋된 트랜잭션과 보조의 트랜잭션 로그로 강화된 트랜잭션 간의 지연 시간(초)을 표시합니다. 예를 들어 지연 시간이 1초이면 현재 가동 중단의 영향을 받고 지역 장애 조치(failover)가 시작되면 마지막 1초에 커밋된 트랜잭션이 손실됩니다.
...
> 
> 또한 지역 보조 지역과 주 last_commit *시간* 간의 차이가 커질 수 있는 조건도 있습니다. 예를 들어 오랜 시간 동안 변경하지 않고 주 에서 커밋이 이루어진 경우 차이는 빠르게 0으로 되돌아가기 전에 큰 값으로 이동합니다. 이러한 두 값 간의 차이가 오랫동안 크게 유지되면 경고를 보내는 것이 좋습니다.

## <a name="programmatically-managing-active-geo-replication"></a>활성 지역 복제를 프로그래밍 방식으로 관리

앞서 설명한 것처럼 활성 지역 복제는 T-SQL, Azure PowerShell 및 REST API 사용하여 프로그래밍 방식으로도 관리될 수 있습니다. 다음 표는 사용 가능한 명령의 집합을 보여 줍니다. 활성 지역 복제는 관리를 위해 [Azure SQL Database REST API](/rest/api/sql/) 및 [Azure PowerShell cmdlet](/powershell/azure/)을 비롯한 Azure Resource Manager API 세트를 포함합니다. 이러한 API는 Azure RBAC(Azure 역할 기반 액세스 제어)를 지원합니다. 액세스 역할을 구현하는 방법에 대한 자세한 내용은 [Azure RBAC(Azure 역할 기반 액세스 제어)](../../role-based-access-control/overview.md)를 참조하세요.
