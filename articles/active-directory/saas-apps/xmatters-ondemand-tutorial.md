...
**B.Simon** 이라는 테스트 사용자를 사용하여 xMatters OnDemand에서 Azure AD SSO를 구성하고 테스트합니다. SSO가 작동하려면 Azure AD 사용자와 xMatters OnDemand의 관련 사용자 간에 연결 관계를 설정해야 합니다.

xMatters OnDemand에서 Azure AD SSO를 구성하고 테스트하려면 다음 단계를 수행합니다.
...
2. **[xMatters OnDemand SSO 구성](#configure-xmatters-ondemand-sso)** - 애플리케이션 쪽에서 Single Sign-On 설정을 구성합니다.
...
6. **xMatters OnDemand 설정** 섹션에서 요구 사항에 따라 적절한 URL을 복사합니다.
...
이 섹션에서는 B.Simon이 Azure Single Sign-On을 사용할 수 있도록 xMatters OnDemand에 대한 액세스 권한을 부여합니다.
...
3. **SAML 구성** 페이지에서 다음 단계를 수행합니다.
    ...
    b. **ID 공급자 ID** 텍스트 상자에, Azure Portal에서 복사한 **Azure AD 식별자** 값을 붙여넣습니다.

    다. **Single Sign-On URL** 텍스트 상자에, Azure Portal에서 복사한 **로그인 URL** 값을 붙여넣습니다.

    d. **로그아웃 URL 리디렉션** 텍스트 상자에 Azure Portal에서 복사한 **로그아웃 URL** 을 붙여넣습니다.

    e. **파일 선택** 을 클릭하여 Azure Portal에서 다운로드한 **인증서(Base64)** 를 업로드합니다. 

    f. 회사 정보 페이지 위쪽에서 **변경 내용 저장** 을 클릭합니다.

    ![회사 세부 정보](./media/xmatters-ondemand-tutorial/save-button.png "회사 세부 정보")
...
2. **사용자 아이콘** > **사용자** 로 이동한 다음, **사용자 추가** 를 클릭합니다.
    ...
    ![사용자](./media/xmatters-ondemand-tutorial/add-user.png "사용자")

3. **사용자 추가** 섹션에서 필수 필드를 채우고 **사용자 추가** 단추를 클릭합니다.
...
* Azure Portal에서 이 애플리케이션 테스트를 클릭하면 SSO를 설정한 xMatters OnDemand에 자동으로 로그인됩니다.

* Microsoft 내 앱을 사용할 수 있습니다. 내 앱에서 xMatters OnDemand 타일을 클릭하면 SSO를 설정한 xMatters OnDemand에 자동으로 로그인됩니다. 내 앱에 대한 자세한 내용은 [내 앱 소개](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510)를 참조하세요.

## <a name="next-steps"></a>다음 단계

xMatters OnDemand가 구성되면 세션 제어를 적용하여 조직의 중요한 데이터의 반출 및 반입을 실시간으로 보호할 수 있습니다. 세션 제어는 조건부 액세스에서 확장됩니다. [Microsoft Defender for Cloud Apps를 사용하여 세션 제어를 적용하는 방법을 알아봅니다](/cloud-app-security/proxy-deployment-any-app).
