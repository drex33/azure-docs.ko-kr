...
ms.openlocfilehash: 8cb32993aa4f2a99d7c9e82256fda56c0318989c
ms.sourcegitcommit: 991268c548dd47e5f7487cd025c7501b9315e477
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/30/2021
ms.locfileid: "133289160"
---
...
* 대상이 RFC1918이 아닌 주소이고 **모두 라우팅을** 사용하도록 설정하지 않았나요?
...
* 지점과 사이트 간 및 ExpressRoute를 모두 지원하는 공존 게이트웨이를 사용하려고 하나요? 가상 네트워크 통합에는 공존 게이트웨이가 지원되지 않습니다.
...
* 지역 가상 네트워크 통합 기능을 사용하여 RFC 1918이 아닌 주소에 도달하려고 합니다.

#### <a name="deleting-the-app-service-plan-or-web-app-before-disconnecting-the-vnet-integration"></a>VNet 통합 연결을 끊기 전에 App Service 계획 또는 웹앱 삭제

VNet 통합 연결을 먼저 끊지 않고 웹앱 또는 App Service 계획을 삭제한 경우 삭제된 리소스와의 통합에 사용된 가상 네트워크 또는 서브넷에서 업데이트/삭제 작업을 수행할 수 없습니다. 서브넷 위임 'Microsoft.Web/serverFarms'는 서브넷에 할당된 상태로 유지되며 업데이트/삭제 작업을 방지합니다. 

서브넷 또는 가상 네트워크를 다시 업데이트/삭제하려면 VNet 통합을 다시 만든 다음 연결을 끊어야 합니다.
1. App Service 계획 및 웹앱을 다시 만듭니다(이전과 정확히 동일한 웹앱 이름을 사용해야 합니다).
1. 웹앱의 '네트워킹' 블레이드로 이동하여 VNet 통합을 구성합니다. 
1. VNet 통합이 구성되면 '연결 끊기' 단추를 선택합니다.
1. App Service 계획 또는 웹앱을 삭제합니다. 
1. 서브넷 또는 가상 네트워크를 업데이트/삭제합니다.

위의 단계를 수행한 후에도 VNet 통합에 문제가 계속 발생하는 경우 Microsoft 지원 문의하세요. 
