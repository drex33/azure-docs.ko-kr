---
title: 온-프레미스 앱에 대한 원격 액세스 - Azure AD 애플리케이션 프록시
...
- **간편한 사용**. 사용자는 Microsoft 365 및 Azure AD와 통합된 기타 SaaS 앱에 액세스할 때와 같은 방식으로 온-프레미스 애플리케이션에 액세스할 수 있습니다. 애플리케이션 프록시를 사용하도록 애플리케이션을 변경하거나 업데이트할 필요가 없습니다.
...
* 폼 기반 또는 [헤더 기반](./application-proxy-configure-single-sign-on-with-headers.md) 액세스를 사용하는 웹 애플리케이션
...
애플리케이션 프록시는 Single Sign-On을 지원합니다. 지원되는 방법에 대한 자세한 내용은 [Single Sign-On 방법 선택](../manage-apps/sso-options.md#choosing-a-single-sign-on-method)을 참조하세요.

애플리케이션 프록시는 원격 사용자에게 내부 리소스에 대한 액세스를 제공하기 위해 권장됩니다. 애플리케이션 프록시는 VPN 또는 역방향 프록시에 대한 필요성을 대체합니다. 회사 네트워크의 내부 사용자를 위한 것은 아닙니다.  내부 사용자들이 애플리케이션 프록시를 불필요하게 사용하면 예기치 않은 성능 문제를 일으킬 수 있습니다.
...
> [!NOTE]
> 대부분의 Azure AD 하이브리드 에이전트와 마찬가지로 애플리케이션 프록시 커넥터는 방화벽을 통해 인바운드 연결을 열지 않아도 됩니다. 3단계의 사용자 트래픽은 Azure AD의 애플리케이션 프록시 서비스에서 종료됩니다. 애플리케이션 프록시 커넥터(온-프레미스)는 나머지 통신을 담당합니다.
>
