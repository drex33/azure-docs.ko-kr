---
title: '데이터 가져오기: 구성 요소 참조'
titleSuffix: Azure Machine Learning
description: Azure Machine Learning 데이터 가져오기 구성 요소를 사용하여 기존 클라우드 데이터 서비스에서 기계 학습 파이프라인으로 데이터를 로드하는 방법을 알아봅니다.
...
ms.locfileid: "131570029"
---
# <a name="import-data-component"></a>데이터 가져오기 구성 요소

이 문서에서는 Azure Machine Learning 디자이너의 구성 요소에 대해 설명합니다.

이 구성 요소를 사용하여 기존 클라우드 데이터 서비스에서 기계 학습 파이프라인으로 데이터를 로드합니다. 

> [!Note]
> 이 구성 요소에서 제공하는 모든 기능은 작업 저장소 방문 페이지의 **데이터 저장소** 및 **데이터 세트를** 통해 수행할 수 있습니다. 데이터 모니터링과 같은 추가 기능을 포함하는 **데이터 저장소** 및 **데이터 세트** 를 사용하는 것이 좋습니다. 자세히 알아보려면 [데이터에 액세스하는 방법](../how-to-access-data.md) 및 [데이터 세트를 등록하는 방법](../how-to-create-register-datasets.md) 문서를 참조하세요.
> 등록한 데이터 세트는 디자이너 인터페이스의 **데이터 세트** -> **내 데이터 세트** 범주에서 찾을 수 있습니다. 이 구성 요소는 익숙한 환경을 위해 Studio(클래식) 사용자를 위해 예약되어 있습니다. 
>

**데이터 가져오기** 구성 요소는 다음 원본에서 데이터 읽기를 지원합니다.
...
1. 데이터 **가져오기** 구성 요소를 파이프라인에 추가합니다. 디자이너의 데이터 입력 **및 출력** 범주에서 이 구성 요소를 찾을 수 있습니다.

1. 구성 요소를 선택하여 오른쪽 창을 엽니다.
...
1. **데이터 원본** 을 선택하고 데이터 원본 유형을 선택합니다. HTTP 또는 데이터 저장소를 선택할 수 있습니다.
    ...
    > [!NOTE]
    > **데이터 가져오기** 구성 요소는 **테이블 형식** 데이터 전용입니다.
...
1. 출력 다시 생성 확인란은 런타임에 **출력을** 다시 생성하기 위해 구성 요소를 실행할지 여부를 결정합니다. 
    ...
    이 옵션을 선택하면 시스템에서 구성 요소를 다시 실행하여 출력을 다시 생성합니다. 따라서 스토리지의 기본 데이터가 업데이트되는 경우 이 옵션을 선택하면 최신 데이터를 가져오는 데 도움이 될 수 있습니다.
...
파이프라인을 실행할 때마다 새 데이터 집합을 가져오는 대신 다시 사용할 데이터를 저장하려면 구성 요소의 오른쪽 패널에 있는 **출력 +로그** 탭에서 **데이터 세트 등록** 아이콘을 선택합니다. 데이터 세트의 이름을 선택합니다. 저장된 데이터 세트는 저장 시 데이터를 유지하지만 파이프라인의 데이터 세트가 변경되는 경우에도 파이프라인을 다시 실행하면 데이터 세트가 업데이트되지 않습니다. 이는 데이터의 스냅샷을 만드는 데 유용할 수 있습니다.
...
- [데이터 세트의 열 선택](./select-columns-in-dataset.md)을 사용하여 변환하거나 모델링에 사용할 열 하위 집합을 선택합니다. 변환되거나 제거된 열은 열 추가 구성 요소를 사용하여 원래 데이터 세트에 쉽게 다시 가입할 수 [있습니다.](./add-columns.md)  

- [파티션 및 샘플](./partition-and-sample.md)을 사용하여 데이터 세트를 나누거나, 샘플링을 수행하거나, 상위 n개 행을 가져옵니다.

## <a name="limitations"></a>제한 사항

datstore 액세스 제한으로 인해 유추 파이프라인에 **데이터 가져오기** 구성 요소가 포함된 경우 실시간 엔드포인트에 배포할 때 자동으로 제거됩니다.
