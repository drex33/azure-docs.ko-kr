...
|`unwind(collectionSelector, [resultSelector], [options], [callback])`|내부 배열에서 셀프 조인을 수행하고 양쪽의 결과를 결과 프로젝션에 튜플로 추가합니다. 예를 들어, person 문서를 person.pets에 조인하면 [person, pet] 튜플이 생성됩니다. 이는 .NET LINQ의 SelectMany와 유사합니다.|
