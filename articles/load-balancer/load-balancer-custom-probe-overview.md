...
프로브 응답 수는 두 가지 모두에 적용됩니다.

- 인스턴스가 가동 상태로 표시될 수 있는 성공한 프로브의 수
- 인스턴스가 가동 중지 상태로 표시되는 시간이 초과된 프로브의 수

지정된 제한 시간 및 간격 값은 인스턴스를 가동 중지 상태로 표시할지 또는 가동 상태로 표시할지를 결정합니다.  간격의 기간에 프로브 응답 수를 곱하여 프로브 응답을 검색해야 하는 기간을 결정합니다.  그리고 서비스는 필요한 프로브를 달성한 후에도 반응합니다.

예를 사용하여 동작을 더 자세히 설명할 수 있습니다. 프로브 응답 수를 2로 설정하고 간격을 5초로 설정한 경우 10초 내에 2개의 프로브 시간 제한 오류가 관찰되어야 합니다.  애플리케이션이 상태를 변경할 수 있는 경우 프로브가 전송되는 시간이 동기화되지 않으므로 다음 두 가지 시나리오에서 검색할 시간을 제한할 수 있습니다.

1. 애플리케이션이 첫 번째 프로브가 도착하기 직전에 시간 제한 프로브 응답을 생성하기 시작하면 이러한 이벤트를 검색하는 데에는 10초(2 x 5초 간격)와 첫 번째 프로브가 도착할 때 애플리케이션이 시간 제한 신호를 보내기 시작하는 시간이 걸립니다.  이 검색은 10초가 약간 넘게 소요된다고 가정할 수 있습니다.
2. 애플리케이션이 첫 번째 프로브가 도착한 직후에 시간 제한 프로브 응답을 생성하기 시작하면 다음 프로브가 도착(및 시간 제한)한 후 10초(2 x 5초 간격)를 더한 시간이 될 때까지 이러한 이벤트 검색은 시작하지 않습니다.  이 검색은 15초 이내에 완료된다고 가정할 수 있습니다.

이 예에서는 검색이 발생하면 플랫폼이 이 변경에 대응하는 데 약간의 시간이 소요됩니다.  즉, 다음 사항에 따라 달라집니다. 

1. 애플리케이션이 상태 변경을 시작하는 경우
2. 이 변경 내용이 검색되고 필요한 조건(지정된 간격으로 전송된 프로브 수)을 충족하는 경우
3. 플랫폼 간에 검색이 전달된 경우 

시간 제한 프로브 응답에 대한 반응에서 애플리케이션의 신호 변경에 반응하는 데 최소 10초에서 최대 15초를 약간 넘는 시간이 소요된다고 가정할 수 있습니다.  이 예는 발생하는 상황을 설명하기 위해 제공되지만, 이 예에 설명된 위의 대략적인 지침 이상으로 정확한 기간을 예측할 수는 없습니다.
...
* 백 엔드 엔드포인트를 정상 상태로 표시하는 데 필요한 지정된 수의 프로브를 달성했습니다.
