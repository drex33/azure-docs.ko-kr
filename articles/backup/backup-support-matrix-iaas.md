...
매일 백업(Azure VM 확장을 통해) | 매일 1회 백업 예약<br/><br/>Azure Backup 서비스는 하루에 최대 3회의 주문형 백업을 지원하고, 예약된 백업 1회를 추가로 지원합니다.
...
머신 시간이 변경된 VM 백업 | 지원되지 않습니다.<br/><br/> VM에 대한 백업을 사용하도록 설정한 후 해당 머신 시간이 이후 날짜-시간으로 변경되었다면 시간 변경이 되돌려진다고 해도 성공적인 백업이 보장되지 않습니다.
...
Docker 탑재 지점이 있는 Linux Azure VM 백업 | 현재 Azure Backup은 Docker 탑재 지점 제외를 지원하지 않습니다. 이는 매번 다른 경로에 탑재되기 때문입니다.
...
공유 디스크, 임시 드라이브, 중복된 디스크, Ultra 디스크 및 쓰기 가속기가 사용하도록 설정된 디스크에서 파일을 복원합니다. | 복원은 지원되지 않습니다. <br/><br/>[Azure VM 스토리지 지원](#vm-storage-support)을 참조하세요.
...
도메인 컨트롤러 VM 복원  | 지원됨. 자세한 내용은 [도메인 컨트롤러 VM 복원](backup-azure-arm-restore-vms.md#restore-domain-controller-vms)을 참조하세요.
...
[Azure Marketplace](https://azuremarketplace.microsoft.com/marketplace/apps?filters=virtual-machine-images)에서 배포한 VM 백업<br/><br/> (Microsoft, 타사에서 게시) |지원됩니다.<br/><br/> VM에서 지원되는 운영 체제도 실행하고 있어야 합니다.<br/><br/> VM에서 파일을 복구할 경우 호환되는 OS(이전 또는 이후 OS 아님)로만 복원할 수 있습니다. VM으로 지원되는 Azure Marketplace VM은 구매 정보가 필요하기 때문에 복원하지 않으며 디스크로만 복원됩니다.
...
[진단 설정](../azure-monitor/essentials/platform-logs-overview.md)으로 백업  | 지원되지 않음 <br/><br/> [새로 만들기](backup-azure-arm-restore-vms.md#create-a-vm) 옵션을 사용하여 진단 설정으로 Azure VM 복원이 트리거되면 복원이 실패합니다.
영역 고정 VM 복원 | 지원됨(2019년 1월 이후 및 [가용성 영역](https://azure.microsoft.com/global-infrastructure/availability-zones/)을 사용할 수 있는 곳에 백업된 VM의 경우)<br/><br/>현재 VM에 고정된 동일한 영역으로 복원을 지원합니다. 그러나 가동 중단으로 인해 영역을 사용할 수 없는 경우 복원이 실패합니다.
...
잠금을 사용하여 Azure VM 백업 | 관리되지 않는 VM의 경우 지원되지 않습니다. <br><br> 관리되는 VM은 지원됩니다.
...
**구성 요소** | **지원**
--- | ---
Azure VM 데이터 디스크 수 | 디스크가 최대 32개인 Azure VM의 백업 지원<br><br> 비관리 디스크 또는 클래식 VM을 사용하는 Azure VM의 백업은 최대 16개의 디스크에만 지원됩니다.
