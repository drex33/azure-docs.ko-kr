...
2. 거버넌스를 사용하기 위해 모든 데이터 원본을 등록하고 단일 Azure Purview 계정에서 연결된 모든 액세스 정책을 관리합니다.
3. 이 기능은 액세스 정책 관리 및 적용 기능이 배포되는 아래 나열된 지역에서만 사용할 수 있습니다.
...
제한에 따라 위에 나열된 지역에 새 Azure Storage 계정을 만듭니다. 스토리지 [계정 만들기 - Azure Storage](../storage/common/storage-account-create.md)
...
-   사용자는 루트 컬렉션 수준에서 Purview 정책 작성자 역할의 일부여야 정책 작성/관리 작업을 수행할 수 있습니다.
...
> [!NOTE]
> 토글의 동작은 지정된 구독의 모든 데이터 원본을 단일 Purview 계정의 데이터 사용 거버넌스에만 등록할 수 있도록 적용합니다. Purview 계정 자체는 테넌트 내의 모든 구독에 있을 수 있습니다.
