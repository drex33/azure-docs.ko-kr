...
description: 진단 호출 기능에 대 한 개요를 제공 합니다.
...
ms.locfileid: "128635891"
---
# <a name="call-diagnostics"></a>통화 진단

Azure 통신 서비스에서 호출을 수행할 때 문제 또는 문제가 발생 하 여 고객에 게 문제를 일으킬 수 있습니다. 이를 지원 하기 위해 호출의 다양 한 속성을 검사 하 여 문제의 원인을 확인할 수 있는 "호출 진단" 이라는 기능을 사용할 수 있습니다.

**호출 진단은 현재 JS/웹 SDK 에서만 지원 됩니다.**

## <a name="accessing-diagnostics"></a>진단 액세스
...
| networkReconnect          | 연결이 끊어졌으며 네트워크에 다시 연결하고 있습니다. | - 미디어 전송 연결이 끊어지면 `Poor`로 설정됩니다. <br/> - 네트워크 연결이 끊어지면 `Bad`로 설정됩니다. <br/> - 새 세션이 연결되면 `Good`으로 설정됩니다.                                                                       | 낮은 대역폭, 인터넷 없음                          |
...
| speakingWhileMicrophoneIsMuted | 음소거 상태에서 말합니다.                                   | - 로컬 마이크가 음소거되고 로컬 사용자가 말하는 경우 `True`로 설정됩니다. <br/> - 로컬 사용자가 말하기를 중지하거나 마이크를 음소거 해제할 때 `False`로 설정됩니다. <br/> \* 참고: 현재는 webrtc에서 오디오 수준 샘플을 가져오므로 아직 safari에서는 지원 되지 않습니다. 않습니다.                 | 통화하는 동안 마이크를 음소거하고 마이크에 대고 말합니다.           |
...
## <a name="get-the-latest-diagnostics"></a>최신 진단 정보 얻기
