...
업계 모범 사례를 준수하기 위해 네트워크 세션 스키마는 **Src** 및 **Dst** 설명자를 사용하여 필드 이름에 **Dvc** 토큰을 포함하지 않고 네트워크 세션 원본 및 대상 디바이스를 식별합니다.
...
다음 필드는 모든 네트워크 세션 활동 로깅에 공통적으로 표시됩니다.
...
| **DstDvcIdType** | 선택 사항 | Enumerated | 알려진 경우 [DstDvcId](#dstdvcid)의 형식입니다. 가능한 값은 다음과 같습니다.<br> - `AzureResourceId`<br>- `MDEidIf`<br><br>여러 개의ID를 사용할 수 있는 경우 위 목록의 첫 번째 ID를 사용하고 다른 것을 각각  **DstDvcAzureResourceId** 또는 **DstDvcMDEid** 필드에 저장합니다.<br><br>**DstDeviceId를** 사용하는 경우 필요합니다.|
...
| **SrcNatIpAddr** | 선택 사항 | IP 주소 | 중간 NAT 디바이스에서 보고되는 경우 NAT 디바이스에서 대상과의 통신에 사용하는 IP 주소입니다.<br><br>예: `4.3.2.1` |
...
HTTP 세션은 HTTP 프로토콜을 사용하는 네트워크 세션입니다. 이러한 세션은 종종 웹 서버, 웹 프록시 및 웹 보안 게이트웨이에서 보고됩니다. HTTP 세션과 관련한 추가 필드는 다음과 같습니다.
