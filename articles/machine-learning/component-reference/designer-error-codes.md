...
description: Azure Machine Learning designer에서 자동화 된 구성 요소 오류 코드를 읽고 문제를 해결 하는 방법에 대해 알아봅니다.
...
- 실패 한 구성 요소를 선택 하 고 **출력 + 로그** 탭으로 이동 하 여 **70_driver_log.txt** 파일의 **azureml-로그** 범주 아래에서 자세한 로그를 찾을 수 있습니다.

- 구성 요소 오류에 대 한 자세한 내용은 **module_statistics** 범주 아래의 error_info에서 확인할 수 있습니다.

다음은 디자이너의 구성 요소에 대 한 오류 코드입니다.

## <a name="error-0001"></a>오류 0001  
 데이터 세트의 지정된 열 중 하나 이상을 찾을 수 없으면 예외가 발생합니다.  

 구성 요소에 대해 열을 선택 했는데 선택한 열이 입력 데이터 집합에 없는 경우이 오류가 표시 됩니다. 이 오류는 열 이름을 수동으로 입력한 경우 또는 파이프라인 실행 시 열 선택기에서 데이터 세트에 존재하지 않는 열을 제공한 경우 발생할 수 있습니다.  

**해결 방법:** 이 예외를 throw 하는 구성 요소를 다시 방문 하 여 열 이름이 올바른지, 그리고 참조 된 모든 열이 존재 하는지 확인 합니다.  
...
**해결 방법:** 구성 요소 요구 사항을 확인 하 고 필요한 값 형식 (문자열, 정수, double 등)을 결정 합니다.  
...
## <a name="error-0003"></a>오류 0003  
 ...
 구성 요소에 대 한 입력 또는 매개 변수가 null 이거나 비어 있는 경우 Azure Machine Learning에서이 오류가 표시 됩니다.  예를 들어, 매개 변수에 대한 값을 입력하지 않을 때 이 오류가 발생할 수 있습니다. 또한 값이 누락된 데이터 세트 또는 빈 데이터 세트를 선택한 경우에도 이 오류가 발생할 수 있습니다.  

**해결 방법:**

+ 예외를 생성 한 구성 요소를 열고 모든 입력이 지정 되었는지 확인 합니다. 모든 필수 입력이 지정되어 있는지 확인합니다. 
...
## <a name="error-0004"></a>오류 0004  
 ...
 메시지의 매개 변수가 구성 요소에서 데이터를 처리 하는 데 필요한 경계 값 보다 낮은 경우 Azure Machine Learning에서이 오류가 표시 됩니다.  

**해결 방법:** 예외를 throw 하는 구성 요소를 다시 방문 하 고 매개 변수를 지정 된 값 보다 크게 수정 합니다.  
...
## <a name="error-0005"></a>오류 0005  
 ...
 메시지의 매개 변수가 구성 요소에서 데이터를 처리 하는 데 필요한 경계 값 보다 작거나 같으면이 오류가 Azure Machine Learning에 표시 됩니다.  

**해결 방법:** 예외를 throw 하는 구성 요소를 다시 방문 하 고 매개 변수를 지정 된 값 보다 크거나 같게 수정 합니다.  
...
## <a name="error-0006"></a>오류 0006  
 ...
 메시지의 매개 변수가 구성 요소에서 데이터를 처리 하는 데 필요한 경계 값 보다 크거나 같은 경우이 오류를 Azure Machine Learning에서 받게 됩니다.  

**해결 방법:** 예외를 throw 하는 구성 요소를 다시 방문 하 고 매개 변수를 지정 된 값 보다 작게 수정 합니다.  
...
## <a name="error-0007"></a>오류 0007  
 ...
 구성 요소에 대 한 속성에서 허용 되는 값 보다 큰 값을 지정한 경우이 오류는 Azure Machine Learning에서 받게 됩니다. 예를 들어 지원되는 날짜의 범위를 벗어난 데이터를 지정할 수 있으며, 또는 3개의 열만 사용 가능한 경우에는 5개의 열을 사용할 것을 나타낼 수 있습니다. 
...
**해결 방법:**
 ...
 + 문제의 구성 요소를 열고 숫자 속성 설정을 검토 합니다.
 + 모든 매개 변수 값이 해당 속성에 대해 지원되는 값의 범위 내에 있는지 확인합니다.
 + 구성 요소에서 여러 입력을 사용 하는 경우 입력의 크기가 동일한 지 확인 합니다.
...
## <a name="error-0008"></a>오류 0008  
 ...
 메시지의 매개 변수가 구성 요소에서 데이터를 처리 하는 데 필요한 범위를 벗어나면이 오류가 Azure Machine Learning에 표시 됩니다.  

 예를 들어 [행 추가](add-rows.md)를 사용하여 열 수가 다른 두 데이터 세트를 결합하려고 할 경우, 이 오류가 표시됩니다.  

**해결 방법:** 예외를 throw 하는 구성 요소를 다시 방문 하 고 매개 변수를 지정 된 범위 내에 있도록 수정 합니다.  
...
이러한 문제들은 계정 이름, 암호 또는 컨테이너 경로를 수동으로 입력하려고 할 때 발생하는 경우가 많습니다. 이름을 조회 하 고 확인 하는 데 도움이 되는 [데이터 가져오기](import-data.md) 구성 요소에 대해 새 마법사를 사용 하는 것이 좋습니다.
...
## <a name="error-0011"></a>오류 0011  
 ...
 열을 선택 하지 않았고 구성 요소가 작동 하는 데 하나 이상의 열이 필요한 경우에도이 오류가 발생할 수 있습니다.  

**해결 방법:** 데이터 집합의 열에 적용 되도록 구성 요소의 열 선택을 수정 합니다.  

 구성 요소에서 레이블 열과 같은 특정 열을 선택 해야 하는 경우 오른쪽 열이 선택 되어 있는지 확인 합니다.  
...
## <a name="error-0013"></a>오류 0013  
 구성 요소에 전달 된 학습자의 유형이 잘못 된 경우 예외가 발생 합니다.  

 이 오류는 학습 된 모델이 연결 된 점수 매기기 구성 요소와 호환 되지 않을 때마다 발생 합니다. <!--For example, connecting the output of [Train Matchbox Recommender](train-matchbox-recommender.md) to [Score Model](score-model.md) (instead of [Score Matchbox Recommender](score-matchbox-recommender.md)) will generate this error when the pipeline is run.  -->

**해결 방법:**

학습 구성 요소에 의해 생성 되는 학습자 유형을 확인 하 고 학습자에 적합 한 점수 매기기 구성 요소를 결정 합니다. 

특수화 된 학습 구성 요소를 사용 하 여 모델을 학습 한 경우 학습 된 모델을 해당 하는 특수 점수 매기기 구성 요소에만 연결 합니다. 
...
오류를 생성 한 구성 요소를 열고 입력으로 사용 된 열을 식별 합니다. 일부 구성 요소의 경우 데이터 집합 입력을 마우스 오른쪽 단추로 클릭 하 고 **시각화** 를 선택 하 여 고유 값의 수 및 분포를 비롯 한 개별 열에 대 한 통계를 가져올 수 있습니다.
...
텍스트 열의 경우 [기능 해시](../algorithm-module-reference/feature-hashing.md) 를 사용 하거나 [텍스트 구성 요소에서 N-문법 기능을 추출](../algorithm-module-reference/extract-n-gram-features-from-text.md) 하 여 텍스트 열을 전처리 할 수 있습니다.
<!--
...
## <a name="error-0016"></a>오류 0016  
 구성 요소에 전달된 입력 데이터 세트에 호환되는 열 형식이 있어야 하지만 그렇지 않으면 예외가 발생합니다.  
...
## <a name="error-0017"></a>오류 0017  
 ...
 예를 들어 수학 연산의 문자열 열 또는 범주 기능 열이 필요한 점수 열과 같이 구성 요소에서 처리할 수 없는 데이터 형식의 열이 열 선택 항목에 포함된 경우 Azure Machine Learning 이 오류가 발생할 수 있습니다.  

**해결 방법:**
 1. 문제가 되는 열을 식별합니다.
 2. 구성 요소의 요구 사항을 검토합니다.
 3. 요구 사항을 준수하도록 열을 수정합니다. 열 및 시도 중인 변환에 따라 다음 구성 요소 중 일부를 사용하여 변경해야 할 수 있습니다.
...
|예외 메시지|
|------------------------|
|현재 형식의 열을 처리할 수 없습니다. 형식은 구성 요소에서 지원되지 않습니다.|
|{col_type} 형식의 열을 처리할 수 없습니다. 형식은 구성 요소에서 지원되지 않습니다.|
|{col_type} 형식의 열 "{col_name}"을(를) 처리할 수 없습니다. 형식은 구성 요소에서 지원되지 않습니다.|
|{col_type} 형식의 열 "{col_name}"을(를) 처리할 수 없습니다. 형식은 구성 요소에서 지원되지 않습니다. 매개 변수 이름: {arg_name}.|


## <a name="error-0018"></a>오류 0018  
 입력 데이트 세트가 올바르지 않으면 예외가 발생합니다.  

**해결 방법:** Azure Machine Learning에서 이 오류는 여러 상황에서 발생할 수 있으므로 절대적인 해결 방법은 없습니다. 일반적으로 오류는 구성 요소에 입력으로 제공된 데이터에 잘못된 수의 열이 있거나 데이터 형식이 구성 요소의 요구 사항과 일치하지 않음을 나타냅니다. 예를 들면 다음과 같습니다.  

-   구성 요소에 레이블 열이 필요하지만 레이블로 표시된 열이 없거나 아직 레이블 열을 선택하지 않았습니다.  
  
-   구성 요소에서는 데이터가 범주적이어야 하지만 데이터는 숫자여야 합니다.  
...
-   열이 비어 있거나 누락된 값이 너무 많이 포함되어 있습니다.  
 ...
 요구 사항 및 데이터 사용 방법을 확인하려면 데이터 세트를 입력으로 사용할 구성 요소에 대한 도움말 항목을 검토하세요.  
...
**해결 방법:** 입력 데이터 세트를 수동으로 수정하여 열 값을 정렬하고 구성 요소를 다시 실행합니다.  
...
## <a name="error-0020"></a>오류 0020  
 구성 요소에 전달된 일부 데이터 세트의 열 수가 너무 작으면 예외가 발생합니다.  

 구성 요소에 대해 열이 충분하지 않은 경우 Azure Machine Learning 이 오류가 표시됩니다.  

**해결 방법:** 구성 요소를 다시 확인하고 열 선택기에서 선택한 열 수가 올바른지 확인합니다.  
...
## <a name="error-0021"></a>오류 0021  
 구성 요소에 전달된 일부 데이터 세트의 행 수가 너무 작으면 예외가 발생합니다.  
...
**해결 방법:**
 ...
 + 입력 데이터 세트가 비어 있지 않고 구성 요소 도움말에 설명된 요구 사항을 충족하기에 충분한 데이터 행이 있는지 확인합니다.  
 + 데이터가 외부 소스에서 로드되는 경우, 데이터 원본을 사용할 수 있는지 확인하며 가져오기 프로세스에서 가져올 행 수가 줄어드는 원인이 될만한 데이터 정의상의 오류나 변경은 없는지 확인합니다.
 + 데이터 형식이나 값 수(예: 정리, 분할 또는 조인 작업)에 영향을 줄 수 있는 구성 요소의 데이터 업스트림에서 작업을 수행하는 경우 해당 작업의 출력을 확인하여 반환되는 행 수를 확인합니다.  
...
## <a name="error-0022"></a>오류 0022  
 ...
 Azure Machine Learning 이 오류는 다운스트림 구성 요소 또는 작업에 특정 개수의 열 또는 입력이 필요하고 너무 적거나 너무 많은 열 또는 입력을 제공한 경우에 발생할 수 있습니다. 다음은 그 예입니다.  

-   단일한 레이블 열 또는 키 열을 지정한 후 여러 개의 열을 실수로 선택했습니다.  
  
-   열 이름을 바꾸고 있지만 실제 존재하는 열의 수보다 더 많거나 더 적은 이름을 제공했습니다.  
  
-   원본 또는 대상의 열 수가 변경되거나 구성 요소에서 사용하는 열 수와 일치하지 않습니다.  
  
-   쉼표로 구분된 입력값 목록을 제공했지만 값 수가 일치하지 않거나 여러 입력이 지원되지 않습니다.  

**해결 방법:** 구성 요소를 다시 확인하고 열 선택을 확인하여 올바른 수의 열이 선택되었는지 확인합니다. 업스트림 구성 요소의 출력 및 다운스트림 작업의 요구 사항을 확인합니다.  
...
입력 데이터 세트의 대상 열이 현재 트레이너 구성 요소에 유효하지 않으면 예외가 발생합니다.  

Azure Machine Learning 이 오류는 대상 열(구성 요소 매개 변수에서 선택한 대로)이 유효한 데이터 형식이 아니거나, 누락된 모든 값을 포함하거나, 예상대로 범주화되지 않은 경우에 발생합니다.  

**해결 방법:** 구성 요소 입력을 다시 검토하여 레이블/대상 열의 내용을 검사합니다. 이 입력에 누락된 값은 없는지 확인합니다. 구성 요소에서 대상 열이 범주화되어야 하는 경우 대상 열에 둘 이상의 고유 값이 있는지 확인합니다.  
...
데이터 세트에 레이블 열이 없으면 예외가 발생합니다.  
 ...
 Azure Machine Learning 이 오류는 구성 요소에 레이블 열이 필요하고 데이터 세트에 레이블 열이 없는 경우에 발생합니다. 예를 들어 채점된 데이터 세트를 평가할 경우, 정확도 메트릭을 계산하려면 일반적으로 레이블 열이 있어야 합니다.  

또한 레이블 열이 데이터 세트 내에 있더라도 Azure Machine Learning에 의해 올바르게 검색되지 않을 수도 있습니다.

**해결 방법:**

+ 오류를 생성한 구성 요소를 열고 레이블 열이 있는지 확인합니다. 이 열에 예측하려는 단일 결과(또는 종속 변수)가 포함되어 있다면 열의 이름 또는 데이터 형식은 중요하지 않습니다. 레이블이 어느 열에 있는지를 모를 경우, 클래스 또는 대상과 같은 일반 이름을 찾습니다. 
+  데이터 세트에 레이블 열이 포함되어 있지 않다면 레이블 열이 명시적으로 또는 실수로 업스트림에서 제거되었을 수도 있습니다. 데이터 세트가 업스트림 점수 매기기 구성 요소의 출력이 아닐 수도 있습니다.
+ 열을 레이블 열로 명시적으로 표시하려면 [메타데이터 편집](edit-metadata.md) 구성 요소를 추가하고 데이터 세트를 연결합니다. 레이블 열만 선택하고 **필드** 드롭다운 목록에서 **레이블** 을 선택합니다. 
...
**해결 방법:** 모델 평가에 대한 입력을 다시 확인하고 이 입력에 하나 이상의 점수 열이 포함되어 있는지 검사합니다. 그렇지 않은 경우 데이터 세트의 점수가 매해지지 않았거나 점수 열이 업스트림 구성 요소에서 삭제되었습니다.  
...
**해결 방법:** 열의 이름이 같으면 입력 데이터 세트와 구성 요소 사이에 [메타데이터 편집](edit-metadata.md) 구성 요소를 삽입합니다. [메타데이터 편집](edit-metadata.md)의 열 선택기를 사용하여 이름을 바꿀 열을 선택하고 새 이름을 **새 열 이름** 텍스트 상자에 입력합니다.  

|예외 메시지|
|------------------------|
|같은 열 이름이 인수에 지정되어 있습니다. 동일한 열 이름은 구성 요소에서 허용되지 않습니다.|
...
**해결 방법:** 이름이 같은 열이 있는 경우 입력 데이터 세트와 오류가 발생하는 구성 요소 사이에 [메타데이터 편집](edit-metadata.md) 인스턴스를 추가합니다. [메타데이터 편집](edit-metadata.md)의 열 선택기를 사용하여 이름을 바꿀 열을 선택하고 새 열 이름을 **새 열 이름** 텍스트 상자에 입력합니다. 여러 열의 이름을 바꾸는 경우, **새 열 이름** 에 입력하는 값이 고유한 값인지 확인합니다.  
...
**해결 방법:** 구성 요소를 다시 확인하고 URI의 형식을 확인합니다. 데이터 원본이 HTTP를 통해 연결되는 웹 URL인 경우, 의도한 원본이 파일 또는 루프백 URI(localhost)가 아닌지 확인합니다.  
...
+ 데이터 세트에서 원하는 데이터 형식으로의 변환을 차단할 수 있는 특수 문자 또는 누락된 값이 있는지 확인합니다. 
    ...
    + 날짜/시간 데이터는 모델링 오류가 발생하지 않도록 일관성을 유지해야 하며, 다만 정리는 다양한 형식으로 인해 복잡해질 수 있습니다. 다음 항목을 사용해보시기 바랍니다. <!--the [Execute R Script](execute-r-script.md) or -->[Python 스크립트](execute-python-script.md) 구성 요소를 실행 하 여 정리를 수행 합니다.  
...
> [!TIP]
> 해결 방법이 불명확하거나 귀하의 사례에 적용할 수 없는 방법인가요? 이 문서에 대 한 사용자 의견을 보내고 구성 요소와 열의 데이터 형식을 포함 하 여 시나리오에 대 한 정보를 제공 하는 것을 환영 합니다. 저희는 이 정보를 사용하여 향후 더 자세한 문제 해결 단계를 제공할 것입니다.  
...
**해결 방법:** 구성 요소를 다시 방문 하 고 디렉터리 경로가 올바르게 포맷 되었으며 현재 자격 증명을 사용 하 여 액세스할 수 있는지 확인 합니다.  
...
## <a name="error-0047"></a>오류 0047  
 구성 요소에 전달 된 데이터 집합 중 일부의 기능 열 수가 너무 적으면 예외가 발생 합니다.  
...
**해결 방법:** 읽고 있는 구성 요소 및 파일을 다시 확인 합니다.  
...
## <a name="error-0049"></a>오류 0049  
 ...
 Azure Machine Learning에서 이 오류는 파일을 구문 분석할 수 없을 때 발생합니다. [데이터 가져오기](import-data.md) 구성 요소에서 선택한 파일 형식이 파일의 실제 형식과 일치 하지 않거나 파일에 인식할 수 없는 문자가 포함 된 경우이 오류가 표시 됩니다.  

**해결 방법:** 구성 요소를 다시 방문 하 여 파일 형식과 일치 하지 않는 경우 파일 형식 선택 사항을 수정 합니다. 가능하다면 파일을 검사하여 잘못된 문자가 포함되어 있지는 않은지 확인합니다.  
...
**해결 방법:** 구성 요소를 다시 방문 하 여 계정에 대 한 Azure 저장소 키가 올바른지 확인 합니다. 필요한 경우 Azure 클래식 포털에서 키를 다시 복사 합니다.  
...
## <a name="error-0056"></a>오류 0056  
 ...
 이 오류는 열이 올바른 데이터 형식이 면 사용 하는 구성 요소에도 해당 열을 기능, 레이블 또는 범주 열로 표시 해야 하는 경우에 발생할 수 있습니다.  
...
3.  열을 선택한 구성 요소에 대 한 도움말 항목을 검토 하 여 데이터 형식 또는 열 사용에 대 한 특정 요구 사항이 있는지 확인 합니다.  
...
## <a name="error-0057"></a>오류 0057  
 ...
 이 예외는 [데이터 내보내기](export-data.md) 구성 요소 또는 기타 구성 요소를 사용 하 여 Azure Machine Learning 파이프라인 결과를 Azure blob 저장소에 저장할 때 발생 하지만 이미 존재 하는 파일이 나 blob을 만들려고 시도 하는 경우에 발생 합니다.   

**해결 방법:**

 이 오류는 이전에 **Azure Blob Storage 쓰기 모드** 속성을 **오류** 로 설정한 경우에만 표시됩니다. 이미 존재 하는 blob에 데이터 집합을 쓰려고 시도 하는 경우이 구성 요소는 기본적으로 오류를 발생 시킵니다.

 - 구성 요소 속성을 열고 **Azure blob storage 쓰기 모드** 속성을 **Overwrite** 로 변경 합니다.
...
**해결 방법:** 해결 방법은 사용 중인 학습자 또는 강사 그리고 데이터 세트에 있는 열의 데이터 유형에 따라 달라집니다. 먼저 machine learning 알고리즘 또는 학습 구성 요소의 요구 사항을 확인 합니다.  

 입력 데이터 세트를 다시 확인합니다. 레이블로 간주될 열에 지금 만들고 있는 모델에 대한 올바른 데이터 형식이 있는지 확인합니다.  

 누락된 값에 대한 입력을 확인하고 필요하다면 이를 제거하거나 바꿉니다.  

 필요한 경우 [메타 데이터 편집](edit-metadata.md) 구성 요소를 추가 하 고 레이블 열이 레이블로 표시 되어 있는지 확인 합니다.  
...
**해결 방법:** 예외를 throw 하는 구성 요소를 다시 방문 합니다. 지정된 Blob이 스토리지 계정의 컨테이너에 존재하며 해당 사용 권한을 통해 Blob을 볼 수 있는지 확인합니다. 인코딩 형식의 Excel 또는 CSV가 있는 경우, 입력이 **containername/filename** 형식인지 확인합니다. SAS URI에 유효한 Blob 이름이 포함되어 있는지 확인합니다.  
...
**해결 방법:** 데이터 [내보내기](export-data.md) 구성 요소를 다시 확인하고 지정된 Blob 경로에 **컨테이너/파일** 이름 형식의 컨테이너와 파일 이름이 모두 포함되어 있는지 확인합니다.  
...
## <a name="error-0080"></a>오류 0080  
 모든 값이 누락된 열이 구성 요소에서 허용되지 않는 경우 예외가 발생합니다.  

 Azure Machine Learning 이 오류는 구성 요소에서 사용하는 하나 이상의 열에 누락된 값이 모두 포함되어 있을 때 생성됩니다. 예를 들어 구성 요소가 각 열에 대한 집계 통계를 계산하는 경우 데이터가 없는 열에서 작동할 수 없습니다. 이러한 경우 이 예외를 제외하고 구성 요소 실행이 중지됩니다.  
...
## <a name="error-0081"></a>오류 0081  
 축소할 차원 수가 하나 이상의 스파스 기능 열을 포함하는 입력 데이터 세트의 기능 열 수와 같으면 PCA 구성 요소에서 예외가 발생합니다.  
...
## <a name="error-0084"></a>오류 0084  
 ...
 Azure Machine Learning 이 오류는 점수가 포함된 R 스크립트의 출력으로 모델을 평가하기 위해 구성 요소 중 하나를 사용하려고 할 때 발생합니다.  
...
|예외 메시지|
|------------------------|
|Hive 테이블을 만들 수 없습니다. HDInsight 클러스터의 경우 클러스터와 연결된 Azure Storage 계정 이름이 구성 요소 매개 변수를 통해 전달되는 이름과 동일한지 확인하세요.|
|Hive 테이블 "{table_name}"을(를) 만들 수 없습니다. HDInsight 클러스터의 경우 클러스터와 연결된 Azure Storage 계정 이름이 구성 요소 매개 변수를 통해 전달되는 이름과 동일한지 확인하세요.|
...
**해결 방법:** 선택한 파일이 유효한 .zip 파일인지 확인하고 지원되는 압축 알고리즘 중 하나를 사용하여 압축되었는지 확인합니다.  
 ...
 압축된 새 압축 폴더에 원하는 파일을 읽고 사용자 지정 구성 요소를 다시 추가해 보세요.  
...
## <a name="error-0105"></a>오류 0105  
 이 오류는 구성 요소 정의 파일에 지원되지 않는 매개 변수 형식이 포함된 경우에 표시됩니다.  
  
 Azure Machine Learning 이 오류는 사용자 지정 구성 요소 xml 정의를 만들고 정의의 매개 변수 또는 인수 형식이 지원되는 형식과 일치하지 않을 때 생성됩니다.  
  
**해결 방법:** 사용자 지정 구성 요소 xml 정의 파일에 있는 **Arg** 요소의 type 속성이 지원되는 형식인지 확인합니다.  
...
## <a name="error-0107"></a>오류 0107  
 구성 요소 정의 파일이 지원되지 않는 출력 형식을 정의할 때 throw됩니다.  
  
 Azure Machine Learning 이 오류는 사용자 지정 구성 요소 xml 정의의 출력 포트 형식이 지원되는 형식과 일치하지 않을 때 생성됩니다.  
  
**해결 방법:** 사용자 지정 구성 요소 xml 정의 파일에 있는 Output 요소의 type 속성이 지원되는 형식인지 확인합니다.  
...
이 오류는 달리 처리되지 않은 내부 엔진 오류를 캡처하기 위해 제공됩니다. 따라서 오류를 생성 한 구성 요소에 따라이 오류의 원인이 다를 수 있습니다.  
...
**Python 스크립트 실행 구성 요소** **70_driver_logs** 에서 **azureml_main** 검색 하 고 발생 한 오류를 찾을 수 있습니다. 예를 들어 "File "/tmp/tmp01_ID/user_script.py", line 17, in azureml_main"은 Python 스크립트의 17번째 줄에서 오류가 발생했음을 나타냅니다.

## <a name="distributed-training"></a>분산 학습

현재 디자이너는 [PyTorch 모델](train-pytorch-model.md) 구성 요소에 대 한 분산 학습을 지원 합니다.
