...
3. 등록 된 데이터 원본에 대 한 검색 설정
    ...
    - **범위 검색**
        - 검색 범위를 설정 하는 동안 세분화 된 수준 또는 부모 수준과 관련 된 자산을 선택 합니다. 이렇게 하면 검색 비용이 최적이 고 성능이 효율적으로 유지 됩니다. 부모를 완전히 확인 하거나 부분적으로 선택 하는 경우 특정 부모에서 이후의 모든 자산이 자동으로 선택 됩니다. 예:
