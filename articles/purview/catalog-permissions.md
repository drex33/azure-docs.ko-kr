...
- **데이터 판독기** - 데이터 자산, 분류, 분류 규칙, 컬렉션, 용어집 용어 및 인사이트에 대한 읽기 전용 액세스를 제공하는 역할입니다.
- **데이터 원본 관리자** - 사용자가 데이터 원본 및 검사를 관리할 수 있도록 하는 역할입니다. 데이터 원본 관리자 역할의 사용자는 Azure Purview Studio에 액세스할 수 없습니다. 이 역할을 모든 컬렉션 범위에서 데이터 판독기 또는 데이터 권한자 역할과 결합하면 Azure Purview Studio에 액세스할 수 있습니다.
...
역할 할당은 컬렉션을 통해 관리됩니다. [컬렉션 관리자 역할](#roles)이 할당된 사용자만이 해당 컬렉션의 다른 사용자에게 권한을 부여할 수 있습니다. 새 권한을 추가해야 하는 경우 컬렉션 관리자는 [Purview Studio에](https://web.purview.azure.com/resource/)액세스하고, 데이터 맵으로 이동한 다음, 컬렉션 탭으로 이동한 다음, 사용자를 추가해야 하는 컬렉션을 선택합니다. [역할 할당] 탭에서는 권한이 필요한 사용자를 추가하고 관리할 수 있습니다.
