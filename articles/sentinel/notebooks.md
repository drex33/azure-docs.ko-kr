...
Microsoft Sentinel의 기반은 데이터 저장소입니다. 고성능 쿼리, 동적 스키마를 결합하고 대규모 데이터 볼륨으로 확장합니다. Azure Portal 및 모든 Microsoft Sentinel 도구는 공통 API를 사용하여 이 데이터 저장소에 액세스합니다.
...
- 사용자 지정 타임라인 및 프로세스 **트리와** 같이 Microsoft Sentinel에서 바로 제공되지 않는 데이터 시각화 만들기
- 온-프레미스 데이터 집합과 같은 Microsoft Sentinel 외부의 데이터 **원본을 통합합니다.**

Jupyter 환경이 Azure Portal에 통합되어 있어서 Notebook을 쉽게 만들고 실행하여 데이터를 분석할 수 있습니다. *Kqlmagic* 라이브러리는 Microsoft Sentinel에서 [KQL](https://kusto.azurewebsites.net/docs/kusto/query/index.html) 쿼리를 받아서 Notebook 내에서 직접 실행할 수 있는 glue를 제공합니다.

Microsoft의 일부 보안 분석가가 개발한 몇 가지 Notebook은 Microsoft Sentinel과 함께 패키지됩니다.
...
다른 Notebook은 [Microsoft Sentinel GitHub 리포지토리에서](https://github.com/Azure/Azure-Sentinel-Notebooks/)가져올 수도 있습니다.
...
Microsoft Sentinel Notebook은 *pandas,* *matplotlib,* *bokeh* 등과 같은 많은 인기 있는 Python 라이브러리를 사용합니다. 그 외 다음과 같은 영역을 지원하는 다양한 Python 패키지를 선택할 수 있습니다.
...
Microsoft Sentinel Notebook은 데이터 검색, 분석, 보강 및 시각화를 위한 사이버 보안 도구 모음인 [MSTICPy라는](https://github.com/Microsoft/msticpy/)Python 패키지를 사용합니다. 
...
[Microsoft Sentinel GitHub 리포지토리는](https://github.com/Azure/Azure-Sentinel-Notebooks/) Microsoft에서 빌드하거나 커뮤니티에서 제공한 향후 Microsoft Sentinel Notebook을 위한 위치입니다.

## <a name="manage-access-to-microsoft-sentinel-notebooks"></a>Microsoft Sentinel Notebook에 대한 액세스 관리
...
## <a name="create-an-azure-ml-workspace-from-microsoft-sentinel"></a>Microsoft Sentinel에서 Azure ML 작업 영역 만들기

이 절차에서는 Microsoft Sentinel Notebook용 Microsoft Sentinel에서 Azure ML 작업 영역을 만드는 방법을 설명합니다.

**작업 영역을 만들려면 다음을 수행합니다.**

1. Azure Portal **Microsoft Sentinel**  >  **Threat Management**  >  **Notebooks로** 이동한 다음, 새 AML 작업 영역 **만들기를** 선택합니다.
...
1. **네트워킹** 탭에서 작업 영역을 연결하는 데 퍼블릭 엔드포인트를 사용할지, 아니면 프라이빗 엔드포인트를 사용할지 선택합니다. Microsoft 센티널 작업 영역에 공용 끝점이 있는 경우 Azure ML 작업 영역에 대 한 공용 끝점을 사용 하 여 네트워크 통신의 잠재적인 문제를 방지 하는 것이 좋습니다. 완료되면 **검토 + 만들기** 를 선택합니다.
...
1. 배포가 완료 되 면 Microsoft 센티널 **노트북** 으로 돌아가서 새 Azure ML 작업 영역에서 노트북을 실행할 수 있습니다.
...
1. Microsoft 센티널 노트북에서 빈 셀에 다음 코드를 입력 하 고 셀을 실행 합니다.
    ...
    GitHub 리포지토리 cotents의 복사본은 azure ML 작업 영역에 있는 사용자 폴더의 **azure-센티널-nb** 디렉터리에 만들어집니다.
