...
| 매개 변수 | Description |  
|-|-|  
|**/r:** <RepairFile\>|**필수 사항입니다.** 복구의 진행 상황을 추적하고 중단된 복구를 다시 시작할 수 있도록 하는 복구 파일의 경로입니다. 각 드라이브에는 하나의 복구 파일만 있어야 합니다. 지정된 드라이브의 복구를 시작할 때 아직 존재하지 않는 복구 파일의 경로를 지정합니다. 중단된 복구를 다시 시작하려면 기존 복구 파일의 이름을 제공해야 합니다. 대상 드라이브에 해당하는 복구 파일을 항상 지정합니다.|  
|**/logdir:** <LogDirectory\>|**선택 사항입니다.** 로그 디렉터리입니다. 이 디렉터리에 자세한 로그 파일이 기록됩니다. 로그 디렉터리를 지정하지 않는 경우 현재 디렉터리가 로그 디렉터리로 사용됩니다.|  
|**/d:**<TargetDirectories\>|**필수 사항입니다.** 가져온 원본 파일을 포함하는 하나 이상의 세미콜론으로 구분된 디렉터리. 가져오기 드라이브도 사용할 수 있으나 원본 파일의 대체 위치를 사용할 수 있는 경우 필요하지 않습니다.|  
|**/bk:** <BitLockerKey\>|**선택 사항입니다.** 이 도구를 사용하여 원본 파일을 사용할 수 있는 암호화된 드라이브의 잠금을 해제하려면 BitLocker 키를 지정합니다.|  
...
|**/CopyLogFile:** <DriveCopyLogFile\>|**필수 사항입니다.** 드라이브 복사 로그 파일(자세한 로그 또는 오류 로그)의 경로입니다. 이 파일은 Microsoft Azure Import/Export 서비스에 의해 생성되고 작업과 연결된 Blob Storage에서 다운로드할 수 있습니다. 복사 로그 파일에는 복구해야 하는 실패한 blob 또는 파일에 대한 정보가 포함되어 있습니다.|  
|**/PathMapFile:**<DrivePathMapFile\>|**선택 사항입니다.** 같은 작업에서 가져오는 여러 파일이 같은 이름을 갖는 경우 모호성을 해결하기 위해 사용하는 텍스트 파일의 경로입니다. 이 도구는 처음 실행될 때 모호한 모든 이름으로 이 파일을 채울 수 있습니다. 이 도구를 나중에 실행하면 이 파일이 모호성을 해결하는 데 사용됩니다.|  
...
상황에 따라 이 도구는 파일을 찾을 수 없거나, 파일에 액세스할 수 없거나, 파일 이름이 모호하거나, 파일 콘텐츠가 더 이상 올바르지 않는 등의 이유로 인해 필요한 파일을 찾거나 열지 못할 수 있습니다.  
