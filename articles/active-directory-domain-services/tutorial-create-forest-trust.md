...
1. 다른 **DNS 도메인** *(예: contoso.com)* 을 입력한 다음, 아래 예제와 같이 해당 네임스페이스에 대한 DNS 서버의 IP 주소를 입력합니다.
    ...
    ![DNS 서버에 대한 조건부 전달자를 추가 및 구성하는 방법에 대한 스크린샷](./media/manage-dns/create-conditional-forwarder.png)

1. **Active Directory에 이 조건부 전달자를 저장하고 다음과 같이 복제** 확인란을 선택하고 다음 예제와 같이 ‘이 도메인의 모든 DNS 서버’ 옵션을 선택합니다.

    ![이 도메인의 모든 DNS 서버를 선택하는 방법에 대한 스크린샷](./media/manage-dns/store-in-domain.png)
...
1. [Azure Bastion](../bastion/bastion-overview.md) 및 Azure AD DS 관리자 자격 증명을 사용하여 Azure AD DS 리소스 포리스트에 조인된 Windows Server VM에 연결합니다.
