...
1. 연결 유형에 따라 다음 탭 중 하나를 선택 합니다.
    ...
    > [!NOTE]
    > 단일 Azure 센티널 작업 영역에서 동일한 리포지토리 및 분기를 사용 하 여 중복 연결을 만들 수 없습니다.
    >
...
소스 제어 리포지토리에 대한 연결을 성공적으로 만든 후 해당 리포지토리의 콘텐츠가 수정되거나 추가될 때마다 배포 워크플로가 다시 실행되어 리포지토리의 모든 콘텐츠를 연결된 모든 Azure Sentinel 작업 영역에 배포합니다.

연결된 리포지토리에 저장된 콘텐츠는 리포지토리에서만 편집하고 Azure Sentinel 편집하지 않는 것이 좋습니다.  예를 들어 분석 규칙을 변경하려면 GitHub 또는 Azure DevOps 직접 변경합니다.
...
1. Azure Sentinel **왼쪽의 콘텐츠 관리** 에서 **리포지토리를** 선택합니다.
