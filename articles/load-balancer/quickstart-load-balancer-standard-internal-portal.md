...
3. **가상 네트워크 만들기** 의 **기본** 탭에서 다음 정보를 입력하거나 선택합니다.
    ...
    | 리소스 그룹   | **새로 만들기** 를 선택합니다. </br> **이름** 에서 **CreateIntLBQS-rg** 를 입력합니다. </br> **확인** 을 선택합니다. |
    | **인스턴스 세부 정보** |                                                                 |
    | 속성             | **myVNet** 입력                                    |
    | 지역           | **(유럽) 서유럽** 선택 |
...
3. **NAT(Network Address Translation) 게이트웨이 만들기** 에서 다음 정보를 입력하거나 선택합니다.
    ...
    | Resource group | **CreateIntLBQS-rg** 를 선택합니다. |
    | **인스턴스 세부 정보** |    |
...
3. **부하 분산 장치 만들기** 페이지의 **기본 사항** 탭에서 다음 정보를 입력하거나 선택합니다. 
    ...
    :::image type="content" source="./media/quickstart-load-balancer-standard-internal-portal/create-standard-internal-load-balancer.png" alt-text="표준 부하 분산 장치 만들기 기본 사항 탭의 스크린샷" border="true":::
...
22. **만들기** 를 선택합니다.
    ...
    > [!NOTE]
    > 이 예제에서는 아웃바운드 인터넷 액세스를 제공하는 NAT 게이트웨이를 만들었습니다. 구성의 아웃바운드 규칙 탭은 NAT 게이트웨이에서 무시되며 필요하지 않습니다. Azure NAT 게이트웨이에 대한 자세한 내용은 [Azure Virtual Network NAT란?](../virtual-network/nat-gateway/nat-overview.md)을 참조하세요.
...
3. **부하 분산 장치 만들기** 페이지의 **기본 사항** 탭에서 다음 정보를 입력하거나 선택합니다. 
    ...
    :::image type="content" source="./media/quickstart-load-balancer-standard-internal-portal/create-basic-internal-load-balancer.png" alt-text="표준 부하 분산 장치 만들기 기본 사항 탭의 스크린샷" border="true":::
...
12. **백 엔드 풀 추가** 의 **이름** 에 **myBackendPool** 을 입력합니다.

13. **다음에 연결됨** 에서 **가상 머신** 을 선택합니다.

14. **IP 버전** 에 대해 **IPv4** 또는 **IPv6** 를 선택합니다.

15. **추가** 를 선택합니다.

16. 페이지 하단에 있는 **다음: 인바운드 규칙** 단추를 선택합니다.

17. **인바운드 규칙** 탭의 **부하 분산 규칙** 에서 **+ 부하 분산 규칙 추가** 를 선택합니다.

18. **부하 분산 규칙 추가** 에서 다음 정보를 입력하거나 선택합니다.
...
20. 페이지 하단에 있는 파란색 **검토 + 만들기** 단추를 선택합니다.
...
5. [네트워킹] 탭에서 다음을 선택하거나 입력합니다.
    ...
    | 네트워크 보안 그룹 구성 | **새로 만들기** 를 선택합니다. </br> **네트워크 보안 그룹 만들기** 의 **이름** 에서 **myNSG** 를 입력합니다. </br> **인바운드 규칙** 아래에서 **+ 인바운드 규칙 추가** 를 선택합니다. </br> **대상 포트 범위** 아래에서 **80** 을 입력합니다. </br> **우선 순위** 아래에서 **100** 을 입력합니다. </br> **이름** 에 **myNSGRule** 을 입력합니다. </br> **추가** 를 선택합니다. </br> **확인** 을 선택합니다. |
    | **부하 분산**.  |
    | 기존 부하 분산 솔루션 뒤에 이 가상 머신을 배치하시겠습니까? | 상자 선택 |
    | **부하 분산 설정** |   |
...
4. **Bastion 사용** 을 선택합니다. 

5. VM을 만드는 동안 입력한 사용자 이름과 암호를 입력합니다.

6. **연결** 을 선택합니다.

7. 서버 데스크톱에서 **Windows 관리 도구** > **Windows PowerShell** > **Windows PowerShell** 로 이동합니다.

8. PowerShell 창에서 다음 명령을 실행합니다.
...
9. **myVM1** 이 포함된 Bastion 세션을 닫습니다.

10. 1~9단계를 반복하여 **myVM2** 및 **myVM3** 에 IIS 및 업데이트된 iisstart.htm 파일을 설치합니다.

## <a name="test-the-load-balancer"></a>부하 분산 장치 테스트

이 섹션에서는 **myTestVM** 에 연결하고 웹 페이지를 확인하여 부하 분산 장치를 테스트합니다.
...
4. 포털 맨 위에 있는 검색 상자에 **가상 머신** 을 입력합니다. 검색 결과에서 **가상 머신** 을 선택합니다.

5. **myTestVM** 을 선택합니다.

6. **개요** 페이지에서 **연결** 을 선택한 다음, **Bastion** 을 선택합니다.

7. **Bastion 사용** 을 선택합니다.

8. VM을 만드는 동안 입력한 사용자 이름과 암호를 입력합니다.

9. **myTestVM** 에서 **Internet Explorer** 를 엽니다.

10. 이전 단계의 IP 주소를 브라우저의 주소 표시줄에 입력합니다. 백 엔드 서버 이름 중 하나를 표시하는 사용자 지정 페이지가 브라우저에 표시됩니다.

    :::image type="content" source="./media/quickstart-load-balancer-standard-internal-portal/load-balancer-test.png" alt-text="스크린샷은 예상대로 사용자 지정된 페이지를 표시하는 브라우저 창을 보여 줍니다." border="true":::
