...
description: Azure Data Factory 또는 Synapse Analytics에서 Machine Learning Studio (클래식)-Batch 실행 작업을 사용 하 여 예측 파이프라인을 만드는 방법에 대해 알아봅니다.
...
ms.custom: synapse
ms.date: 09/09/2021
ms.openlocfilehash: 99f9e57c1b9b07cd763ea7a8022bbc2069c98f18
ms.sourcegitcommit: b00a2d931b0d6f1d4ea5d4127f74fc831fb0bca9
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/20/2021
ms.locfileid: "132863674"
---
# <a name="create-a-predictive-pipeline-using-machine-learning-studio-classic-with-azure-data-factory-or-synapse-analytics"></a>Azure Data Factory 또는 Synapse Analytics를 사용 하 여 Machine Learning Studio (클래식)를 사용 하 여 예측 파이프라인 만들기
...
> [!NOTE] 
> 2021 년 12 월 1 일 이후에는 Machine Learning Studio (클래식) 리소스를 더 이상 만들 수 없으므로 사용자는 일괄 처리 실행 작업을 사용 하 여 실행 하는 대신 [파이프라인 실행 작업 Machine Learning](transform-data-machine-learning-service.md) [Azure Machine Learning](https://azure.microsoft.com/services/machine-learning/) 사용 하는 것이 좋습니다 Machine Learning  Studio (클래식) 일괄 처리.
...
### <a name="using-machine-learning-studio-classic-with-azure-data-factory-or-synapse-analytics"></a>Azure Data Factory 또는 Synapse Analytics에서 Machine Learning Studio (클래식) 사용
Azure Data Factory 및 Synapse analytics를 사용 하면 예측 분석을 위해 게시 된 [Machine Learning Studio (클래식)](https://azure.microsoft.com/documentation/services/machine-learning) 웹 서비스를 사용 하는 파이프라인을 쉽게 만들 수 있습니다. 파이프라인에서 **batch 실행 작업** 을 사용 하 여 Machine Learning Studio (클래식) 웹 서비스를 호출 하 여 일괄 처리로 데이터를 예측할 수 있습니다.

시간이 지남에 따라 Machine Learning Studio (클래식) 점수 매기기 실험의 예측 모델은 새 입력 데이터 집합을 사용 하 여 다시 학습 해야 합니다. 다음 단계를 수행 하 여 파이프라인에서 모델을 다시 학습 수 있습니다.
...
Machine Learning studio (클래식) 웹 서비스를 연결 하는 **Machine Learning studio (클래식)** 연결 된 서비스를 만듭니다. 연결 된 서비스는 Machine Learning Studio (클래식) Batch 실행 작업 및 [리소스 업데이트 작업](update-machine-learning-models.md)에 사용 됩니다.
...
Machine Learning Studio (클래식)는 예측 실험을 위해 기존 웹 서비스와 새 웹 서비스를 모두 지원 합니다. Data Factory 또는 Synapse 작업 영역에서 사용할 올바른 항목을 선택할 수 있습니다. Machine Learning Studio (클래식) 연결 된 서비스를 만드는 데 필요한 정보를 얻으려면로 이동 https://services.azureml.net 합니다. 여기에는 모든 (신규) 웹 서비스 및 클래식 웹 서비스가 나열 됩니다. 액세스하려는 웹 서비스를 클릭하고 **사용** 페이지를 클릭합니다. **apiKey** 속성의 **기본 키** 및 **mlEndpoint** 속성의 **Batch 요청** 을 복사합니다.
...
이 시나리오에서 Machine Learning Studio (클래식) 웹 서비스는 Azure blob 저장소에 있는 파일의 데이터를 사용 하 여 예측을 수행 하 고 예측 결과를 blob 저장소에 저장 합니다. 다음 JSON은 AzureMLBatchExecution 활동을 사용 하 여 파이프라인을 정의 합니다. Azure 블로그 스토리지의 입력 및 출력 데이터는 LinkedName 및 FilePath 쌍을 사용하여 참조됩니다. 입력 및 출력의 연결 된 샘플 서비스가 서로 다르기 때문에 서비스에 대 한 각 입/출력에 대해 서로 다른 연결 된 서비스를 사용 하 여 올바른 파일을 선택 하 고 Machine Learning Studio (클래식) 웹 서비스로 보낼 수 있습니다.
