...
description: 이 문서에서는 Azure Stack Edge Azure Video Analyzer를 배포하는 방법을 설명합니다.
ms.topic: how-to
ms.date: 11/04/2021
...
ms.locfileid: "132487634"
---
# <a name="deploy-azure-video-analyzer-on-azure-stack-edge"></a>Azure Stack Edge에 Azure Video Analyzer 배포

이 문서에서는 Azure Stack Edge 디바이스에 Azure Video Analyzer를 배포하기 위한 전체 지침을 제공합니다. 디바이스를 설정하고 활성화하면 Video Analyzer 배포가 준비됩니다. 

이 문서에서는 Azure IoT Hub를 사용하여 Video Analyzer를 배포하지만, Azure Stack Edge 리소스는 Kubernetes API를 노출하며, 이를 통해 Video Analyzer와 인터페이스할 수 있는 비 IoT Hub 인식 솔루션을 추가로 배포할 수 있습니다. 

> [!TIP]
> 사용자 지정 배포에 Kubernetes API를 사용하는 것은 고급 사례입니다. Kubernetes API를 사용하는 대신 에지 모듈을 만들고 IoT Hub 통해 각 Azure Stack Edge 리소스에 배포하는 것이 좋습니다. 이 문서에서는 IoT Hub 사용하여 Video Analyzer 모듈을 배포하는 방법을 보여줍니다.

## <a name="prerequisites"></a>사전 요구 사항

* Azure Video Analyzer 계정

    이 [클라우드 서비스는](../overview.md) Video Analyzer 에지 모듈을 등록하고 녹화된 비디오 및 비디오 분석을 재생하는 데 사용됩니다.
* 관리 ID

    스토리지 계정에 대한 액세스를 관리하는 데 사용하는 사용자 할당 [관리 ID입니다.](../../../active-directory/managed-identities-azure-resources/overview.md)
* [Azure Stack Edge 리소스](../../../databox-online/azure-stack-edge-gpu-deploy-prep.md)
* [IoT Hub](../../../iot-hub/iot-hub-create-through-portal.md)
* 스토리지 계정

    [범용 v2 스토리지 계정 을](../../../storage/common/storage-account-upgrade.md?tabs=azure-portal)사용하는 것이 좋습니다.  
* [개발](https://code.visualstudio.com/)머신에 설치된 Visual Studio Code
*  Visual Studio Code 설치된 [Azure IoT Tools 확장](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-tools)입니다.
* 개발 컴퓨터가 연결된 네트워크에서 5671 포트를 통해 고급 메시지 큐 프로토콜을 허용하는지 확인합니다. 이 설정을 사용하면 Azure IoT Tools Azure IoT 허브와 통신할 수 있습니다.

## <a name="configure-azure-stack-edge-to-use-video-analyzer"></a>Video Analyzer를 사용하도록 Azure Stack Edge 구성

Azure Stack Edge 하드웨어-as-a-Service 솔루션 및 네트워크 데이터 전송 기능이 있는 AI 지원 에지 컴퓨팅 디바이스입니다. 자세한 내용은 [Azure Stack Edge 및 자세한 설치 지침을 참조하세요.](../../../databox-online/azure-stack-edge-gpu-deploy-prep.md) 
...
1. [GPU를 Azure Stack Edge Pro 설치하고 설정합니다.](../../../databox-online/azure-stack-edge-gpu-deploy-install.md)  
1. 다음을 수행하여 리소스를 커넥트 활성화합니다.

    a. [로컬 웹 UI 설정 에](../../../databox-online/azure-stack-edge-gpu-deploy-connect.md)커넥트.  
    b. [네트워크 를 구성합니다.](../../../databox-online/azure-stack-edge-gpu-deploy-configure-network-compute-web-proxy.md)  
    다. [디바이스를 구성합니다.](../../../databox-online/azure-stack-edge-gpu-deploy-set-up-device-update-time.md)  
    d. [인증서를 구성합니다.](../../../databox-online/azure-stack-edge-gpu-deploy-configure-certificates.md)  
    e. [디바이스 활성화](../../../databox-online/azure-stack-edge-gpu-deploy-activate.md).  

1. [Azure Stack Edge IoT Hub를 연결합니다.](../../../databox-online/azure-stack-edge-gpu-deploy-configure-compute.md#configure-compute)

### <a name="meet-the-compute-prerequisites-on-the-azure-stack-edge-local-ui"></a>Azure Stack Edge 로컬 UI에서 컴퓨팅 필수 조건 충족

계속하기 전에 다음을 완료했는지 확인합니다.

* Azure Stack Edge 리소스를 활성화 했습니다.
* PowerShell 5.0 이상에서 실행되는 Windows 클라이언트 시스템에 액세스하여 Azure Stack Edge 리소스에 액세스할 수 있습니다.
* Kubernetes 클러스터를 배포하기 위해 [로컬 웹 UI](../../../databox-online/azure-stack-edge-deploy-connect-setup-activate.md#connect-to-the-local-web-ui-setup)에서 Azure Stack Edge 리소스를 구성했습니다. 

    1. 다음을 수행하여 리소스를 커넥트 구성합니다. a. [로컬 웹 UI 설정 에](../../../databox-online/azure-stack-edge-gpu-deploy-connect.md)커넥트.  
        b. [네트워크 를 구성합니다.](../../../databox-online/azure-stack-edge-gpu-deploy-configure-network-compute-web-proxy.md)  
        다. [디바이스 구성](../../../databox-online/azure-stack-edge-gpu-deploy-set-up-device-update-time.md)  
        d. [인증서를 구성합니다.](../../../databox-online/azure-stack-edge-gpu-deploy-configure-certificates.md)  
        e. [디바이스 활성화](../../../databox-online/azure-stack-edge-gpu-deploy-activate.md).

    1. 컴퓨팅을 사용하도록 설정하려면 디바이스의 로컬 웹 UI에서 **컴퓨팅** 페이지로 이동합니다.
    
        a. 컴퓨팅에 사용할 네트워크 인터페이스를 **선택한** 다음 사용을 선택합니다. 컴퓨팅을 사용하도록 설정하면 해당 네트워크 인터페이스의 디바이스에 가상 스위치가 만들어집니다.  
        b. Kubernetes 테스트 노드 IP와 Kubernetes 외부 서비스 IP를 비워 둡니다.  
        다. **적용** 을 선택합니다. 작업에는 약 2분이 소요됩니다.
        
        > [!div class="mx-imgBorder"]

        > :::image type="content" source="../../../databox-online/media/azure-stack-edge-gpu-deploy-configure-network-compute-web-proxy/compute-network-2.png" alt-text="Azure Stack Edge 로컬 UI의 컴퓨팅 필수 조건 스크린샷.":::

        Azure DNS Kubernetes API 및 Azure Stack Edge 리소스에 대해 구성되지 않은 경우 다음을 수행하여 Windows 호스트 파일을 업데이트할 수 있습니다.
        
        a. 텍스트 편집기를 관리자 권한으로 엽니다.  
        b. *C:\Windows\System32\drivers\etc에서 \\* *호스트* 파일을 엽니다.  
        다. Kubernetes API 디바이스 이름의 인터넷 프로토콜 버전 4(IPv4) 및 호스트 이름을 파일에 추가합니다. 이 정보는 Azure Stack Edge 포털의 디바이스 아래에서 찾을 수 **있습니다.**  
        d. 파일을 저장하고 닫습니다.

### <a name="deploy-video-analyzer-edge-modules-by-using-the-azure-portal"></a>Azure Portal 사용하여 Video Analyzer Edge 모듈 배포

Azure Portal 배포 매니페스트를 만들고 IoT Edge 디바이스에 배포를 푸시할 수 있습니다.  

#### <a name="select-your-device-and-set-modules"></a>디바이스를 선택하고 모듈 설정

1. [Azure Portal](https://ms.portal.azure.com/)로그인한 다음, IoT Hub로 이동합니다.
...
배포 매니페스트는 배포할 모듈, 모듈 간에 데이터가 흐르는 방식 및 모듈 쌍의 desired 속성을 설명하는 JSON 문서입니다. Azure Portal에는 배포 매니페스트를 만들 수 있도록 안내하는 마법사가 있습니다. 세 단계는 **모듈,** **경로** 및 **검토 + 만들기** 탭으로 구성됩니다.

#### <a name="add-modules"></a>모듈 추가

1. IoT Edge **모듈** 섹션의 **추가** 드롭다운 목록에서 **IoT Edge 모듈을** 선택하여 IoT Edge **모듈 추가** 페이지를 표시합니다.
1. **모듈 설정** 탭을 선택하고 모듈의 이름을 제공한 다음 컨테이너 이미지 URI를 지정합니다. 예제 값은 다음 이미지에 표시됩니다.     
    
    > [!div class="mx-imgBorder"]
    > :::image type="content" source="./media/deploy-on-stack-edge/add-module.png" alt-text="IoT Edge 모듈 추가 페이지의 모듈 설정 창 스크린샷.":::
    
    > [!TIP]
    > 이 절차에 설명된 대로 모듈 설정 , **컨테이너 만들기 옵션** 및 **모듈 쌍 설정** 탭에서 값을 지정할 때까지  **추가를** 선택하지 마세요.
    
    > [!IMPORTANT]
    > Azure IoT Edge 값은 모듈을 호출할 때 대/소문자를 구분합니다. 모듈 이름으로 사용 중이던 정확한 문자열을 기록해 두십시오.
1. 환경 **변수 탭을** 선택한 다음, 다음 이미지와 같이 값을 입력합니다.
   
    > [!div class="mx-imgBorder"]
    > :::image type="content" source="./media/deploy-on-stack-edge/environment-variables.png" alt-text="'IoT Edge 모듈 추가' 페이지의 '환경 변수' 창 스크린샷.":::
1. 컨테이너 **만들기 옵션** 탭을 선택합니다.

    > [!div class="mx-imgBorder"]
    > :::image type="content" source="./media/deploy-on-stack-edge/container-create-options.png" alt-text="IoT Edge 모듈 추가 페이지의 컨테이너 만들기 옵션 창 스크린샷.":::
 
    **컨테이너 만들기 옵션** 창의 상자에 다음 JSON 코드를 붙여넣습니다. 이 작업은 모듈에서 생성되는 로그 파일의 크기를 제한합니다.
...
1. **모듈 설정** 탭을 선택하고 모듈의 이름을 제공한 다음 컨테이너 이미지 URI를 지정합니다. 예:   
    ...
    * **이미지 URI:** mcr.microsoft.com/ava-utilities/rtspsim-live555:1.2 
...
마지막으로 IoT Edge 디바이스 모듈이 예상대로 연결되어 실행되고 있는지 확인합니다. 모듈의 런타임 상태를 확인하려면 다음을 수행합니다.
...
1. 추가 **옵션** 아이콘을 선택하여 상황에 맞는 메뉴를 표시한 다음, **연결 문자열 IoT Hub 설정을** 선택합니다.
   ...
   연결에 성공하면 Azure Stack Edge 디바이스를 포함한 에지 디바이스 목록이 표시됩니다. 이제 상황에 맞는 메뉴를 통해 IoT Edge 디바이스를 관리하고 Azure IoT 허브와 상호 작용할 수 있습니다. 
...
* **(고급) Pod 공동 위치**
    ...
    * **옵션 1:** 공동 위치에 *노드 선호도* 및 기본 제공 노드 레이블을 사용합니다.  
    ...
    현재 NodeSelector 사용자 지정 구성은 옵션으로 표시되지 않습니다. 사용자가 노드에서 레이블을 설정할 수 있는 액세스 권한이 없기 때문입니다. 그러나 사용자의 토폴로지 및 명명 규칙에 따라 기본 제공 노드 레이블 을 사용할 수 [있습니다.](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#built-in-node-labels) 공동 위치를 얻으려면 Video Analyzer를 사용하여 Azure Stack Edge 리소스를 참조하는 nodeAffinity 섹션을 유추 Pod 매니페스트에 추가할 수 있습니다.    
    * **옵션 2:**(권장) 공동 위치에 *Pod 선호도를* 사용합니다.  

        Kubernetes는 동일한 노드에서 [Pod를](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#inter-pod-affinity-and-anti-affinity)예약할 수 있는 Pod 선호도 를 지원합니다. 공동 위치를 얻으려면 Video Analyzer 모듈을 참조하는 podAffinity 섹션인 유추 Pod 매니페스트에 를 추가할 수 있습니다.
