...
1. 다음 그림에 표시 된 것 처럼 모든 서비스 끝점에 대 한 [사용자 지정 도메인 이름을](configure-custom-domain.md) 설정 합니다.
...
2. DNS 서버에 레코드를 생성하여 VNET 내에서 액세스할 수 있는 엔드포인트에 액세스합니다. 끝점 레코드를 서비스의 [개인 가상 IP 주소](#routing) 에 매핑합니다.

## <a name="routing"></a>라우팅

다음 가상 IP 주소는 내부 가상 네트워크의 API Management 인스턴스에 대해 구성 됩니다. [API Management의 IP 주소](api-management-howto-ip-addresses.md)에 대해 자세히 알아보세요.

| 가상 IP 주소 | 설명 |
| ----- | ----- |
| **개인 가상 IP 주소** | API 게이트웨이, 개발자 포털, 관리 및 Git 끝점에 액세스할 수 있는 API Management 인스턴스의 서브넷 범위 (DIP) 내에서 부하가 분산 된 IP 주소입니다.<br/><br/>VNET에서 사용하는 DNS 서버에 이 주소를 등록합니다.     |  
| **공용 가상 IP 주소** | 포트 3443을 통해 관리 끝점에 대 한 제어 평면 트래픽용 *으로만* 사용 됩니다. [ApiManagement][ServiceTags] 서비스 태그로 잠글 수 있습니다. | 
...
DIP (동적 IP) 주소는 서비스의 각 기본 가상 머신에 할당 되며 VNET *내의* 리소스에 액세스 하는 데 사용 됩니다. API Management 서비스의 공용 VIP (가상 IP) 주소는 VNET *외부* 리소스에 액세스 하는 데 사용 됩니다. IP 제한 목록이 VNET 내의 리소스를 보호하는 경우, 서비스에서 액세스 권한을 부여하거나 제한하도록 API Management 서비스가 배포된 서브넷의 전체 범위를 지정해야 합니다.

[권장 서브넷 크기](virtual-network-concepts.md#subnet-size)에 대해 자세히 알아보세요.

#### <a name="example"></a>예제

API Management의 [용량 단위](api-management-capacity.md) 하나를 내부 VNET의 Premium 계층에 배포 하는 경우 3 개의 IP 주소가 사용 됩니다. 개인 VIP의 경우 1, 두 vm의 경우 각 dip에 대해 1 개의 IP 주소가 사용 됩니다. 4 개 단위로 규모를 확장 하는 경우 서브넷에서 추가 Dip에 더 많은 Ip가 소비 됩니다.  

대상 끝점에서 허용 된 Dip 집합만 허용 하는 경우 나중에 새 단위를 추가 하면 연결 오류가 발생 합니다. 이러한 이유로 서브넷은 전적으로 컨트롤에 있기 때문에 백 엔드에서 전체 서브넷을 나열 하도록 허용 하는 것이 좋습니다.

## <a name="next-steps"></a>다음 단계

다음에 대해 자세히 알아봅니다.

* [VNET에서 Azure API Management을 설정할 때의 네트워크 구성][Common network configuration problems]
