...
* **고성능**: 데이터를 Data Lake Storage Gen1에 최대 1GB/s 속도로 로드합니다. 자세한 내용은 [복사 작업 성능](copy-activity-performance.md)을 참조하세요.

이 문서에서는 Data Factory 복사 데이터 도구를 사용하여 _Amazon S3의 데이터를 Data Lake Storage Gen1으로 로드_ 하는 방법을 설명합니다. 다른 데이터 저장소 유형에서 데이터를 복사할 때도 이와 유사한 단계를 따를 수 있습니다.
...
1. 왼쪽 메뉴에서 **리소스 만들기** > **분석** > **Data Factory** 를 선택합니다.
   ...
   :::image type="content" source="./media/quickstart-create-data-factory-portal/new-azure-data-factory-menu.png" alt-text="새로 Data Factory &quot; &quot; 창에서 선택":::

2. **새 데이터 팩터리** 페이지에서 다음 그림에 표시된 필드의 값을 제공합니다. 
      
   :::image type="content" source="./media/load-data-into-azure-data-lake-store//new-azure-data-factory.png" alt-text="새 데이터 팩터리 페이지":::
 
    * **Name**: Azure Data Factory의 전역적으로 고유 이름을 입력합니다. "데이터 팩터리 이름 \"LoadADLSG1Demo\"를 사용할 수 없습니다" 오류가 발생하면 데이터 팩터리의 다른 이름을 입력합니다. 예를 들어 _**yourname**_**ADFTutorialDataFactory** 라는 이름을 사용할 수 있습니다. 데이터 팩터리를 다시 만들어 봅니다. 데이터 팩터리 아티팩트에 대한 명명 규칙은 [데이터 팩터리 명명 규칙](naming-rules.md)을 참조하세요.
...
2. **속성** 페이지에서 **작업 이름** 필드를 **CopyFromAmazonS3ToADLS** 로 지정하고 **다음** 을 선택합니다.
    ...
    :::image type="content" source="./media/load-data-into-azure-data-lake-store/copy-data-tool-properties-page.png" alt-text="속성 페이지":::
3. **원본 데이터 저장소** 페이지에서 **+ 새 연결 만들기** 를 클릭합니다.

    :::image type="content" source="./media/load-data-into-azure-data-lake-store/source-data-store-page.png" alt-text="원본 데이터 저장소 페이지":::
    
    **Amazon S3**, **계속** 을 차례로 선택합니다.
    
    :::image type="content" source="./media/load-data-into-azure-data-lake-store/source-data-store-page-s3.png" alt-text="원본 데이터 저장소 s3 페이지":::
...
4. **Amazon S3 연결 지정** 페이지에서 다음 단계를 수행합니다. 
   ...
   3. **마침** 을 선택합니다.
      ...
      :::image type="content" source="./media/load-data-into-azure-data-lake-store/specify-amazon-s3-account.png" alt-text="스크린샷은 값을 입력할 수 있는 새롭게 연결된 서비스 창을 보여 줍니다.":::
   
   4. 새 연결이 표시됩니다. **다음** 을 선택합니다.
   
   :::image type="content" source="./media/load-data-into-azure-data-lake-store/specify-amazon-s3-account-created.png" alt-text="스크린샷은 새 연결을 보여 줍니다.":::
   
5. **입력 파일 또는 폴더 선택** 페이지에서, 복사하려는 폴더 및 파일로 이동합니다. 폴더/파일을 선택하고 **선택**, **다음** 을 차례로 선택합니다.

    :::image type="content" source="./media/load-data-into-azure-data-lake-store/choose-input-folder.png" alt-text="입력 파일 또는 폴더 선택":::

6. **재귀적으로 파일 복사** 및 **이진 복사**(있는 그대로 파일 복사) 옵션을 선택하여 복사 동작을 선택합니다. **다음** 을 선택합니다.

    :::image type="content" source="./media/load-data-into-azure-data-lake-store/specify-binary-copy.png" alt-text="스크린샷은 재귀적 파일 복사 및 이진 복사를 선택할 수 있는 입력 파일 또는 폴더를 선택을 보여 줍니다.":::
    
7. **대상 데이터 저장소** 페이지에서 **+ 새 연결 만들기** 를 클릭한 다음, **Azure Data Lake Storage Gen1** 을 선택하고 **계속** 을 선택합니다.

    :::image type="content" source="./media/load-data-into-azure-data-lake-store/destination-data-storage-page.png" alt-text="대상 데이터 저장소 페이지":::
...
8. **New Linked Service (Azure Data Lake Storage Gen1)** [새 연결된 서비스(Azure Data Lake Storage Gen1)] 페이지에서 다음 단계를 수행합니다. 
   ...
   :::image type="content" source="./media/load-data-into-azure-data-lake-store/specify-adls.png" alt-text="Data Lake Storage Gen1 계정 지정":::
9. **출력 파일 또는 폴더 선택** 페이지에서 출력 폴더 이름으로 **copyfroms3** 를 입력하고 **다음** 을 선택합니다. 

    :::image type="content" source="./media/load-data-into-azure-data-lake-store/specify-adls-path.png" alt-text="스크린샷은 입력한 폴더 경로를 보여 줍니다.":::

10. **설정** 페이지에서 **다음** 을 선택합니다.

    :::image type="content" source="./media/load-data-into-azure-data-lake-store/copy-settings.png" alt-text="설정 페이지":::
11. **요약** 페이지에서 설정을 검토하고, **다음** 을 선택합니다.

    :::image type="content" source="./media/load-data-into-azure-data-lake-store/copy-summary.png" alt-text="요약 페이지":::
12. **배포 페이지** 에서 **모니터** 를 선택하여 파이프라인(작업)을 모니터링합니다.

    :::image type="content" source="./media/load-data-into-azure-data-lake-store/deployment-page.png" alt-text="배포 페이지":::
13. 왼쪽의 **모니터** 탭이 자동으로 선택됩니다. **작업** 열에는 활동 실행 세부 정보를 보고 파이프라인을 다시 실행하기 위한 링크가 있습니다.

    :::image type="content" source="./media/load-data-into-azure-data-lake-store/monitor-pipeline-runs.png" alt-text="파이프라인 실행 모니터링":::
14. 파이프라인 실행과 연결된 활동 실행을 보려면 **작업** 열에서 **활동 실행 보기** 링크를 선택합니다. 파이프라인에는 하나의 작업(복사 작업)만 있으므로 하나의 항목만 표시됩니다. 파이프라인 실행 보기로 전환하려면 위쪽의 **파이프라인** 링크를 선택합니다. **새로 고침** 을 선택하여 목록을 새로 고칩니다. 
    ...
    :::image type="content" source="./media/load-data-into-azure-data-lake-store/monitor-activity-runs.png" alt-text="작업 실행 모니터링":::
...
15. 각 복사 작업의 실행 세부 정보를 모니터링하려면 작업 모니터링 보기의 **작업** 아래에서 **세부 정보** 링크를 선택합니다. 원본에서 싱크로 복사되는 데이터 볼륨, 데이터 처리량, 해당 기간의 실행 단계, 사용되는 구성 등의 세부 정보를 모니터링할 수 있습니다.
    ...
    :::image type="content" source="./media/load-data-into-azure-data-lake-store/monitor-activity-run-details.png" alt-text="작업 실행 세부 정보 모니터링":::

16. 데이터가 Data Lake Storage Gen1 계정에 복사되었는지 확인합니다. 

    :::image type="content" source="./media/load-data-into-azure-data-lake-store/adls-copy-result.png" alt-text="Data Lake Storage Gen1 출력 확인":::
