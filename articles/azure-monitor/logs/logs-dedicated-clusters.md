...
- **[가용성 영역](../../availability-zones/az-overview.md)** - 영역이 위치로 물리적으로 분리되고 독립적인 전원, 냉각 및 네트워킹을 갖춘 데이터 센터 오류로부터 데이터를 보호합니다. 영역 및 독립 인프라의 물리적 분리는 작업 영역이 영역의 리소스에 의존할 수 있기 때문에 인시던트 발생 가능성이 훨씬 낮아집니다. 전용 클러스터는 Azure에 가용성 영역이 있는 지역에서 데이터 복원력을 사용하도록 설정된 가용성 영역을 사용하여 [만들어집니다.](../../availability-zones/az-overview.md#azure-regions-with-availability-zones) 클러스터의 가용성 영역 구성을 만든 후에는 변경할 수 없으며 클러스터의 속성 에서 설정을 확인할 수 `isAvailabilityZonesEnabled` 있습니다. [Azure Monitor 가용성 영역은](./availability-zones.md) 서비스의 광범위한 부분을 다루며 해당 지역에서 사용할 수 있는 경우 Azure Monitor 복원력을 자동으로 확장합니다.
...
전용 클러스터는 Azure Monitor 로그 클러스터를 나타내는 Azure 리소스를 사용하여 관리됩니다. 작업은 [CLI,](/cli/azure/monitor/log-analytics/cluster) [PowerShell](/powershell/module/az.operationalinsights) 또는 [REST](/rest/api/loganalytics/clusters)를 사용하여 프로그래밍 방식으로 수행됩니다.
...
2. **작업 영역:** 클러스터의 약정 계층 비용은 각 작업 영역의 데이터 수집 볼륨에 따라 클러스터의 작업 영역에 비례하여 계산됩니다(각 작업 영역에 대한 [Microsoft Defender for Cloud의](../../security-center/index.yml) 노드별 할당을 고려한 후). 이 가격 책정 모델의 전체 세부 정보는 여기에 설명되어 [있습니다.](./manage-cost-storage.md#log-analytics-dedicated-clusters) 

작업 영역에서 레거시 노드당 가격 책정 계층을 사용하는 경우 클러스터에 연결되면 클러스터의 약정 계층에 대해 수집된 데이터에 따라 요금이 청구되고 노드당 더 이상 청구되지 않습니다. Microsoft Defender for Cloud의 노드별 데이터 할당은 계속 적용됩니다.
...
하나의 클러스터에 최대 1,000개의 작업 영역을 연결할 수 있습니다. 연결된 작업 영역은 클러스터와 동일한 지역에 위치합니다. 시스템 백 엔드를 보호하고 데이터 조각화를 방지하기 위해 작업 영역을 한 달에 두 번 이상 클러스터에 연결할 수 없습니다.
...
클러스터가 고객 관리형 키로 구성된 경우 연결 작업 완료 후 작업 영역에 수집된 데이터는 관리형 키로 암호화되어 저장됩니다. 작업 영역 링크 작업을 완료 하는 데 최대 90 분이 걸릴 수 있으며, 작업 영역에 Get 요청을 전송 하 여 상태를 확인 하 고, *기능의* 응답에 *clusterresourceid* 속성이 있는지 확인할 수 있습니다.
...
연결된 작업 영역의 데이터 볼륨이 시간이 지남에 따라 변경되어 약정 계층 수준을 적절하게 업데이트하려는 경우입니다. 계층은 GB 단위로 지정되며 500, 1,000, 2,000 또는 5,000GB/일 값을 가질 수 있습니다. 전체 REST 요청 본문을 제공할 필요는 없지만 sku는 포함 해야 합니다.
...
삭제 하기 전에 전용 클러스터에서 모든 작업 영역의 연결을 해제 하는 것이 좋습니다. 클러스터 리소스에 대 한 *쓰기* 권한이 있어야 합니다. 클러스터를 삭제 하면 연결 된 작업 영역 및 이전에 연결 된 작업 영역에서 클러스터에 대 한 모든 데이터 수집에 대 한 액세스가 손실 됩니다. 이 작업은 되돌릴 수 없습니다. 작업 영역이 연결 되 면 클러스터를 삭제 하면 자동으로 연결이 끊어지고 새 데이터가 Log Analytics 저장소로 수집 됩니다.

지난 14 일 동안 삭제 된 클러스터 리소스는 일시 삭제 상태로 유지 되 고 이름은 예약 된 상태로 유지 됩니다. 일시 삭제 기간이 지나면 클러스터가 영구적으로 삭제 되 고 이름을 사용할 수 있습니다.

> [!WARNING] 
> - 일시 삭제 된 클러스터의 복구는 지원 되지 않으므로 삭제 한 후에는 복구할 수 없습니다.
> - 구독 당 클러스터는 4 개로 제한 됩니다. 활성 삭제 및 일시 삭제된 클러스터가 모두 여기에 계산됩니다. 고객은 클러스터를 만들고 삭제하는 되풀이 프로시저를 만들면 안 됩니다. 이렇게 하면 Log Analytics 백 엔드 시스템에 상당한 영향을 미치게 됩니다.
