---
title: 관리형 온라인 엔드포인트를 사용하여 기계 학습 모델 배포(미리 보기)
...
> [!NOTE]
> 엔드포인트 이름은 Azure 지역 내에서 고유해야 합니다. 예를 들어 Azure westus2 지역에는 이름이 `my-endpoint`인 엔드포인트가 하나만 있을 수 있습니다. 

## <a name="define-the-endpoint-configuration"></a>엔드포인트 구성 정의

온라인 엔드포인트에 모델을 배포하려면 특정 입력이 필요합니다.
...
다음 조각은 모든 필수 입력을 캡처하는 *endpoints/online/managed/simple-flow/1-create-endpoint-with-blue.yml* 파일을 보여 줍니다. 
...
| `auth_mode` | 키 기반 인증에는 `key`를 사용하고 Azure Machine Learning 토큰 기반 인증에는 `aml_token`을 사용합니다. `key`는 만료되지 않지만 `aml_token`은 만료됩니다. (`az ml endpoint get-credentials` 명령을 사용하여 가장 최근 토큰을 가져옵니다.) |
| `deployments` | 엔드포인트에서 만들 배포의 목록입니다. 이 경우 `blue`라는 하나의 배포만 있습니다. 여러 배포에 대한 자세한 내용은 [온라인 엔드포인트에 대한 안전한 롤아웃(미리 보기)](how-to-safely-rollout-managed-endpoints.md)을 참조하세요.|

다음 표에서는 `deployments`의 특성을 설명합니다.

| 키 | 설명 |
| --- | --- |
| `name`  | 배포의 이름입니다. |
| `model` | 이 예에서는 `name`, `version` 및 `local_path` 모델 속성을 인라인으로 지정합니다. 모델 파일이 자동으로 업로드되어 등록됩니다. 인라인 사양의 단점은 모델 파일을 업데이트하려는 경우 버전을 수동으로 증분시켜야 한다는 것입니다. 관련 모범 사례는 다음 섹션의 팁을 참조하세요. |
...
| `environment` | 모델 및 코드를 호스트할 환경의 세부 정보를 포함합니다. 이 예제에는 `name`, `version` 및 `path`를 포함하는 인라인 정의가 있습니다. 이미지에 `environment.docker.image`를 사용합니다. `conda_file` 종속성은 이미지 위에 설치됩니다. 자세한 내용은 다음 섹션의 팁을 참조하세요. |
...
| `scale_settings.instance_count` | 배포의 인스턴스 수입니다. 예상되는 워크로드 값을 기준으로 합니다. 고가용성을 위해 `scale_settings.instance_count`를 `3` 이상으로 설정하는 것이 좋습니다. |
...
이 예에서는 모델 및 환경 속성을 인라인으로 지정합니다. `name`, `version` 및 `local_path`(파일을 업로드할 위치). CLI는 자동으로 파일을 업로드하고 모델과 환경을 등록합니다. 프로덕션 환경에서는 모델 및 환경을 등록하고 YAML에 별도로 등록된 이름 및 버전을 지정하는 것이 모범 사례입니다. `model: azureml:my-model:1` 또는 `environment: azureml:my-env:1` 형식을 사용합니다.
...
디버깅 시간을 절약하려면 엔드포인트를 로컬에서 테스트 실행하는 것이 *매우 좋습니다*.
...
> - 로컬 엔드포인트는 엔드포인트당 하나의 배포만 지원합니다. 즉, 로컬 배포에서는 Azure Machine Learning 작업 영역에 등록된 모델 또는 환경에 대한 참조를 사용할 수 없습니다. 
...
REST 클라이언트(예: curl)를 사용하려면 채점 URI가 있어야 합니다. 채점 URI를 얻으려면 `az ml endpoint show --local -n $ENDPOINT_NAME`을 실행합니다. 반환된 데이터에서 `scoring_uri` 특성을 찾습니다. 
...
YAML 구성을 클라우드에 배포하려면 다음 코드를 실행합니다.
...
REST 클라이언트를 사용하려면 `scoring_uri` 값과 인증 키 또는 토큰이 있어야 합니다. `scoring_uri` 값은 `show` 명령의 출력에서 볼 수 있습니다.
 
::: code language="azurecli" source="~/azureml-examples-main/cli/deploy-managed-online-endpoint.sh" ID="get_scoring_uri" :::

현재 `--query` 플래그를 사용하여 특성을 필터링해 필요한 사항만 확인하고 있습니다. `--query`에 대한 자세한 내용은 [Azure CLI 명령 출력 쿼리](/cli/azure/query-azure-cli)를 참조하세요.
...
1. 다음 명령을 실행합니다.
    ...
    > [!IMPORTANT]
    > YAML을 사용한 업데이트는 선언적입니다. 즉, YAML의 변경 내용이 기본 Azure Resource Manager 리소스(엔드포인트 및 배포)에 반영됩니다. 선언적 접근 방식은 [GitOps](https://www.atlassian.com/git/tutorials/gitops)를 용이하게 합니다. 엔드포인트 및 배포에 대한 *모든* 변경 사항(`instance_count` 포함)은 YAML을 거칩니다. 따라서 YAML에서 배포를 제거하고 파일을 사용하여 `az ml endpoint update`를 실행할 경우 해당 배포가 삭제된다는 것입니다. `--set` 플래그를 사용하여 YAML을 사용하지 않고 업데이트할 수 있습니다.
...
드문 경우이지만 해결할 수 없는 문제로 인해 배포를 삭제하고 다시 만들려면 다음을 실행합니다.

```azurecli
az ml endpoint delete -n $ENDPOINT_NAME --deployment blue
```

`update` 명령은 로컬 엔드포인트에서도 작동합니다. 동일한 `az ml endpoint update` 명령에 `--local` 플래그를 사용합니다.

> [!TIP]
> `az ml endpoint update` 명령을 통해 [Azure CLI의 `--set` 매개 변수](/cli/azure/use-cli-effectively#generic-update-arguments)를 사용하여 YAML의 특성을 *재정의하거나* YAML 파일을 전달하지 않고 특정 특성을 설정할 수 있습니다. 단일 특성에 `--set`를 사용하는 것은 개발 및 테스트 시나리오에서 특히 중요합니다. 예를 들어 첫 번째 배포의 `instance_count` 값을 스케일 업하려면 `--set deployments[0].scale_settings.instance_count=2` 플래그를 사용하면 됩니다. 그러나 YAML이 업데이트되지 않으므로 이 기법을 사용하면 [GitOps](https://www.atlassian.com/git/tutorials/gitops)가 용이하지 않습니다.
