...
ms.locfileid: "132064445"
---
# <a name="create-a-search-index-in-azure-cognitive-search"></a>Azure Cognitive Search 검색 인덱스 만들기

검색 인덱스의 대상 검색 가능 텍스트에 Azure Cognitive Search 요청을 쿼리합니다. 이 문서에서는 Azure Cognitive Search 지원하는 형식을 사용하여 검색 인덱스 정의 및 게시 단계를 알아봅니다. 

인덱서 를 사용하지 않는 한 [인덱스](search-howto-create-indexers.md)만들기 및 인덱스 채우기는 별도의 작업입니다. 인덱서가 아닌 시나리오의 경우 인덱스를 만든 후 다음 단계는 [데이터 가져오기입니다.](search-what-is-data-import.md) 자세한 배경은 [Azure Cognitive Search 인덱스 검색을](search-what-is-an-index.md)참조하세요.

## <a name="prerequisites"></a>전제 조건

인덱스를 만들고 로드하려면 검색 서비스에 대한 쓰기 권한이 필요합니다. 대부분의 작업을 수행하려면 인덱스 만들기 요청에 [관리자 API 키를](search-security-api-keys.md) 제공해야 합니다. 또는 Azure Active Directory [역할 기반 액세스 제어 공개 미리 보기](search-security-rbac.md)에 참여하는 경우 검색 참가자 역할의 멤버로 요청을 발급할 수 있습니다.

인덱스 만들기는 주로 스키마 정의 연습입니다. 만들기 전에 다음이 있어야 합니다.

+ 인덱스의 검색 가능, 검색 가능, 필터링 가능, facetable 및 정렬 가능을 만들려는 필드를 명확하게 파악할 수 있습니다.

  [필드 특성 할당은](search-what-is-an-index.md#index-attributes) 검색 서비스의 실제 스토리지 구조를 결정합니다. 디자인 및 개발 중에 필드 특성을 마무리할 때 인덱스도 쉽게 삭제하고 다시 작성할 수 있도록 샘플 데이터로 시작합니다.

+ 원본 데이터의 각 행, 레코드 또는 항목을 고유하게 식별하는 원본 필드입니다. Blob Storage 인덱싱하는 경우 스토리지 경로가 문서 키로 사용되는 경우가 많습니다. 

  모든 인덱스에는 *문서 키로* 사용되는 하나의 필드("문서 ID"라고도 함)가 필요하며, 이 키는 고유 식별자를 포함하는 원본 필드에 매핑됩니다. 검색 인덱스에서 특정 문서를 검색하고 원본 데이터에서 오른쪽 문서를 끌어와 선택적 데이터를 처리하는 데 특정 검색 문서를 고유하게 식별하는 기능이 필요합니다.

+ 인덱스 위치입니다. 기존 인덱스 를 다른 검색 서비스로 이동하는 것은 지원되지 않습니다. 애플리케이션 요구 사항을 다시 확인하고 기존 검색 서비스, 해당 용량 및 위치가 요구 사항에 충분한지 확인합니다.
...
[인덱스 만들기는](/rest/api/searchservice/create-index) 검색 서비스에 물리적 데이터 구조(파일 및 반전된 인덱스)를 만드는 작업입니다. [인덱스 업데이트를](/rest/api/searchservice/update-index) 사용하여 변경 내용을 적용하는 기능은 수정으로 해당 물리적 구조가 무효화되는지 여부에 따라 달라집니다. 인덱스로 필드를 만든 후에는 대부분의 필드 특성을 변경할 수 없습니다.
...
| 키 | 아니요 |
| 필드 이름 및 형식 | 아니요 |
| 필드 특성(검색 가능, 필터링 가능, facetable, 정렬 가능) | 아니요 |
| 필드 특성(검색 가능) | 예 |
...
| [확인기](index-add-suggesters.md) | 아니요 |
...
1. [지원되는 데이터 형식을 검토합니다.](/rest/api/searchservice/supported-data-types) 데이터 형식은 필드 사용 방법에 영향을 미칩니다. 예를 들어 숫자 콘텐츠는 필터링 가능하지만 전체 텍스트를 검색할 수는 없습니다.

1. 고유 값이 포함된 데이터 원본에서 하나의 필드를 식별하여 인덱스의 키 필드로 작동할 수 있도록 합니다.

1. 인덱스 검색 가능한 콘텐츠에 기여할 수 있는 데이터 원본의 필드를 식별합니다. 검색 가능한 콘텐츠는 전체 텍스트 검색 엔진을 사용하여 쿼리되는 짧거나 긴 문자열입니다. 콘텐츠가 자세한(작은 구 또는 더 큰 청크)인 경우 다양한 분석기로 실험하여 텍스트가 토큰화되는 방법을 확인합니다.
...
개발하는 동안 다시 빌드하는 계획을 자주 세워야 합니다. 실제 구조는 서비스에서 생성되므로 많은 수정을 위해 [인덱스를 삭제하고 다시 만들어야](search-howto-reindex.md) 합니다. 보다 빠르게 다시 작성할 수 있도록 데이터 하위 집합을 사용하는 방안을 고려해 볼 수 있습니다.
