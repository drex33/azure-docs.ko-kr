...
보안 관리자는 IoT Edge 디바이스에서 보안 실리콘 하드웨어를 추상화하고 추가 보안 서비스에 대한 확장성 프레임워크를 제공합니다.

<!-- add new graphic here -->
:::moniker-end

IoT Edge 보안 관리자는 IoT Edge 디바이스 및 모든 내재 소프트웨어 작업의 무결성을 방어하는 것을 목표로 합니다. 보안 관리자는 신뢰할 수 있는 하드웨어(사용 가능한 경우)의 기본 하드웨어 루트에서 트러스트를 전환하여 IoT Edge 런타임을 부트스트랩하고 진행 중인 작업을 모니터링합니다.  IoT Edge 보안 관리자는 보안 실리콘 하드웨어(사용 가능한 경우)와 연동되어 가능한 최고 수준의 보안을 보장하는 소프트웨어입니다.  

<!--1.2-->
:::moniker range=">=iotedge-2020-11"
또한 IoT Edge 보안 관리자는 호스트 수준 모듈을 통해 보안 서비스 확장에 대한 안전한 프레임워크를 제공합니다. 이러한 서비스에는 디바이스의 일부 구성 요소에 대한 권한 있는 액세스 권한이 있는 디바이스 내의 에이전트가 필요한 보안 모니터링 및 업데이트가 포함됩니다. 확장성 프레임워크는 이러한 통합이 전반적인 시스템 보안을 일관되게 유지하도록 합니다.
:::moniker-end
...
* 공증 서비스를 통해 신뢰할 수 있는 디바이스 하드웨어 루트에 대한 액세스를 제어합니다.
* 런타임 시 IoT Edge 작업의 무결성 모니터링.
* HSM(하드웨어 보안 모듈)에서 트러스트 위임을 받습니다.
* 디바이스 ID를 프로비전하고 해당하는 경우 트러스트 전환을 관리합니다.
* Device Provisioning Service와 같은 클라우드 서비스의 디바이스 구성 요소 호스트 및 보호.
* 고유한 ID를 IoT Edge 모듈을 프로비전합니다.
:::moniker-end
...
* 런타임 시 IoT Edge 작업의 무결성 모니터링.
* 디바이스 ID를 프로비전하고 해당하는 경우 트러스트 전환을 관리합니다.
* IoT Hub 디바이스 업데이트 및 IoT용 Microsoft Defender를 포함하여 서비스에 대한 클라이언트 에이전트의 안전한 작업을 보장합니다.
:::moniker-end

IoT Edge 보안 관리자는 다음 세 가지 구성 요소로 구성됩니다.
...
* 신뢰 또는 HSM의 하드웨어 실리콘 루트(선택 사항이지만 권장)
:::moniker-end
...
IoT Edge 버전 1.0 및 1.1에서는 **보안 디먼이라는** 구성 요소가 보안 관리자의 논리적 보안 작업을 담당했습니다. 버전 1.2 업데이트에서는 몇 가지 주요 책임이 [Azure IoT Identity Service](https://azure.github.io/iot-identity-service/) 보안 하위 시스템으로 위임되었습니다. 이러한 보안 기반 작업이 보안 디먼에서 제거되면 해당 이름이 더 이상 의미가 없습니다. 버전 1.2 이상에서 이 구성 요소가 수행하는 작업을 더 잘 반영하기 위해 **모듈 런타임** 로 이름을 변경했습니다.
:::moniker-end
...
IoT Edge 모듈 런타임은 [Azure IoT Identity Service](https://azure.github.io/iot-identity-service/) 보안 하위 시스템으로부터 신뢰를 위임하여 IoT Edge 컨테이너 런타임 환경을 보호합니다. 모듈 런타임은 보안 관리자의 논리적 보안 작업을 담당합니다. 이는 IoT Edge 디바이스의 Trusted Computing Base에서 상당한 부분을 차지합니다. 모듈 런타임은 IoT ID 서비스의 보안 서비스를 사용하며, 이 서비스는 디바이스 제조업체가 선택한 HSM(하드웨어 보안 모듈)에 의해 강화됩니다. 디바이스 강화를 위해 HSM을 사용하는 것이 좋습니다.
:::moniker-end

### <a name="design-principles"></a>디자인 원칙

IoT Edge 운영 무결성을 최대화하고 블로트 및 변동을 최소화하는 두 가지 핵심 원칙을 따릅니다.
...
<!--1.1-->
:::moniker range=">=iotedge-2020-11"
IoT Edge 모듈 런타임은 지정된 신뢰 하드웨어 루트의 방어 기능 내에서 가능한 가장 높은 무결성으로 작동합니다. 적절한 통합을 통해 트러스트 하드웨어의 루트는 변조를 방지하기 위해 보안 디먼을 런타임 시 정적으로 측정하고 모니터링합니다.
:::moniker-end

디바이스에 대한 악의적인 물리적 액세스는 항상 IoT에서 위협입니다. 신뢰의 하드웨어 루트는 IoT Edge 디바이스의 무결성을 방어하는 데 중요한 역할을 합니다.  신뢰할 수 있는 하드웨어 루트는 두 가지로 제공됩니다.

* 비밀 및 암호화 키와 같은 중요한 정보를 보호하기 위한 요소를 보호합니다.
* 키와 같은 비밀 및 기밀 기계 학습 모델 및 계량 작업과 같은 중요한 워크로드를 보호하기 위해 enclave를 보호합니다.
...
<!--1.1-->
:::moniker range="iotedge-2018-06"
보안 Enclave를 신뢰의 하드웨어 루트로 사용하는 디바이스의 경우 IoT Edge 보안 디먼 내의 중요한 논리는 enclave 내에 있어야 합니다.  보안 디먼의 중요하지 않은 부분은 TEE 외부에 있을 수 있습니다.  모든 경우에 ODM(원래 디자인 제조업체) 및 OEM(원래 장비 제조업체)은 부팅 및 런타임에 IoT Edge 보안 디먼의 무결성을 측정하고 방어하기 위해 HSM에서 신뢰를 확장하는 것이 좋습니다.
:::moniker-end

<!--1.2-->
:::moniker range=">=iotedge-2020-11"
보안 enclave를 신뢰의 하드웨어 루트로 사용하는 디바이스의 경우 IoT Edge 모듈 런타임 내의 중요한 논리는 enclave 내에 있어야 합니다.  모듈 런타임의 중요하지 않은 부분은 TEE 외부에 있을 수 있습니다.  모든 경우에 ODM(원래 디자인 제조업체) 및 OEM(원래 장비 제조업체)은 HSM에서 신뢰를 확장하여 부팅 및 런타임 시 IoT Edge 모듈 런타임의 무결성을 측정하고 방어하는 것이 좋습니다.
:::moniker-end
...
<!--1.1-->
:::moniker range="iotedge-2018-06"
IoT Edge 보안 디먼이 따르는 또 다른 핵심 원칙은 변동의 최소화합니다.  최고 수준의 신뢰를 보장하기 위해 IoT Edge 보안 디먼은 신뢰할 수 있는 디바이스 하드웨어 루트와 밀접하게 결합되어 네이티브 코드로 작동할 수 있습니다.  이러한 경우 일반적으로 운영 체제의 업데이트 메커니즘이 아닌 트러스트의 보안 업데이트 경로의 하드웨어 루트를 통해 IoT Edge 소프트웨어를 업데이트하는 것이 어려울 수 있습니다.  IoT 디바이스에는 보안 갱신이 권장되지만 과도한 업데이트 요구 사항 또는 대규모 업데이트 페이로드는 여러 가지 방법으로 위협 표면을 확장할 수 있습니다. 예를 들어 디바이스 가용성을 최대화하기 위해 일부 업데이트를 건너뛰려고 할 수 있습니다. 따라서 IoT Edge 보안 디먼의 디자인은 자주 업데이트를 권장하기 위해 잘 격리된 신뢰할 수 있는 컴퓨팅 기반을 작게 유지하는 데 간결합니다.
:::moniker-end

<!--1.2-->
:::moniker range=">=iotedge-2020-11"
IoT Edge 모듈 런타임의 또 다른 핵심 원칙은 변동을 최소화하는 것입니다.  가장 높은 신뢰 수준을 위해 IoT Edge 모듈 런타임은 신뢰할 수 있는 디바이스 하드웨어 루트와 긴밀하게 결합되고 네이티브 코드로 작동할 수 있습니다.  이러한 경우 일반적으로 운영 체제의 업데이트 메커니즘이 아닌 트러스트의 보안 업데이트 경로의 하드웨어 루트를 통해 IoT Edge 소프트웨어를 업데이트하는 것이 어려울 수 있습니다.  IoT 디바이스에는 보안 갱신이 권장되지만 과도한 업데이트 요구 사항 또는 대규모 업데이트 페이로드는 여러 가지 방법으로 위협 표면을 확장할 수 있습니다. 예를 들어 디바이스 가용성을 최대화하기 위해 일부 업데이트를 건너뛰려고 할 수 있습니다. 따라서 IoT Edge 모듈 런타임의 디자인은 자주 업데이트를 권장하기 위해 잘 격리된 신뢰할 수 있는 컴퓨팅 기반을 작게 유지하는 데 간결합니다.
:::moniker-end
...
IoT Edge 모듈 런타임은 보안 강화를 위해 사용 가능한 신뢰 기술의 하드웨어 루트를 활용합니다.  또한 하드웨어 기술에서 신뢰할 수 있는 실행 환경을 제공할 때는 표준 REE(Rich Execution Environment)와 TEE(Trusted Executive Environment) 간의 분할 작업도 가능합니다. 역할별 인터페이스를 통해 IoT Edge의 주요 구성 요소가 IoT Edge 디바이스와 해당 디바이스 작동의 무결성을 보장할 수 있습니다.
...
클라우드 인터페이스를 사용하면 디바이스 보안을 보완하는 클라우드 서비스에 액세스할 수 있습니다.  예를 들어 이 인터페이스를 사용하면 디바이스 ID 수명 주기 관리를 위해 [Device Provisioning Service에](../iot-dps/index.yml) 액세스할 수 있습니다.  

#### <a name="management-api"></a>관리 API

<!--1.1-->
:::moniker range="iotedge-2018-06"
관리 API는 IoT Edge 모듈을 생성/시작/중지/제거할 때 IoT Edge 에이전트에 의해 호출됩니다. 보안 디먼은 모든 활성 모듈에 대한 "등록"을 저장합니다. 이러한 등록은 모듈의 ID를 모듈의 일부 속성에 매핑합니다. 예를 들어 이러한 모듈 속성에는 컨테이너에서 실행되는 프로세스의 프로세스 식별자(pid)와 Docker 컨테이너 콘텐츠의 해시가 포함됩니다.
...
관리 API는 권한이 있는 API로 IoT Edge 에이전트에서만 호출할 수 있습니다.  IoT Edge 모듈 런타임은 IoT Edge 에이전트를 부트스트랩하고 시작하기 때문에 IoT Edge 에이전트가 변조되지 않았는지 확인한 다음 IoT Edge 에이전트에 대한 암시적 등록을 만들 수 있습니다. 또한 워크로드 API가 사용하는 동일한 증명 프로세스는 관리 API에 대한 액세스를 IoT Edge 에이전트에만 제한합니다.
:::moniker-end
