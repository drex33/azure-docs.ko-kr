...
>[!NOTE]
>Vm과 호스트 풀이 다른 구독에 있는 경우에는 Vm이 있는 구독에 RBAC 역할을 할당 해야 합니다.
...
3. **액세스 제어(IAM)** 로 이동하고 **사용자 지정 역할 추가** 를 선택합니다.

    > [!div class="mx-imgBorder"]
    > ![액세스 제어(IAM)에 있는 추가 단추의 드롭다운 메뉴 스크린샷. “사용자 지정 역할 추가”가 빨간색으로 강조 표시되어 있음](media/add-custom-role.png)

4. 다음으로, 사용자 지정 역할의 이름을 지정하고 설명을 추가합니다. 이름은 “start VM on connect”로 지정하는 것이 좋습니다.

5. **권한** 탭에서 역할을 할당 하는 구독에 대해 다음 두 가지 권한 집합 중 하나를 추가 합니다. 
