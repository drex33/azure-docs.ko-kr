...
- 유지 관리를 다시 부팅해야 하는 경우 유지 관리가 계획된 시기에 대해 알림을 받을 수 있습니다. 사용자가 원하는 시점에 스스로 유지 관리를 시작할 수 있는 약 35일의 기간이 지정됩니다.


다시 부팅해야 하는 계획된 유지 관리는 웨이브에서 예약됩니다. 각 웨이브는 범위(지역)이 다릅니다.

- 웨이브는 고객에게 알림을 보내면서 시작합니다. 기본적으로 구독 관리자 및 공동 관리자에게 알림이 전송됩니다. [활동 로그 경고](../service-health/alerts-activity-log-service-notifications-portal.md)를 사용하여 더 많은 수신자 및 이메일, SMS 및 웹후크와 같은 메시징 옵션을 추가할 수 있습니다.  
- 알림이 나가면 *셀프 서비스 기간* 을 사용할 수 있습니다. 이 기간에는 영향을 받는 가상 머신을 쿼리하고 사용자 자신의 일정 예약 요구에 따라 유지 보수를 시작할 수 있습니다. 셀프 서비스 기간은 일반적으로 약 35일입니다.
...
**가용성 집합** 을 사용하는 배포에는 셀프 서비스 유지 관리를 사용하지 않는 것이 좋습니다. 가용성 집합은 이미 한 번에 하나의 업데이트 도메인만 업데이트되었습니다. 

- Azure에서 유지 관리를 트리거할 수 있습니다. 다시 부팅이 필요한 유지 관리의 경우 유지 관리가 업데이트 도메인별로 도메인을 업데이트합니다. 업데이트 도메인이 유지 관리를 순차적으로 받을 필요는 없으며 업데이트 도메인 사이에 30분 동안 일시 중지됩니다. 
- 일부 용량(업데이트 도메인 1개)이 일시적으로 손실된 경우 유지 관리 기간 동안 인스턴스를 추가할 수 있습니다. 
...
- 두 UD(업데이트 도메인) 간에 30분 이상의 VM 복구 시간이 필요합니다. 업데이트 도메인 사이의 시간을 제어하려면 한 번에 하나의 UD(업데이트 도메인)에서 VM 유지 관리를 트리거해야 합니다.
...
고가용성에 대한 자세한 내용은 [Azure에서 가상 머신의 가용성](availability.md)을 참조하세요.

**Q: 계획된 유지 관리에 관한 알림은 어떻게 받나요?**

**A:** 계획된 유지 관리 주기는 하나 이상의 Azure 지역에 예약을 설정하는 것에서 출발합니다. 곧 메일 알림이 구독 관리자, 공동 관리자, 소유자 및 기여자에게 전달됩니다(모든 수신자가 추가된 구독당 하나의 메일). 이 알림에 대한 추가 채널과 받는 사람은 활동 로그 경고를 통해 구성할 수 있습니다. 계획된 유지 관리가 이미 예약된 지역에 가상 머신을 배포하는 경우 알림이 전달되지 않으므로 VM의 유지 관리 상태를 확인해야 합니다.

**Q: Portal, Powershell 또는 CLI에서 계획된 유지 관리의 표시가 전혀 나타나지 않습니다. 무엇이 문제인가요?**
...
**A:** 이제 계획된 유지 관리는 Virtual Machine Scale Sets에 지원됩니다. 셀프 서비스 유지 관리를 시작하는 방법에 대한 지침은 [가상 머신 확장 집합에 대한 계획된 유지 관리](../virtual-machine-scale-sets/virtual-machine-scale-sets-maintenance-notifications.md) 문서를 참조하세요.
...
**Q: VM에서 유지 관리 정보가 보이지 않습니다. 무엇이 문제인가요?**
...
3. VM 할당을 취소한 다음 다시 시작했습니다. 이로 인해 VM이 계획된 유지 관리를 예약하지 않은 위치로 이동할 수 있습니다. 따라서 VM은 유지 관리 정보를 더 이상 표시하지 않습니다. 
...
[Azure CLI](maintenance-notifications-cli.md), [Azure PowerShell](maintenance-notifications-powershell.md) 또는 [Portal](maintenance-notifications-portal.md)을 사용하여 예정된 유지 관리를 처리할 수 있습니다.
