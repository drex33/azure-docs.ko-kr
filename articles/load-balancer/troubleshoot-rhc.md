...
* OS를 확인합니다. Vm이 프로브 포트에서 수신 대기 하 고 있는지 확인 하 고 OS 방화벽 규칙을 검토 하 여 IP 주소에서 시작 된 프로브 트래픽을 차단 하지 않는지 확인 합니다. `168.63.129.16`
  * `netstat -a`Windows 명령 프롬프트 또는 `netstat -l` Linux 터미널에서를 실행 하 여 수신 대기 포트를 확인할 수 있습니다.
