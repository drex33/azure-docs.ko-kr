---
title: Microsoft Defender for Cloud에서 사용자 지정 보안 정책 만들기 | Microsoft Docs
description: 클라우드 용 Microsoft Defender에서 모니터링 하는 Azure 사용자 지정 정책 정의입니다.
author: memildin
manager: rkarlin
...
ms.locfileid: "131009988"
---
# <a name="create-custom-security-initiatives-and-policies"></a>사용자 지정 보안 이니셔티브 및 정책 만들기

[!INCLUDE [Banner for top of topics](./includes/banner.md)]

시스템 및 환경에 대 한 보안을 유지 하기 위해 Microsoft Defender for Cloud는 보안 권장 사항을 생성 합니다. 이러한 권장 사항은 모든 고객에게 제공되는 일반적인 기본 보안 정책에 통합되는 업계 모범 사례를 기반으로 합니다. 클라우드의 업계 및 규제 표준에 대 한 정보를 Defender에서 가져올 수도 있습니다.
...
1. 사용자 지정 이니셔티브를 추가할 구독이나 관리 그룹을 선택합니다.
    ...
    > [!NOTE]
    > 사용자 지정 이니셔티브를 평가 하 고 클라우드에 대해 Defender에 표시 하려면 구독 수준 (이상)에서 추가 해야 합니다. 사용 가능한 가장 넓은 범위를 선택하는 것이 좋습니다.
...
1. 새 사용자 지정 이니셔티브를 만들려면 다음을 수행합니다.
    ...
    > [!NOTE]
    > 새 이니셔티브를 만들려면 구독 소유자 자격 증명이 필요합니다. Azure 역할에 대 한 자세한 내용은 [클라우드 용 Microsoft Defender의 권한](permissions.md)을 참조 하세요.

    새 이니셔티브가 적용되며 다음과 같은 두 가지 방법으로 영향을 확인할 수 있습니다.

    * 클라우드 용 Defender 메뉴에서 **규제 준수** 를 선택 합니다. 기본 제공 이니셔티브와 함께 새로운 사용자 지정 이니셔티브를 표시하는 규정 준수 대시보드가 열립니다.
...
Azure Policy와의 기본 통합의 일환으로 Microsoft Defender for Cloud를 사용 하면 Azure Policy의 REST API을 활용 하 여 정책 할당을 만들 수 있습니다. 다음 지침은 기존 할당에 대한 사용자 지정은 물론 정책 할당 생성 과정을 안내합니다. 
...
Defender for Cloud에는 모든 보안 정책을 포함 하는 기본 제공 이니셔티브 인 [Azure 보안 벤치 마크가](/security/benchmark/azure/introduction)있습니다. Azure 리소스에서 클라우드 정책의 Defender를 평가 하려면 관리 그룹 또는 평가 하려는 구독에 대 한 할당을 만들어야 합니다.

기본 제공 이니셔티브에는 기본적으로 클라우드 정책의 모든 Defender가 사용 하도록 설정 되어 있습니다. 기본 제공 이니셔티브에서 특정 정책을 사용하지 않도록 설정할 수 있습니다. 예를 들어 **웹 응용 프로그램 방화벽** 을 제외 하 고 클라우드의 모든 정책에 대 한 모든 Defender를 적용 하려면 정책 효과 매개 변수의 값을 **사용 안 함** 으로 변경 합니다.
...
이 예제에서는 구독 또는 관리 그룹에서 클라우드 이니셔티브에 대해 기본 제공 Defender를 할당 하는 방법을 보여 줍니다.
...
이 예제에서는 다음 정책을 사용 하지 않도록 설정 하 여 구독에서 클라우드 이니셔티브에 대해 기본 제공 Defender를 할당 하는 방법을 보여 줍니다. 
...
Microsoft Defender for Cloud와 함께 제공 되는 기본 제공 권장 사항에는 심각도 수준 및 수정 지침과 같은 세부 정보가 포함 됩니다. Microsoft Azure Portal이나 권장 사항에 액세스하는 모든 위치에 표시되도록 이러한 유형의 정보를 사용자 지정 권장 사항에 추가하려면 REST API를 사용해야 합니다. 
