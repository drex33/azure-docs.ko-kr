---
title: Azure Cosmos DB에서 대량 실행기 Java 라이브러리를 사용하여 대량 가져오기 및 업데이트 작업 수행
...
이 자습서는 Azure Cosmos DB Bulk Executor Java 라이브러리를 사용하여 Azure Cosmos DB 문서를 가져오고 업데이트하는 데 관한 지침을 제공합니다. Bulk Executor 라이브러리와 방대한 처리량 및 스토리지를 활용하는 방법에 대한 자세한 내용은 [Bulk Executor 라이브러리 개요](../bulk-executor-overview.md) 문서를 참조하세요. 이 자습서에서는 임의의 문서를 생성하는 Java 애플리케이션을 빌드하고 이러한 문서를 Azure Cosmos 컨테이너에 대량으로 가져옵니다. 가져온 후 문서의 일부 속성을 대량 업데이트합니다. 

현재 Azure Cosmos DB SQL API 및 Gremlin API 계정에서만 대량 실행기 라이브러리가 지원됩니다. 이 문서에서는 SQL API 계정으로 대량 실행기 Java 라이브러리를 사용하는 방법을 설명합니다. Bulk Executor .NET 라이브러리 사용에 대해 알아보려면 [Azure Cosmos DB Gremlin API에서 대량 작업 수행](../graph/bulk-executor-graph-dotnet.md)을 참조하세요. 설명된 대량 실행기 라이브러리는 [Azure Cosmos DB Java 동기화 SDK v2](sql-api-sdk-java.md)에서만 사용할 수 있으며 현재 Java 대량 지원에 권장되는 솔루션입니다. 현재 3.x, 4.x 또는 기타 상위 SDK 버전에서는 사용할 수 없습니다.
...
* [JDK(Java Development Kit) 1.7+](/java/azure/jdk/)  
...
4. 임의 문서를 생성하여 Azure Cosmos 컨테이너에 대량으로 가져오는 importAll API를 호출합니다. CmdLineConfiguration.java 파일 내에서 명령줄 구성을 구성할 수 있습니다.
...
2. 임의 문서를 생성한 다음, Azure Cosmos 컨테이너에 대량으로 가져오는 updateAll API를 호출합니다. CmdLineConfiguration.java 파일에 전달할 명령줄 구성을 구성할 수 있습니다.
   ...
   **대량 가져오기 응답 개체 정의** 대량 가져오기 API 호출의 결과에는 다음 get 메서드가 포함되어 있습니다.
   ...
   |List\<Exception> getErrors()   |       업데이트 작업과 관련된 운영 또는 네트워킹 문제 목록을 가져옵니다.      |
   |List\<BulkUpdateFailure> getFailedUpdates()   |       실패로 이어지는 특정 예외와 함께 완료할 수 없는 업데이트 목록을 가져옵니다.|
...
* 특정 Azure Cosmos 컨테이너에 해당하는 단일 가상 머신 내에서 전체 애플리케이션에 대한 단일 DocumentBulkExecutor 개체를 인스턴스화하는 것이 좋습니다.  
