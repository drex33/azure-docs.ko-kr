...
ms.locfileid: "131859268"
---
# <a name="connect-to-and-manage-postgresql-in-azure-purview-preview"></a>Azure Purview에서 PostgreSQL 커넥트 및 관리(미리 보기)

이 문서에서는 PostgreSQL을 등록하는 방법 및 Azure Purview에서 PostgreSQL을 인증하고 상호 작용하는 방법을 설명합니다. Azure Purview에 대한 자세한 내용은 [소개 문서](overview.md)를 읽어보십시오.
...
- PostgreSQL 서버, 데이터베이스, 스키마, 테이블, 뷰, 테이블/뷰 열 등을 포함한 메타데이터 추출
...
* [JDK 11](https://www.PostgreSQL.com/java/technologies/javase-jdk11-downloads.html)이 자체 호스팅 통합 런타임이 설치된 가상 머신에 설치되어 있는지 확인합니다.

* Visual Studio 2012용 Visual C++ 재배포 가능 패키지 업데이트 4가 자체 호스팅 통합 런타임 컴퓨터에 설치되어 있는지 확인합니다. 이 업데이트가 설치되지 않은 경우 [여기에서 다운로드할 수 있습니다.](https://www.microsoft.com/download/details.aspx?id=30679)

* 고급 메타데이터에 액세스하려면 PostgreSQL 사용자가 시스템 테이블에 대한 읽기 권한이 있어야 합니다.
...
1. **등록** 을 선택합니다.
1. 원본 등록에서 **PostgreSQL** 을 선택합니다. **계속** 을 선택합니다.
...
아래 단계에 따라 PostgreSQL을 검사하여 자산을 자동으로 식별하고 데이터를 분류합니다. 일반적인 검사에 대한 자세한 내용은 [검색 및 검색 소개를 참조하세요.](concept-scans-and-ingestion.md)
...
1. 아래 세부 정보를 제공합니다.
    ...
    1. **통합 런타임을 통해 커넥트:** 구성된 자체 호스팅 통합 런타임을 선택합니다.
