...
description: 이 자습서에서는 Azure Active Directory와 Predictix Ordering 간에 Single Sign-On을 구성하는 방법에 대해 알아봅니다.
...
이 자습서에서는 Predictix Ordering을 Azure AD(Azure Active Directory)와 통합하는 방법에 대해 알아봅니다.
이 통합은 다음과 같은 이점을 제공합니다.

* Azure AD를 사용하여 Predictix Ordering에 대해 액세스 권한이 있는 사용자를 제어할 수 있습니다.
* 사용자가 자신의 Azure AD 계정으로 Predictix Ordering에 자동으로 로그온(Single Sign-On)되도록 설정할 수 있습니다.
...
Predictix Ordering과 Azure AD 통합을 구성하려면 다음이 필요합니다.

* Azure AD 구독 Azure AD 환경이 없으면 [체험 계정](https://azure.microsoft.com/pricing/free-trial/)을 얻을 수 있습니다.
* Single Sign-On이 설정된 Predictix Ordering 구독

## <a name="scenario-description"></a>시나리오 설명

이 자습서에서는 테스트 환경에서 Azure AD Single Sign-On을 구성하고 테스트합니다.

* Predictix Ordering에서 SP 시작 SSO를 지원합니다.

## <a name="add-predictix-ordering-from-the-gallery"></a>갤러리에서 Predictix Ordering 추가

Azure AD로 Predictix Ordering 통합을 설정하려면 갤러리의 Predictix Ordering을 관리되는 SaaS 앱 목록에 추가해야 합니다.
...
4. 검색 상자에 **Predictix Ordering** 을 입력합니다. 검색 결과에서 **Predictix Ordering** 을 선택하고 **추가** 를 선택합니다.

     ![검색 결과](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>Azure AD Single Sign-On 구성 및 테스트

이 섹션에서는 Britta Simon이라는 테스트 사용자를 사용하여 Predictix Ordering에서 Azure AD Single Sign-On을 구성하고 테스트합니다.
Single Sign-On을 사용하도록 설정하려면 Azure AD 사용자와 Predictix Ordering의 해당 사용자 간에 연결 관계를 설정해야 합니다.

Predictix Ordering에서 Azure AD Single Sign-On을 구성하고 테스트하려면 다음 단계를 완료해야 합니다.

1. 사용자가 이 기능을 사용할 수 있도록 **[Azure AD Single Sign-On을 구성](#configure-azure-ad-single-sign-on)** 합니다.
2. 애플리케이션 쪽에서 **[Predictix Ordering Single Sign-On을 구성](#configure-predictix-ordering-single-sign-on)** 합니다.
...
5. 사용자의 Azure AD 표현과 연결된 **[Predictix Ordering 테스트 사용자를 만듭니다](#create-a-predictix-ordering-test-user)** .
...
Predictix Ordering에서 Azure AD Single Sign-on을 구성하려면 다음 단계를 수행합니다.

1. [Azure Portal](https://portal.azure.com/)의 **Predictix Ordering** 애플리케이션 통합 페이지에서 **Single Sign-On** 을 선택합니다.
...
4. **기본 SAML 구성** 대화 상자에서 다음 단계를 수행합니다.
    ...
    > [!NOTE]
    > 이러한 값은 자리 표시자입니다. 실제 로그온 URL 및 식별자를 사용해야 합니다. 이러한 값을 얻으려면 [Predictix Ordering 지원 팀](https://www.predix.io/support/)에 문의하세요. Azure Portal의 **기본 SAML 구성** 대화 상자에 표시된 패턴을 참조할 수도 있습니다.

5. **SAML로 Single Sign-On 설정** 페이지의 **SAML 서명 인증서** 섹션에서 요구 사항에 따라 **인증서(Base64)** 옆의 **다운로드** 링크를 선택하고 해당 인증서를 컴퓨터에 저장합니다.

    ![인증서 다운로드 링크](common/certificatebase64.png)

6. **Predictix Ordering 설정** 섹션에서 요구 사항에 따라 적절한 URL을 복사합니다.
...
### <a name="configure-predictix-ordering-single-sign-on"></a>Predictix Ordering Single Sign-On 구성

Predictix Ordering 쪽에서 Single Sign-On을 구성하려면 Azure Portal에서 다운로드한 인증서와 복사한 URL을 [Predictix Ordering 지원 팀](https://www.predix.io/support/)으로 보내야 합니다. 이 팀은 SAML SSO 연결이 양쪽에서 올바르게 설정되도록 보장합니다.
...
이 섹션에서는 Azure AD Single Sign-On을 사용할 수 있도록 Britta Simon에게 Predictix Ordering에 대한 액세스 권한을 부여합니다.

1. Azure Portal에서 **엔터프라이즈 애플리케이션** 을 선택하고 **모든 애플리케이션** 을 선택한 다음, **Predictix Ordering** 을 선택합니다.

    ![Enterprise 애플리케이션](common/enterprise-applications.png)

2. 애플리케이션 목록에서 **Predictix Ordering** 을 선택합니다.
...
### <a name="create-a-predictix-ordering-test-user"></a>Predictix Ordering 테스트 사용자 만들기

다음으로 Predictix Ordering에서 Britta Simon이라는 사용자를 만들어야 합니다. [Predictix Ordering 지원 팀](https://www.predix.io/support/)과 협력하여 사용자를 추가하세요. Single Sign-On을 사용하려면 먼저 사용자를 만들고 활성화해야 합니다.
