...
description: Azure Active Directory B2C를 사용하여 Python 웹 애플리케이션에 로그인하고 사용자를 로그인할 수 있습니다.
...
ms.locfileid: "122642351"
---
# <a name="configure-authentication-in-a-sample-python-web-application-using-azure-active-directory-b2c"></a>Azure Active Directory B2C를 사용하여 샘플 Python 웹 애플리케이션에서 인증 구성
...
OIDC(OpenID Connect)는 애플리케이션에 사용자가 안전하게 로그인하는 데 사용할 수 있는 OAuth 2.0 기반의 인증 프로토콜입니다. 이 웹앱 샘플에서는 [Python용 MSAL](https://github.com/AzureAD/microsoft-authentication-library-for-python)을 사용합니다. Python용 MSAL을 사용하면 Python 웹앱에 인증 및 권한 부여 지원을 간편하게 추가할 수 있습니다. 
...
1. 앱은 인증 코드를 ID 토큰으로 교환합니다. 그런 후에 ID 토큰의 유효성을 검사하고, 클레임을 읽고, 사용자에게 보안 페이지를 반환합니다.
...
### <a name="21-register-the-app"></a>2.1 앱 등록
...
1. **리디렉션 URI** 에서 **웹** 을 선택한 다음 URL 텍스트 상자에 `http://localhost:5000/getAToken`를 입력합니다.
1. **사용 권한** 에서 **openid 및 오프라인 액세스 권한에 대한 관리자 동의 부여** 확인란을 선택합니다.
1. **등록** 을 선택합니다.
1. **개요** 를 선택합니다.
1. 웹 애플리케이션을 구성하는 이후 단계에 사용할 수 있도록 **애플리케이션(클라이언트) ID** 를 기록합니다.

    ![애플리케이션 ID 가져오기](./media/configure-authentication-sample-python-web-app/get-azure-ad-b2c-app-id.png)  


### <a name="22-create-a-web-app-client-secret"></a>2.2 웹앱 클라이언트 암호 만들기
...
|키  |값  |
|---------|---------|
|`b2c_tenant`| Azure AD B2C [테넌트 이름](tenant-management.md#get-your-tenant-name)의 첫 번째 부분입니다. 예들 들어 `contoso`입니다.|
|`CLIENT_ID`| [2.1단계](#21-register-the-app)의 웹 API 애플리케이션 ID입니다.|
|`CLIENT_SECRET`| [2.2단계](#22-create-a-web-app-client-secret)에서 만든 클라이언트 암호입니다. 보안 강화를 위해 설명에 권장된 대로 환경 변수에 대신 저장하는 것이 좋습니다. |
...
1. http://localhost:5000으로 이동하여 로컬 머신에서 실행 중인 웹 애플리케이션을 봅니다. 

1. **로그인** 을 선택합니다.

    ![Azure AD B2C 로그인을 보여 주는 스크린샷](./media/configure-authentication-sample-python-web-app/web-app-sign-in.png)


1. 가입 또는 로그인 프로세스를 완료합니다.

1. 성공적으로 인증되면 표시 이름이 표시됩니다. 
...
### <a name="register-the-web-api-application"></a>웹 API 애플리케이션 등록
...
### <a name="grant-the-web-app-permissions"></a>웹앱 권한 부여

[!INCLUDE [active-directory-b2c-app-integration-grant-permissions](../../includes/active-directory-b2c-app-integration-grant-permissions.md)]

### <a name="configure-your-web-api"></a>웹 API 구성
...
### <a name="configure-the-sample-application-with-the-web-api"></a>웹 API를 통해 샘플 애플리케이션 구성

*app_config.py* 파일을 엽니다. 이 파일에는 Azure AD B2C ID 공급자에 대한 정보가 포함되어 있습니다. 앱 설정의 다음 속성을 업데이트합니다. 

|키  |값  |
|---------|---------|
|`ENDPOINT`| 웹 API의 URI입니다. https://localhost:44332/hello)을 입력합니다.|
|`SCOPE`| 만든 웹 API [범위](#configure-scopes)입니다.|
...
* [Azure Active Directory B2C를 사용하여 Python 웹 애플리케이션에서 인증 옵션을 구성](enable-authentication-python-web-app-options.md)하는 방법 알아보기