---
title: 로그 전달자를 배포하여 Syslog 및 CEF 로그를 Azure Sentinel로 수집 | Microsoft Docs
description: Syslog 및 CEF 로그를 Azure Sentinel로 수집하는 프로세스의 일환으로 Syslog 디먼 및 Log Analytics 에이전트로 구성되는 로그 전달자 배포 방법을 알아봅니다.
services: sentinel
...
- 로그를 Azure Sentinel에 전달하는 **Log Analytics 에이전트**(OMS 에이전트라고도 함).
...
- **Data**
    - 이 프로세스의 특정 시점에 Azure Sentinel 작업 영역의 **작업 영역 ID** 와 **작업 영역 기본 키** 가 필요할 수 있습니다. 이는 작업 영역 설정의 **에이전트 관리** 에서 찾을 수 있습니다.
...
1. Azure Sentinel 탐색 메뉴에서 **데이터 커넥터** 를 선택합니다. 커넥터 갤러리(또는 제품이 나열되지 않은 경우 **CEF(Common Event Format)** )에서 제품에 대한 커넥터를 선택한 다음, 오른쪽 아래의 **Open Connector 페이지** 단추를 선택합니다. 
