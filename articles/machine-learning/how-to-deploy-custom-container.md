...
## <a name="create-a-yaml-file-for-your-endpoint-and-deployment"></a>끝점 및 배포에 대 한 YAML 파일 만들기

YAML을 사용하여 클라우드 배포를 구성할 수 있습니다. 이 예제에 대 한 샘플 YAML를 살펴보세요.

__tfserving-endpoint__

:::code language="yaml" source="~/azureml-examples-main/cli/endpoints/online/custom-container/tfserving-endpoint.yml":::

__tfserving-deployment__
...
HTTP 서버는 _선거의_ 와 _준비_ 모두에 대 한 경로를 정의 합니다. 활동성 경로는 서버가 실행 중인지 여부를 확인하는 데 사용됩니다. 준비 경로는 서버가 작업을 수행할 준비가 되었는지 여부를 확인 하는 데 사용 됩니다. 기계 학습 유추에서 서버는 모델을 로드하기 전에 활동성 요청에 200 정상으로 응답할 수 있습니다. 서버는 모델이 메모리에 로드된 후에만 준비 요청에 200 정상으로 응답할 수 있습니다.
...
예를 들어 로컬 컴퓨터에의 디렉터리 구조가 있는 경우 `/azureml-examples/cli/endpoints/online/custom-container` 모델의 이름은 `half_plus_two` 다음과 같습니다.

:::image type="content" source="./media/how-to-deploy-custom-container/local-directory-structure.png" alt-text="로컬 디렉터리 구조의 트리 뷰를 표시 하는 다이어그램입니다.":::
...
그러면 모델은 `/var/azureml-app/azureml-models/tfserving-deployment/1` 배포의 아래에 배치 됩니다.

:::image type="content" source="./media/how-to-deploy-custom-container/deployment-location.png" alt-text="배포 디렉터리 구조의 트리 뷰를 표시 하는 다이어그램입니다.":::

필요에 따라을 구성할 수 있습니다 `model_mount_path` . 모델을 탑재 한 경로를 변경할 수 있습니다. 예를 들어 `model_mount_path` _tfserving-deployment_ 에 매개 변수를 사용할 수 있습니다.

> [!IMPORTANT]
> 는 `model_mount_path` Linux의 올바른 절대 경로 (컨테이너 이미지의 OS) 여야 합니다.
...
그러면 모델은 배포의에 배치 됩니다 `/var/tfserving-model-mount/tfserving-deployment/1` . 이 파일은 더 이상 아래 `azureml-app/azureml-models` 에 있지만 지정 된 탑재 경로 아래에 있습니다.

:::image type="content" source="./media/how-to-deploy-custom-container/mount-path-deployment-location.png" alt-text="Mount_model_path를 사용 하는 경우 배포 디렉터리 구조의 트리 뷰를 표시 하는 다이어그램입니다.":::

### <a name="create-your-endpoint-and-deployment"></a>끝점 및 배포 만들기
