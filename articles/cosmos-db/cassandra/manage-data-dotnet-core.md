---
title: '빠른 시작: .NET Core가 포함된 Cassandra API - Azure Cosmos DB'
description: 이 빠른 시작에서는 Azure Portal 및 .NET Core와 함께 Azure Cosmos DB Cassandra API를 사용하여 프로필 애플리케이션을 만드는 방법을 보여 줍니다.
...
이 빠른 시작에서는 GitHub에서 예제를 복제하여 프로필 앱을 빌드하기 위해 .NET Core와 Azure Cosmos DB [Cassandra API](cassandra-introduction.md)를 사용하는 방법을 보여 줍니다. 또한 웹 기반 Azure Portal을 사용하여 Azure Cosmos DB 계정을 만드는 방법도 보여 줍니다.
...
이 단계는 선택 사항입니다. 코드로 데이터베이스 리소스를 만드는 방법을 알아보려는 경우 다음 코드 조각을 검토할 수 있습니다. 이 코드 조각은 모두 `C:\git-samples\azure-cosmos-db-cassandra-dotnet-core-getting-started\CassandraQuickStart` 폴더에 설치된 `async Task ProcessAsync()` 메서드 내의 `Program.cs` 파일에서 가져온 것입니다. 그렇지 않으면 [연결 문자열 업데이트](#update-your-connection-string)로 건너뛸 수 있습니다.
...
1. 화면 오른쪽에 있는 :::image type="icon" source="./media/manage-data-dotnet/copy.png"::: 단추를 사용하여 USERNAME 값을 복사합니다.

   :::image type="content" source="./media/manage-data-dotnet/keys.png" alt-text="Azure Portal, 연결 문자열 페이지에서 액세스 키 보기 및 복사":::
...
1. 포털의 USERNAME 값을 줄 13의 `<PROVIDE>`에 붙여넣습니다.
    ...
    CONTACT POINT 값에 대해 줄 15의 `<PROVIDE>`에 동일한 값을 붙여넣을 수도 있습니다.
...
1. 포털로 돌아가서 CONTACT POINT 값을 복사합니다. 포털의 CONTACT POINT 값을 줄 16의 `<PROVIDE>`에 붙여넣습니다.

    이제 Program.cs의 줄 16은 다음과 유사합니다. 
...
3. Ctrl+F5를 눌러 애플리케이션을 실행합니다. 앱이 콘솔 창에 표시됩니다. 
    ...
    :::image type="content" source="./media/manage-data-dotnet/output.png" alt-text="출력 보기 및 확인":::

    프로그램 실행을 중지하고 콘솔 창을 닫으려면 CTRL+C를 누릅니다. 
    
4. Azure Portal에서 **데이터 탐색기** 를 열어 이 새 데이터를 쿼리/수정/사용합니다.

    :::image type="content" source="./media/manage-data-dotnet/data-explorer.png" alt-text="데이터 탐색기에서 데이터 보기":::
