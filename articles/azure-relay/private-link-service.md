...
ms.openlocfilehash: a8936508eafeb2b8ceffa837fef97f389dfd8b43
ms.sourcegitcommit: 66b6e640e2a294a7fbbdb3309b4829df526d863d
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/01/2021
ms.locfileid: "133359563"
---
...
> [!NOTE]
> 개인 링크를 통해 **릴레이 수신기** 를 사용 하는 경우 표준 릴레이 포트와 함께 나가는 통신용 포트 **9400-9599** 를 엽니다. **릴레이 수신기** 에 대해서만이 단계를 수행 해야 합니다.
...
7. **기본** 페이지에서 다음 단계를 수행합니다. 
    ...
    5. 프라이빗 엔드포인트에 대한 **지역** 을 선택합니다. 개인 끝점은 가상 네트워크와 동일한 지역에 있어야 하지만 연결 하는 Azure Relay 네임 스페이스와 다른 지역에 있을 수 있습니다. 
...
8. **리소스** 페이지에서 다음 단계를 수행합니다.
    1. 연결 방법의 경우 **내 디렉터리의 Azure 리소스에 커넥트를** 선택 하면 해당 네임 스페이스에 대 한 소유자 또는 참가자 액세스 권한이 있고 네임 스페이스는 개인 끝점과 동일한 디렉터리에 있습니다. 다음 단계를 수행 합니다. 
    ...
    2. 네임 스페이스가 개인 끝점과 동일한 디렉터리에 있지 않기 때문에 **리소스 ID 또는 별칭으로 Azure 리소스에 커넥트를** 선택 하는 경우 다음 단계를 수행 합니다.
        1. **리소스 ID** 또는 **별칭** 을 입력합니다. 다른 사람과 공유한 리소스 ID 또는 별칭일 수 있습니다. 리소스 ID를 가져오는 가장 쉬운 방법은 Azure Portal에서 Azure Relay 네임스페이스로 이동하여 `/subscriptions/`부터 URI의 일부를 복사하는 것입니다. 예를 들면 다음과 같습니다. `/subscriptions/000000000-0000-0000-0000-000000000000000/resourceGroups/myresourcegroup/providers/Microsoft.Relay/namespaces/myrelaynamespace.` 
...
12. **프라이빗 엔드포인트** 페이지에서 프라이빗 엔드포인트 연결의 상태를 볼 수 있습니다. Relay 네임 스페이스의 소유자 이거나이에 대 한 액세스를 관리 하 고 **연결 방법** 에 대해 **내 디렉터리에서 Azure 리소스에 커넥트를** 선택한 경우 끝점 연결이 **자동으로 승인** 됩니다. **보류 중** 인 경우 [Azure Portal을 사용하여 프라이빗 엔드포인트 관리](#manage-private-endpoints-using-azure-portal) 섹션을 참조하세요.
...
프라이빗 엔드포인트를 만들 때 연결이 승인되어야 합니다. 개인 끝점을 만드는 데 사용한 리소스 (릴레이 네임 스페이스)가 디렉터리에 있으면 릴레이 네임 스페이스에 대 한 권한이 관리 되는 경우 연결 요청을 승인할 수 있습니다. 관리 액세스 권한이 없는 릴레이 네임스페이스에 연결하는 경우 해당 리소스의 소유자가 연결 요청을 승인할 때까지 기다려야 합니다.
...
1. 거부하려는 프라이빗 엔드포인트 연결이 있는 경우 보류 중인 요청인지 아니면 이전에 승인된 기존 연결인지에 관계없이 엔드포인트 연결을 선택하고 **거부** 단추를 클릭합니다.
...
3. 상태가 **연결 끊김** 으로 변경되어 있어야 합니다. 그러면 목록에 엔드포인트가 표시되지 않습니다. 
...
1. **가상 네트워크** 와 **서브넷** 을 지정합니다. 프라이빗 엔드포인트를 배포한 Virtual Network 선택합니다.
2. **공용 IP** 리소스를 지정합니다.
