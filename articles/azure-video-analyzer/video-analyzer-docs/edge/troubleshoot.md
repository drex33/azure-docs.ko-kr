...
도움이 필요할 수 있는 추가 문제가 있으면 **[로그를 수집하여 지원 티켓을 제출](#collect-logs-for-submitting-a-support-ticket)** 하세요. **[videoanalyzerhelp@microsoft.com](mailto:videoanalyzerhelp@microsoft.com)** 으로 이메일을 보내서 연락하셔도 됩니다.
...
1. 포털의 **IoT Hub** 페이지에서 가상 머신에 연결
   ...
   1. _debugLogs_ 폴더의 모든 파일을 압축합니다. 이러한 로그 파일은 자체 진단용이 아닙니다. Azure 엔지니어링 팀이 문제를 분석하기 위한 것입니다.
...
1. `support-bundle` 명령을 _--since_ 플래그와 함께 실행하여 로그에 포함할 시간을 지정합니다. 예를 들어 2h는 지난 2시간 동안의 로그를 가져옵니다. 다른 기간에 대한 로그를 포함하도록 이 플래그의 값을 변경할 수 있습니다.
   ...
   이 명령은 명령을 실행한 디렉터리에 _support_bundle.zip_ 라는 파일을 만듭니다.
...
[.NET의 gRPC에서 로깅 및 진단 ](/aspnet/core/grpc/diagnostics?view=aspnetcore-5.0&preserve-view=true)에는 gRPC 서버에서 일부 진단 로그를 수집하기 위한 몇 가지 지침이 제공됩니다.
