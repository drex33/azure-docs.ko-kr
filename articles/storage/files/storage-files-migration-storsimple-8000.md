...
ms.locfileid: "131421944"
---
# <a name="storsimple-8100-and-8600-migration-to-azure-file-sync"></a>Azure 파일 동기화로 StorSimple 8100 및 8600 마이그레이션

StorSimple 8000 시리즈는 8100 또는 8600 물리적 온-프레미스 어플라이언스와 해당 클라우드 서비스 구성 요소로 표시됩니다. StorSimple 8010 및 8020 가상 어플라이언스는이 마이그레이션 가이드에도 설명 되어 있습니다. 선택적 Azure 파일 동기화를 사용 하 여 이러한 어플라이언스의 데이터를 Azure 파일 공유로 마이그레이션할 수 있습니다. Azure 파일 동기화은 StorSimple 온-프레미스 기능을 대체 하는 기본적인 전략적 장기 Azure 서비스입니다.
...
마이그레이션을 시작 하기 전에 StorSimple Data Manager 및 Azure 파일 공유에는 마이그레이션을 방지 하기 위해 고려해 야 할 몇 가지 제한 사항이 있습니다.
* StorSimple 어플라이언스의 NTFS 볼륨만 지원 됩니다.
* 서비스가 BitLocker로 암호화된 볼륨에서 작동하지 않습니다.
* 손상 된 StorSimple 백업은 마이그레이션할 수 없습니다.
* 방화벽, 개인 끝점 전용 통신 등의 특수 네트워킹 옵션은 StorSimple 백업이 저장 되는 원본 저장소 계정 및 Azure 파일 공유를 보유 하는 대상 저장소 계정에서 사용 하도록 설정할 수 없습니다.


### <a name="file-fidelity"></a>파일 충실도

[알려진 제한](#known-limitations) 사항으로 인해 마이그레이션을 방해 하는 제한 사항이 없는 경우 주의 해야 하는 Azure 파일 공유에 저장 될 수 있는 항목에 대 한 제한 사항이 여전히 있습니다.
_파일 충실도_ 는 파일을 구성 하는 다양 한 특성, 타임 스탬프 및 데이터를 나타냅니다. 마이그레이션에서 파일 충실도는 원본 (StorSimple 볼륨)에 대 한 정보를 대상 (Azure 파일 공유)으로 변환 (마이그레이션) 할 수 있는 정도를 측정 한 것입니다.
Azure Files [NTFS 파일 속성](/windows/win32/fileio/file-attribute-constants)의 [하위 집합을 지원](/rest/api/storageservices/set-file-properties) 합니다. Acl, 공통 메타 데이터 및 일부 타임 스탬프는 마이그레이션됩니다. 다음 항목은 마이그레이션을 방지 하지 않지만 마이그레이션 중에 항목당 문제를 일으킵니다.

* 타임 스탬프: 파일 변경 시간이 설정 되지 않습니다. 현재 REST 프로토콜을 통해 읽기 전용입니다. 파일의 마지막 액세스 타임 스탬프는 이동 되지 않으므로 현재 Azure 파일 공유에 저장 된 파일에 대해 지원 되는 특성이 아닙니다.
* [대체 데이터 스트림](/openspecs/windows_protocols/ms-fscc/b134f29a-6278-4f3f-904f-5e58a713d2c5) 는 Azure 파일 공유에 저장할 수 없습니다. 대체 데이터 스트림을 보유 하는 파일이 복사 되지만 대체 데이터 스트림 프로세스의 파일에서 제거 됩니다.
* 마이그레이션 중에는 기호화 된 링크, 하드 링크, 연결 및 재분석 지점이 생략 됩니다. 마이그레이션 복사 로그는 건너뛴 항목 및 이유를 모두 나열 합니다.
* EFS로 암호화 된 파일을 복사할 수 없습니다. 로그 복사는 "액세스가 거부 되었습니다."와 함께 복사 하지 못한 항목을 표시 합니다.
* 손상 된 파일을 건너뜁니다. 복사 로그에 StorSimple 디스크에서 손상 된 각 항목에 대해 다른 오류가 나열 될 수 있습니다. "심각한 장치 하드웨어 오류로 인해 요청이 실패 했습니다." 또는 "파일 또는 디렉터리가 손상 되었거나 읽을 수 없습니다." 또는 "ACL (access control 목록) 구조가 잘못 되었습니다" 라는 메시지가 표시 될 수 있습니다.
* 4 TiB 보다 큰 개별 파일은 건너뜁니다.
* 파일 경로 길이는 2048 자이 하 여야 합니다. 경로가 긴 파일 및 폴더는 건너뜁니다.
...
* 프리미엄 파일 공유의 경우 저장소 계정 만들기 마법사에서 *파일 공유* 를 선택 합니다.
...
:::row:::
    :::column:::
        :::image type="content" source="media/storage-files-migration-storsimple-8000/storage-files-migration-storsimple-8000-new-job.png" alt-text="마이그레이션 작업에 대 한 새 작업 생성 양식의 스크린샷":::
    :::column-end:::
    :::column:::
...
- 마이그레이션 작업은 라이브 볼륨 데이터가 아닌 백업만 이동할 수 있습니다. 따라서 최근 백업은 라이브 데이터와 가장 가까이 있으며 항상 마이그레이션에서 이동할 백업 목록에 포함되어야 합니다. 백업 선택 대화 상자를 열면 기본적으로 선택 됩니다.
...
- 백업을 선택 하려면 마이그레이션하려는 StorSimple 볼륨이 온라인 상태 여야 합니다.
...
열리는 작업 블레이드에서 작업의 현재 상태와 선택한 백업 목록을 볼 수 있습니다. 백업 목록은 가장 오래된 것에서 최신으로 정렬되며 이 순서대로 Azure 파일 공유로 마이그레이션됩니다.  

:::row:::
    :::column:::        
        :::image type="content" source="media/storage-files-migration-storsimple-8000/storage-files-migration-storsimple-8000-job-never-ran-focused.png" alt-text="작업을 시작하는 명령 주위에 강조 표시가 있는 마이그레이션 작업 블레이드의 스크린샷 또한 마이그레이션을 위해 예약된 선택한 백업을 표시합니다." lightbox="media/storage-files-migration-storsimple-8000/storage-files-migration-storsimple-8000-job-never-ran.png":::
    :::column-end:::
    :::column:::
        처음에는 마이그레이션 작업에 다음 상태가 표시됩니다. 를 **실행하지 않았습니다.** </br>준비가 되면 이 마이그레이션 작업을 시작할 수 있습니다. (더 높은 해상도의 버전에 대한 이미지를 선택합니다.) </br> 백업이 성공적으로 마이그레이션되면 자동 Azure 파일 공유 스냅샷이 생성됩니다. StorSimple 백업의 원래 백업 날짜는 Azure 파일 공유 스냅샷의 *설명* 섹션에 배치됩니다. 이 필드를 사용하면 파일 공유 스냅샷을 생성한 시간과 비교하여 데이터가 원래 백업된 시간을 확인할 수 있습니다.
        </br></br>
        > [!CAUTION]
        > 백업은 가장 오래된 백업에서 최신 백업으로 처리해야 합니다. 마이그레이션 작업이 만들어지면 선택한 StorSimple 볼륨 백업 목록을 변경할 수 없습니다. 백업 목록이 올바르지 않거나 불완전한 경우 작업을 시작하지 마세요. 작업을 삭제하고 올바른 백업이 선택된 새 작업을 만듭니다.
    :::column-end:::
:::row-end:::

### <a name="per-item-errors"></a>항목별 오류

마이그레이션 작업에는 복사 중에 발생했을 수 있는 문제를 나열하는 두 개의 열이 백업 목록에 있습니다.

* 복사 오류 </br>이 열에는 복사해야 하지만 복사되지 않은 파일 또는 폴더가 나열됩니다. 이러한 오류는 종종 복구할 수 있습니다. 백업에 이 열의 항목 문제가 나열되면 복사 로그를 검토합니다. 이러한 파일을 마이그레이션해야 하는 경우 **백업 다시 시도** 를 선택합니다. 백업 처리가 완료되면 이 옵션을 사용할 수 있습니다. [마이그레이션 작업 관리](#manage-a-migration-job) 섹션에서는 옵션을 자세히 설명합니다.
* 지원되지 않는 파일 </br>이 열에는 마이그레이션할 수 없는 파일 또는 폴더가 나열됩니다. Azure Storage 현재 또는 논리적으로 Azure 파일 공유에 저장할 수 없는 파일 이름, 경로 길이 및 파일 형식에 제한이 있습니다. 마이그레이션 작업은 이러한 종류의 오류에 대해 일시 중지되지 않습니다. 백업 마이그레이션을 다시 시도해도 결과가 변경되지 않습니다. 백업에 이 열의 항목 문제가 나열되면 복사 로그를 검토하고 기록해 둡니다. 마지막 백업에서 이러한 문제가 발생하고 복사 로그에서 영향을 주는 파일 이름, 경로 길이 또는 기타 문제로 인해 오류가 발생한 경우 라이브 StorSImple 볼륨의 문제를 해결하고 StorSimple 볼륨 백업을 수행하며 해당 백업만으로 새 마이그레이션 작업을 만들 수 있습니다. 그런 다음 이 해결된 네임스페이스를 마이그레이션하고 Azure 파일 공유의 최신/라이브 버전이 됩니다. 수동 및 시간이 많이 걸리는 프로세스입니다. 복사 로그를 신중하게 검토하고 가치가 있는지 평가합니다.

이러한 복사 로그는 네임스페이스 항목이 성공적으로 나열된 파일과 복사에 실패한 항목을 *\*.csv.* 오류는 이전에 설명한 범주로 추가로 분할됩니다.
로그 파일 위치에서 "실패"를 검색하여 실패한 파일에 대한 로그를 찾을 수 있습니다. 결과는 복사에 실패한 파일에 대한 로그 집합이어야 합니다. 이러한 로그를 크기별로 정렬합니다. 크기가 17바이트인 추가 로그가 생성될 수 있습니다. 이러한 로그는 비어 있으며 무시해도 됩니다. 정렬을 사용하면 콘텐츠가 있는 로그에 집중할 수 있습니다.

복사 성공이 기록된 로그 파일에도 동일한 프로세스가 적용됩니다.

### <a name="manage-a-migration-job"></a>마이그레이션 작업 관리

마이그레이션 작업에는 다음과 같은 상태가 있습니다.
* **실행 안 했습니다.** </br>정의되었지만 이전에 실행되지 않은 새 작업입니다.
* **대기 중** </br>이 상태의 작업은 마이그레이션 서비스에서 리소스가 프로비전되기를 기다리고 있습니다. 준비가 되면 자동으로 다른 상태로 전환됩니다.
* **실패** </br>실패한 작업이 더 많은 백업을 처리하지 못하게 하는 심각한 오류에 도달했습니다. 작업이 이 상태로 들어가지 않습니다. 지원 요청은 가장 좋은 조치입니다.
...
> Microsoft는 microsoft dot :::image type="content" source="media/storage-files-migration-storsimple-8000/storage-files-migration-storsimple-8000-migration-email.png" alt-text="com에서 마이그레이션을 Azure Files"::: 도움을 드립니다. 한 단어로 메일 주소 보내기
