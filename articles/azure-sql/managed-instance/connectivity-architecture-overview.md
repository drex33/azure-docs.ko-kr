...
- **Azure 정책에서 허용:** [Azure Policy](../../governance/policy/overview.md) 를 사용 하 여 SQL Managed Instance 서브넷/가상 네트워크를 포함 하는 범위에서 리소스를 만들거나 수정 하는 것을 거부할 경우 해당 정책은 Managed Instance 내부 리소스를 관리 하는 것을 방지 해서는 안 됩니다. 정상 작업을 사용 하려면 다음 리소스를 거부 효과에서 제외 해야 합니다.
  ...
  - Microsoft. Network/virtualNetworks/서브넷/contextualServiceEndpointPolicies 형식의 모든 리소스
- **가상 네트워크 잠금:** 전용 서브넷의 가상 네트워크, 해당 부모 리소스 그룹 또는 구독에 대 한 [잠금은](../../azure-resource-manager/management/lock-resources.md) 때때로 SQL Managed Instance의 관리 및 유지 관리 작업을 방해할 수 있습니다. 이러한 잠금을 사용할 때는 특별히 주의 해야 합니다.
