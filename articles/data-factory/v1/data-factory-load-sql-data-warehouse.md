---
title: 수 테라바이트 분량의 데이터를 Azure Synapse Analytics에 로드
...
> [!NOTE]
> 이 아티클은 Data Factory 버전 1에 적용됩니다. 현재 버전의 Data Factory 서비스를 사용 중인 경우 [Data Factory를 통한 Azure Synapse Analytics 간 데이터 복사](../connector-azure-sql-data-warehouse.md)를 참조하세요.


[Azure Synapse Analytics](../../synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is.md)는 대용량 관계형 및 비관계형 데이터를 처리할 수 있는 클라우드 기반 스케일 아웃 데이터베이스입니다.  MPP(대용량 병렬 처리) 아키텍처를 기반으로 하는 Azure Synapse Analytics는 엔터프라이즈 데이터 웨어하우스 작업에 최적화되어 있습니다.  스토리지를 확장하고 개별적으로 계산할 수 있는 클라우드 탄력성을 유연하게 제공합니다.

Azure Synapse Analytics로 시작하는 것은 이제 **Azure Data Factory** 를 사용하는 것보다 쉬워졌습니다.  Azure Data Factory는 완벽한 관리형 클라우드 기반 데이터 통합 서비스로서, 기존 시스템에서 나온 데이터로 Azure Synapse Analytics를 채우는 데 사용되므로 Azure Synapse Analytics를 평가하고 분석 솔루션을 빌드하는 데 소요되는 시간을 절약할 수 있습니다. Azure Data Factory를 사용하여 Azure Synapse Analytics로 데이터를 로드하는 방법에는 다음과 같은 주요 장점이 있습니다.
...
* **PolyBase를 통해 제공되는 뛰어난 성능**: 데이터를 Azure Synapse Analytics로 이동시키는 가장 효율적인 방법은 Polybase를 사용하는 것입니다. 스테이징 Blob 기능을 사용하면 Polybase가 기본적으로 지원하는 Azure Blob Storage를 제외한 모든 유형의 데이터 저장소에서 높은 로드 속도를 얻을 수 있습니다.

이 문서에서는 Data Factory 복사 마법사를 사용하여 Azure Blob Storage에서 나온 1TB 데이터를 15분 내 처리량 1.2GBps 이상의 속도로 Azure Synapse Analytics로 로드하는 방법을 보여 줍니다.

또한 복사 마법사를 사용하여 Azure Synapse Analytics로 데이터를 이동하기 위한 단계별 지침을 제공합니다.

> [!NOTE]
>  Azure Synapse Analytics 간에 데이터를 이동하는 Data Factory 기능에 대한 일반적인 내용은 [Azure Data Factory를 통한 Azure Synapse Analytics 간 데이터 이동](data-factory-azure-sql-data-warehouse-connector.md) 문서를 참조하세요.
>
> Visual Studio, PowerShell 등을 사용하여 파이프라인을 빌드할 수도 있습니다. Azure Data Factory에서 복사 작업을 사용하는 단계별 지침을 간략하게 살펴보려면, [자습서: Azure Blob에서 Azure SQL Database에 데이터 복사](data-factory-copy-data-from-azure-blob-storage-to-sql-database.md)를 참조하세요.  
>
>
...
* Azure Synapse Analytics: 본 실험은 6,000 DWU로 만든 Azure Synapse Analytics에 데이터를 로드합니다.

    Azure Synapse Analytics 데이터베이스를 만드는 방법에 대한 자세한 지침은 [Azure Synapse 분석 만들기](../../synapse-analytics/sql-data-warehouse/create-data-warehouse-portal.md)를 참조하세요.  Polybase를 사용하여 Azure Synapse Analytics에 대한 최상의 로드 성능을 얻기 위해 성능 설정에서 허용되는 최대 DWU(데이터 웨어하우스 단위) 수(6,000 DWU)를 선택합니다.

  > [!NOTE]
  > Azure Blob에서 로드할 때 데이터 로드 성능은 Azure Synapse Analytics에서 구성하는 DWU 수에 정비례합니다.
  >
  > 1TB를 1,000DWU Azure Synapse Analytics에 로드하는 데는 87분(처리량 ~200MBps), 1TB를 2,000DWU Azure Synapse Analytics에 로드하는 데는 46분(처리량 ~380MBps), 1TB를 6,000 DWU Azure Synapse Analytics에 로드하는 데는 14분(처리량 ~1.2GBps)이 걸립니다.
  >
  >
    ...
    **크기 조정** 단추를 클릭하여 다음 패널을 열고, 슬라이더를 최대값으로 이동하고 나서, **저장** 단추를 클릭합니다.
    ...
    최상의 처리량을 위해서는 `xlargerc` 리소스 클래스에 속한 Azure Synapse Analytics 사용자를 통해 복사해야 합니다.  [사용자 리소스 클래스 변경 예제](../../synapse-analytics/sql-data-warehouse/resource-classes-for-workload-management.md)에 따라 이 작업을 하는 방법을 알아보세요.  
* 다음 DDL 명세서를 실행하여 Azure Synapse Analytics 데이터베이스에서 대상 테이블 스키마를 만듭니다.
...
3. **새 데이터 팩터리** 창에서 다음을 수행합니다.
   ...
   1. **이름** 으로 **LoadIntoSQLDWDataFactory** 를 입력합니다.
       Azure Data Factory 이름은 전역적으로 고유해야 합니다. **데이터 팩터리 이름 ‘LoadIntoSQLDWDataFactory’ 사용할 수 없음** 이라는 오류가 표시되는 경우, 데이터 팩터리 이름을 변경하고(예: yournameLoadIntoSQLDWDataFactory) 다시 만듭니다. 데이터 팩터리 아티팩트에 대한 명명 규칙은 [데이터 팩터리 - 명명 규칙](data-factory-naming-rules.md) 항목을 참조하세요.  
   2. Azure **구독** 을 선택합니다.
...
* Azure Synapse Analytics에 로드를 완료한 후에 통계를 만듭니다.

자세한 내용은 [Azure Synapse Analytics 모범 사례](../../synapse-analytics/sql/best-practices-dedicated-sql-pool.md)를 참조하세요.
