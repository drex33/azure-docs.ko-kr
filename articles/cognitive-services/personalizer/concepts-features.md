...
* **문자열**: 문자열 형식의 경우 키와 값의 모든 조합은 One-Hot 기능으로 처리됩니다. 예를 들어 장르 "ScienceFiction" 및 장르 "다큐멘터리"는 기계 학습 모델에 대한 두 개의 새로운 입력 기능을 만듭니다.
...
다음 JSON에서,, `user` `environment` `device` 및 `activity` 는 기능 네임 스페이스입니다. 

> [!Note]
> 현재는 UTF-8을 기반으로 하고 다른 문자로 시작하는 기능 네임스페이스 이름을 사용하는 것이 좋습니다. 예를 들어,, 및는,, `user` `environment` `device` `activity` 및로 시작 `u` `e` `d` `a` 합니다. 현재 동일한 첫 문자를 포함하는 네임스페이스가 있으면 기계 학습에 사용되는 인덱스에서 충돌이 발생할 수 있습니다.
