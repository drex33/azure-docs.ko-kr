---
title: AD FS 2.0과 함께 Azure MFA Server 사용 - Azure Active Directory
...
이 문서에서는 AD FS 2.0과 함께 Azure Multi-Factor Authentication 서버를 사용하는 방법을 소개합니다. AD FS에 대한 자세한 정보는 [Windows Server와 Azure Multi-factor Authentication 서버를 사용하여 클라우드 및 온-프레미스 리소스 보안 유지](howto-mfaserver-adfs-windows-server.md)를 참조하세요.
...
11. 고급 검색 옵션을 보려면 **고급...** 고급 설정을 확인합니다. 구성할 수 있는 설정은 다음과 같습니다.
...
2. 설정 탭에서 **특정 LDAP 구성 사용** 라디오 단추를 선택합니다.
   ...
   ![특정 LDAP 설정에 대한 LDAP 설정 구성](./media/howto-mfaserver-adfs-2/ldap1.png)
...
Active Directory에서 서버로 사용자를 가져왔는지 확인합니다. 해당 위치에서 웹 사이트에 로그인할 때 2단계 인증이 필요하지 않도록 내부 IP 주소를 허용하려면 [신뢰할 수 있는 IP](#trusted-ips) 섹션을 참조하세요.

![회사 설정 구성을 위한 레지스트리 편집기](./media/howto-mfaserver-adfs-2/reg.png)
...
Active Directory에서 서버로 사용자를 가져왔는지 확인합니다. 해당 위치에서 웹 사이트에 로그인할 때 2단계 인증이 필요하지 않도록 내부 IP 주소를 허용하려면 신뢰할 수 있는 IP 섹션을 참조하세요.
...
4. 허용되어야 할 IP 주소, IP 주소 범위 또는 서브넷을 입력합니다. 서브넷을 입력하는 경우 적합한 넷마스크를 선택하고 **확인** 단추를 클릭합니다.

![MFA 서버에 대한 신뢰할 수 있는 IP 구성](./media/howto-mfaserver-adfs-2/trusted.png)
