---
title: 장애 조치(failover) 클러스터 인스턴스 마이그레이션
description: Azure Migrate를 사용하여 Azure VM의 SQL Server로 Always On 장애 조치(failover) 클러스터 인스턴스 고가용성 솔루션을 리프트 앤 시프트하는 방법을 알아봅니다.
...
이 문서에서는 [Azure Migrate: 서버 마이그레이션 도구](../../../migrate/migrate-services-overview.md#azure-migrate-server-migration-tool)를 사용하여 Always On FCI(장애 조치 클러스터 인스턴스)를 Azure VM의 SQL Server로 마이그레이션하는 방법을 설명합니다. 마이그레이션 도구를 사용하면 장애 조치 클러스터 인스턴스의 각 노드를 클러스터 및 FCI 메타데이터뿐만 아니라 SQL Server를 호스트하는 Azure VM으로 마이그레이션할 수 있습니다.  
...
> * 복제를 모니터링합니다.
> * 전체 VM 마이그레이션을 실행합니다.
> * Azure 공유 디스크로 SQL 장애 조치 클러스터를 다시 구성합니다.
...
**Azure 가상 네트워크 설정** | Azure VNet(가상 네트워크)을 [설정](../../../virtual-network/manage-virtual-network.md#create-a-virtual-network)합니다. Azure에 복제하면 Azure VM이 만들어지고 마이그레이션을 설정할 때 지정한 Azure VNet에 조인됩니다.
...
서버 마이그레이션을 준비하려면 물리적 서버 설정을 확인하고 복제 어플라이언스 배포를 준비해야 합니다.
...
2. Azure에 복제하는 원본 머신이 [Azure VM 요구 사항](../../../migrate/migrate-support-matrix-physical-migration.md#azure-vm-requirements)을 준수하는지 확인합니다.
...
원본 머신을 준비하려면 클러스터의 정보가 필요합니다. 

> [!CAUTION]
> - 최종 중단될 때까지 복제 프로세스 전체에서 디스크 소유권을 유지합니다. 디스크 소유권이 변경되는 경우 볼륨이 손상되어 복제를 다시 트리거해야 할 가능성이 있습니다. 각 디스크에 대한 기본 설정 소유자를 설정하여 복제 프로세스 중에 소유권을 전송하지 않도록 합니다. 
> - 복제 프로세스 중에 패치 작업 및 시스템 재부팅을 피하여 디스크 소유권이 전송되지 않도록 합니다.

소스 머신을 준비하려면 다음을 수행합니다. 

1. **디스크 소유권 식별**: 클러스터 노드 중 하나에 로그인하고 장애 조치 클러스터 관리자를 엽니다. 디스크의 소유자 노드를 식별하여 각 서버와 함께 마이그레이션해야 하는 디스크를 결정합니다.  
2. **클러스터 정보 검색:** 클러스터 노드에서 `Get-ClusterInfo.ps1` 스크립트를 실행하여 클러스터 리소스에 대한 정보를 검색합니다. 스크립트는 `Cluster-Config.csv` 파일의 역할 이름, 리소스 이름, IP 및 프로브 포트를 출력합니다. 이 문서의 뒷부분에서 이 CSV 파일을 사용하여 Azure에서 리소스를 만들고 할당합니다.  
...
1. `Cluster-Config.csv` 파일의 열을 작성합니다. 
    ...
    **열 머리글** | **설명**
    ...
    ServicePort | CSV 파일의 각 리소스에서 사용할 서비스 포트를 지정합니다. SQL 클러스터 리소스의 경우 CSV의 프로브 포트와 동일한 서비스 포트 값을 사용합니다. 다른 클러스터 역할의 경우 사용되는 기본값은 1433이지만 현재 설정에서 구성된 포트 번호를 계속 사용할 수 있습니다. 


1. `Create-ClusterLoadBalancer.ps1` 스크립트를 실행하여 다음 필수 매개 변수를 사용해 부하 분산 장치를 만듭니다. 
...
1. Azure Migrate 프로젝트 > **서버** 의 **Azure Migrate: 서버 마이그레이션** 에서 **복제** 를 선택합니다.
    ...
    ![Azure Migrate - 서버 화면의 스크린샷에는 마이그레이션 도구 아래의 Azure Migrate: 서버 마이그레이션에서 선택한 복제 단추가 표시됩니다.](../../../migrate/media/tutorial-migrate-physical-virtual-machines/select-replicate.png)
...
1. **게스트 자격 증명** 에서 [복제 설치 관리자 설정](#download-replication-appliance-installer) 중에 이전에 만든 더미 계정을 선택합니다. 그런 다음, **다음: 가상 머신** 을 선택합니다.
...
1. **디스크** 에서 VM 디스크를 Azure에 복제해야 하는지 여부를 지정하고, Azure에서 디스크 유형(표준 SSD/HDD 또는 프리미엄 관리 디스크)을 선택합니다. **다음** 을 선택합니다.
    - 이전에 만든 목록을 사용하여 각 서버에 복제할 디스크를 선택합니다. 복제에서 다른 디스크를 제외합니다.
...
- 초기 복제가 완료된 후 각 VM에 대한 컴퓨팅 및 네트워크 항목을 구성합니다. 클러스터에는 일반적으로 여러 개의 NIC가 있지만 마이그레이션에는 하나의 NIC만 필요합니다(다른 NIC는 생성하지 않으므로 설정).
...
1. 마이그레이션된 서버가 원본 서버와 동기화되도록 하려면 클러스터 디스크가 온라인 상태인지 확인하는 동안 SQL Server 리소스를 중지합니다(**장애 조치 클러스터 관리자** > **역할** > **기타 리소스**).   
1. **머신 복제 중** > 서버 이름에서 선택 > **개요** 에서 다음 단계로 이동하기 전에 마이그레이션할 서버에서 SQL Server 리소스를 중지한 후 마지막으로 동기화된 타임스탬프가 있는지 확인합니다. 몇 분밖에 안 걸립니다. 
1. **머신 복제 중** 에서 마우스 오른쪽 단추로 VM > **마이그레이션** 을 차례로 클릭합니다.
1. **마이그레이션** > **가상 머신을 종료하고 데이터 손실 없이 계획된 마이그레이션을 수행하시겠어요?** 에서 **아니요** > **확인** 을 차례로 선택합니다.
...
1. 마이그레이션된 머신을 부하 분산 장치의 백 엔드 풀에 추가합니다. **Load Balancer** > **백 엔드 풀** > 백 엔드 풀 선택 > **마이그레이션된 머신 추가** 로 이동합니다. 

1. `Create-SharedDisks.ps1` 스크립트를 실행하여 서버의 마이그레이션된 디스크를 공유 디스크로 다시 구성합니다. 이 스크립트는 대화형이며 머신 목록을 묻는 메시지를 표시한 다음, 추출할 수 있는 디스크(데이터 디스크만)를 표시합니다. 공유 디스크로 사용할 드라이브가 포함된 머신을 선택하라는 메시지가 한 번 표시됩니다. 이를 선택하면 특정 디스크를 선택하라는 메시지가 머신마다 한 번 표시됩니다. 
...
1. `Attach-SharedDisks.ps1` 스크립트를 실행하여 공유 디스크를 마이그레이션된 서버에 연결합니다. 
...
1. Azure에서 마이그레이션된 서버를 시작하고 모든 노드에 로그인합니다. 
...
SQL Server 장애 조치 클러스터 인스턴스가 준비되었습니다. 
...
1. 마이그레이션된 머신에 Azure VM [Windows](../../../virtual-machines/extensions/agent-windows.md) 에이전트를 설치합니다.
