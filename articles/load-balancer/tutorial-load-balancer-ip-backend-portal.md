---
title: '자습서: IP 기반 백 엔드를 사용하여 공용 부하 분산 장치 만들기 - Azure Portal'
titleSuffix: Azure Load Balancer
description: 이 자습서에서는 IP 기반 백 엔드 풀을 사용하여 공용 부하 분산 장치를 만드는 방법에 대해 알아봅니다.
author: asudbring
ms.author: allensu
...
ms.locfileid: "106123017"
---
# <a name="tutorial-create-a-public-load-balancer-with-an-ip-based-backend-using-the-azure-portal"></a>자습서: Azure Portal을 사용하여 IP 기반 백 엔드로 공용 부하 분산 장치 만들기
...
3. **NAT(Network Address Translation) 게이트웨이 만들기** 의 **기본 사항** 탭에서 다음 정보를 입력하거나 선택합니다.
    ...
    | 리소스 그룹   | **새로 만들기** 를 선택하고 텍스트 상자에 **TutorPubLBIP-rg** 를 입력합니다. </br> **확인** 을 선택합니다. |
    | **인스턴스 세부 정보** |                                                                 |
    | 속성             | **myNATgateway** 입력                                    |
    | 지역           | **(미국) 미국 동부** 를 선택합니다.  |
    | 가용성 영역 | **없음** 을 선택합니다. |
...
1. [Azure Portal](https://portal.azure.com)에 로그인합니다.
2. **리소스 만들기** 를 선택합니다. 
3. 검색 상자에 **부하 분산 장치** 를 입력합니다. 검색 결과에서 **부하 분산 장치** 를 선택합니다.
4. **부하 분산 장치** 페이지에서 **만들기** 를 선택합니다.
5. **부하 분산 장치 만들기** 페이지에서 다음 정보를 입력하거나 선택합니다. 

    | 설정                 | 값                                              |
    | ---                     | ---                                                |
    | **프로젝트 세부 정보** |   |
    | Subscription               | 구독을 선택합니다.    |    
    | Resource group         | **TutorPubLBIP-rg** 를 선택합니다.|
    | **인스턴스 세부 정보** |   |
    ...
    | 계층          | 기본 **지역** 을 그대로 둡니다. |
    | **공용 IP 주소** |   |
    | 공용 IP 주소 | **새로 만들기** 를 선택합니다. </br> 사용하려는 기존 공용 IP가 있는 경우 **기존 항목 사용** 을 선택합니다. |
    | 공용 IP 주소 이름 | 텍스트 상자에 **myPublicIP-LB** 를 입력합니다.|
...
6. 나머지 설정에는 기본값을 적용한 다음, **검토 + 만들기** 를 선택합니다.

7. **검토 + 만들기** 탭에서 **만들기** 를 선택합니다.
...
3. **백 엔드 풀 추가** 페이지에서 다음 정보를 입력하거나 선택합니다.

    | 설정 | 값 |
    | ------- | ----- |
    | 이름 | **myBackendPool** 을 입력합니다. |
    | 가상 네트워크 | **myVNet** 을 선택합니다. |
    | 백 엔드 풀 구성 | **IP 주소** 를 선택합니다. |
    | IP 버전 | **IPv4** 를 선택합니다. |
...
2. **설정** 에서 **부하 분산 규칙** 을 선택한 다음, **+ 추가** 를 선택합니다.

3. 부하 분산 장치 규칙에 대한 다음 정보를 입력하거나 선택합니다.
...
4. 기본값의 나머지를 그대로 둔 다음, **추가** 를 선택합니다.
...
2. **가상 머신 만들기** 에서 **기본** 탭에서 값을 입력하거나 선택합니다.
...
4. [네트워킹] 탭에서 다음을 선택하거나 입력합니다.
    ...
    | 네트워크 보안 그룹 구성 | **새로 만들기** 를 선택합니다. </br> **네트워크 보안 그룹 만들기** 의 **이름** 에서 **myNSG** 를 입력합니다. </br> **인바운드 규칙** 내에서 **+ 인바운드 규칙 추가** 를 선택합니다. </br> **서비스** 에서 **HTTP** 를 선택합니다. </br> **우선 순위** 에서 **100** 을 입력합니다. </br> **이름** 에서 **myHTTPRule** 을 입력합니다. </br> **추가** 를 선택합니다. </br> **확인** 을 선택합니다. |
    | **부하 분산**.  |
    | 기존 부하 분산 솔루션 뒤에 이 가상 머신을 배치하시겠습니까? | 확인란을 선택합니다.|
    | **부하 분산 설정** |
    | 부하 분산 옵션 | **Azure Load Balancer** 선택 |
...
7. 1 ~ 6단계에 따라 다음 값 및 **myVM1** 과 동일한 다른 모든 설정을 사용하여 VM을 만듭니다.
...
1. 왼쪽 메뉴에서 **모든 서비스** 를 선택하고 **모든 리소스** 를 선택한 다음, 리소스 목록에서 **TutorPubLBIP-rg** 리소스 그룹에 있는 **myVM1** 을 선택합니다.

2. **개요** 페이지에서 **연결** 을 선택한 다음, **Bastion** 을 선택합니다.

3. **Bastion 사용** 단추를 선택합니다.
...
9. 1 ~ 7단계를 반복하여 **myVM2** 에 IIS 및 업데이트된 iisstart.htm 파일을 설치합니다.

## <a name="test-the-load-balancer"></a>부하 분산 장치 테스트

1. **개요** 화면에서 부하 분산 장치의 공용 IP 주소를 찾습니다. 왼쪽 메뉴에서 **모든 서비스** 를 선택하고 **모든 리소스** 를 선택한 다음, **myPublicIP-LB** 를 선택합니다.
...
2. **TutorPubLBIP-rg** 리소스 그룹을 선택합니다.

3. **리소스 그룹 삭제** 를 선택합니다.

4. **TutorPubLBIP-rg** 를 입력하고 **삭제** 를 선택합니다.
...
> [Azure Portal을 사용하여 지역 간 Azure Load Balancer 만들기](tutorial-cross-region-portal.md)
