...
Azure Database for MySQL 백업 복사본을 여러 개 저장하여 일시적인 하드웨어 오류, 네트워크 또는 정전, 대규모 자연 재해 등 계획되거나 계획되지 않은 이벤트로부터 데이터를 보호합니다. Azure Database for MySQL 기본, 범용 및 메모리 최적화 계층에서 로컬 중복, 영역 중복 또는 지역 중복 백업 스토리지 중에서 유연하게 선택할 수 있습니다. 기본적으로 Azure Database for MySQL 서버 백업 스토리지는 동일한 영역 HA(고가용성) 또는 고가용성 구성이 없는 서버의 경우 로컬로 중복되고 영역 중복 HA 구성이 있는 서버의 경우 영역이 중복됩니다.

백업 중복성은 오류가 발생하더라도 데이터베이스가 가용성 및 내구성 목표를 충족하도록 보장하며, Azure Database for MySQL 사용자에게 세 가지 옵션을 확장합니다. 
...
- **지역 중복 백업 스토리지:** 백업이 지역 중복 백업 스토리지에 저장되면 서버가 호스트되는 지역 내에 여러 복사본이 저장될 뿐만 아니라 지역 쌍을 이루는 지역에도 복제됩니다. 이렇게 하면 재해 발생 시 다른 지역에서 서버를 복원하는 데 더 효율적인 보호와 기능을 제공합니다. 또한 이 기능은 지정된 1년 동안 Backups 개체의 최소 99.999999999999999%(16개의 9) 내구성을 제공합니다. 서버 생성 시 Geo-Redundancy 옵션을 사용하도록 설정하여 지역 중복 백업 스토리지를 보장할 수 있습니다. 지역 중복은 Azure 쌍을 이루는 지역 에서 호스트되는 서버에 대해 [지원됩니다.](../../best-practices-availability-paired-regions.md) 
...
- **로컬 중복에서 지역 중복 백업 스토리지로 이동** - 로컬 중복 스토리지에서 지역 중복 스토리지로 백업 스토리지를 이동하려면 지정 시간 복원 작업을 수행하고 컴퓨팅 + Storage 서버 구성을 변경하여 로컬 중복 원본 서버에 지역 중복을 사용하도록 설정할 수 있습니다. 동일한 영역 중복 HA 서버는 기본 백업 스토리지가 로컬로 중복되는 것과 비슷한 방식으로 지역 중복 서버로 복원할 수도 있습니다. 
...
> [!IMPORTANT]
> 서버 삭제 시점으로부터 5일 이내에 삭제된 MySQL 유연한 서버 리소스를 복구할 수 있습니다. 삭제된 서버를 복원하는 방법에 대한 자세한 가이드는 문서화된 단계 를 [참조하세요.](../flexible-server/how-to-restore-dropped-server.md) 배포 후 실수로 인한 삭제 또는 예기치 않은 변경으로부터 서버 리소스를 보호하기 위해 관리자는 [관리 잠금을](../../azure-resource-manager/management/lock-resources.md)활용할 수 있습니다.

## <a name="geo-restore"></a>지역 복원

지역 중복 백업을 위해 서버를 구성한 경우 서비스를 사용할 수 있는 [지역 쌍의 지역으로](../../best-practices-availability-paired-regions.md) 서버를 복원할 수 있습니다. 다른 지역으로의 지역 복원은 현재 지원되지 않습니다. 
...
> [!NOTE]
> 영역 중복 고가용성으로 구성된 유연한 서버를 지역 복원하는 경우 복원된 서버는 지역 쌍을 이루는 지역 및 주 서버와 동일한 영역에서 구성되고 HA가 아닌 모드에서 단일 유연한 서버로 배포됩니다. 유연한 서버에 대해서는 [영역 중복 고가용성](concepts-high-availability.md)을 참조하세요.

> [!IMPORTANT]
> 주 지역이 다운되면 주 지역에서 스토리지를 프로비전할 수 없으므로 해당 지역 쌍 지역에 지역 중복 서버를 만들 수 없습니다. 주 지역이 지역 쌍을 이루는 지역에서 지역 중복 서버를 프로비전할 때까지 기다려야 합니다. 주 지역이 다운된 경우 복원 포털 환경에서 Compute + Storage 구성 서버 설정에서 지역 중복 옵션을 사용하지 않도록 설정하여 원본 서버를 지리적으로 쌍을 이루는 지역으로 복원하고 로컬 중복 서버로 복원하여 비즈니스 연속성을 보장할 수 있습니다.  
...
성공적으로 완료된 백업의 가용성을 확인하는 가장 좋은 방법은 백업 및 복원 블레이드에서 보존 기간 내에 수행된 전체 자동화된 백업을 보는 것입니다. 백업이 실패하면 사용 가능한 백업 목록에 나열되지 않으며 백업 서비스는 성공적인 백업이 수행될 때까지 20분마다 백업을 수행하려고 시도합니다. 이러한 백업 오류는 서버의 트랜잭션 프로덕션 부하로 인해 발생합니다.
