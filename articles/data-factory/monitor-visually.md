...
:::image type="content" source="media/monitor-visually/pipeline-runs.png" alt-text="파이프라인 실행 모니터링용 목록 보기":::
...
:::image type="content" source="media/monitor-visually/refresh.png" alt-text="새로 고침 단추":::
...
:::image type="content" source="media/monitor-visually/view-activity-runs.png" alt-text="활동 실행 보기":::

목록 보기에는 각 파이프라인 실행에 해당하는 작업 실행이 표시됩니다. 특정 활동 실행을 마우스로 가리키면 JSON 입력, JSON 출력 및 자세한 활동 관련 모니터링 환경과 같은 실행 관련 정보가 표시됩니다.

:::image type="content" source="media/monitor-visually/activity-runs.png" alt-text="SalesAnalyticsMLPipeline에 대한 정보와 작업 실행 목록이 차례로 표시됩니다.":::
...
:::image type="content" source="media/monitor-visually/activity-run-error.png" alt-text="오류 코드, 오류 유형 및 오류 정보를 포함한 오류 세부 정보를 표시하는 알림이 나타납니다.":::
...
:::image type="content" source="media/monitor-visually/promote-user-properties.png" alt-text="사용자 속성 만들기":::

사용자 속성을 만든 후에 모니터링 목록 보기에서 모니터링할 수 있습니다.

:::image type="content" source="media/monitor-visually/choose-user-properties.png" alt-text="활동 실행 목록에 사용자 속성에 대한 열 추가":::

 복사 작업의 원본이 테이블 이름인 경우 원본 테이블 이름을 활동 실행 목록 보기의 열로 모니터링할 수 있습니다.

:::image type="content" source="media/monitor-visually/view-user-properties.png" alt-text="사용자 속성의 열을 포함한 활동 실행 목록":::
...
:::image type="content" source="media/monitor-visually/rerun-pipeline.png" alt-text="파이프라인 다시 실행":::

특정 지점에서 시작을 다시 실행하려면 활동 실행 보기에서 다시 실행하면 됩니다. 시작하려는 활동을 선택하고 **활동에서 다시 실행** 을 선택합니다. 

:::image type="content" source="media/monitor-visually/rerun-activity.png" alt-text="활동 실행 다시 실행":::
...
:::image type="content" source="media/monitor-visually/rerun-failed-activity.png" alt-text="실패한 작업 다시 실행":::

### <a name="view-rerun-history"></a>다시 실행 기록 보기

목록 보기에서 모든 파이프라인 실행의 다시 실행 기록을 볼 수 있습니다.

:::image type="content" source="media/monitor-visually/rerun-history-1.png" alt-text="기록 보기":::

특정 파이프라인 실행의 다시 실행 기록을 볼 수도 있습니다.

:::image type="content" source="media/monitor-visually/view-rerun-history.png" alt-text="파이프라인 실행의 기록 보기":::

## <a name="monitor-consumption"></a>사용량 모니터링

실행 옆에 있는 사용량 아이콘을 클릭하여 파이프라인 실행에서 사용된 리소스를 볼 수 있습니다. 

:::image type="content" source="media/monitor-visually/monitor-consumption-1.png" alt-text="파이프라인에서 사용하는 리소스를 볼 수 있는 위치를 보여 주는 스크린샷":::

아이콘을 클릭하면 해당 파이프라인 실행에 사용되는 리소스의 사용량 보고서가 열립니다. 

:::image type="content" source="media/monitor-visually/monitor-consumption-2.png" alt-text="사용량 모니터링":::
...
:::image type="content" source="media/monitor-visually/select-gantt.png" alt-text="Gantt 차트의 예":::

막대의 길이는 파이프라인의 기간을 알려줍니다. 막대를 선택하여 자세한 내용을 볼 수도 있습니다.

:::image type="content" source="media/monitor-visually/view-gantt-run.png" alt-text="Gantt 차트 기간":::

## <a name="alerts"></a>경고

Data Factory의 지원되는 메트릭에 대해 경고를 발생시킬 수 있습니다. 시작하려면 Data Factory 모니터링 페이지에서 **모니터링** > **경고 및 메트릭** 을 선택합니다.

:::image type="content" source="media/monitor-visually/start-page.png" alt-text="데이터 팩터리 모니터링 페이지":::
...
1.  **새로운 경고 규칙** 을 선택하여 새 경고를 만듭니다.
    ...
    :::image type="content" source="media/monitor-visually/new-alerts.png" alt-text="새 경고 규칙 단추":::

1.  규칙 이름을 지정하고 경고 심각도를 선택합니다.

    :::image type="content" source="media/monitor-visually/name-and-severity.png" alt-text="규칙 이름 및 심각도 상자":::

1.  경고 조건을 선택합니다.

    :::image type="content" source="media/monitor-visually/add-criteria-1.png" alt-text="대상 조건을 위한 상자":::

    :::image type="content" source="media/monitor-visually/add-criteria-2.png" alt-text="경고 조건을 설정하기 위해 하나의 메트릭을 선택하는 위치를 보여 주는 스크린샷입니다.":::

    :::image type="content" source="media/monitor-visually/add-criteria-3.png" alt-text="조건 목록":::
...
1.  경고 논리를 구성합니다. 모든 파이프라인 및 해당 활동의 선택한 메트릭에 대해 경고를 만들 수 있습니다. 특정 활동 유형, 활동 이름, 파이프라인 이름 또는 오류 유형을 선택할 수도 있습니다.
    ...
    :::image type="content" source="media/monitor-visually/alert-logic.png" alt-text="경고 논리를 구성하기 위한 옵션":::

1.  경고에 대해 이메일, SMS, 푸시, 음성 알림을 구성합니다. 경고 알림에 대해 작업 그룹을 만들거나 기존 작업 그룹을 선택합니다.

    :::image type="content" source="media/monitor-visually/configure-notification-1.png" alt-text="경고 알림 구성 옵션":::

    :::image type="content" source="media/monitor-visually/configure-notification-2.png" alt-text="알림 추가 옵션":::

1.  경고 규칙을 만듭니다.

    :::image type="content" source="media/monitor-visually/create-alert-rule.png" alt-text="경고 규칙을 만드는 옵션":::
