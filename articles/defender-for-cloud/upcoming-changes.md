...
|경고 이름| Description|
|----------------------|---------------------------|
|**미리 보기 - 위험한 IP 주소에서 활동 발생**<br>(ARM.MCAS_ActivityFromAnonymousIPAddresses)|익명 프록시 IP 주소로 식별된 IP 주소에서 사용자 활동이 감지되었습니다.<br>이 프록시는 해당 장치의 IP 주소를 숨기려는 사용자가 사용하며 악의적인 의도로 사용될 수 있습니다. 이 감지는 조직의 다른 사용자가 널리 사용하는 태그가 잘못 지정된 IP 주소와 같은 가양성을 줄이는 기계 학습 알고리즘을 이용합니다.<br>활성 Microsoft Defender for Cloud Apps 라이선스가 필요합니다.|
|||
...
### <a name="container-security-features-to-be-grouped-under-defender-for-containers"></a>Defender for Containers에서 그룹화할 컨테이너 보안 기능

**변경 예상 날짜:** 2021년 12월

Microsoft Defender for Cloud의 컨테이너 보안 기능은 현재 두 개의 Microsoft Defender 요금제인 [Microsoft Defender for Kubernetes](defender-for-kubernetes-introduction.md) 및 [컨테이너 레지스트리용 Microsoft Defender](defender-for-container-registries-introduction.md)를 통해 제공됩니다.

이 변경으로

- 이러한 두 계획은 더 이상 사용되지 않습니다.
- 현재 계획을 사용하도록 설정하는 두 가지 기존 권장 사항(**Azure Defender for Kubernetes를 사용하도록 설정** 및 **컨테이너 레지스트리용 Azure Defender를 사용하도록 설정**)은 더 이상 사용되지 않습니다.
- 새로운 통합 계획인 **Microsoft Defender for Containers** 에는 컨테이너 솔루션을 보호하는 데 도움이 되는 보다 간소화되고 풍부한 기능을 제공하는 모든 기능이 포함되어 있습니다. 

Defender for Kubernetes 또는 컨테이너 레지스트리용 Defender가 이미 활성화된 구독에는 변경 사항이 없습니다. 기존 구독을 Microsoft Defender for Containers로 업그레이드하는 옵션이 제공됩니다.

일반 공급용으로 Microsoft Defender for Containers를 릴리스할 때 새 구독은 더 이상 사용되지 않는 계획을 사용하는 옵션을 제공하지 않습니다.

[Microsoft Defender for Cloud의 컨테이너 보안](container-security.md)에 대해 자세히 알아보세요.
