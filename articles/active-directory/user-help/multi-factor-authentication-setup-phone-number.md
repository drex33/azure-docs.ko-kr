...
description: 2단계 인증 방법으로 모바일 디바이스를 설정하는 방법을 알아봅니다.
...
ms.locfileid: "122535731"
---
# <a name="set-up-a-mobile-device-as-your-two-step-verification-method"></a>2단계 인증 방법으로 모바일 디바이스 설정
...
1. **추가 보안 확인** 페이지의 **1단계: 사용자에게 연락하는 방법** 영역에서 **인증 전화** 를 선택하고 드롭다운 목록에서 국가 또는 지역을 선택한 다음, 모바일 디바이스 전화 번호를 입력합니다.

2. **방법** 영역에서 **문자 메시지로 코드 보내기** 를 선택하고 **다음** 을 선택합니다.

    ![“인증 전화” 및 “문자 메시지로 내게 코드 보내기”가 선택된 “추가 보안 확인” 페이지 스크린샷](media/multi-factor-authentication-verification-methods/multi-factor-authentication-text-message.png)

3. Microsoft에서 보낸 문자 메시지의 확인 코드를 **2단계: 휴대폰으로 문자 메시지를 전송했음** 영역에 입력하고 **확인** 을 선택합니다.
