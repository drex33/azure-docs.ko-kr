...
이벤트 처리기는 수신 클라이언트 이벤트를 처리합니다. 이벤트 처리기는 포털 또는 Azure CLI를 통해 서비스에 미리 등록되고 구성되므로, 클라이언트 이벤트가 트리거되면 서비스에서 이벤트 처리가 예상되는지 여부를 식별할 수 있습니다. 이제 모드를 사용 하 여 이벤트 `PUSH` 처리기를 호출 합니다 .이 이벤트 처리기는 서버 쪽으로 이벤트를 트리거할 때 서비스에 대 한 공용 액세스 가능 끝점을 노출 합니다. **웹후크** 역할을 합니다. 
...
서비스는 서버에서 연결 관리를 수행 하기 위한 REST Api를 제공 합니다.
