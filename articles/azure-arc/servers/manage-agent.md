...
description: 이 문서에서는 Azure 연결 된 컴퓨터 에이전트의 수명 주기 동안 일반적으로 수행 하는 다양 한 관리 작업에 대해 설명 합니다.
ms.date: 10/28/2021
ms.topic: conceptual
ms.openlocfilehash: 7a5bb0ce72d5a8d8d3ee62106b3d0a2ec7d04ce0
ms.sourcegitcommit: b00a2d931b0d6f1d4ea5d4127f74fc831fb0bca9
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/20/2021
ms.locfileid: "132868023"
---
# <a name="managing-and-maintaining-the-connected-machine-agent"></a>Connected Machine 에이전트 관리 및 유지 관리

Windows 또는 Linux 용 Azure 연결 된 컴퓨터 에이전트의 초기 배포 후 에이전트를 다시 구성 하거나, 업그레이드 하거나, 컴퓨터에서 제거 해야 할 수 있습니다. 이러한 일상적인 유지 관리 작업을 수동으로 또는 자동화를 통해 간단히 관리하여 조작 오류와 비용을 모두 줄일 수 있습니다.
...
2. [Azure CLI](manage-vm-extensions-cli.md#remove-extensions) 또는 [Azure PowerShell](manage-vm-extensions-powershell.md#remove-extensions)을 사용하여 [Azure Portal](manage-vm-extensions-portal.md#remove-extensions)에서 설치된 VM 확장을 제거합니다.
...
Azcmagent 도구 (Azcmagent.exe)는 설치 중에 Azure 연결 된 컴퓨터 에이전트를 구성 하거나 설치 후 에이전트의 초기 구성을 수정 하는 데 사용 됩니다. Azcmagent.exe는 에이전트를 사용자 지정하고 해당 상태를 볼 수 있는 명령줄 매개 변수를 제공합니다.

* **연결** -Azure Arc에 컴퓨터를 연결 하려면

* **연결 끊기** -Azure Arc에서 컴퓨터 연결을 끊으려면

* **표시** -에이전트와 관련 된 문제를 해결 하는 데 도움이 될 수 있는 에이전트 상태 및 해당 구성 속성 (리소스 그룹 이름, 구독 ID, 버전 등)을 표시 합니다. `-j` 매개 변수를 포함하여 JSON 형식으로 결과를 출력합니다.

* **구성** -기능 및 컨트롤 에이전트 동작을 사용 하도록 설정 보기 및 변경

* **로그** -문제를 해결 하는 동안 지원 하기 위해 로그를 포함 하는 현재 디렉터리에 .zip 파일을 만듭니다.

* **버전** -연결 된 컴퓨터 에이전트 버전을 표시 합니다.

* **-useStderr**-오류 및 자세한 출력을 stderr로 보냅니다. `-json` 매개 변수를 포함하여 JSON 형식으로 결과를 출력합니다.
...
대화형으로 로그온하는 동안 수동으로 **Connect** 및 **Disconnect** 를 수행할 수 있으며, 또는 여러 에이전트를 온보딩할 때 사용한 동일한 서비스 주체를 사용하거나 Microsoft ID 플랫폼 [액세스 토큰](../../active-directory/develop/access-tokens.md)을 사용하여 자동화할 수 있습니다. 서비스 주체를 사용하여 Azure Arc 지원 서버에 머신을 등록하지 않은 경우 서비스 주체를 만들려면 다음 [문서](onboard-service-principal.md#create-a-service-principal-for-onboarding-at-scale)를 참조하세요.

>[!NOTE]
>**azcmagent** 을 실행 하려면 Linux 컴퓨터에 대 한 Windows 또는 *루트* 액세스 권한에 대 한 *관리자* 권한이 있어야 합니다.
...
이 매개 변수를 사용 하 여 에이전트 동작을 제어 하는 설정을 보고 구성할 수 있습니다.

모든 구성 속성 및 해당 값의 목록을 보려면 다음 명령을 실행 합니다.

`azcmagent config list`

특정 구성 속성에 대 한 값을 가져오려면 다음 명령을 실행 합니다.

`azcmagent config get <propertyName>`

구성 속성을 변경 하려면 다음 명령을 실행 합니다.

`azcmagent config set <propertyName> <propertyValue>`

구성 속성의 값을 지우려면 다음 명령을 실행 합니다.
...
1. [Azure Portal](manage-vm-extensions-portal.md#remove-extensions)에서 설치된 VM 확장을 제거하거나 [Azure CLI](manage-vm-extensions-cli.md#remove-extensions)를 사용하거나 머신에 유지하지 않으려는 [Azure PowerShell](manage-vm-extensions-powershell.md#remove-extensions)을 사용합니다.
...
에이전트 버전 1.13을 사용 하 여 프록시 설정은 `azcmagent config` 명령 또는 시스템 환경 변수를 사용 하 여 구성할 수 있습니다. 에이전트 구성과 시스템 환경 변수 모두에서 프록시 서버를 지정 하는 경우 에이전트 구성이 우선적으로 적용 되 고 유효 설정이 됩니다. `azcmagent show` 에이전트의 유효한 프록시 구성을 반환 합니다.

> [!NOTE]
> Azure Arc 사용 서버는 인증, TLS (HTTPS) 연결 또는 [Log Analytics 게이트웨이가](../../azure-monitor/agents/gateway.md) 연결 된 컴퓨터 에이전트에 대 한 프록시로 필요한 프록시 서버를 사용 하도록 지원 하지 않습니다.

### <a name="universal-proxy-configuration"></a>유니버설 프록시 구성

유니버설 프록시 구성은 Azure 연결 된 컴퓨터 에이전트 버전 1.13부터 사용할 수 있으며 프록시 서버 설정을 구성 하는 기본 방법입니다.

프록시 서버를 통해 통신 하도록 에이전트를 구성 하려면 다음 명령을 실행 합니다.

```bash
azcmagent config set proxy.url "http://ProxyServerFQDN:port"
```

네트워크에 필요한 경우 FQDN 대신 IP 주소 또는 간단한 호스트 이름을 사용할 수 있습니다. 프록시 서버가 포트 80에서 실행 되는 경우 끝에 ": 80"를 생략할 수 있습니다.

에이전트 설정에서 프록시 서버 URL이 구성 되어 있는지 확인 하려면 다음 명령을 실행 합니다.

```bash
azcmagent config get proxy.url
```

에이전트가 프록시 서버를 통해 통신 하는 것을 중지 하려면 다음 명령을 실행 합니다.

```bash
azcmagent config clear proxy.url
```

명령을 사용 하 여 프록시 설정을 다시 구성할 때 서비스를 다시 시작할 필요는 없습니다 `azcmagent config` .

### <a name="windows-environment-variables"></a>Windows 환경 변수

Windows에서 Azure 연결 된 컴퓨터 에이전트는 먼저 에이전트 `proxy.url` 구성 속성 (에이전트 버전 1.13부터)을 확인 한 다음 시스템 차원의 환경 변수를 확인 `HTTPS_PROXY` 하 여 사용할 프록시 서버를 결정 합니다. 둘 다 비어 있는 경우 기본값 Windows 시스템 수준 프록시 설정이 구성 된 경우에도 프록시 서버가 사용 되지 않습니다.

시스템 환경 변수 대신 에이전트 구성 속성을 사용 하는 것이 좋습니다.
...
프록시 서버를 통한 통신을 중지 하도록 에이전트를 구성 하려면 다음 명령을 실행 합니다.
...
### <a name="linux-environment-variables"></a>Linux 환경 변수

Linux에서 Azure 연결 된 컴퓨터 에이전트는 먼저 에이전트 `proxy.url` 구성 속성 (에이전트 버전 1.13부터)을 확인 한 다음 `HTTPS_PROXY` himds, GC_Ext 및 GCArcService 디먼에 대해 설정 된 환경 변수를 확인 합니다. 지정 된 프록시 서버를 사용 하도록 컴퓨터의 Azure 연결 된 컴퓨터 에이전트 및 기타 모든 서비스에 대 한 systemd의 기본 프록시 설정을 구성 하는 스크립트가 포함 되어 있습니다.
...
환경 변수를 제거 하려면 다음 명령을 실행 합니다.

```bash
sudo /opt/azcmagent/bin/azcmagent_proxy remove
```

### <a name="migrating-from-environment-variables-to-universal-proxy-configuration"></a>환경 변수에서 유니버설 프록시 구성으로 마이그레이션

이미 환경 변수를 사용 하 여 Azure 연결 된 컴퓨터 에이전트에 대 한 프록시 서버를 구성 하 고 로컬 에이전트 설정에 따라 유니버설 프록시 구성으로 마이그레이션하려면 다음 단계를 수행 합니다.

1. [Azure 연결 된 컴퓨터 에이전트](#upgrading-agent) 를 최신 버전 (버전 1.13부터)으로 업그레이드 하 여 새 프록시 구성 설정을 사용 합니다.
1. 을 실행 하 여 프록시 서버 정보를 사용 하 여 에이전트 구성 `azcmagent config set proxy.url "http://ProxyServerFQDN:port"`
1. [Windows](#windows-environment-variables) 또는 [Linux에](#linux-environment-variables) 대한 단계를 수행하여 사용하지 않는 환경 변수를 제거합니다.
