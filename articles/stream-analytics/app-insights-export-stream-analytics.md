...
[Azure Stream Analytics](https://azure.microsoft.com/services/stream-analytics/)는 [Application Insights에서 내보낸](../azure-monitor/app/export-telemetry.md) 데이터를 처리하는 위한 이상적인 도구입니다. Stream Analytics는 다양한 원본의 데이터를 가져와서 변환하고 필터링한 다음 다양한 싱크로 라우팅할 수 있습니다.
...
![Stream Analytics를 통해 PowerBI로 내보내기에 대 한 블록 다이어그램](./media/app-insights-export-stream-analytics/020.png)
...
1. [Azure 포털](https://portal.azure.com)에서 구독에 &quot;클래식&quot; 스토리지 계정을 만듭니다.
   ...
   ![Azure Portal의 스크린샷, 새로 만들기, 데이터, Storage를 선택 합니다.](./media/app-insights-export-stream-analytics/030.png)
2. 컨테이너 만들기
   
    ![새 저장소의 스크린샷에서 컨테이너를 선택 하 고 컨테이너 타일을 클릭 한 다음 추가를 클릭 합니다.](./media/app-insights-export-stream-analytics/040.png)
3. 스토리지 액세스 키 복사
   
    스트림 분석 서비스에 대한 입력을 설정하려면 곧 이 키가 필요합니다.
   
    ![저장소의 스크린샷, 설정 및 키를 열고 기본 액세스 키의 복사본을 가져옵니다.](./media/app-insights-export-stream-analytics/045.png)

## <a name="start-continuous-export-to-azure-storage"></a>Azure Storage로 연속 내보내기 시작

[연속 내보내기](../azure-monitor/app/export-telemetry.md)는 Application Insights에서 Azure Storage로 데이터를 이동합니다. 

1. Azure 포털에서 애플리케이션에 대해 만든 Application Insights 리소스를 찾습니다.
   
    ![Azure Portal의 모든 리소스에 대 한 스크린샷 Application Insights 다음 응용 프로그램을 선택 합니다.](./media/app-insights-export-stream-analytics/050.png)
2. 연속 내보내기를 만듭니다.
   
    ![Application Insights의 스크린샷 및 설정, 연속 내보내기, 추가를 차례로 선택 합니다.](./media/app-insights-export-stream-analytics/060.png)

    이전에 만든 스토리지 계정을 선택합니다.

    ![연속 내보내기 스크린샷, 추가를 선택 하 고 내보내기 대상을 설정 합니다.](./media/app-insights-export-stream-analytics/070.png)

    보려는 이벤트 유형을 설정합니다.

    ![연속 내보내기 추가 및 이벤트 유형 선택의 스크린샷](./media/app-insights-export-stream-analytics/080.png)
...
2. 내보낸 데이터를 검사합니다. Visual Studio에서 **보기/클라우드 탐색기** 를 선택하고 Azure/스토리지를 엽니다. (이 메뉴 옵션이 없는 경우 Azure SDK를 설치해야 합니다. 새 프로젝트 대화 상자를 열고 시각적 개체 C# / 클라우드 / .NET용 Microsoft Azure SDK 가져오기를 엽니다.)
    ...
    ![보려는 이벤트 유형을 설정하는 방법을 보여 주는 스크린샷](./media/app-insights-export-stream-analytics/04-data.png)
...
![Azure Portal에서 Stream Analytics 작업을 만들기 위한 기본 페이지를 보여 주는 스크린샷](./media/app-insights-export-stream-analytics/001.png)

![새로운 Stream Analytics 작업을 만들 때 필요한 세부 정보를 보여 주는 스크린샷](./media/app-insights-export-stream-analytics/002.png)

새 작업이 만들어질 때 **리소스로 이동** 을 선택합니다.

![새로운 Stream Analytics 작업 배포에 성공하는 경우 수신된 메시지를 보여 주는 스크린샷](./media/app-insights-export-stream-analytics/003.png)

### <a name="add-a-new-input"></a>새 입력 추가

![Stream Analytics 작업에 입력을 추가하는 방법을 보여 주는 스크린샷](./media/app-insights-export-stream-analytics/004.png)

연속 내보내기 Blob에서 입력을 가져오도록 설정합니다.

![연속 내보내기 Blob에서 입력을 가져오도록 Stream Analytics 작업을 구성하는 방법을 보여 주는 스크린샷](./media/app-insights-export-stream-analytics/0005.png)
...
![Stream Analytics 작업을 선택하여 새 출력을 추가하는 것을 보여 주는 스크린샷.](./media/app-insights-export-stream-analytics/006.png)


![새 출력의 스크린샷, 새 채널, 출력, 추가, Power BI을 차례로 선택 합니다.](./media/app-insights-export-stream-analytics/010.png)
...
![Stream Analyics의 스크린샷에서 작업을 선택 하 고 쿼리를 클릭 합니다. 아래 샘플을 붙여넣습니다.](./media/app-insights-export-stream-analytics/008.png)
...
![데이터 세트 및 필드를 선택하는 Power BI 스크린샷.](./media/app-insights-export-stream-analytics/200.png)

이제 [Power BI](https://powerbi.microsoft.com)의 보고서 및 대시보드에서 이 데이터 세트를 사용할 수 있습니다.

![Power BI의 데이터 세트에서 만든 보고서 예시를 보여 주는 스크린샷](./media/app-insights-export-stream-analytics/210.png)
...
* [연속 내보내기](../azure-monitor/app/export-telemetry.md)
