---
title: CLI 스크립트 - Azure Database for MySQL - 유연한 서버(미리 보기)에서 읽기 복제본 만들기 및 관리
description: 이 Azure CLI 샘플 스크립트에서는 Azure Database for MySQL - 유연한 서버에서 읽기 복제본을 만들고 관리하는 방법을 보여줍니다.
author: shreyaaithal
ms.author: shaithal
ms.service: mysql
ms.devlang: azurecli
...
ms.locfileid: "128701951"
---
# <a name="create-and-manage-read-replicas-in-an-azure-database-for-mysql---flexible-server-preview-using-azure-cli"></a>Azure CLI를 사용하여 Azure Database for MySQL - 유연한 서버(미리 보기)에서 읽기 복제본 만들기 및 관리

이 샘플 CLI 스크립트는 Azure Database for MySQL - 유연한 서버에서 [읽기 복제본](../concepts-read-replicas.md)을 만들고 관리합니다.
...
|[az mysql flexible-server replica create](/cli/azure/mysql/flexible-server/replica#az_mysql_flexible_server_replica_create)|서버용 읽기 복제본을 만듭니다.|
|[az mysql flexible-server replica list](/cli/azure/mysql/flexible-server/replica#az_mysql_flexible_server_replica_list)|지정된 서버의 모든 읽기 복제본을 나열합니다.|
|[az mysql flexible-server replica stop-replication](/cli/azure/mysql/flexible-server/replica#az_mysql_flexible_server_replica_stop_replication)|읽기 복제본에 대한 복제를 중지하고 읽기/쓰기 서버로 만듭니다.|
