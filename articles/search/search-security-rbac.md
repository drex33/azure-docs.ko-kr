...
+ [사용자 지정 역할](../role-based-access-control/custom-roles.md) 은 지원 되지 않습니다.
...
| [Search 서비스 기여자](../role-based-access-control/built-in-roles.md#search-service-contributor) | 서비스 ops (일반적으로 사용 가능) 및 최상위 개체 및 콘텐츠 (미리 보기) | 이 역할은 서비스 수준에서 참가자의 조합 이지만, 인덱스, 동의어 맵, 인덱서, 데이터 원본 및 기술력과의 모든 동작에 대 한 모든 액세스 권한을 [`Microsoft.Search/searchServices/*`](/azure/role-based-access-control/resource-provider-operations#microsoftsearch) 콘텐츠 수준에서 사용 합니다. 이 역할은 서비스와 해당 콘텐츠를 완전히 관리 해야 하는 검색 서비스 관리자를 위한 것입니다. 콘텐츠 관리를 위해 미리 보기에 등록 해야 합니다. </br></br>기여자와 마찬가지로이 역할의 멤버는 역할 할당을 만들거나 관리 하거나 권한 부여 옵션을 변경할 수 없습니다. |
