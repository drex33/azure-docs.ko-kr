...
종종 서로 다른 하위 파서에서 파서 빌드를 원하며, 각각 다른 구문 분석 논리가 필요한 이벤트의 다른 변형을 처리합니다. 각각 자체적으로 쿼리인 이 하위 파서는 `union` 연산자를 사용하여 통합됩니다. 이 접근 방식은 편리하지만 파서 성능에 크게 영향을 미치므로 권장되지 ‘않습니다’.
...
|다른 변형은 *동일한* 이벤트 형식을 나타내지만 다르게 구조화됩니다.     |   구문 분석 전에 이벤트를 구별하는 메서드가 있는 경우와 같이 변형이 알려진 경우 [Infoblox DNS 파서](https://aka.ms/AzSentinelInfobloxParser)에서 설명된 대로 `case` 연산자를 사용하여 실행할 올바른 `extract_all`과 필드 매핑을 선택합니다.      |
...
이렇게 하려면 만든 원본별 파서가 포함하도록 관련 소스 독립적 파서 를 수정합니다. 예를 들어 `imDns` 파서를 문에 있는 파서 목록에 추가하여 파서를 포함하도록 소스에 구애받지 않은 파서를 `union` 변경합니다. 파서가 매개 변수를 지원하는 경우 아래 vimDnsYyyXxx 파서와 같은 매개 변수 시그니처와 함께 포함합니다. 그렇지 않은 경우 아래 vimDnsWwwZzz 파러와 같이 서명 없이 포함합니다.
...
이 문서에서는 ASIM(고급 SIEM 정보 모델) 파서에 대해 설명합니다.
