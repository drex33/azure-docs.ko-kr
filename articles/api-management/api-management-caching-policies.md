...
> [!NOTE]
> `vary-by-query-parameter`를 사용할 때 rewrite-uri 템플릿에서 매개 변수를 선언하거나 `copy-unmatched-params` 특성을 `false`로 설정할 수 있습니다. 이 플래그를 사용하지 않도록 설정하면 선언되지 않은 매개 변수가 백 엔드로 전송됩니다.
