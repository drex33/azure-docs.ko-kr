...
> - **수동** - HTML 페이지(**표준** 인증서만) 또는 DNS TXT 레코드를 사용하여 도메인을 확인합니다. 옵션을 선택하면 지침이 제공됩니다. HTML 페이지 옵션은 웹앱에서 "Https 전용"을 사용하도록 설정된 경우에는 작동하지 않습니다.
...
> [!NOTE]
> 2021년 9월 23일부터 App Service 인증서는 395일마다 도메인 유효성 검사를 수행해야 합니다. 이는 인증 기관이 준수해야 하는 CA/브라우저 포럼에 의해 적용되는 새로운 지침 때문입니다. 
> 
> App Service Managed Certificate와 달리 App Service Certificate에 대한 도메인 다시 유효성 검사는 자동화되지 않습니다. App Service 인증서를 확인하는 방법에 대한 자세한 내용은 [도메인 소유권 확인](#verify-domain-ownership)을 참조하세요.
