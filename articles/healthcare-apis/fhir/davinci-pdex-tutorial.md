...
description: 이 자습서에서는 Da Vinci Payer 데이터 Exchange 구현 가이드에 대한 테스트를 통과하도록 FHIR 서비스를 설정하는 방법을 안내합니다.
...
이 자습서에서는 Azure Healthcare API(여기서는 FHIR 서비스라고 함)에서 FHIR 서비스를 설정하여 [PDex IG(Da Vinci Payer Data Exchange Implementation Guide)에](http://hl7.org/fhir/us/davinci-pdex/toc.html) 대한 [Touchstone](https://touchstone.aegis.net/touchstone/) 테스트를 통과하는 방법을 안내합니다.

> [!NOTE]
> 이러한 모든 테스트에 대해 JSON 테스트에 대해 실행합니다. FHIR 서비스는 JSON과 XML을 모두 지원하지만 JSON 또는 XML에 액세스하기 위한 별도의 엔드포인트가 없습니다. 이로 인해 모든 XML 테스트가 실패합니다. XML에서 capability 문을 보려면 \_ 형식 매개 변수 GET \` {fhirurl}/metadata를 전달하기만 하면 됩니다. \_ format=xml\`

## <a name="touchstone-capability-statement"></a>Touchstone 기능 문

첫 번째 테스트 세트는 PDex IG 기능 문에 대해 FHIR 서비스를 테스트하는 것입니다. 여기에는 세 가지 테스트가 포함됩니다.
...
* 세 번째 테스트는 [모든 $patient 작업이](patient-everything.md) 지원되는지 확인합니다.
