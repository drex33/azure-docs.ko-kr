---
title: 'Python 스크립트 실행: 구성 요소 참조'
titleSuffix: Azure Machine Learning
description: Azure Machine Learning designer에서 python 스크립트 실행 구성 요소를 사용 하 여 python 코드를 실행 하는 방법을 알아봅니다.
...
ms.locfileid: "131570041"
---
# <a name="execute-python-script-component"></a>Python 스크립트 실행 구성 요소

이 문서에서는 Azure Machine Learning designer에서 Python 스크립트 실행 구성 요소에 대해 설명 합니다.

이 구성 요소를 사용 하 여 Python 코드를 실행 합니다. Python의 아키텍처 및 디자인 원칙에 대한 자세한 내용은 [Azure Machine Learning 디자이너에서 Python 코드를 실행하는 방법](../how-to-designer-python.md)을 참조하세요.

Python을 사용 하면 다음과 같이 기존 구성 요소가 지원 하지 않는 작업을 수행할 수 있습니다.

+ `matplotlib`를 사용하여 데이터 시각화
+ Python 라이브러리를 사용하여 작업 영역에서 데이터 세트 및 모델 열거
+ [데이터 가져오기](./import-data.md) 구성 요소가 지원 하지 않는 원본의 데이터를 읽고 로드 하 고 조작 합니다.
+ 자체 딥 러닝 코드 실행 
...
> [!NOTE]
> 파이프라인이 사전 설치 된 목록에 없는 패키지를 필요로 하는 여러 개의 Python 스크립트 실행 구성 요소를 포함 하는 경우 각 구성 요소에 패키지를 설치 합니다.

> [!WARNING]
> Excute Python 스크립트 구성 요소는 Java, PyODBC 등의 명령을 사용 하 여 추가 네이티브 라이브러리에 종속 된 패키지를 설치 하는 것을 지원 하지 않습니다. 이 구성 요소는 Python이 사전 설치 되어 있고 비관리자 권한이 있는 간단한 환경에서 실행 되기 때문입니다.  
...
python 스크립트 실행 구성 요소는 [Azure Machine Learning python SDK](/python/api/azureml-core/azureml.core.run%28class%29#upload-file-name--path-or-stream-)를 사용 하 여 파일 업로드를 지원 합니다.

다음 예제에서는 Python 스크립트 실행 구성 요소에서 이미지 파일을 업로드 하는 방법을 보여 줍니다.
...
파이프라인 실행이 완료 되 면 구성 요소의 오른쪽 패널에서 이미지를 미리 볼 수 있습니다.
...
Python 스크립트 실행 구성 요소에는 시작 지점으로 사용할 수 있는 샘플 Python 코드가 포함 되어 있습니다. Python 스크립트 실행 구성 요소를 구성 하려면 **python 스크립트** 텍스트 상자에서 실행할 입력 및 python 코드 집합을 제공 합니다.

1. 파이프라인에 **Python 스크립트 실행** 구성 요소를 추가 합니다.

2. **Dataset1** 에서 입력에 사용할 디자이너의 모든 데이터 세트를 추가하고 연결합니다. Python 스크립트에서 이 데이터 세트를 **DataFrame1** 로 참조합니다.

    데이터 세트 사용은 선택 사항입니다. Python을 사용 하 여 데이터를 생성 하려는 경우 또는 Python 코드를 사용 하 여 데이터를 구성 요소로 직접 가져오는 경우이를 사용 합니다.

    이 구성 요소는 **Dataset2** 에 두 번째 데이터 집합을 추가할 수 있도록 지원 합니다. Python 스크립트에서 두 번째 데이터 세트를 **DataFrame2** 로 참조합니다.

    이 구성 요소를 사용 하 여 로드 하면 Azure Machine Learning에 저장 된 데이터 집합이 자동으로 pandas 데이터 프레임으로 변환 됩니다.
...
4. 새 Python 패키지 또는 코드를 포함하려면 이러한 사용자 지정 리소스가 포함된 압축 파일을 **스크립트 번들** 포트에 연결합니다. 또는 스크립트가 16kb보다 큰 경우에는 **스크립트 번들** 포트를 사용하여 *명령줄에서 16597자 제한 초과* 같은 오류를 방지합니다. 
    ...
    1. 디자이너 제작 페이지의 왼쪽 구성 요소 창에 *있는 데이터 집합 목록에서* 데이터 집합 구성 요소를 끌어 옵니다. 
    1. 데이터 집합 구성 요소를 **Python 스크립트 실행** 구성 요소의 **스크립트 번들** 포트로 커넥트 합니다.
...
5. **Python 스크립트** 텍스트 상자에 올바른 Python 스크립트를 입력하거나 붙여넣습니다.
    ...
    > [!NOTE]
    >  스크립트를 작성할 때는 주의해야 합니다. 선언 되지 않은 변수를 사용 하거나 가져오지 않은 구성 요소 또는 함수를 사용 하는 것과 같은 구문 오류가 없는지 확인 합니다. 사전 설치 된 구성 요소 목록에 주의를 기울여야 합니다. 나열 되지 않은 구성 요소를 가져오려면 다음과 같이 스크립트에 해당 패키지를 설치 합니다.
    >  ``` Python
    > import os
    > os.system(f"pip install scikit-misc")
    > ```
    ...
    **Python 스크립트** 텍스트 상자는 주석의 몇 가지 지침과 데이터 액세스 및 출력에 대한 샘플 코드를 사용하여 미리 채워져 있습니다. 이 코드를 편집하거나 바꾸어야 합니다. 들여쓰기 및 대/소문자 구분에 대한 Python 규칙을 따르세요.
    ...
    + 스크립트는 `azureml_main` 이 구성 요소에 대 한 진입점으로 이라는 함수를 포함 해야 합니다.
    ...
    > [!WARNING]
    > **Python 스크립트 실행 구성 요소** 에서 데이터베이스 또는 다른 외부 저장소에 연결 하지 **않는** 것이 좋습니다. [데이터 가져오기 구성 요소](./import-data.md) 및 [데이터 내보내기 구성 요소](./export-data.md) 를 사용할 수 있습니다.     

6. 파이프라인을 제출합니다.

    구성 요소가 완료 되 면 예상 대로 출력을 확인 합니다.

    구성 요소가 실패 한 경우 몇 가지 문제 해결을 수행 해야 합니다. 구성 요소를 선택 하 고 오른쪽 창에서 **출력 + 로그** 를 엽니다. **70_driver_log.txt** 를 열고 **in azureml_main** 을 검색하면 오류가 발생한 줄을 찾을 수 있습니다. 예를 들어 "File "/tmp/tmp01_ID/user_script.py", line 17, in azureml_main"은 Python 스크립트의 17번째 줄에서 오류가 발생했음을 나타냅니다.

## <a name="results"></a>결과

포함된 Python 코드의 모든 계산 결과는 `pandas.DataFrame`으로 제공되어야 하며, 이는 Azure Machine Learning 데이터 세트 형식으로 자동 변환됩니다. 그런 다음 파이프라인의 다른 구성 요소와 결과를 사용할 수 있습니다.

구성 요소는 두 개의 데이터 집합을 반환 합니다.  
