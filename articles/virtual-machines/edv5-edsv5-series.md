...
description: Edv5 및 Edsv5 시리즈 VM에 대한 사양입니다.
author: styli365
ms.author: sttsinar
...
ms.topic: conceptual
ms.date: 10/20/2021
ms.openlocfilehash: a8e5317418862fe44c6a37aff01b2790842c95a7
ms.sourcegitcommit: 702df701fff4ec6cc39134aa607d023c766adec3
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/03/2021
ms.locfileid: "131482967"
---
...
Edv5 및 Edsv5 시리즈는 하이퍼 스레드 구성의 3세대 Intel &reg; Xeon &reg; 8370C(Ice Lake) 프로세서에서 [실행되어](https://www.intel.com/content/www/us/en/architecture-and-technology/hyper-threading/hyper-threading-technology.html) 대부분의 범용 워크로드에 더 나은 가치 제안을 제공할 Virtual Machines 있습니다. 이 새로운 프로세서는 [Intel Turbo Boost &reg; Technology, Intel](https://www.intel.com/content/www/us/en/architecture-and-technology/turbo-boost/turbo-boost-technology.html)Advanced-Vector [ &reg; Extensions 512(Intel AVX-512) 및 Intel Deep Learning Boost가 있는 &reg; 3.5GHz의](https://www.intel.com/content/www/us/en/architecture-and-technology/avx-512-overview.html) 모든 코어 Turbo 클록 속도를 제공합니다. [ &reg; ](https://software.intel.com/content/www/us/en/develop/topics/ai/deep-learning-boost.html) 최대 672 GiB RAM을 갖춘 이러한 가상 머신은 메모리 집약적 엔터프라이즈 애플리케이션, 관계형 데이터베이스 서버 및 메모리 내 분석 워크로드에 적합합니다. 또한 이러한 VM은 빠르고 큰 로컬 SSD 스토리지(최대 3,900 GiB)를 제공합니다.

## <a name="edv5-series"></a>Edv5 시리즈

Edv5 시리즈 가상 머신은 3세대 Intel® Xeon® 8370C(Ice Lake) 프로세서에서 실행되어 최대 3.5GHz의 모든 코어 turbo 클록 속도에 도달합니다.  이러한 가상 머신은 최대 104개의 vCPU와 672 GiB RAM뿐만 아니라 최대 3800 GiB의 빠른 로컬 SSD 스토리지를 제공합니다. Edv5 시리즈 가상 머신은 짧은 대기 시간, 고속 로컬 스토리지를 활용하는 메모리 집약적 엔터프라이즈 애플리케이션 및 애플리케이션에 적합합니다.

Edv5 시리즈 가상 머신은 표준 SSD 및 표준 HDD 디스크 유형을 지원합니다. Premium SSD 또는 Ultra Disk 스토리지를 사용하려면 Edsv5 시리즈 가상 머신을 선택합니다. 디스크 스토리지는 가상 머신과 별도로 청구됩니다. [디스크에 대한 가격 책정을 참조하세요.](https://azure.microsoft.com/pricing/details/managed-disks/)
...
<sup>1</sup> 가속화된 네트워킹이 필요하며 모든 Edv5 가상 머신에서 기본적으로 켜져 있습니다.<br>
<sup>2</sup> 가속화된 네트워킹은 두 개의 NIC에 적용할 수 있습니다.<br>
<sup>3</sup> 인스턴스는 단일 고객 전용 하드웨어로 [격리됩니다.](../security/fundamentals/isolation-choices.md#compute-isolation)


## <a name="edsv5-series"></a>Edsv5 시리즈

Edsv5 시리즈 가상 머신은 3세대 Intel® Xeon® 8370C(Ice Lake) 프로세서에서 실행되어 최대 3.5GHz의 모든 코어 turbo 클록 속도에 도달합니다.  이러한 가상 머신은 최대 104개의 vCPU와 672 GiB RAM뿐만 아니라 최대 3800 GiB의 빠른 로컬 SSD 스토리지를 제공합니다. Edsv5 시리즈 가상 머신은 짧은 대기 시간, 고속 로컬 스토리지를 활용하는 메모리 집약적 엔터프라이즈 애플리케이션 및 애플리케이션에 적합합니다.

Edsv5 시리즈 가상 머신은 표준 SSD 및 표준 HDD 디스크 유형을 지원합니다. 표준 SSD, 표준 HDD 및 Premium SSD 디스크 스토리지를 이러한 VM에 연결할 수 있습니다. 지역별 가용성에 따라 Ultra Disk 스토리지를 연결할 수도 있습니다. 디스크 스토리지는 가상 머신과 별도로 청구됩니다. [디스크에 대한 가격 책정을 참조하세요.](https://azure.microsoft.com/pricing/details/managed-disks/)
...
| 크기 | vCPU | 메모리: GiB | 임시 스토리지(SSD) GiB | 최대 데이터 디스크 수 | 캐시된 최대 및 임시 스토리지 처리량: IOPS/MBps<sup>*</sup> | 캐시되지 않은 최대 디스크 처리량: IOPS/MBps | 캐시되지 않은 최대 버스트 디스크 처리량: IOPS/MBps<sup>5</sup> | 최대 NIC 수 | 최대 네트워크 대역폭(Mbps) |
|---|---|---|---|---|---|---|---|---|---|
...
<sup>*</sup> [Gen2 VM](generation-2.md)을 사용하여 이러한 IOP 값을 보장할 수 있습니다.<br>
<sup>1</sup> 가속화된 네트워킹이 필요하며 모든 Edsv5 가상 머신에서 기본적으로 켜져 있습니다.<br>
<sup>2</sup> 가속화된 네트워킹은 두 개의 NIC에 적용할 수 있습니다.<br>
...
<sup>5</sup> Edsv5 시리즈 가상 머신은 디스크 성능을 [버스트하고](disk-bursting.md) 한 번에 최대 30분 동안 최대 버스트 최대값을 얻을 수 있습니다.
