...
7. 작업의 상태를 확인하고 작업의 모든 속성을 목록에 표시하려면 반환 변수를 `Format-List` cmdlet에 파이프합니다. 
   ...
   다음 출력은 계정에서 호환되지 않는 기능이 사용됨을 나타냅니다. 이 경우 기능을 사용하지 않도록 설정하고 유효성 검사 프로세스를 다시 시작합니다.
...
8. 계정의 유효성이 성공적으로 검사된 후 다음 명령을 실행하여 업그레이드를 시작합니다.
...
3. 다음 명령을 사용하여 스토리지 계정의 유효성을 검사합니다.
   ...
   유효성 검사에 성공하면 프로세스가 완료되고 오류가 표시되지 않습니다.
...
5. 계정의 유효성이 성공적으로 검사된 후 다음 명령을 실행하여 업그레이드를 시작합니다.
...
마이그레이션이 완료되기 전에 중지할 수 있습니다. 여기서는 중지할 때 발생하는 일에 대해 자세히 알아보겠습니다.
...
작업이 완료 되기 전에 업그레이드를 중지 하려면 명령을 사용 `Stop-AzStorageAccountHierarchicalNamespaceUpgrade` 합니다.
...
작업이 완료 되기 전에 업그레이드를 중지 하려면 명령을 사용 `az storage account hns-migration stop` 합니다.
