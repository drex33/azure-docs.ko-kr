...
4. **SAML로 Single Sign-On 설정** 페이지에서 다음 단계를 수행합니다.
    ...
    a. **식별자** 텍스트 상자에 `https://<domain>.tyeexpress.com` 패턴을 사용하여 URL로 값을 입력합니다.

    b. **회신 URL** 텍스트 상자에서 `https://<domain>.tyeexpress.com/authorize/samlConsume.aspx` 패턴을 사용하여 URL을 입력합니다.

    > [!NOTE]
    > 이러한 값은 실제 값이 아닙니다. 실제 식별자 및 회신 URL로 해당 값을 업데이트합니다. 식별자에는 고유한 문자열 값을 사용하는 것이 좋습니다. 이러한 값을 얻으려면 [T&E Express 클라이언트 지원 팀](https://www.tyeexpress.com/contacto.aspx)에 문의하세요. Azure Portal의 **기본 SAML 구성** 섹션에 표시된 패턴을 참조할 수도 있습니다.
...
1. **관리** 탭 아래에서 **SAML 도메인** 을 클릭하여 SAML 설정 페이지를 엽니다.
    ...
    ![스크린샷은 관리 메뉴에서 선택한 SAML 도메인을 보여줍니다.](./media/tyeexpress-tutorial/tye-SAML.png)

1. **Activar(활성화)** 옵션을 **아니요** 에서 **SI(예)** 로 선택합니다. **ID 공급 기업 메타데이터** 텍스트 상자에 Azure Portal에서 다운로드한 메타데이터 XML을 붙여넣습니다.

    ![스크린샷은 메타데이터를 입력할 수 있는 Dominio SAML 페이지를 보여줍니다.](./media/tyeexpress-tutorial/tyeAdmin.png)
...
1. 관리 탭 아래에서 사용자를 클릭하여 사용자 마스터 페이지를 엽니다.
    ...
    ![스크린샷은 관리 메뉴에서 선택한 사용자를 보여줍니다.](./media/tyeexpress-tutorial/tye-adminusers.png)

1. 홈 페이지에서 **+** 를 클릭하여 사용자를 추가합니다.

    ![스크린샷은 사용자를 추가하기 위한 더하기 아이콘을 보여줍니다.](./media/tyeexpress-tutorial/tye-usershome.png)

1. 양식에서 요청한 대로 모든 필수 세부 정보를 입력하고 저장 단추를 클릭하여 세부 정보를 저장합니다.

    ![스크린샷은 적절한 값을 입력할 수 있는 사용자 정보 섹션을 보여줍니다.](./media/tyeexpress-tutorial/tye-usersadd.png)

    ![스크린샷은 적절한 값을 입력할 수 있는 승인자 및 보조자 섹션을 보여줍니다.](./media/tyeexpress-tutorial/tye-userssave.png)

### <a name="test-single-sign-on"></a>Single Sign-On 테스트

이 섹션에서는 액세스 패널을 사용하여 Azure AD Single Sign-On 구성을 테스트합니다.

액세스 패널에서 T&E Express 타일을 클릭하면, SSO를 설정한 T&E Express에 자동으로 로그인되어야 합니다. 액세스 패널에 대한 자세한 내용은 [액세스 패널 소개](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510)를 참조하세요.
