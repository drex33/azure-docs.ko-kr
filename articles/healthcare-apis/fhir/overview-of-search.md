...
| _sort                         | Partial              | Partial                   | sort=_lastUpdated Azure API for FHIR 및 FHIR 서비스에서 지원됩니다. FHIR 서비스 및 OSS SQL DB FHIR 서버의 경우 문자열 및 dateTime 필드를 기준으로 정렬이 지원됩니다. 2021년 4월 20일 이후에 생성된 Azure API for FHIR 및 OSS Cosmos DB 데이터베이스의 경우 이름, 성, 생년월일 및 임상 날짜에 정렬이 지원됩니다.             |
| _contained                    | 아니요                   | 아니요                        |
| _containedType                | 아니요                   | 아니요                        |
| _score                        | 아니요                   | 아니요                        |

> [!NOTE]
> 기본적으로 `_sort` 레코드를 오름차순으로 정렬합니다. 접두사 를 사용하여 `'-'` 내림차순으로 정렬할 수 있습니다. 또한 FHIR 서비스 및 Azure API for FHIR 한 번에 하나의 필드만 정렬할 수 있습니다.
...
마찬가지로 역방향 체인 검색을 수행할 수 있습니다. 이를 통해 참조하는 다른 리소스에 대한 조건을 지정하는 리소스를 얻을 수 있습니다. 연결된 검색 및 역방향 연결 검색의 더 많은 예제는 FHIR 검색 예제 페이지를 [참조하세요.](search-samples.md) 
