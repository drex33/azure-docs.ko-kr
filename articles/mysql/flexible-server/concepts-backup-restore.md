...
ms.topic: conceptual
ms.date: 09/21/2020
ms.openlocfilehash: 997d007a69ef8360ebcdfc4add56254290cdca8d
ms.sourcegitcommit: 93c7420c00141af83ed3294923b4826dd4dc6ff2
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/02/2021
ms.locfileid: "133438330"
---
...
[Azure Portal](how-to-restore-server-portal.md)를 통해 최신 복원 지점, 사용자 지정 복원 지점 및 가장 빠른 복원 지점 (전체 백업을 사용 하 여 복원) 중에서 선택할 수 있습니다.
...
-   **가장 빠른 복원 지점**:이 옵션을 사용 하면 사용자가 유연한 서버에 대해 정의 된 보존 기간 내에 지정 된 날짜에 가능한 가장 빠른 시간 내에 서버를 복원할 수 있습니다. 전체 백업이 완료 된 복원 지정 시간을 선택 하 여 가장 빠른 복원을 수행할 수 있습니다. 이 복원 작업은 전체 스냅숏 백업을 복원 하기만 하 고 로그의 복원 또는 복구를 보장 하지 않으며 로그를 빠르게 만듭니다. 복원 작업을 성공적으로 수행 하려면 가장 이른 복원 시점 보다 큰 전체 백업 타임 스탬프를 선택 하는 것이 좋습니다.
...
> [!IMPORTANT]
> 서버 삭제 시점부터 5 일 이내에 삭제 된 MySQL의 유연한 서버 리소스를 복구할 수 있습니다. 삭제 된 서버를 복원 하는 방법에 대 한 자세한 지침은 [문서화 된 단계를 참조](../flexible-server/how-to-restore-dropped-server.md)하세요. 서버 리소스를 보호 하기 위해 실수로 삭제 되거나 예기치 않은 변경이 발생 하지 않도록 관리자가 [관리 잠금을](../../azure-resource-manager/management/lock-resources.md)활용할 수 있습니다.

## <a name="geo-restore"></a>지역 복원

지역 중복 백업을 위한 서버를 구성한 경우 해당 서비스를 사용할 수 있는 [지리적으로 쌍을 이루는 지역](../../availability-zones/cross-region-replication-azure.md) 으로 서버를 복원할 수 있습니다. 다른 지역으로의 지역 복원은 현재 지원 되지 않습니다. 
...
성공적으로 완료 된 백업에 대 한 가용성의 유효성을 검사 하는 가장 좋은 방법은 백업 및 복원 블레이드의 보존 기간 내에 수행 된 전체 자동화 된 백업을 확인 하는 것입니다. 백업이 실패 하면 사용 가능한 백업 목록에 나열 되지 않으며 백업 서비스가 성공적으로 백업 될 때까지 백업을 수행 하는 데 20 분 마다 시도 합니다. 이러한 백업 오류는 서버의 트랜잭션 프로덕션 부하가 많기 때문에 발생 합니다.
