...
> [!IMPORTANT]
> 사용자가 삭제 된 서버를 복원할 **수 없습니다** . 서버를 삭제하면 해당 서버에 속한 모든 데이터베이스도 삭제되고 복구할 수 없습니다. 배포 후에 실수로 인한 삭제 또는 예기치 않은 변경에서 서버 리소스를 보호하려면 관리자는 [관리 잠금](../../azure-resource-manager/management/lock-resources.md)을 활용할 수 있습니다. 서버를 실수로 삭제 한 경우 지원에 문의 하세요. 경우에 따라 데이터 손실 없이 서버를 복원할 수 있습니다.

## <a name="geo-redundant-backup-and-restore-preview"></a>지역 중복 백업 및 복원 (미리 보기)

서버를 만들 때 지역 중복 백업을 구성할 수 있습니다. Compute + Storage 블레이드에서 지역 중복 백업을 사용 하도록 설정 하는 방법에 대 한 [빠른 시작 가이드](./quickstart-create-server-portal.md) 를 참조 하세요. 

>[!IMPORTANT]
> 지역 중복 백업은 서버를 만들 때만 구성할 수 있습니다. 

지역 중복 백업을 사용 하 여 서버를 구성한 후에는 [지리적으로 쌍을 이루는 지역](../../availability-zones/cross-region-replication-azure.md)으로 복원할 수 있습니다. 지역 중복 백업 지원 [지역](overview.md#azure-regions)을 참조 하세요.
...
지역에서 복원 수행에 대 한 [방법 가이드](how-to-restore-server-portal.md#performing-geo-restore-preview) 를 참조 하세요.
