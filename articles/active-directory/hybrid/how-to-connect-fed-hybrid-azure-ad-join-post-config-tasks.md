...
Windows 10 및 Windows Server 2016을 실행하는 모든 도메인 조인 디바이스는 모든 구성 단계가 완료되면 자동으로 Azure AD에 등록됩니다. 이 자동 등록보다 제어된 롤아웃을 선호하는 경우 그룹 정책을 사용하여 자동 롤아웃을 선택적으로 사용하거나 사용하지 않을 수 있습니다.  이 그룹 정책은 다른 구성 단계를 시작하기 전에 설정해야 합니다.
...
* [도메인에 가입된 컴퓨터를 디바이스로 등록] 설정을 사용합니다.
* 적용하고 [확인]을 클릭합니다.
...
## <a name="3-implement-wpad-for-windows-10-devices"></a>3. Windows 10 디바이스에 대해 WPAD를 구현
조직이 아웃바운드 프록시를 통해 인터넷에 액세스하는 경우 Windows 10 컴퓨터에서 Azure AD에 등록할 수 있게 WPAD(웹 프록시 자동 검색)를 구현합니다.

## <a name="4-configure-the-scp-in-any-forests-that-were-not-configured-by-azure-ad-connect"></a>4. Azure AD Connect에서 구성되지 않은 포리스트에 SCP를 구성 
...
## <a name="5-configure-any-federation-service-that-was-not-configured-by-azure-ad-connect"></a>5. Azure AD Connect에서 구성되지 않은 모든 페더레이션 서비스를 구성
...
## <a name="9-install-microsoft-workplace-join-on-windows-down-level-devices"></a>9. Windows 하위 수준 디바이스에 Microsoft Workplace Join을 설치
...
## <a name="10-configure-group-policy-to-allow-device-registration"></a>10. 디바이스 등록을 허용하도록 그룹 정책을 구성

개별 디바이스에 하이브리드 Azure AD 조인을 허용하는 방법에 대한 자세한 내용은 [하이브리드 Azure AD 조인의 제어된 유효성 검사](../devices/hybrid-azuread-join-control.md)를 참조하세요.
