...
**--cpk-by-value**                클라이언트에서 이름으로 제공한 키를 사용하면 Azure Blob에 대한 요청을 하는 클라이언트가 요청별로 암호화 키를 제공하는 옵션을 Storage 수 있습니다. 제공된 키 및 해당 해시는 환경 변수에서 페치됩니다.
...
**--s2s-preserve-access-tier**  서비스 간 복사가 진행되는 동안 액세스 계층을 유지합니다. 대상 스토리지 계정이 액세스 계층 설정을 지원하는지 확인하려면 [Blob 데이터에 대한 핫, 쿨 및 보관](../blobs/access-tiers-overview.md) 액세스 계층을 참조하세요. 액세스 계층 설정이 지원되지 않는 경우 를 사용하여 `--s2s-preserve-access-tier=false` 액세스 계층 복사를 무시하세요. (기본값 `true`).
