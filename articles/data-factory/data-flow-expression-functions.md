...
ms.topic: conceptual
ms.date: 07/04/2021
ms.openlocfilehash: 893d62847c6017b21e612b5f2906b39bb13e88ce
ms.sourcegitcommit: 0046757af1da267fc2f0e88617c633524883795f
ms.translationtype: HT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/13/2021
ms.locfileid: "122642439"
---
...
___
### <code>mapIf</code>
<code><b>mapIf (<i>\<value1\></i> : array, <i>\<value2\></i> : binaryfunction, \<value3\>: binaryFunction) => any</b></code><br/><br/> 배열을 같거나 짧은 길이의 다른 배열에 조건부로 매핑합니다. 값은 structTypes을 포함한 모든 데이터 형식이 될 수 있습니다. 배열의 항목을 #item으로, 현재 인덱스를 #index로 지정할 수 있는 매핑 함수를 사용합니다. 깊게 중첩된 맵의 경우 t maps using the ``#item_[n](#item_1,#index_1...)`` notation.
*    ``mapIf([10, 20, 30], #item > 10, #item + 5) -> [25, 35]``
* ``mapIf(['icecream', 'cake', 'soda'], length(#item) > 4, upper(#item)) -> ['ICECREAM', 'C을 사용하여 상위 맵을 참조할 수 있습니다.AKE']``
___
### <code>mapIndex</code>
<code><b>mapIndex(<i>&lt;value1&gt;</i> : array, <i>&lt;value2&gt;</i> : binaryfunction) => any</b></code><br/><br/> 제공된 식을 사용하여 배열의 각 요소를 새 요소에 매핑합니다. 맵은 식 함수의 한 요소에 참조를 #item으로 그리고 요소에 대한 참조를 예상합니다. index as #index.  
* ``mapIndex([1, 2, 3, 4], #item + 2 + #index) -> [4, 6, 8, 10]``  
___
### <code>mapLoop</code>
<code><b>mapLoop(<i>\<value1\></i> : integer, <i>\<value2\></i> : unaryfunction) => any</b></code><br/><br/> 1에서 length까지 반복하여 해당 길이의 배열을 만듭니다. 배열의 인덱스를 #index로 지정할 수 있는 매핑 함수를 사용합니다. 깊게 중첩된 맵의 경우 #index_n(#index_1, #index_2...) notation.
*    ``mapLoop(3, #index * 10) -> [10, 20을 사용하여 상위 맵을 참조할 수 있습니다., 30]``
___
### <code>reduce</code>
<code><b>reduce(<i>&lt;value1&gt;</i> : array, <i>&lt;value2&gt;</i> : any, <i>&lt;value3&gt;</i> : binaryfunction, <i>&lt;value4&gt;</i> : unaryfunction) => any</b></code><br/><br/> 배열의 요소를 누적합니다. Reduce는 첫 번째 식 함수의 누적기와 한 요소에 대한 참조를 #acc 및 #item으로 예상하며 두 번째 식 함수에 사용될 결과 값을 #result로 예상합니다. ession function.  
* ``toString(reduce(['1', '2', '3', '4'], '0', #acc + #item, #result)) -> '01234'``  
___
### <code>size</code>
<code><b>size(<i>&lt;value1&gt;</i> : any) => integer</b></code><br/><br/> rray or map type  
* ``size(['element1', 'element2']) -> 2``
* ``size([1,2,3])의 크기를 찾습니다. -> 3``
___
### <code>slice</code>
<code><b>slice(<i>&lt;array to slice&gt;</i> : array, <i>&lt;from 1-based index&gt;</i> : integral, [<i>&lt;number of items&gt;</i> : integral]) => array</b></code><br/><br/> 위치에서 배열의 하위 집합을 추출합니다. 위치는 1부터 시작합니다. 길이를 생략하면 기본적으로 문자열 끝까지입니다. d of the string.  
...
___
### <code>setBitSet</code>
<code><b>setBitSet (<i>\<value1\></i>: array, <i>\<value2\></i>:array) => array</b></code>에 비트 위치가 설정되어 있는지 확인합니다.<br/><br/>
이 비트 세트 * ``setBitSet(toBitSet([10, 32]), [98]) => [4294968320L, 17179869184L]``
___  
### <code>isBoolean</code>
<code><b>isBoolean(<i>\<value1\></i>: string) => boolean</b></code>에 비트 위치 설정<br/><br/>
문자열 값이 ``toBoolean()``
* ``isBoolean('true') -> true``
* ``isBoolean('no') -> true``
...
___
### <code>isByte</code>
<code><b>isByte(<i>\<value1\></i> : string) => boolean</b></code> 규칙에 따라 부울 값인지 확인합니다.<br/><br/>
문자열 값이 ``toByte()``
* ``isByte('123') -> true``
...
___
### <code>isDecimal</code>
<code><b>isDecimal (<i>\<value1\></i> : string) => boolean</b></code> 규칙에 따라 지정된 선택적 형식의 double 값인지 검사합니다.<br/><br/>
문자열 값이 ``toDecimal()``
...
16진 문자열 표현을 이진 값으로 표시합니다. 이는 sha2, md5와 함께 사용하여 문자열을 이진 표현으로 변환할 수 있습니다. *    ``unhex('1fadbe') -> toBinary([toByte(0x1f), toByte(0xad), toByte(0xbe)])``
