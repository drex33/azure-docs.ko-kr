...
### <a name="step-1---create-the-data-source"></a>1단계 - 데이터 원본 만들기

데이터 원본 정의는 데이터 원본 유형뿐만 아니라 인덱싱할 콘텐츠에 대한 인증 및 연결의 기타 속성을 지정합니다.

Data Lake Storage Gen2 데이터 원본 정의는 아래 예와 유사합니다.
...
컨테이너에 대한 자격 증명을 제공하는 방법은 다음 중 하나입니다.
...
Data Lake Storage Gen2 Blob에는 콘텐츠 및 메타데이터가 포함됩니다. `dataToExtract` 구성 매개 변수를 사용하여 Blob에서 인덱싱할 부분을 제어할 수 있습니다. 사용되는 값은 다음과 같습니다.
...
1. 데이터를 여러 BLOB 컨테이너 또는 가상 폴더로 분할합니다.

1. 데이터 원본을 컨테이너 또는 폴더당 하나씩 여러 개 설정합니다. BLOB 폴더를 가리키려면 `query` 매개 변수를 사용합니다.
...
1. 각 데이터 소스에 해당하는 인덱서를 만듭니다. 모든 인덱서가 동일한 대상 검색 인덱스를 가리켜야 합니다.  
