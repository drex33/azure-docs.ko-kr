...
ms.date: 05/13/2019
ms.author: allensu
ms.openlocfilehash: 101e636d3349581dae8c7c759e0c3e073c1d0947
ms.sourcegitcommit: 93c7420c00141af83ed3294923b4826dd4dc6ff2
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/02/2021
ms.locfileid: "133436687"
---
# <a name="manage-a-public-ip-address-prefix"></a>공용 IP 주소 접두사 관리

공용 IP 주소 접두사는 표준 SKU 공용 IP 주소의 연속 범위입니다.  공용 IP 주소 리소스를 만드는 경우 접두사에서 고정 공용 IP를 할당하고 Azure 리소스에 주소를 연결할 수 있습니다. 자세한 내용은 [공용 IP 주소 접두사 개요](public-ip-address-prefix.md)를 참조하세요.  이 문서에서는 공용 IP 주소 접두사를 생성, 수정 또는 삭제 하는 방법 뿐만 아니라 기존 접두사에서 공용 IP를 만드는 방법을 설명 합니다.
...
다음 섹션에서는 접두사에서 고정 공용 IP 주소를 만들 때 필요한 매개 변수에 대해 자세히 설명합니다.
   ...
   |유휴 제한 시간(분)|No|연결 유지 메시지를 보내는 데 클라이언트를 사용하지 않고 TCP 또는 HTTP 연결을 유지하는 데 걸리는 시간(분)입니다. |
...
>[!NOTE]
>공용 IP 접두사에서 공용 IP 주소를 요청할 때 할당은 결정적 이거나 순차적이 아닙니다. 공용 IP 접두사의 특정 공용 IP 주소가 필요한 경우 PowerShell 또는 CLI 명령에서이를 허용 합니다.  PowerShell의 경우 `IpAddress` 매개 변수를 사용 해야 합니다 (원하는 ip가 뒤에 옴). CLI의 경우 `ip-address` 매개 변수 (뒤에 원하는 ip)를 사용 해야 합니다.
