...
<sup>2</sup> 로컬 NVMe 디스크는 삭제 되지 않으며, VM을 중지/할당 취소 하는 경우 이러한 디스크의 데이터가 손실 됩니다. [호스트에서 암호화](disk-encryption.md#supported-vm-sizes)를 사용 하도록 설정한 경우에도 로컬 NVMe 디스크는 [Azure Storage 암호화](disk-encryption.md)로 암호화 되지 않습니다.
