...
인덱싱 경로, 종류, 모드 및 쿼리 실행에 어떻게 영향을 주는지에 대한 내용은 [인덱싱 정책 구성](index-policy.md)을 참조하세요. 기본적으로 인덱싱 정책은 문자열에 대해 범위 인덱싱을 사용하며, 같음 쿼리에 효과적입니다. 문자열에 대해 범위 쿼리가 필요한 경우 모든 문자열에 대해 범위 인덱스 유형을 지정하는 것이 좋습니다. 
