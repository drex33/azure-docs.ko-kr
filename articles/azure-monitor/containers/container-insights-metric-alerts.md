...
ms.locfileid: "131455134"
---
# <a name="recommended-metric-alerts-preview-from-container-insights"></a>컨테이너 인사이트에서 권장되는 메트릭 경고(미리 보기)

최대 수요가 발생하고 수용작업량까지 실행되고 있을 때 시스템 리소스 문제에 대해 경고하려면 컨테이너 인사이트를 사용하여 Azure Monitor 로그에 저장된 성능 데이터를 기반으로 로그 경고를 만듭니다. 이제 컨테이너 인사이트에는 AKS 및 Azure Arc 지원 Kubernetes 클러스터에 대해 미리 구성된 메트릭 경고 규칙이 포함되며, 이는 공개 미리 보기로 제공됩니다.
...
> [!NOTE]
> 2021년 10월 8일부터 세 가지 경고가 업데이트되어 경고 조건을 올바르게 계산했습니다. **컨테이너 CPU %**, 컨테이너 작업 집합 메모리 **%** 및 영구 **볼륨 사용량 %** 이러한 새 경고는 이전에 사용 가능한 해당 경고와 동일한 이름을 갖지만 업데이트된 새 메트릭을 사용합니다. 이 문서에 설명된 "이전" 메트릭을 사용하는 경고를 사용하지 않도록 설정하고 "새" 메트릭을 사용하도록 설정하는 것이 좋습니다. "이전" 메트릭은 사용하지 않도록 설정한 후 권장 경고에서 더 이상 사용할 수 없지만 수동으로 다시 사용하도록 설정할 수 있습니다.
...
* 메트릭 경고 및 추가 메트릭 도입을 지원하기 위해 필요한 최소 에이전트 버전은 AKS용으로 **mcr.microsoft.com/azuremonitor/containerinsights/ciprod:ciprod05262020** **Azure Arc** 지원 Kubernetes 클러스터에 대한 mcr.microsoft.com/azuremonitor/containerinsights/ciprod:ciprod09252020.
    ...
    * **노드** 탭에서 클러스터 노드를 선택하고 오른쪽의 **속성** 창에서 **에이전트 이미지 태그** 아래의 값을 확인합니다.
    ...
    AKS에 대해 표시 된 값은 버전 **ciprod05262020** 이상 이어야 합니다. Azure Arc 지원 Kubernetes 클러스터에 대해 표시된 값은 **ciprod09252020** 이상 버전이어야 합니다. 클러스터에 이전 버전이 있는 경우에 최신 버전을 다운로드 하는 단계는 [컨테이너 인사이트 에이전트를 업그레이드 하는 방법](container-insights-manage-agent.md#upgrade-agent-on-aks-cluster)을 참조 하세요.
...
중요한 사항에 대해 경고하기 위해 컨테이너 인사이트에는 AKS 및 Azure Arc 지원 Kubernetes 클러스터에 대한 다음과 같은 메트릭 경고가 포함됩니다.

|Name| 설명 |default_threshold |
|----|-------------|------------------|
|**(신규) 평균 컨테이너 CPU%** |컨테이너 당 사용 되는 평균 CPU를 계산합니다.|컨테이너 당 평균 CPU 사용량이 95% 보다 큰 경우.| 
|**(신규) 평균 컨테이너 작업 집합 메모리 %** |컨테이너 당 사용 되는 평균 작업 집합 메모리를 계산합니다.|컨테이너당 평균 작업 집합 메모리 사용량이 95%를 초과하는 경우. |
|평균 CPU % |노드당 사용 되는 평균 CPU를 계산합니다. |평균 노드 CPU 사용률이 80%를 초과 하는 경우 |
| 일일 데이터 상한 위반 | 데이터 한도 위반 시| Log Analytics 작업 영역에 대한 총 데이터 수집이 [지정된 할당량을](../logs/manage-cost-storage.md#manage-your-maximum-daily-data-volume) 초과하는 경우 |
|평균 디스크 사용량 (%) |노드의 평균 디스크 사용량을 계산 합니다.|노드의 디스크 사용량이 80% 보다 큰 경우. |
|**(신규) 평균 영구적 볼륨 사용량 %** |Pod 당 평균 PV 사용 현황을 계산합니다. |Pod 당 평균 PV 사용량이 80% 보다 큰 경우.|
...
이 기능의 일부로 달리 지정하지 않는 한 다음 메트릭이 활성화되고 수집됩니다. 레이블이 "Old"인 **굵게** 표시된 메트릭은 올바른 경고 평가를 위해 수집된 "신규" 메트릭으로 대체됩니다.

|메트릭 네임스페이스 |메트릭 |Description |
|---------|----|------------|
|Insights.container/nodes |cpuUsageMillicores |호스트별 관한 CPU 사용률(밀리코어)입니다.|
|Insights.container/nodes |cpuUsagePercentage, cpuUsageAllocatablePercentage(미리 보기) |노드 및 할당 가능한 CPU 사용률입니다.|
|Insights.container/nodes |memoryRssBytes |호스트의 메모리 RSS 사용률 (바이트)입니다.|
|Insights.container/nodes |memoryRssPercentage, memoryRssAllocatablePercentage(미리 보기) |호스트 및 할당 가능한 메모리 RSS 사용률입니다.|
|Insights.container/nodes |memoryWorkingSetBytes |호스트의 메모리 작업 집합 사용률 (바이트)입니다.|
|Insights.container/nodes |memoryWorkingSetPercentage, memoryRssAllocatablePercentage(미리 보기) |메모리 작업 집합 사용률(각각 호스트 및 할당 가능)입니다.|
...
|Insights.container/containers |**(Old)memoryWorkingSetExceededPercentage** |컨테이너 이름, 컨트롤러 이름, Kubernetes 네임 스페이스, pod 이름에 의해 사용자 구성 가능 임계값 (기본값은 95.0)을 초과 하는 컨테이너에 대한 메모리 작업 집합 비율입니다.|
|Insights.container/containers |**(New)memoryWorkingSetThresholdViolated** |사용자 구성 가능한 임계값(기본값: 95.0)을 초과하는 컨테이너의 메모리 작업 집합 백분율이 컨테이너 이름, 컨트롤러 이름, Kubernetes 네임스페이스, Pod 이름으로 트리거됩니다.|
