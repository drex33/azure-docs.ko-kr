...
ms.date: 11/09/2021
ms.openlocfilehash: 00636fbcc807f157b268a60bee52989d87aab1bc
ms.sourcegitcommit: 1244a72dbec39ac8cf16bb1799d8c46bde749d47
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/18/2021
ms.locfileid: "132762445"
---
...
1. [Microsoft Sentinel GitHub 리포지토리에서](https://github.com/Azure/Azure-Sentinel/tree/master/Solutions/SAP/CR)다음 SAP 변경 요청 중 하나를 다운로드하여 설치합니다.
    ...
    - **SAP 버전 750 이상:** SAP 변경 요청 *NPLK900170* 설치
    - **SAP 버전 740:** SAP 변경 요청 *NPLK900169* 설치
...
1. SAP 변경 요청 *NPLK900163* 을 **가져와서 /MS MSPN/SENTINEL_CONNECTOR** 라는 새 SAP 역할을 만듭니다. **STMS_IMPORT** SAP 트랜잭션 코드를 사용합니다.

    다음과 같이 필요한 권한으로 역할이 만들어졌는지 확인합니다.

    :::image type="content" source="media/sap/required-sap-role-authorizations.png" alt-text="Microsoft Sentinel SAP 데이터 커넥터에 대한 SAP 역할 권한이 필요합니다.":::

    자세한 내용은 [ABAP 사용자에 대한 권한 부여](sap-solution-detailed-requirements.md#required-abap-authorizations)를 참조하세요.

1. SAP 데이터 커넥터에 대한 대화 상자가 아닌 RFC/NetWeaver 사용자를 만들고 새로 만든 */MSIDN/SENTINEL_CONNECTOR* 역할을 연결합니다.

    - 역할을 연결한 후 역할 권한이 사용자에게 배포되었는지 확인합니다.
    - 이 프로세스에서는 ABAP 사용자에 대한 사용자 이름 및 암호를 사용해야 합니다. 새 사용자를 만들고 필요한 권한을 가지고 나면 ABAP 사용자 암호를 변경해야 합니다.

1. 설치 프로세스 중에 필요하므로 VM의 x86_64 64비트 버전에서 Linux용 SAP NetWeaver RFC SDK 7.50을 다운로드하여 배치합니다.
...
1. (선택 사항) SAP *Auditlog* 파일은 시스템 차원에서 사용되며 여러 SAP 클라이언트를 지원합니다. 그러나 Microsoft Sentinel SAP 솔루션의 각 인스턴스는 단일 SAP 클라이언트만 지원합니다.

    따라서 다중 클라이언트 SAP 시스템이 있는 경우 데이터 중복을 방지하려면 SAP 솔루션을 배포하는 클라이언트에 대해서만 *Auditlog* 파일을 사용하도록 설정하는 것이 좋습니다.
...
1. 다음 명령을 예로 사용합니다. 리소스 그룹 및 VM 이름에 대한 값을 삽입해야 합니다.
...
> [!IMPORTANT]
> 다른 VM과 마찬가지로 조직에 보안 모범 사례를 적용해야 합니다.
>

자세한 내용은 [빠른 시작: Azure CLI를 사용하여 Linux 가상 머신 만들기](../virtual-machines/linux/quick-create-cli.md)를 참조하세요.

## <a name="create-a-key-vault-for-your-sap-credentials"></a>SAP 자격 증명에 대한 키 자격 증명 모음 만들기

이 자습서에서는 새로 만든 Azure Key Vault 또는 전용 [Azure Key Vault를](../key-vault/index.yml) 사용하여 SAP 데이터 커넥터에 대한 자격 증명을 저장합니다.

Azure Key Vault를 만들거나 전치려면 다음을 수행합니다.

1. 새 Azure Key Vault를 만들거나 SAP 데이터 커넥터 배포에만 적용할 기존 Key Vault를 선택합니다.

    예를 들어 새 키 자격 증명 모음을 만들려면 다음 명령을 실행합니다. 키 자격 증명 모음 리소스 그룹의 이름을 사용하고 키 자격 증명 모음 이름을 입력해야 합니다.
...
1. 다음 방법 중 하나를 사용하여 VM의 관리 ID에 GET, LIST 및 SET 권한을 포함한 액세스 정책을 할당합니다.
    ...
    - **Azure Portal 입니다.**
        ...
        1. Azure 키 자격 증명 모음에서 **액세스 정책** 액세스 정책 추가 - 비밀  >  권한: 보안 주체 선택 **을 선택하고, Get, List 및 Set를**  >  **선택합니다.**  
        1. [VM 이름](#deploy-a-linux-vm-for-your-sap-data-connector)를 입력한 다음, **저장 추가를**  >  선택합니다.

        자세한 내용은 [Key Vault 설명서](../key-vault/general/assign-access-policy-portal.md)를 참조하세요.

    - **Azure CLI:**

        1. 다음 명령을 실행하여 [VM의 보안 주체 ID](#deploy-a-linux-vm-for-your-sap-data-connector)를 확인합니다. Azure 리소스 그룹의 이름을 입력해야 합니다.  
        ...
        1. 다음 명령을 실행하여 키 자격 증명 모음에 VM 액세스 권한을 할당합니다. 리소스 그룹의 이름과 이전 단계에서 반환된 보안 주체 ID 값을 입력해야 합니다.
...
Microsoft Sentinel SAP 데이터 커넥터의 배포 스크립트는 [필요한 소프트웨어를](#automatically-installed-software) 설치한 다음 새로 [만든 VM](#deploy-a-linux-vm-for-your-sap-data-connector)에 커넥터를 설치합니다. 또한 자격 증명을 전용 키 자격 증명 모음 에 [저장합니다.](#create-a-key-vault-for-your-sap-credentials)

SAP 데이터 커넥터의 배포 스크립트는 [Microsoft Sentinel GitHub 리포지토리 > DataConnectors > SAP 에 저장됩니다.](https://raw.githubusercontent.com/Azure/Azure-Sentinel/master/Solutions/SAP/sapcon-sentinel-kickstart.sh)

SAP 데이터 커넥터 배포 스크립트를 실행하려면 다음 항목이 필요합니다.

- [필수 조건](#prerequisites) 섹션에 나열된 Microsoft Sentinel 작업 영역 세부 정보입니다.
- 필수 조건 섹션에 나열된 SAP 시스템 세부 [정보입니다.](#prerequisites)
- sudo 권한이 있는 VM 사용자에 대한 액세스
- **/MS MSPN/SENTINEL_CONNECTOR** 역할이 적용된 [SAP 시스템 구성에서](#configure-your-sap-system)만든 SAP 사용자입니다.
- SAP 팀의 도움말.
...
1. 화면의 지침에 따라 SAP 및 키 자격 증명 모음 세부 정보를 입력하고 배포를 완료합니다. 배포가 완료되면 확인 메시지가 표시됩니다.

    ```azurecli
    The process has been successfully completed, thank you!
    ```

    Microsoft Sentinel은 초기화 시간 24시간 전까지 구성된 시간 범위에 대한 SAP 로그 검색을 시작합니다.

1. 시스템 로그를 검토하여 데이터 커넥터가 데이터를 전송하는지 확인하는 것이 좋습니다. 다음을 실행합니다.
...
Microsoft Sentinel **솔루션** 및 감시 목록 영역에서 [SAP 보안 콘텐츠를](sap-solution-security-content.md) **배포합니다.**

**SAP용 Microsoft Sentinel - Continuous Threat Monitoring** 솔루션을 사용하면 SAP 데이터 커넥터를 Microsoft Sentinel 데이터 커넥터 영역에 표시할 수 **있습니다.** 또한 이 솔루션은 **SAP - 시스템 애플리케이션 및 제품** 통합 문서와 SAP 관련 분석 규칙을 배포합니다.

Microsoft Sentinel 작업 영역에 SAP 관련 감시 목록을 수동으로 추가합니다.

SAP 솔루션 보안 콘텐츠를 배포하려면 다음을 수행합니다.

1. Microsoft Sentinel의 왼쪽 창에서 **솔루션(미리 보기) 을** 선택합니다.

    **솔루션** 페이지에는 필터링되고 검색 가능한 솔루션 목록이 표시됩니다.

1. SAP 솔루션 페이지를 열려면 **Microsoft Sentinel - SAP용 지속적인 위협 모니터링(미리 보기) 을** 선택합니다.

    :::image type="content" source="media/sap/sap-solution.png" alt-text="'Microsoft Sentinel - SAP용 지속적인 위협 모니터링(미리 보기)' 솔루션 창의 스크린샷.":::

1. 솔루션 배포 마법사를 시작하려면 **만들기를** 선택한 다음, 솔루션을 배포할 Azure 구독, 리소스 그룹 및 Log Analytics 작업 영역의 세부 정보를 입력합니다.
...
1. **다음** 을 선택하여 **데이터 커넥터** **분석** 및 **통합 문서** 탭을 순환합니다. 여기에서 이 솔루션과 함께 배포되는 구성 요소에 대해 알아볼 수 있습니다.
    ...
    자세한 내용은 [Microsoft Sentinel SAP 솔루션: 보안 콘텐츠 참조(공개 미리 보기)](sap-solution-security-content.md)를 참조하세요.

1. 검토 **+ 만들기 탭** 창에서 **유효성 검사 통과** 메시지를 기다린 다음, **만들기를** 선택하여 솔루션을 배포합니다.

    > [!TIP]
    > 링크에 대해 **템플릿 다운로드** 를 선택하여 솔루션을 코드로 배포할 수도 있습니다.

1. 배포가 완료되면 오른쪽 위에 확인 메시지가 나타납니다.
...
1. 검색, 검색 규칙, 위협 헌팅 및 응답 플레이북에서 사용할 SAP 관련 관심 목록을 추가합니다. 이러한 감시 목록은 Microsoft Sentinel SAP Continuous Threat Monitoring 솔루션에 대한 구성을 제공합니다. 다음을 수행합니다.

    a. 의 Microsoft Sentinel GitHub 리포지토리에서 SAP 감시 목록을 https://github.com/Azure/Azure-Sentinel/tree/master/Solutions/SAP/Analytics/Watchlists 다운로드합니다.  
    b. Microsoft Sentinel **감시 목록** 영역에서 Microsoft Sentinel 작업 영역에 감시 목록을 추가합니다. 다운로드한 CSV 파일을 원본으로 사용하고 사용자 환경에 필요한 대로 사용자 지정합니다.  

    [![Microsoft Sentinel에 추가된 SAP 관련 감시 목록입니다. ](media/sap/sap-watchlists.png) ](media/sap/sap-watchlists.png#lightbox)

    자세한 내용은 [Microsoft Sentinel 감시 목록 사용](watchlists.md) 및 사용 가능한 SAP 감시 목록 [을 참조하세요.](sap-solution-security-content.md#available-watchlists)

1. Microsoft Sentinel에서 SAP 데이터 **커넥터용 Microsoft Sentinel Continuous Threat Monitoring으로** 이동하여 연결을 확인합니다.

    [![SAP 데이터 커넥터에 대한 Microsoft Sentinel Continuous Threat Monitoring 페이지의 스크린샷. ](media/sap/sap-data-connector.png) ](media/sap/sap-data-connector.png#lightbox)

    SAP ABAP 로그는 Microsoft Sentinel **로그** 페이지의 사용자 지정 로그 아래에 **표시됩니다.**

    [![Microsoft Sentinel의 '사용자 지정 로그' 영역에 있는 SAP ABAP 로그의 스크린샷. ](media/sap/sap-logs-in-sentinel.png) ](media/sap/sap-logs-in-sentinel.png#lightbox)
...
Microsoft Sentinel github 리포지토리에서 최신 버전의 관련 배포 스크립트가 있는지 확인합니다. 
...
- [Microsoft Sentinel GitHub 리포지토리의](https://github.com/Azure/Azure-Sentinel/tree/master/Solutions/SAP/CR)관련 SAP 변경 요청.
- Microsoft Sentinel SAP용 지속적인 위협 모니터링 솔루션의 **Microsoft Sentinel SAP** 보안 콘텐츠입니다.
- [Microsoft Sentinel GitHub 리포지토리의](https://github.com/Azure/Azure-Sentinel/tree/master/Solutions/SAP/Analytics/Watchlists)관련 감시 목록입니다.
...
1. SAP 실행 [패드 지원 사이트에서](https://launchpad.support.sap.com/#/notes/0002624117)액세스할 수 있는 SAP Note 0002624117 에 설명된 대로 *SAP HANA* 감사 로그 내역이 Syslog를 사용하도록 구성되어 있는지 확인합니다. 자세한 내용은 다음을 참조하세요.
...
1. 머신에 Log Analytics 에이전트를 설치하고 구성합니다.
    ...
    b. Azure Portal에서 Log Analytics 작업 영역으로 이동합니다. 왼쪽 창의 **설정** 아래에서 에이전트 관리   >  **Linux 서버를** 선택합니다.  
    c. **Linux용 에이전트 다운로드 및 온보딩에서** 상자에 표시된 코드를 터미널에 복사한 다음 스크립트를 실행합니다.

    Log Analytics 에이전트가 머신에 설치되고 작업 영역에 연결됩니다. 자세한 내용은 [Linux 컴퓨터에 Log Analytics 에이전트 설치](../azure-monitor/agents/agent-linux.md) 및 Microsoft [GitHub](https://github.com/microsoft/OMS-Agent-for-Linux) 리포지토리에 Linux용 OMS 에이전트.

1. 에이전트 **관리 > Linux 서버** 탭을 새로 고쳐 **1개의 Linux 컴퓨터가 연결되어** 있음을 확인합니다.

1. 왼쪽 창의 **설정** 아래에서 **에이전트 구성 을** 선택한 **다음, Syslog 탭을** 선택합니다.

1. **기능 추가** 를 선택하여 수집하려는 기능을 추가합니다. 

    > [!TIP]
    > HANA 데이터베이스 이벤트가 저장되는 기능이 서로 다른 배포 간에 변경 될 수 있으므로 모든 기능을 추가하고 Syslog 로그에 대해 확인한 다음 관련이 없는 모든 기능을 제거하는 것이 좋습니다.
    >

1. Microsoft Sentinel에서 HANA 데이터베이스 이벤트가 이제 검색된 로그에 표시되어 있는지 확인합니다.

## <a name="next-steps"></a>다음 단계

Microsoft Sentinel SAP 솔루션에 대해 자세히 알아보세요.
...
자세한 내용은 [Microsoft Sentinel 솔루션 을 참조하세요.](sentinel-solutions.md)
