---
title: Azure Data Protection REST API 사용하여 Azure 디스크를 백업합니다.
description: 이 문서에서는 REST API 사용하여 Azure 디스크의 백업 작업을 구성, 시작 및 관리하는 방법을 알아봅니다.
ms.topic: conceptual
ms.date: 10/06/2021
...
ms.locfileid: "130264715"
---
# <a name="back-up-azure-disks-using-azure-data-protection-via-rest-api"></a>REST API 통해 Azure Data Protection을 사용하여 Azure 디스크 백업

이 문서에서는 REST API 통해 Azure Disks에 대한 백업을 관리하는 방법을 설명합니다.
...
보호할 ARM ID 및 디스크의 위치를 확인합니다. 디스크의 식별자 역할을 합니다.
...
디스크 스냅샷은 구독 내의 리소스 그룹에 저장됩니다. 지침으로 Azure Backup 서비스에서 사용할 스냅샷 데이터 저장소로 전용 리소스 그룹을 만드는 것이 좋습니다. 전용 리소스 그룹을 사용하면 리소스 그룹에 대한 액세스 권한을 제한할 수 있기 때문에 안전하고 쉽게 백업 데이터를 관리할 수 있습니다. 디스크 스냅샷을 배치하려는 리소스 그룹의 ARM ID를 기록해 둡니다.
...
백업 자격 증명 모음을 사용하려면 디스크에 대한 권한이 필요합니다. 자격 증명 모음의 시스템이 할당한 관리 ID는 해당 권한을 할당하는 데 사용됩니다.
...
관련 권한이 자격 증명 모음 및 디스크로 설정되고 자격 증명 모음 및 정책이 구성되면 백업 구성 요청을 준비할 수 있습니다. 다음은 Azure 디스크에 대한 백업을 구성하기 위한 요청 본문입니다. Azure 디스크의 AZURE RESOURCE MANAGER ID(ARM ID)와 세부 정보는 _datasourceinfo_ 섹션에 언급되고 정책 정보는 스냅샷 리소스 그룹이 정책 매개 변수 중 하나로 제공되는 _policyinfo_ 섹션에 있습니다.
...
앞에서 준비한 [요청 본문은](#prepare-the-request-to-configure-backup) 보호할 Azure 디스크의 세부 정보를 제공하는 데 사용됩니다.
...
모든 권한을 부여하는 경우 유효성 검사 요청을 다시 제출하고 결과 작업을 추적하며 모든 조건이 충족되면 성공 응답을 200(확인)으로 반환합니다.
...
요청의 유효성이 검사되면 [백업 인스턴스 만들기 API](/rest/api/dataprotection/backup-instances/create-or-update)에 동일한 항목을 제출할 수 있습니다. Backup 인스턴스는 Backup 자격 증명 모음 내에서 Azure Backup 데이터 보호 서비스로 보호되는 항목을 나타냅니다. 여기서 Azure 디스크는 백업 인스턴스이며, 위에서 유효성을 검사한 동일한 요청 본문을 사소한 추가와 함께 사용할 수 있습니다.

백업 인스턴스에 고유한 이름을 사용합니다. 따라서 리소스 이름과 고유 식별자의 조합을 사용하는 것이 좋습니다. 예를 들어 다음 작업에서는 _msdiskbackup-2dc6eb5b-d008-4d68-9e49-7132d99da0ed를_ 사용하고 백업 인스턴스 이름으로 표시합니다.
...
그런 다음, 간단한 *GET* 명령으로 _Azure-AsyncOperation_ 헤더를 사용하여 결과 작업을 추적합니다.
