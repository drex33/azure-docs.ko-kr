---
title: Microsoft Sentinel | 플레이북 인증 Microsoft Docs
description: 수정 작업을 수행할 수 있도록 플레이북에 Microsoft Sentinel 및 권한 부여에 대한 액세스 권한을 부여하는 방법을 알아봅니다.
services: sentinel
...
ms.locfileid: "132715248"
---
# <a name="authenticate-playbooks-to-microsoft-sentinel"></a>Microsoft Sentinel에 플레이북 인증

[!INCLUDE [Banner for top of topics](./includes/banner.md)]

Logic Apps 작동하는 방식에서는 Microsoft Sentinel 자체를 포함하여 상호 작용하는 모든 유형의 모든 리소스에 개별적으로 연결하고 독립적으로 인증해야 합니다. Logic Apps는 이 용도로 [특수 커넥터](/connectors/connector-reference/)를 사용하며, 각 리소스 종류에는 고유한 커넥터가 있습니다. 이 문서에서는 플레이북이 작업 영역의 테이블에 있는 정보에 액세스하기 위해 [Microsoft Sentinel과](/connectors/azuresentinel/)상호 작용하는 데 사용할 수 있는 Logic Apps Microsoft Sentinel 커넥터 의 연결 및 인증 유형에 대해 설명합니다.

이 문서는 [플레이북에서 트리거 및 작업 사용에](playbook-triggers-actions.md)대한 가이드와 함께 다른 플레이북 설명서인 [자습서: Microsoft Sentinel에서 자동화 규칙과 함께 플레이북 사용과 함께](tutorial-respond-threats-playbook.md)제공됩니다.
...
Logic Apps Microsoft Sentinel 커넥터와 해당 구성 요소 트리거 및 작업은 관련 작업 영역에 필요한 권한(읽기 및/또는 쓰기)이 있는 ID를 대신하여 작동할 수 있습니다. 커넥터는 다양한 ID 유형을 지원합니다.
...
이 인증 방법을 사용하면 플레이북(논리 앱 워크플로 리소스)에 직접 권한을 부여할 수 있으므로 플레이북에서 수행된 Microsoft Sentinel 커넥터 작업은 Microsoft Sentinel에 대한 자체 권한이 있는 독립 개체인 것처럼 플레이북을 대신하여 작동합니다. 이 방법을 사용하면 관리해야 하는 ID 수가 줄어듭니다. 

> [!NOTE]
> 관리 ID에 다른 리소스(예: Microsoft Sentinel 작업 영역)에 대한 액세스 권한을 부여하려면 로그인한 사용자에게 Microsoft Sentinel 작업 영역의 소유자 또는 사용자 액세스 관리자와 같은 역할 할당을 작성할 수 있는 권한이 있는 역할이 있어야 합니다.
...
1. [해당 ID에](../logic-apps/create-managed-service-identity.md#give-identity-access-to-resources) Microsoft Sentinel 작업 영역에 대한 액세스 권한을 부여합니다.
    1. Microsoft Sentinel 메뉴에서 **설정** 선택합니다.
    ...
    1. 표시되는 새 패널에서 적절한 역할을 할당합니다.
        ...
        | 역할 | 상황 |
        | --- | --- |
        | [**Microsoft Sentinel 응답기**](../role-based-access-control/built-in-roles.md#microsoft-sentinel-responder) | 플레이북에는 인시던트 또는 관심 목록을 업데이트하는 단계가 있습니다. |
        | [**Microsoft Sentinel 판독기**](../role-based-access-control/built-in-roles.md#microsoft-sentinel-reader) | 플레이북은 인시던트만 받습니다. |
        |

        [Microsoft Sentinel에서](./roles.md)사용 가능한 역할에 대해 자세히 알아봅니다.
    1. **액세스 권한 할당** 에서 **논리 앱** 을 선택합니다.
    1. 플레이북이 속한 구독을 선택하고 플레이북 이름을 선택합니다.
    1. **저장** 을 선택합니다.

1. Microsoft Sentinel Logic Apps 커넥터에서 관리 ID 인증 방법을 사용하도록 설정합니다.

    1. Logic Apps 디자이너에서 Microsoft Sentinel Logic Apps 커넥터 단계를 추가합니다. 기존 연결에 대한 커넥터를 이미 사용하도록 설정한 경우 **연결 변경** 링크를 클릭합니다.
...
Microsoft Sentinel 커넥터에서 사용자 고유의 애플리케이션을 사용하려면 다음 단계를 수행합니다.
...
1. Microsoft Sentinel 작업 영역에 권한을 부여합니다.

    이 단계에서 앱은 Microsoft Sentinel 작업 영역에서 작업할 수 있는 권한을 얻습니다.

    1. Microsoft Sentinel 작업 영역에서 **설정**  ->  **작업 영역 설정** 액세스  ->  **제어(IAM)로** 이동합니다.

    1. **역할 할당 추가** 를 선택합니다.

    1. 애플리케이션에 할당할 역할을 선택합니다. 예를 들어 애플리케이션이 인시던트 업데이트와 같이 Sentinel 작업 영역을 변경하는 작업을 수행하도록 허용하려면 **Microsoft Sentinel 기여자** 역할을 선택합니다. 데이터만 읽는 작업의 경우 **Microsoft Sentinel 판독기** 역할로 충분합니다. [Microsoft Sentinel에서 사용 가능한 역할에 대해 자세히 알아봅니다.](./roles.md)
...
처음으로 인증을 만들 때마다 API 연결 형식의 새로운 Azure 리소스가 만들어집니다. 동일한 리소스 그룹의 모든 Microsoft Sentinel 작업 및 트리거에서 동일한 API 연결을 사용할 수 있습니다.
...
이 문서에서는 Microsoft Sentinel에 Logic Apps 기반 플레이북을 인증하는 다양한 방법에 대해 배웠습니다.
