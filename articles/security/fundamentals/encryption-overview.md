...
description: Azure의 암호화 옵션에 대해 알아보기 미사용 암호화, 진행 중 암호화 및 Azure Key Vault를 사용한 키 관리에 대한 정보를 참조하세요.
services: security
author: msmbaldwin
ms.assetid: ''
ms.service: security
...
마지막으로 Java용 Azure Storage 클라이언트 라이브러리를 사용하여 Azure Storage에 데이터를 업로드하기 전에 클라이언트 쪽 암호화를 수행하고 클라이언트에 데이터가 다운로드될 때 암호 해독할 수 있습니다. 또한 이 라이브러리는 스토리지 계정 키 관리를 위해 [Key Vault](https://azure.microsoft.com/services/key-vault/)와의 통합을 지원합니다.
...
[TDE](/sql/relational-databases/security/encryption/transparent-data-encryption-tde)는 복구 중에 사용 가능하도록 데이터베이스 부트 레코드에 저장된 DEK(데이터베이스 암호화 키)를 사용하여 [SQL Server](https://www.microsoft.com/sql-server/sql-server-2016), [Azure SQL Database](../../azure-sql/database/sql-database-paas-overview.md) 및 [Azure Synapse Analytics](../../synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is.md) 데이터 파일을 실시간으로 암호화하는 데 사용됩니다.
...
Azure 고객 트래픽이 Microsoft가 제어하지 않는 물리적 경계 외부의(또는 Microsoft를 대신하여) 데이터 센터 간에 이동할 때마다 [IEEE 802.1AE MAC 보안 표준](https://1.ieee802.org/security/802-1ae/)(MACsec라고도 함)을 사용하는 데이터 링크 계층 암호화 방법은 기본 네트워크 하드웨어에 걸쳐 지점 간에 적용됩니다. 패킷이 전송되기 전에 디바이스에서 암호화 및 암호 해독되어 물리적 "중간자(man-in-the-middle)" 또는 스누핑/wiretapping 공격을 방지합니다. 이 기술은 네트워크 하드웨어 자체에 통합되어 있기 때문에 측정 가능한 링크 대기 시간 증가 없이 네트워크 하드웨어에 회선 속도 암호화를 제공합니다. 이 MACsec 암호화는 한 지역 내에서 또는 지역 간에 이동하는 모든 Azure 트래픽에 대해 기본적으로 켜져 있으며 고객이 사용하도록 설정하기 위해 필요한 작업은 없습니다. 

### <a name="tls-encryption-in-azure"></a>Azure에서 TLS 암호화

Microsoft는 클라우드 서비스와 고객 간에 이동할 때 [TLS(전송 계층 보안)](https://en.wikipedia.org/wiki/Transport_Layer_Security) 프로토콜을 사용하여 데이터를 보호하는 기능을 제공합니다. Microsoft 데이터 센터는 Azure 서비스에 연결되는 클라이언트 시스템과 TLS 연결을 협상합니다. TLS는 강력한 인증, 메시지 개인 정보 및 무결성(메시지 변조, 가로채기 및 위조의 검색 가능), 상호 운용성, 알고리즘 유연성, 배포 및 사용 편의성을 제공합니다.
...
Windows를 실행하는 VM 간에 전송되는 데이터는 연결 특성에 따라 다양한 방법으로 암호화될 수 있습니다.
