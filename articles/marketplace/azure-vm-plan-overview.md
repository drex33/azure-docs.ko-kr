...
ms.author: iqshah
ms.date: 11/19/2021
ms.openlocfilehash: b268725d85327ba5c4fcafdbd565b4e0a2a2789c
ms.sourcegitcommit: b00a2d931b0d6f1d4ea5d4127f74fc831fb0bca9
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/20/2021
ms.locfileid: "132871588"
---
...
각 제품에 대해 최대 100 요금제를 만들 수 있으며, 이러한 요금제는 45 모두 전용으로 지정할 수 있습니다. [Microsoft 상업용 마켓플레이스의 프라이빗 제품](private-offers.md)에서 프라이빗 플랜에 대해 자세히 알아봅니다.
...
**Azure Government** 를 선택하여 [Azure Government](../azure-government/documentation-government-welcome.md) 지역에서 플랜을 사용할 수 있도록 합니다. 이 지역은 미국 연방, 주, 지방 또는 부족의 고객에 대 한 제어 된 액세스 및 서비스를 제공 하는 파트너를 위해 제공 됩니다. 게시자는 규정 준수 제어, 보안 조치 및 모범 사례를 담당합니다. Azure Government는 물리적으로 격리된 데이터 센터 및 네트워크를 사용합니다(미국에만 있음).
...
사용량 기반 월별 청구 계획의 경우 Microsoft는 시간별 사용량에 대해 고객에 게 요금을 청구 하 고 매달 청구 됩니다. 이 요금제는 고객이 사용한 시간에 대해서만 요금이 청구 되는 _종 량_ 제 요금제입니다. 이 계획을 선택 하는 경우 다음 가격 책정 옵션 중 하나를 선택 합니다.

- **무료** – VM 제품은 무료입니다.
- **Flat rate (권장)** -VM 제품은 실행 되는 하드웨어에 관계 없이 동일한 시간당 가격입니다.
- **코어 당** -VM 제공 가격은 CPU 코어 수를 기준으로 합니다. 단일 CPU 코어에 대 한 가격을 제공 하 고 하드웨어 크기를 기준으로 가격 책정을 증가 시킵니다.
- **코어 크기 당** – VM 제품은 배포 된 하드웨어의 CPU 코어 수를 기준으로 가격이 책정 됩니다.
- **시장 및 코어 크기 당** – 배포 된 하드웨어의 CPU 코어 수와 모든 시장에 대 한 가격을 기준으로 가격을 할당 합니다. 통화 변환은 게시자에 의해 수행 됩니다. 가격 책정 기능을 사용 하는 경우이 옵션을 사용 하는 것이 더 쉽습니다.

**코어 크기** 및 **시장 및 코어 크기** 당 **가격** 을 입력 한 다음 **가격 생성** 을 선택 합니다. 가격/시간 계산의 테이블이 채워집니다. 그런 다음, 선택 하는 경우 코어 당 가격을 조정할 수 있습니다. _시장별 및 코어 크기_ 가격 책정 옵션을 사용하는 경우 이 계획에 대해 선택한 각 시장에 대한 가격/시간 계산 테이블을 추가로 사용자 지정할 수 있습니다.

> [!NOTE]
> 가격을 게시하기 직전에 가격이 적절한지 확인하려면 가격 책정 스프레드시트를 내보내고 각 시장의 가격을 검토합니다. 가격 책정 데이터를 내보내기 전에 먼저 페이지 아래쪽에 **있는 초안 저장을** 선택하여 가격 책정 변경 내용을 저장합니다.

가격 책정 옵션을 선택할 때 고려해야 할 몇 가지 사항:
- 처음 네 가지 옵션의 경우 Microsoft는 통화 변환을 수행합니다.
- Microsoft는 소프트웨어 솔루션에 대해 고정 요금 가격 책정을 사용하는 것이 좋습니다.
- 가격은 고정되므로 플랜이 게시된 후에는 가격을 조정할 수 없습니다. 그러나 VM 제안에 대한 가격을 줄이려면 [지원 티켓을](support.md)열 수 있습니다.

### <a name="free-trial"></a>평가판

고객에게 1개월, 3개월 또는 6개월 **무료 평가판을** 제공할 수 있습니다.

### <a name="plan-visibility"></a>플랜 표시 여부

모든 사용자에게 표시되거나 미리 선택한 사용자에게만 표시되도록 각 플랜을 설계할 수 있습니다. Azure 테넌트 ID, 구독 ID 또는 둘 모두를 사용하여 이 제한된 대상의 멤버 자격을 할당합니다.
...
*Azure 테넌트 ID,* 구독 ID 또는 둘 모두를 사용하여 이 프라이빗 플랜에 액세스할 수 있는 *대상을 할당합니다.* 필요에 따라 할당한 각 Azure 테넌트 ID 또는 구독 ID에 대한 **설명을** 포함합니다. 최대 10개의 구독 ID 및 테넌트 ID를 수동으로 추가하거나 10개 이상의 ID가 필요한 경우 CSV 스프레드시트를 가져옵니다.
...
숨겨진 플랜은 미리 보기 링크를 생성하지 않습니다. 그러나 [다음 단계에 따라](azure-vm-create-faq.yml#how-do-i-test-a-hidden-preview-image-)테스트할 수 있습니다.
...
이 옵션을 사용하면 동일한 제안 내의 계획에서 동일한 기술 구성 설정을 사용할 수 있으므로 동일한 이미지 집합을 활용할 수 있습니다. 기술 구성 _재사용_ 옵션을 사용하도록 설정하면 계획이 선택한 기본 계획과 동일한 기술 구성 설정을 상속합니다.  기본 계획을 변경하면 구성을 다시 사용할 계획에 변경 내용이 반영됩니다.

다른 계획에서 기술 구성 설정을 다시 사용할 수 있는 몇 가지 일반적인 이유는 다음과 같습니다.

1. *종량제* 및 *BYOL* 모두에 동일한 이미지를 사용할 수 있습니다.
2. 다른 가격으로 프라이빗 플랜에 대한 공용 플랜의 동일한 기술 구성을 다시 사용합니다. 
3. 솔루션은 사용자가 배포하도록 선택한 계획에 따라 다르게 동작합니다. 예를 들어 소프트웨어는 동일하지만 기능은 계획에 따라 다릅니다.

[AZURE IMDS(Instance Metadata Service)를](../virtual-machines/windows/instance-metadata-service.md) 활용하여 라이선스의 유효성을 검사하거나 적절한 기능을 사용하도록 설정하기 위해 솔루션이 배포되는 계획을 식별합니다.

나중에 계획 간에 다른 변경 내용을 게시하기로 결정한 경우 분리할 수 있습니다. 계획에서 이 옵션을 선택 취소하여 기술 구성을 다시 사용하는 계획을 분리합니다. 분리되면 계획이 마지막 설정 대신 동일한 기술 구성 설정을 수행하고 계획이 구성에서 달라질 수 있습니다. 과거에 독립적으로 게시된 계획은 나중에 기술 구성을 다시 사용할 수 없습니다.
...
- **확장 지원**: 이 플랜의 이미지가 확장을 지원하는 경우, 이 속성을 사용하도록 설정합니다. 확장은 Azure VM에서 배포 후 구성 및 자동화를 제공하는 작은 애플리케이션입니다. [Azure 가상 머신 확장](./azure-vm-certification-faq.yml#vm-extensions)에 대해 자세히 알아보세요.
...
- **원격 데스크톱 또는 SSH 사용** 안 함: 이러한 이미지와 함께 배포된 가상 머신에서 고객이 원격 데스크톱 또는 SSH를 사용하여 액세스할 수 없는 경우 이 속성을 사용하도록 설정합니다. [잠긴 VM 이미지](./azure-vm-certification-faq.yml#locked-down-or-ssh-disabled-offer)에 대해 자세히 알아보세요.

- **배포용 사용자 지정 ARM 템플릿 필요**:이 플랜의 이미지를 사용자 지정 ARM 템플릿만 사용하여 배포할 수 있는 경우, 이 속성을 사용하도록 설정합니다. 자세히 알아보려면 [가상 컴퓨터 인증 문제 해결의 사용자 지정 템플릿 섹션](./azure-vm-certification-faq.yml#custom-templates)을 참조하세요.
...
> [!NOTE]
> 게시된 세대에는 고객이 사용할 수 있는 이미지 버전이 하나 이상 있어야 합니다. 전체 계획(모든 세대 및 이미지와 함께)을 제거하려면 계획 **개요** 페이지에서 **계획 사용 안 함(이** 문서의 첫 번째 섹션 참조)을 선택합니다.
...
- **OS VHD 링크:** Azure Compute 갤러리에 저장된 이미지(이전의 Shared Image Gallery)입니다. [Azure Compute 갤러리에서](azure-vm-create-using-approved-base.md#capture-image)이미지를 캡처하는 방법을 알아봅니다.
...
> [!NOTE]
> SAS를 사용하여 이미지를 제공하고 데이터 디스크가 있는 경우, SAS URI로도 제공해야 합니다. 공유 이미지를 사용하는 경우 Azure Compute 갤러리에서 이미지의 일부로 캡처됩니다. 제안이 Azure Marketplace 게시되면 Azure Storage 또는 Azure Compute 갤러리에서 이미지를 삭제할 수 있습니다.
...
- [CSP를 통한 재판매](azure-vm-resell-csp.md)
