...
| `endpoint` | 문자열 | 스토리지 계정 이름과 를 결합하여 스토리지 계정 엔드포인트 URL을 만드는 데 사용되는 스토리지 서비스의 엔드포인트 접미사입니다. `endpoint` 스토리지 계정 URL 예: `https://<storage-account-name>.blob.core.windows.net` . | | `core.windows.net` |
| `protocol` | 문자열 | 컨테이너에 연결하는 데 사용할 프로토콜입니다. | `https`, `wasbs` | `https` |
