---
title: Go용 Azure HDInsight SDK
description: Go 및 Apache Hadoop 클러스터용 Azure HDInsight SDK 사용에 대한 참조 자료
...
Go용 HDInsight SDK는 HDInsight 클러스터 관리를 위한 클래스 및 함수를 제공합니다. 여기에는 HDInsight 클러스터의 속성 만들기, 삭제, 업데이트, 나열, 크기 조정, 스크립트 작업 실행, 모니터링, 가져오기 작업을 포함합니다.

> [!NOTE]  
>이 SDK에 대한 GoDoc 참조 자료도 [여기](https://godoc.org/github.com/Azure/azure-sdk-for-go/services/preview/hdinsight/mgmt/2015-03-01-preview/hdinsight)서 사용할 수 있습니다.

Azure 구독이 아직 없는 경우 시작하기 전에 [무료 계정](https://azure.microsoft.com/free/?WT.mc_id=A261C142F)을 만듭니다.

## <a name="prerequisites"></a>사전 요구 사항

* [`go get` 도구](https://github.com/golang/go/wiki/GoGetTools).
...
Azure 구독을 사용해서 SDK를 먼저 인증해야 합니다.  아래 예제에 따라 서비스 주체를 만들고 이를 인증에 사용합니다. 이 작업이 완료되면 관리 작업 수행을 위해 사용할 수 있는 여러 함수(아래 섹션 참조)가 포함된 `ClustersClient` 인스턴스가 준비됩니다.
...
> [!IMPORTANT]  
> Azure Portal을 통해 HDInsight Cluster를 만드는 등 다른 함수를 사용하여 HDInsight Resource Provider를 등록하지 않은 경우, 이 작업을 먼저 수행해야 인증할 수 있습니다. 이 작업은 [Azure Cloud Shell](https://shell.azure.com/bash)에서 다음 명령을 실행하여 수행할 수 있습니다.
>```azurecli-interactive
...
이제 다음 명령을 실행하여 스토리지 계정에 대한 키를 가져옵니다(클러스터를 만드는 데 필요함).
...
---
아래의 Go 코드 조각은 2개의 헤드 노드와 1개의 작업자 노드로 구성된 Spark 클러스터를 만듭니다. 주석 설명에 따라 빈 변수를 채우고 특정 요구에 따라 다른 매개변수를 변경합니다.
...
`get`를 사용하여 클러스터를 성공적으로 생성했는지 확인할 수 있습니다.
...
> [!NOTE]  
> OMS 모니터링을 사용하려면 기존 Log Analytics 작업 영역이 있어야 합니다. 아직 만들지 않았으면 [Azure Portal에서 Log Analytics 작업 영역 만들기](../azure-monitor/logs/quick-create-workspace.md)에서 이를 수행하는 방법을 확인할 수 있습니다.
...
[GoDoc 참조 자료](https://godoc.org/github.com/Azure/azure-sdk-for-go/services/preview/hdinsight/mgmt/2015-03-01-preview/hdinsight)를 탐색합니다. GoDocs는 SDK의 모든 함수에 대한 참조 설명서를 제공합니다.
