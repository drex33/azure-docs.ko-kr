...
**ServiceDefinition.csdef** 파일은 클라우드 서비스를 구성하기 위해 Azure에서 사용되는 설정을 지정합니다. [Azure 서비스 정의 스키마(.csdef 파일)](schema-csdef-file.md) 는 서비스 정의 파일에 허용되는 형식을 제공합니다. 다음 예제에서는 웹 및 작업자 역할에 정의할 수 있는 설정을 보여 줍니다.
...
클라우드 서비스에 대한 설정 구성은 **ServiceConfiguration.cscfg** 파일의 값에 따라 결정됩니다. 이 파일의 각 역할에 배포하려는 인스턴스 수를 지정합니다. 서비스 정의 파일에서 정의한 구성 설정에 대한 값이 서비스 구성 파일에 추가됩니다. 클라우드 서비스와 연결된 모든 관리 인증서의 지문도 파일에 추가됩니다. [Azure 서비스 구성 스키마(.cscfg 파일)](schema-cscfg-file.md) 에서 서비스 구성 파일에 허용되는 형식을 제공합니다.
...
여기에 사용되는 XML 스키마를 더 잘 이해하려면 [서비스 구성 스키마](schema-cscfg-file.md)를 참조하면 됩니다. 그러나 여기서 간략하게 요소를 설명합니다.
...
Azure 런타임 라이브러리는 Microsoft.WindowsAzure.ServiceRuntime 네임스페이스를 포함하여 역할에서 Azure 환경과 상호 작용하기 위한 클래스를 제공합니다. RoleEnvironment 클래스는 구성 변경 전후 발생하는 다음 이벤트를 정의합니다.
...
**CSPack** 은 서비스 정의 파일 및 서비스 구성 파일의 콘텐츠를 사용하여 패키지의 콘텐츠를 정의합니다. **CSPack** 은 [Azure Portal](../cloud-services/cloud-services-how-to-create-deploy-portal.md#create-and-deploy)을 사용하여 Azure에 업로드할 수 있는 애플리케이션 패키지 파일(.cspkg)을 생성합니다. 기본적으로 패키지의 이름은 `[ServiceDefinitionFileName].cspkg`이지만 **CSPack** 의 `/out` 옵션을 사용하여 다른 이름을 지정할 수 있습니다.
