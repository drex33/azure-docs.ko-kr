...
| **경고함** | Azure 구독은 경고 상태에 있으며 경고 이유가 해결되지 않으면 곧 비활성화됩니다. 구독 기간이 지났거나, 사용자가 취소했거나, 구독이 만료된 경우 구독이 경고 상태일 수 있습니다.<br><br>리소스를 검색하거나 삭제할 수 있지만(GET/DELETE) 리소스를 만들 수 없습니다(PUT/PATCH/POST). |
