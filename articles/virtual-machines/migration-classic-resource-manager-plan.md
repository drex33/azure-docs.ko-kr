...
4. 마이그레이션 API로 지원되는 시나리오는 무엇인가요?  [지원되지 않는 기능 및 구성](migration-classic-resource-manager-overview.md)을 검토하세요.
...
- 플랫폼에서 지원하는 마이그레이션 API가 모든 시나리오를 설명한다고 가정 지원되는 시나리오를 이해하려면 [지원되지 않는 기능 및 구성](migration-classic-resource-manager-overview.md)을 참조하세요.
...
**환경 복제 및 테스트 마이그레이션 수행**
  ...
  정확한 시나리오(컴퓨팅, 네트워킹 및 스토리지)에 대한 랩 테스트를 수행하는 것이 원활한 마이그레이션을 보장하는 가장 좋은 방법입니다. 이렇게 하면 다음과 같은 이점을 얻을 수 있습니다.
...
다음은 다양한 대규모 마이그레이션에서 발견된 문제입니다. 전체적인 목록이 아니므로 자세한 내용에 대해서는 [지원되지 않는 기능 및 구성](migration-classic-resource-manager-overview.md)을 참조해야 합니다. 이러한 기술적인 문제가 발생할 수도 있고 그렇지 않을 수도 있지만, 마이그레이션을 시도하기 전에 문제를 해결하면 더 원활한 환경을 갖출 수 있습니다.
