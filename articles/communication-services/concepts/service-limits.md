...
ms.subservice: data
ms.openlocfilehash: 35edb99fd779969eee852bcb178928e3f370078d
ms.sourcegitcommit: 66b6e640e2a294a7fbbdb3309b4829df526d863d
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/01/2021
ms.locfileid: "133368386"
---
...
전화 번호를 얻기 전에 구독이 [지리적 및 구독](./telephony/plan-solution.md) 요구 사항을 충족 하는지 확인 하세요. 그렇지 않으면 전화 번호를 구입할 수 없습니다. 다음 제한은 [전화 번호 SDK](./reference.md) 및 [Azure Portal](https://portal.azure.com/)를 통해 구매 번호에 적용 됩니다.
...
자세한 내용은 [전화 번호 유형](./telephony/plan-solution.md) 개념 페이지 및 [전화 통신 개념](./telephony/telephony-concept.md) 개요 페이지를 참조 하세요.
...
SMS SDK 및 서비스에 대 한 자세한 내용은 sms [sdk 개요](./sms/sdk-features.md) 페이지 또는 [sms FAQ](./sms/sms-faq.md) 페이지를 참조 하십시오.
...
|메시지 업데이트         |               |60             |30 |
|메시지 업데이트         |                   |180            |60 | 
|스레드를 얻습니다.             |스레드당 사용자당    |5              |10 |
|스레드를 얻습니다.             |사용자 단위           |5          |20 |
|스레드를 얻습니다.            |사용자 단위           |5              |40 |
|UpdateThreadRoster     |스레드당             |300            |25 |
|UpdateThreadRoster   |사용자 단위               |60           |25 |
|UpdateThreadRoster   |스레드당 사용자당  |1          |- |
...
|참가자 배치 - AddParticipant|200 |

### <a name="action-to-take"></a>수행할 작업

모든 채팅 SDK에는 지수 백오프가 기본적으로 설정되어 있는 재시도 정책이 있습니다(원하는 경우 재정의할 수 있습니다). API는 메시지 보내기를 다시 시도하기 전에 대기하도록 제안된 시간(초)을 나타내는 retry-after 헤더를 반환합니다.

채팅 SDK 및 서비스에 대한 자세한 내용은 [채팅 SDK 개요](./chat/sdk-features.md) 페이지를 참조하세요.

## <a name="voice-and-video-calling"></a>음성 및 비디오 통화

### <a name="call-maximum-limitations"></a>호출 최대 제한 사항
...
음성 및 비디오 통화 SDK 및 서비스에 대한 자세한 내용은 [통화 SDK 개요](./voice-video-calling/calling-sdk-features.md) 페이지 또는 [알려진 문제를](./known-issues.md)참조하세요.

## <a name="teams-interoperability-and-microsoft-graph"></a>Teams 상호 운용성 및 Microsoft Graph
Teams 상호 운용성 시나리오를 사용하는 경우 일부 Microsoft Graph API를 사용하여 회의를 만들 수 [있습니다.](/graph/cloud-communications-online-meetings.md)  

Microsoft Graph 통해 제공되는 각 서비스에는 서로 다른 제한 사항이 있습니다. 서비스별 제한은 여기에 자세히 [설명되어](/graph/throttling.md) 있습니다.

### <a name="action-to-take"></a>수행할 작업
오류 처리를 구현하는 경우 HTTP 오류 코드 429를 사용하여 제한을 검색합니다. 실패한 응답에는 ```Retry-After``` 응답 헤더가 포함됩니다. 클라이언트가 제한되는 동안 Microsoft Graph 리소스 사용량을 계속 기록하기 때문에 지연을 사용하여 요청을 백업하는 ```Retry-After``` 것이 제한에서 복구하는 가장 빠른 방법입니다.

Microsoft Graph [제한](/graph/throttling.md) 제한에 대한 자세한 내용은 Microsoft [Graph](/graph/overview.md) 설명서에서 확인할 수 있습니다.

## <a name="network-traversal"></a>Network Traversal

| 작업(Operation) | 시간 프레임(초) | 제한(요청 수) |
|---|--|--|
...
자세한 내용은 네트워크 [순회 개념 개요](./network-traversal.md) 페이지를 참조하세요.

## <a name="still-need-help"></a>여전히 도움이 필요하세요?
사용 가능한 [도움말 및 지원](../support.md) 옵션을 참조하세요.
