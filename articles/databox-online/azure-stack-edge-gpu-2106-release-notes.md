---
title: Azure Stack Edge 2106 릴리스 정보
description: 2106 릴리스가 실행 중인 Azure Stack Edge에 대한 주요 미해결 문제 및 해결 방법을 설명합니다.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
다음 릴리스 정보에서는 Azure Stack Edge 디바이스용 2106 릴리스에 대한 중요한 미해결 문제와 해결된 문제를 식별합니다. 이러한 릴리스 정보는 Azure Stack Edge Pro GPU, Azure Stack Edge Pro R 및 Azure Stack Edge Mini R 디바이스에만 해당됩니다. 특정 모델에 해당하는 기능 및 문제가 설명됩니다(해당하는 경우).

릴리스 정보는 계속 업데이트되며, 해결 방법이 필요한 중대한 문제가 발견되면 해당 내용이 추가됩니다. 디바이스를 배포하기 전에 릴리스 정보에 포함된 정보를 주의 깊게 검토하세요.

이 문서는 소프트웨어 버전 번호 **2.2.1636.3457** 에 매핑되는 **Azure Stack Edge 2106** 릴리스에 적용됩니다. Azure Stack Edge 2010(2.1.1377.2170) 소프트웨어 이상을 실행 중인 경우 이 소프트웨어를 디바이스에 적용할 수 있습니다.

## <a name="whats-new"></a>새로운 기능

Azure Stack Edge 2106 릴리스에서는 다음과 같은 새로운 기능을 사용할 수 있습니다. 

- **Windows 업데이트 및 보안 수정 사항** - [Windows에 대한 LCU(최신 누적 업데이트) 및 6월 보안 수정 사항](https://support.microsoft.com/en-us/topic/june-8-2021-kb5003697-monthly-rollup-457aa997-18a0-46e9-8612-497f01ccaa54)이 Azure Stack Edge용 업데이트 패키지로 롤업되었습니다.
- **Azure Private 다중 액세스 에지 컴퓨팅에 대한 버그 수정 사항** - Azure Private MEC 배포에 대한 여러 문제가 수정되었습니다.

    - 연결 플래핑, 부팅 로그의 오류, 다시 부팅 등 게스트 VM 상태 모니터링과 관련된 문제
    - 시간에 따른 메모리 리소스 사용량
    - Mellanox 드라이버, 펌웨어 및 도구
    - VM 관련 문제 및 네트워크 상태 검사를 디버그하는 도구
    - SR-IOV(단일 루트 I/O 가상화) VM 아웃바운드 패킷을 발생시킨 문제 또는 LAN/WAN VM NetAdapters의 트래픽을 삭제하는 문제
- **로그 수집 개선** - 이 릴리스에는 Azure Stack Edge 업데이트 시나리오와 관련된 로그 수집 향상 기능이 있습니다.



## <a name="issues-fixed-in-2106-release"></a>2106 릴리스에서 수정된 이슈

다음 테이블에는 이전 릴리스의 릴리스 정보에 언급되고 현재 릴리스에서 수정된 이슈가 나열되어 있습니다.

| 아니요. | 기능 | 문제 | 
| --- | --- | --- |
|**1.**|Azure Private MEC |VM 네트워크 어댑터 링크 상태는 부팅 시 및 주기적으로 플랩됩니다.|
|**2.**|Azure Private MEC  |VFtoPF DHCP 리디렉션 플래그는 Mellanox 네트워크 인터페이스에서 사용되는 경우 패킷이 삭제될 수 있습니다.|
|**3.**|Azure Private MEC  |Mellanox 네트워크 인터페이스 드라이버, 펌웨어 및 도구를 버전 2.60으로 업그레이드해야 합니다.|
|**4.**|VM |디바이스의 PowerShell 인터페이스를 통해 연결할 때 VM 게스트 로그의 컬렉션에 사용할 수 있는 `Get-VMInguestLogs` cmdlet이 실패합니다.|
|**5.**|Azure Private MEC  |웹 프록시가 구성된 경우 웹 프록시 바이패스 설정으로 인해 VM 프로비전이 실패합니다. |
|**6.**|Azure Private MEC |2105 업데이트 전의 MEC/NFM 배포의 경우, LAN/WAN VM NetAdapters의 트래픽을 삭제하면 드물게 문제가 발생할 수 있습니다. 2106에서는 VM이 2105 이전에 만들어졌는지 아니면 2105 릴리스 후에 만들어졌는지에 관계 없이 VM LAN/WAN 네트워크 인터페이스에서 enableIPForwarding을 true로 설정하여 이 문제를 해결할 수 있습니다.   |
|**7.**|Azure Private MEC  |SR-IOV(단일 루트 I/O 가상화) VM의 아웃바운드 패킷은 Mellanox 드라이버, SR-IOV VF(가상 함수) 및 vftopfDHCPRedirect 기능을 함께 사용하는 경우 Mellanox 네트워크 인터페이스(디바이스의 포트 5 및 포트 6)에 의해 삭제될 수 있습니다. 2106에서는 vftopfDHCPRedirect 기능을 비활성화하여 문제를 해결합니다.  |


## <a name="known-issues-in-2106-release"></a>2106 릴리스의 알려진 문제

다음 표에서는 2106 릴리스의 알려진 문제에 관해 간략하게 설명합니다.
