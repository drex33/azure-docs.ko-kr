...
연결 문자열을 가져오려면 스토리지 계정 [액세스 키 관리](../articles/storage/common/storage-account-keys-manage.md)에 표시된 단계를 수행합니다. 연결 문자열은 [Blob Storage 계정](../articles/storage/common/storage-account-overview.md#types-of-storage-accounts)이 아닌 범용 스토리지 계정의 문자열이어야 합니다.

이 연결 문자열은 바인딩 구성의 속성에 지정된 값과 이름이 일치하는 애플리케이션 설정에 저장되어야 `connection` 합니다.
...
|---------------------------|-----------------------------------------------------|--------------------------------------------|---------|
| Blob Service URI | `<CONNECTION_NAME_PREFIX>__blobServiceUri`<sup>1</sup>  | 연결하려는 Blob 서비스의 데이터 평면 URI입니다. | <storage_account_name>.blob.core.windows.net |

<sup>1은</sup> `<CONNECTION_NAME_PREFIX>__serviceUri` 별칭으로 사용할 수 있습니다. 두 별칭이 모두 제공되면 `blobServiceUri` 양식이 사용됩니다. `serviceUri`Blob, 큐 및/또는 테이블에서 전체 연결 구성을 사용하는 경우에는 폼을 사용할 수 없습니다.

> [!NOTE]
> 기본적으로 Blob 트리거는 내부적으로 Azure 큐를 사용합니다. `<CONNECTION_NAME_PREFIX>__queueServiceUri` 을 지정할 수도 있지만 이 없으면 기본 동작은 "AzureWebJobsStorage"로 정의된 연결을 사용하는 것입니다. 트리거는 이러한 큐에 사용할 연결에 [Storage 큐 데이터 기여자가](../articles/role-based-access-control/built-in-roles.md#storage-queue-data-contributor) 필요합니다.
