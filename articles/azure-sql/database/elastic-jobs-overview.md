...
탄력적 작업을 사용한 적이 없으면 [Azure SQL Database의 작업 자동화 개념에 대해 자세히 알아봅니다](job-automation-overview.md).

## <a name="create-and-configure-the-agent"></a>에이전트 만들기 및 구성

1. 빈 S0 이상의 데이터베이스를 만들거나 식별합니다. 이 데이터베이스는 탄력적 작업 에이전트를 만드는 동안 *작업 데이터베이스* 로 사용됩니다.
2. [포털](https://portal.azure.com/#create/Microsoft.SQLElasticJobAgent)에서, 또는 [PowerShell](elastic-jobs-powershell-create.md#create-the-elastic-job-agent)로 탄력적 작업 에이전트를 만듭니다.
...
1. 자격 증명 작업 실행을 *작업 데이터베이스* 에서 [PowerShell](elastic-jobs-powershell-create.md) 또는 [T-SQL](elastic-jobs-tsql-create-manage.md#create-a-credential-for-job-execution)을 사용해 만듭니다.
2. [PowerShell](elastic-jobs-powershell-create.md) 또는 [T-SQL](elastic-jobs-tsql-create-manage.md#create-a-target-group-servers)을 사용하여 대상 그룹(작업을 실행하려는 데이터베이스)을 정의합니다.
3. 작업을 실행할 각 데이터베이스에서 작업 에이전트 자격 증명을 만듭니다[(그룹의 각 데이터베이스에서 사용자(또는 역할)를 추가합니다)](logins-create-manage.md). 예를 들어 [PowerShell 자습서](elastic-jobs-powershell-create.md)를 참조합니다.
4. [PowerShell](elastic-jobs-powershell-create.md) 또는 [T-SQL](elastic-jobs-tsql-create-manage.md#deploy-new-schema-to-many-databases)을 사용하여 작업을 만듭니다.
5. [PowerShell](elastic-jobs-powershell-create.md) 또는 [T-SQL](elastic-jobs-tsql-create-manage.md#deploy-new-schema-to-many-databases)을 사용하여 작업 단계를 추가합니다.
6. [PowerShell](elastic-jobs-powershell-create.md#run-the-job) 또는 [T-SQL](elastic-jobs-tsql-create-manage.md#begin-unplanned-execution-of-a-job)을 사용하여 작업을 실행합니다.
7. 포털, [PowerShell](elastic-jobs-powershell-create.md#monitor-status-of-job-executions) 또는 [T-SQL](elastic-jobs-tsql-create-manage.md#monitor-job-execution-status)을 사용하여 작업 실행 상태를 모니터링합니다.
...
- 자격 증명은 작업 단계를 수행하는 데 필요한 최소한의 권한만 있어야 합니다. 자세한 정보는 [권한 부여 및 사용 권한](/dotnet/framework/data/adonet/sql/authorization-and-permissions-in-sql-server)을 참조하세요.
