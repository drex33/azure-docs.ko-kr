...
ms.locfileid: "132289984"
---
# <a name="micro-agent-event-collection-preview"></a>마이크로 에이전트 이벤트 수집(미리 보기)

Defender for IoT 보안 에이전트는 로컬 디바이스에서 데이터 및 시스템 이벤트를 수집하고 처리 및 분석을 위해 데이터를 Azure 클라우드로 보냅니다. Defender for IoT 마이크로 에이전트는 새 프로세스 및 모든 새 연결 이벤트를 포함하여 다양한 유형의 디바이스 이벤트를 수집합니다. 새 프로세스와 새 연결 이벤트는 모두 디바이스에서 자주 발생할 수 있습니다. 이 기능은 포괄적인 보안에 중요합니다. 그러나 보안 에이전트가 보내는 메시지 수가 IoT Hub 할당량 및 비용 제한을 빠르게 충족하거나 초과할 수 있습니다. 이러한 메시지 및 이벤트에는 디바이스를 보호하는 데 중요한 매우 중요한 보안 정보가 포함되어 있습니다.
...
이벤트 기반 수집기는 디바이스 내에서 해당 활동에 따라 트리거되는 수집기입니다. 예: ``a process was started in the device``.  
...
에이전트는 이미 메모리에 저장된 이벤트와 동일한 이벤트를 수집합니다. 이 컬렉션을 사용하면 에이전트가 이 특정 이벤트의 적중 횟수를 늘려 에이전트의 메모리 공간을 줄일 수 있습니다. 집계 기간이 지나면 에이전트는 발생한 이벤트 유형 각각에 대한 적중 횟수를 보냅니다. 이벤트 집계는 수집된 각 이벤트 유형의 적중 횟수에 대한 집계일 뿐입니다.
...
프로세스 이벤트의 기본 버퍼는 256개 프로세스입니다. 이 제한이 충족되면 버퍼가 순환되고 가장 오래된 프로세스 이벤트가 삭제되어 최신 처리된 이벤트를 위한 공간이 확보됩니다. 캐시 크기를 늘리라는 경고가 기록됩니다.

각 이벤트에 대해 수집되는 데이터는 다음과 같습니다.
...
네트워크 연결 이벤트의 기본 버퍼는 256입니다. 캐시가 가득 찬 경우:

- **Azure RTOS 디바이스:** 다음 컬렉션 주기가 시작될 때까지 새 네트워크 이벤트가 캐시되지 않습니다.  
...
| **애플리케이션 프로토콜** | 연결과 연결된 애플리케이션 프로토콜입니다. |
...
현재 SSH 및 텔넷은 완전히 지원됩니다.  

다음 데이터가 수집됩니다.

| 매개 변수 | Description|
|--|--|
| **operation** | 로그인, 로그아웃 또는 LoginFailed입니다. |
| **process_id** | Linux PID입니다. |
| **user_name** | Linux 사용자입니다. |
| **실행** | 터미널 디바이스입니다. 예를 들어 tty1입니다. 6 또는 pts/n. |
| **remote_address** | 연결 원본으로, IPv6 또는 IPv4 형식의 원격 IP 또는 로컬 연결을 나타내는 127.0.0.1/0.0.0.0입니다. |

> [!Note]
> 사용자가 실제로 로그인하기 전에 디바이스에서 터미널을 열 때 로그인 이벤트가 캡처됩니다. 이 이벤트에는 TTY 프로세스, 로그인 이벤트 유형 및 사용자 이름이 있습니다. 예: `LOGIN`.
