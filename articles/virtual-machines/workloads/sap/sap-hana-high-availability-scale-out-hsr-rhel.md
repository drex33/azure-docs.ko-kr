...
description: hsr (HANA system replication) 및 RHEL (Pacemaker on Red Hat Enterprise Linux)를 사용 하 여 확장 SAP HANA
author: rdeltcheva
manager: juergent
...
이 문서에서는 스케일 아웃 구성에서 항상 사용 가능한 SAP HANA 시스템을 배포 하는 방법을 설명 합니다. 특히이 구성은 Azure Red Hat Enterprise Linux vm (가상 머신)에서 hsr (HANA system replication) 및 Pacemaker를 사용 합니다. [Azure NetApp Files](../../../azure-netapp-files/azure-netapp-files-introduction.md) 는 제공 된 아키텍처에서 공유 파일 시스템을 제공 하며 이러한 파일 시스템은 NFS (네트워크 파일 시스템)를 통해 탑재 됩니다.  

구성 및 설치 명령 예에서는 HANA 인스턴스가이 `03` 고 hana 시스템 ID는 `HN1` 입니다. 예제는 HANA 2.0 SP4 및 SAP 7.6 용 Red Hat Enterprise Linux (RHEL)를 기반으로 합니다. 

일부 독자는이 문서의 항목을 계속 진행 하기 전에 다양 한 SAP 노트와 리소스를 제공 하는 것이 좋습니다. 이러한 리소스는이 문서의 끝 부분에 있는 "추가 리소스" 섹션에서 확인할 수 있습니다.

## <a name="overview"></a>개요

Hana 스케일 아웃 설치를 위한 HANA 고가용성을 위해 HANA 시스템 복제를 구성 하 고 자동 장애 조치 (failover)를 허용 하도록 Pacemaker 클러스터를 사용 하 여 솔루션을 보호할 수 있습니다. 활성 노드에 오류가 발생하면 클러스터는 HANA 리소스를 다른 사이트로 장애 조치(failover)합니다.  

다음 다이어그램에는 각 사이트에 세 개의 HANA 노드가 있고 "분할-두뇌" 시나리오를 방지 하기 위한 과반수 작성자 노드가 있습니다. 추가 Vm을 HANA DB 노드로 포함 하도록 지침을 적용할 수 있습니다.  

[Azure NetApp Files](../../../azure-netapp-files/azure-netapp-files-introduction.md) 는 HANA 공유 파일 시스템을 제공 `/hana/shared` 합니다. 동일한 HANA 시스템 복제 사이트의 각 HANA 노드에 NFS v 4.1을 통해 탑재 됩니다. 파일 시스템 `/hana/data` 및 `/hana/log` 은 로컬 파일 시스템 이며 HANA DB 노드 간에 공유 되지 않습니다. SAP HANA는 비공유 모드로 설치됩니다. 

> [!TIP]
> 권장 SAP HANA 스토리지 구성은 [SAP HANA Azure VM 스토리지 구성](./hana-vm-operations-storage.md)을 참조하세요.   

[![HSR 및 Pacemaker 클러스터를 사용 하는 SAP HANA 확장의 다이어그램입니다.](./media/sap-hana-high-availability-rhel/sap-hana-high-availability-scale-out-hsr-rhel.png)](./media/sap-hana-high-availability-rhel/sap-hana-high-availability-scale-out-hsr-rhel-detail.png#lightbox)

위의 다이어그램은 SAP HANA 네트워크 권장 사항에 따라 하나의 Azure virtual network 내에 표시 되는 세 개의 서브넷을 보여 줍니다. 
...
1. Azure VM을 배포합니다. 이 구성에서는 7 개의 가상 컴퓨터를 배포 합니다. 
   
   - Hana 복제 사이트 1에 대 한 HANA DB 노드로 사용할 3 개의 가상 머신: **hana-s1-db1**, **hana-s1** - **db3**.  
   - Hana 복제 사이트 2: hana-s2- **db1**, **hana-s2** - **DB3** 에 대 한 hana DB 노드로 사용할 3 개의 가상 머신  
   - 과반수 작성자 역할을 하는 작은 가상 머신: **hana-s-mm**.

   SAP DB HANA 노드로 배포 된 Vm은 [SAP HANA 하드웨어 디렉터리](https://www.sap.com/dmc/exp/2014-09-02-hana-hardware/enEN/iaas.html#categories=Microsoft%20Azure)에 게시 된 대로 SAP에서 hana로 인증 되어야 합니다. HANA DB 노드를 배포 하는 경우 [가속화 된 네트워크](../../../virtual-network/create-vm-accelerated-networking-cli.md)를 선택 해야 합니다.  
  
   과반수 작성자 노드의 경우이 VM이 SAP HANA 리소스를 실행 하지 않으므로 작은 VM을 배포할 수 있습니다. 대다수 작성자 VM은 분할-두뇌 시나리오에서 클러스터 노드 수와 홀수를 얻기 위해 클러스터 구성에서 사용 됩니다. 이 예제에서는 주 결정자 VM이 `client` 서브넷에서 하나의 가상 네트워크 인터페이스만 필요로 합니다.        
   ...
   `client` 가상 네트워크 서브넷의 각 VM별 기본 네트워크 인터페이스를 배포합니다. Azure Portal을 통해 VM을 배포하는 경우 네트워크 인터페이스 이름이 자동으로 생성됩니다. 이 문서에서는 자동으로 생성 된, 기본 네트워크 인터페이스를 **hana-db1**, **hana**-s1-db3, hana- **s1-** 클라이언트 등으로 참조 합니다. 이러한 네트워크 인터페이스는 `client` Azure virtual network 서브넷에 연결 됩니다.  

   > [!IMPORTANT]
   > 선택한 운영 체제에서 사용 중인 특정 VM 유형에 대해 SAP HANA에 대해 SAP 인증을 수행 해야 합니다. 이러한 유형에 대 한 SAP HANA 인증 VM 유형 및 운영 체제 릴리스 목록은 [SAP HANA 인증 된 IaaS 플랫폼](https://www.sap.com/dmc/exp/2014-09-02-hana-hardware/enEN/iaas.html#categories=Microsoft%20Azure)을 참조 하세요. 나열 된 VM 형식의 세부 정보를 자세히 분석 하 여 해당 형식에 대 한 SAP HANA 지원 되는 운영 체제 릴리스의 전체 목록을 가져옵니다.  
...
1. 새로 만든 가상 네트워크 인터페이스를 해당 가상 머신에 연결합니다.  
    ...
    1. 왼쪽 창에서 **Virtual Machines** 을 선택 합니다. 가상 머신 이름(예: **hana-s1-db1**)을 필터링한 다음, 가상 머신을 선택합니다.  

    1. **개요** 창에서 **중지** 를 선택 하 여 가상 컴퓨터의 할당을 취소 합니다.  

    1. **네트워킹** 을 선택하고 네트워크 인터페이스를 연결합니다. **네트워크 인터페이스 연결** 드롭다운 목록에서 및 서브넷에 대해 이미 생성 된 네트워크 인터페이스를 선택 합니다 `inter` `hsr` .  
    
    1. **저장** 을 선택합니다. 
 
    1. 나머지 가상 머신도 b~e단계를 반복합니다(이 예에서는 **hana-s1-db2**, **hana-s1-db3**, **hana-s2-db1**, **hana-s2-db2**, **hana-s2-db3**).
 
    1. 지금은 가상 컴퓨터를 중지 됨 상태로 둡니다.   

1. [](../../../virtual-network/create-vm-accelerated-networking-cli.md) `inter` `hsr` 다음을 수행 하 여 및 서브넷에 대 한 추가 네트워크 인터페이스에 대해 가속화 된 네트워킹을 사용 하도록 설정 합니다.  

    1. [Azure Portal](https://portal.azure.com/#home)에서 [Azure Cloud Shell](https://azure.microsoft.com/features/cloud-shell/)을 엽니다.  

    1. 다음 명령을 실행 하 여 및 서브넷에 연결 된 추가 네트워크 인터페이스에 가속화 된 네트워킹을 사용 하도록 설정 `inter` `hsr` 합니다.  
...
1. HANA DB 가상 컴퓨터를 시작 합니다.

### <a name="deploy-azure-load-balancer"></a>Azure Load Balancer 배포

표준 부하 분산 장치를 사용 하는 것이 가장 좋습니다. 방법은 다음과 같습니다.
...
1. 부하 분산 규칙 만들기:
   ...
   > [!IMPORTANT]
   > 부동 IP는 부하 분산 시나리오의 NIC 보조 IP 구성에서 지원 되지 않습니다. 자세한 내용은 [Azure Load Balancer 제한 사항](../../../load-balancer/load-balancer-multivip-overview.md#limitations)을 참조 하세요. VM에 대 한 추가 IP 주소가 필요한 경우 두 번째 NIC를 배포 합니다.    
   
표준 부하 분산 장치를 사용 하는 경우 다음과 같은 제한 사항을 알고 있어야 합니다. 내부 부하 분산 장치의 백 엔드 풀에 공용 IP 주소 없이 Vm을 배치한 경우 아웃 바운드 인터넷 연결이 없습니다. 공용 끝점으로의 라우팅을 허용 하려면 추가 구성을 수행 해야 합니다. 자세한 내용은 [SAP 고가용성 시나리오에서 Azure Standard Load Balancer를 사용하는 가상 머신에 대한 퍼블릭 엔드포인트 연결](./high-availability-guide-standard-load-balancer-outbound-connections.md)을 참조하세요.  

Azure Load Balancer 뒤에 배치 되는 Azure Vm에서 TCP 타임 스탬프를 사용 하도록 설정 하지 마세요. TCP 타임 스탬프를 사용 하도록 설정 하면 상태 프로브가 실패 합니다. 매개 변수를 `net.ipv4.tcp_timestamps` 로 설정 `0` 합니다. 자세한 내용은 [Load Balancer 상태 프로브](../../../load-balancer/load-balancer-custom-probe-overview.md) 및 SAP note [2382421](https://launchpad.support.sap.com/#/notes/2382421)을 참조 하세요.  

### <a name="deploy-the-azure-netapp-files-infrastructure"></a>Azure NetApp Files 인프라 배포 

파일 시스템에 대 한 Azure NetApp Files 볼륨을 배포 `/hana/shared` 합니다. `/hana/shared`각 HANA 시스템 복제 사이트 마다 별도의 볼륨이 필요 합니다. 자세한 내용은 [Azure NetApp Files 인프라 설정](./sap-hana-scale-out-standby-netapp-files-rhel.md#set-up-the-azure-netapp-files-infrastructure)을 참조하세요.

이 예제에서는 다음 Azure NetApp Files 볼륨을 사용 합니다. 
...
다음을 수행 하 여 운영 체제를 구성 하 고 준비 합니다.

1. **[A]** 가상 머신에서 호스트 파일을 유지 관리합니다. 모든 서브넷에 대한 항목을 포함합니다. 이 예제에서는 다음 항목이에 추가 됩니다 `/etc/hosts` .  
...
1. **[A]** SAP HANA 실행을 위해 운영 체제를 준비 합니다. 자세한 내용은 [Azure NetApp Files를 사용하는 Microsoft Azure의 NetApp SAP 애플리케이션][anf-sap-applications-azure]을 참조하세요. Azure NetApp Files 구성 설정에 대 한 구성 파일 */etc/sysctl.d/netapp-hana.conf* 를 만듭니다.  
...
1. **[A]** 추가 최적화 설정을 사용하여 */etc/sysctl.d/ms-az.conf* 구성 파일을 만듭니다.  
    ...
    > [!TIP]
    > `net.ipv4.ip_local_port_range` `net.ipv4.ip_local_reserved_ports` `sysctl` SAP 호스트 에이전트가 포트 범위를 관리할 수 있도록 구성 파일에서 및을 명시적으로 설정 하지 마십시오. 자세한 내용은 SAP note [2382421](https://launchpad.support.sap.com/#/notes/2382421)을 참조 하세요.  

1. **[A]** `sunrpc` [Azure NetApp Files를 사용 하 Microsoft Azure netapp SAP 응용 프로그램][anf-sap-applications-azure]에서 권장 하는 대로 설정을 조정 합니다.  
...
1. **[AH]** HANA 구성을 위한 Red Hat입니다.  
   ...
   [Red Hat 고객 포털](https://access.redhat.com/solutions/2447641) 및 다음 SAP 참고 사항에 설명 된 대로 RHEL를 구성 합니다.
...
다음 섹션에서는 파일 시스템을 준비 하는 단계를 제공 합니다.

### <a name="mount-the-shared-file-systems"></a>공유 파일 시스템 탑재

이 예제에서 공유 HANA 파일 시스템은 Azure NetApp Files에 배포 되 고 NFS v4를 통해 탑재 됩니다.  
...
1. **[AH]** NFS 도메인 설정을 확인합니다. 도메인이 기본 Azure NetApp Files 도메인으로 구성 되어 있는지 확인 `defaultv4iddomain.com` 합니다. 매핑이로 설정 되었는지 확인 `nobody` 합니다.  
   (이 단계는 Azure NetAppFiles NFS v 4.1을 사용 하는 경우에만 필요 합니다.)  

    > [!IMPORTANT]
    > VM의 `/etc/idmapd.conf`에서 NFS 도메인을 Azure NetApp Files의 기본 도메인 구성( `defaultv4iddomain.com` )과 일치하도록 설정해야 합니다. NFS 클라이언트와 NFS 서버의 도메인 구성이 일치 하지 않는 경우 Vm에 탑재 된 Azure NetApp 볼륨의 파일에 대 한 사용 권한이로 표시 됩니다 `nobody` .  
...
1. **[AH]** `nfs4_disable_idmapping`을 확인합니다. `Y`로 설정되어야 합니다. `nfs4_disable_idmapping`이 있는 디렉터리 구조를 만들려면 mount 명령을 실행합니다. 액세스는 커널 또는 드라이버용으로 예약 되어 있기 때문에 */dv/sd* 아래에 있는 디렉터리는 수동으로 만들 수 없습니다.  
   ...
   매개 변수를 변경 하는 방법에 대 한 자세한 내용은 `nfs4_disable_idmapping` [Red Hat 고객 포털](https://access.redhat.com/solutions/1749883)을 참조 하세요.
...
1. **[AH]** `/hana/shared/` NFS 프로토콜 버전 **NFSv4** 를 사용 하 여 해당 파일 시스템이 모든 HANA DB vm에 탑재 되어 있는지 확인 합니다.  
...
제공 된 구성에서는 파일 시스템과 관리 디스크를 배포 하 `/hana/data` `/hana/log` 고 이러한 파일 시스템을 각 HANA DB VM에 로컬로 연결 합니다. 각 HANA DB 가상 컴퓨터에서 로컬 데이터 및 로그 볼륨을 만들려면 다음 단계를 실행 합니다. 
...
1. **[1]** **hana-s1-db2** 및 **hana-db3** 에서 암호를 입력 하 라는 메시지가 표시 되지 않고 SSH (보안 셸)를 통해 로그인 할 수 있는지 확인 합니다. 이 경우 `ssh` [키 기반 인증 사용](https://access.redhat.com/documentation/red_hat_enterprise_linux/6/html/deployment_guide/s2-ssh-configuration-keypairs)에 설명 된 대로 교환 키가 아닌 것입니다.  
    ```bash
    ssh root@hana-s1-db2
    ssh root@hana-s1-db3
    ```

1. **[2]** db3 **에서 암호** 를 입력 하 라는 메시지가 표시 되지 않고 SSH를 통해 로그인 할 수 있는지 확인 합니다. 이 경우 `ssh` [키 기반 인증 사용](https://access.redhat.com/documentation/red_hat_enterprise_linux/6/html/deployment_guide/s2-ssh-configuration-keypairs)에 설명 된 대로 교환 키가 아닌 것입니다.  
    ```bash
...
1. **[1]** 보조 HANA 노드를 설치합니다. 이 단계의 예제 지침은 SITE 1에 해당합니다.  
   ...
   1. 프롬프트에서 다음 값을 입력합니다.
     ...
     * **SAP 호스트 에이전트 사용자 (sapadm) 암호 확인** 에 암호를 입력 합니다.
     * **호스트 hana-s1-d b 2에 대 한 인증서 호스트 이름** [hana-s1-db2]에 대해 enter 키를 눌러 기본값을 적용 합니다.
     * 호스트 hana-s1-db3 [hana-s1-db3] **에 대 한 인증서 호스트 이름** 에 대해 enter 키를 눌러 기본값을 적용 합니다.
     * **계속 하 시겠습니까 (y/n)**, 요약의 유효성을 검사 하 고 모든 항목이 양호 하면 **y** 를 입력 합니다.
...
다음 단계를 수행 하 여 시스템 복제를 설정할 수 있습니다.
...
1. **[1]** 복제 상태를 확인 하 고 모든 데이터베이스가 동기화 될 때까지 기다립니다.
...
1. **[1, 2]** hana 시스템 복제에 대 한 통신이 hana 시스템 복제 가상 네트워크 인터페이스를 통해 전달 되도록 hana 구성을 변경 합니다.   
   1. 두 사이트에서 HANA를 중지 합니다.
      ```bash
      sudo -u hn1adm /usr/sap/hostctrl/exe/sapcontrol -nr 03 -function StopSystem HDB
      ```

   1. HANA 시스템 복제에 대 한 호스트 매핑을 추가 하려면 *global.ini* 를 편집 합니다. 서브넷의 IP 주소를 사용 `hsr` 합니다.  
      ```bash
      sudo vi /usr/sap/HN1/SYS/global/hdb/custom/config/global.ini
      #Add the section
...
1. **[AH]** 방화벽을 다시 사용 하도록 설정 하 고 필요한 포트를 엽니다.  
   1. 방화벽을 다시 사용 하도록 설정 합니다.
       ```bash
       # Execute as root
   ...
   1. 필요한 방화벽 포트를 엽니다. HANA 인스턴스 번호에 대한 포트를 조정해야 합니다.  
       ...
       > [!IMPORTANT]
       > HANA 노드 간 통신 및 클라이언트 트래픽을 허용 하는 방화벽 규칙을 만듭니다. 필요한 포트는 [모든 SAP 제품의 tcp/ip 포트](https://help.sap.com/viewer/ports)에 나열 됩니다. 다음 명령은 예제입니다. 이 시나리오에서는 시스템 번호 03을 사용 합니다.
...
이 HANA 서버에 대 한 기본 Pacemaker 클러스터를 만들려면 [Azure에서 Pacemaker on Red Hat Enterprise Linux 설정](high-availability-guide-rhel-pacemaker.md)의 단계를 따르세요. 클러스터의 주 결정자를 비롯한 모든 가상 머신을 포함합니다.  
...
1. **[1]** 비활성화된 상태에서 에 대한 파일 시스템 클러스터 `/hana/shared` 리소스를 만듭니다. `--disabled`탑재를 사용하도록 설정하기 전에 위치 제약 조건을 정의해야 하므로 를 사용합니다.  
   ...
   `OCF_CHECK_LEVEL=20`특성은 모니터 작업에 추가되므로 모니터 작업에서 파일 시스템에서 읽기/쓰기 테스트를 수행합니다. 이 특성이 없으면 모니터 작업은 파일 시스템이 탑재되어 있는지만 확인합니다. 연결이 끊어지면 액세스할 수 없더라도 파일 시스템이 탑재된 상태를 유지할 수 있기 때문에 문제가 될 수 있습니다.  
...
1. **[1,2]** 각 시스템 복제 사이트의 HANA DB 노드 하나에 HANA 시스템 복제 후크를 설치합니다. SAP HANA 여전히 다운되어야 합니다.        
...
1. **[1]** HANA 클러스터 리소스를 만듭니다. 다음 명령을 로 `root` 실행합니다.    
   ...
   1. HANA 인스턴스 리소스를 만듭니다.  
      ...
      ```
      > [!IMPORTANT]
      > `AUTOMATED_REGISTER` `false` 장애 조치(failover) 테스트를 수행하는 동안 을 로 설정하여 실패한 주 인스턴스가 자동으로 보조 인스턴스로 등록되지 않도록 하는 것이 좋습니다. 테스트 후에는 인수 후 시스템 복제가 자동으로 다시 시작될 수 있도록 를 로 설정하는 `AUTOMATED_REGISTER` `*true` 것이 좋습니다. 
...
SAP HANA 2.0 SPS 01부터 SAP는 SAP HANA 시스템 복제를 위한 활성/읽기 가능 설정을 허용합니다. 이 기능을 사용하면 읽기 집약적인 워크로드에 SAP HANA 시스템 복제의 보조 시스템을 적극적으로 사용할 수 있습니다. 클러스터에서 이러한 설정을 지원하려면 클라이언트가 보조 읽기 사용 SAP HANA 데이터베이스에 액세스할 수 있는 두 번째 가상 IP 주소가 필요합니다. 인수가 발생한 후에도 보조 복제 사이트에 계속 액세스할 수 있도록 하려면 클러스터에서 가상 IP 주소를 SAP HANA 리소스의 보조와 함께 이동해야 합니다.

이 섹션에서는 두 번째 가상 IP 주소를 사용하여 Red Hat 고가용성 클러스터에서 이러한 유형의 시스템 복제를 관리하기 위해 수행해야 하는 추가 단계를 설명합니다.  

계속하기 전에 이 문서의 앞부분에서 설명한 대로 SAP HANA 데이터베이스를 관리하는 Red Hat 고가용성 클러스터를 완전히 구성했는지 확인합니다.  
...
### <a name="additional-setup-in-azure-load-balancer-for-activeread-enabled-setup"></a>활성/읽기 사용 설정에 대한 Azure Load Balancer 추가 설정

두 번째 가상 IP 프로비저닝을 계속하려면 배포 Azure Load Balancer 에 설명된 대로 [Azure Load Balancer](#deploy-azure-load-balancer)구성했는지 확인합니다.

*표준* 부하 분산의 경우 이전 섹션에서 만든 것과 동일한 부하 분산에서 이러한 추가 단계를 수행합니다.

1. 두 번째 프런트 엔드 IP 풀 만들기: 

   1. 부하 분산 장치를 열고, **프런트 엔드 IP 풀** 을 선택하고, **추가** 를 선택합니다.
   1. 두 번째 프런트 엔드 IP 풀의 이름을 입력합니다(예: *hana-secondaryIP*).
   1. **할당을** **정적** 로 설정하고 IP 주소(예: **10.23.0.19)를** 입력합니다.
   1. **확인** 을 선택합니다.
   1. 새 프런트 엔드 IP 풀을 만든 후, 풀 IP 주소를 적어 둡니다.
...
HANA 시스템 복제를 구성하는 단계는 SAP HANA [2.0 시스템 복제 구성](#configure-sap-hana-20-system-replication) 섹션에 설명되어 있습니다. 읽기 가능 보조 시나리오를 배포 하는 경우 두 번째 노드에서 시스템 복제를 구성 하는 동안 **hanasid** adm으로 다음 명령을 실행 합니다.
...
이 문서에서 설명 하는 자료에 대 한 추가 배경이 필요할 수 있는 독자의 혜택을 위해 다음 리소스를 수집 했습니다.

* SAP note [1928533] 에는 다음이 포함 됩니다.  
  * SAP 소프트웨어 배포에 지원 되는 Azure VM 크기 목록입니다.
  * Azure VM 크기에 대한 중요한 용량 정보.
  * 지원 되는 SAP 소프트웨어 및 운영 체제 및 데이터베이스 조합입니다.
  * Microsoft Azure에서 Windows 및 Linux에 필요한 SAP 커널 버전.
* SAP note [2015553]: AZURE에서 SAP 지원 sap 소프트웨어 배포에 대 한 필수 조건을 나열 합니다.
* SAP note [2002167]: RHEL에 대 한 권장 운영 체제 설정을가지고 있습니다.
* SAP note [2009879]: RHEL에 대 한 SAP HANA 지침이 있습니다.
...
* SAP note [2243692]: Azure에서 LINUX의 sap 라이선스에 대 한 정보를 포함 합니다.
* SAP note [1999351]: Sap 용 Azure 고급 모니터링 확장에 대 한 추가 문제 해결 정보를 포함 합니다.
...
* [Linux에서 SAP 용 Azure Virtual Machines 계획 및 구현][planning-guide]입니다.
* [Linux에서 SAP 용 Azure Virtual Machines 배포][deployment-guide]
* [Linux에서 SAP 용 Azure VIRTUAL MACHINES DBMS 배포][dbms-guide].
* [네트워크 요구 사항을 SAP HANA](https://www.sap.com/documents/2016/08/1cd2c2fb-807c-0010-82c7-eda71af511fa.html)합니다.
* 일반 RHEL 설명서:
  ...
  * 고가용성 [추가 기능 참조](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html/high_availability_add-on_reference/index)입니다.
  * [Red Hat Enterprise Linux 네트워킹 가이드를 참조](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html/networking_guide)하세요.
  * [NFS 공유의 HANA 파일 시스템을 사용 하 여 Pacemaker 클러스터에서 확장 시스템 복제 SAP HANA 구성 어떻게 할까요?](https://access.redhat.com/solutions/5423971)합니다.
  * [활성/활성 (읽기 사용): SAP HANA 규모 확장 및 시스템 복제를 위한 RHEL HA 솔루션](https://access.redhat.com/sites/default/files/attachments/v8_ha_solution_for_sap_hana_scale_out_system_replication_1.pdf)입니다.
* Azure 특정 RHEL 설명서:
  * [Microsoft Azure에서 사용할 Red Hat Enterprise Linux SAP HANA를 설치](https://access.redhat.com/public-cloud/microsoft-azure)합니다.
  * [SAP HANA 확장 및 시스템 복제를 위한 Red Hat Enterprise Linux 솔루션](https://access.redhat.com/solutions/4386601)입니다.
* [Azure NetApp Files를 사용 하 여 Microsoft Azure에서 netapp SAP 응용 프로그램][anf-sap-applications-azure].
* [설명서를 Azure NetApp Files][anf-azure-doc]. 
* [SAP HANA에 대 한 Azure NetApp Files의 NFS v 4.1 볼륨](./hana-vm-operations-netapp.md)
