...
| 기능 | 세부 정보 |
| ---| --- |
| [중요 비즈니스용 16 TB 지원](resource-limits.md#service-tier-characteristics) | 새로운 메모리 최적화 프리미엄 시리즈 하드웨어 생성을 사용하여 중요 비즈니스용 서비스 계층의 SQL Managed Instance 최대 16 TB의 공간 할당을 지원합니다. | 
|[엔드포인트 정책](../../azure-sql/managed-instance/service-endpoint-policies-configure.md) | SQL Managed Instance 서브넷에서 액세스할 수 있는 Azure Storage 계정을 구성합니다. 의도치 않거나 악의적인 데이터 반출에 대한 추가 보호 계층을 부여합니다.
| [인스턴스 풀](instance-pools-overview.md) | 더 작은 SQL Server 인스턴스를 클라우드로 마이그레이션하는 편리하고 비용 효율적인 방법입니다. |
| [링크 기능](link-feature.md)| Azure SQL Managed Instance 어디서나 호스팅되는 SQL Server 데이터베이스의 온라인 복제. |
...
| [메모리 최적화 프리미엄 시리즈 하드웨어 생성](resource-limits.md#service-tier-characteristics) | 최신 Intel Ice Lake CPU를 활용하기 위해 새 메모리 최적화 프리미엄 시리즈 하드웨어 세대에 SQL Managed Instance 배포합니다. 메모리 최적화 하드웨어 생성은 더 높은 메모리 대 vCore 비율을 제공합니다. | 
...
| [Premium 시리즈 하드웨어 생성](resource-limits.md#service-tier-characteristics) | 새로운 프리미엄 시리즈 하드웨어 세대에 SQL Managed Instance 배포하여 최신 Intel Ice Lake CPU를 활용합니다.  | 
...
| 기능 | GA 월 | 세부 정보 |
| ---| --- |--- |
|[범용 16 TB 지원](resource-limits.md)| 2021년 11월 |  범용 서비스 계층의 SQL Managed Instance 최대 16 TB의 공간 할당을 지원합니다. |
[Azure Active Directory 전용 인증](../database/authentication-azure-ad-only-authentication.md) | 2021년 11월 |  이제 인증을 Azure SQL Managed Instance Azure Active Directory 사용자로만 제한할 수 있습니다. |
| [분산 트랜잭션](../database/elastic-transactions-overview.md) | 2021년 11월 | Azure SQL Managed Instance 대한 분산 데이터베이스 트랜잭션을 사용하면 여러 인스턴스에 걸쳐 있는 분산 트랜잭션을 실행할 수 있습니다. | 
|[연결된 서버 - 관리 ID Azure AD 인증](/sql/relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql#h-create-sql-managed-instance-linked-server-with-managed-identity-azure-ad-authentication) |2021년 11월 | Azure SQL Managed Instance 대한 관리 ID 인증을 사용하여 연결된 서버를 만듭니다.|
|[연결된 서버 - 통과 Azure AD 인증](/sql/relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql#i-create-sql-managed-instance-linked-server-with-pass-through-azure-ad-authentication) |2021년 11월 | Azure SQL Managed Instance 대한 통과 Azure AD 인증을 사용하여 연결된 서버를 만듭니다. |
| [다른 서브넷으로 인스턴스 이동](vnet-subnet-move-instance.md)| 2021년 11월 | Azure Portal, Azure PowerShell 또는 Azure CLI 사용하여 SQL Managed Instance 다른 서브넷으로 이동합니다.  |
...
| 변경 | 세부 정보 |
| --- | --- |
| **중요 비즈니스용 미리 보기에 대한 16 TB 지원** | SQL Managed Instance 중요 비즈니스용 서비스 계층은 현재 미리 보기로 제공되는 새로운 프리미엄 시리즈 및 메모리 최적화 프리미엄 시리즈 하드웨어 세대로 최대 16 TB의 최대 인스턴스 스토리지 용량을 늘렸습니다. 자세한 내용은 [리소스 제한을](resource-limits.md#service-tier-characteristics) 참조하세요. |
|**범용 GA에 대한 16 TB 지원** | 이제 범용 서비스 계층에 16 TB 인스턴스를 배포하는 것이 일반 제공됩니다.  자세한 내용은 [리소스 제한을](resource-limits.md) 참조하세요. |
| **Azure AD 전용 인증 GA** |  이제 Azure Active Directory 사용자로만 Azure SQL Managed Instance 인증을 제한할 수 있습니다. 자세한 내용은 [Azure AD 전용 인증을](../database/authentication-azure-ad-only-authentication.md)참조하세요. |
| **분산 트랜잭션 GA** | 이제 관리되는 인스턴스에서 분산 트랜잭션을 실행하는 기능을 일반적으로 사용할 수 있습니다. 자세한 내용은  [분산 트랜잭션을](../database/elastic-transactions-overview.md) 참조하세요. |
|**엔드포인트 정책 미리 보기** | 이제 엔드포인트 정책을 구성하여 SQL Managed Instance 서브넷에서 Azure Storage 계정으로 액세스를 제한할 수 있습니다. 이렇게 하면 의도치 않거나 악의적인 데이터 반출에 대한 추가 보호 계층이 부여됩니다. 자세한 내용은 [엔드포인트 정책을](../../azure-sql/managed-instance/service-endpoint-policies-configure.md) 참조하세요. |
|**링크 기능 미리 보기** | SQL Managed Instance 대한 링크 기능을 사용하여 데이터를 Azure로 이동하지 않고 Azure의 이점을 활용하고, 워크로드를 오프로드하거나, 재해 복구를 위해 또는 클라우드로 마이그레이션할 수 있도록 SQL Server 호스트된 위치에서 Azure SQL Managed Instance 데이터를 복제합니다. 자세한 내용은 [SQL Managed Instance 링크 기능을](link-feature.md) 참조하세요. 링크 기능은 현재 제한된 공개 미리 보기로 제공됩니다. |
...
| **Azure AD 전용 인증 미리 보기** | 이제 인증을 Azure SQL Managed Instance Azure Active Directory 사용자로만 제한할 수 있습니다. 이 기능은 현재 미리 보기로 제공됩니다. 자세한 내용은 [Azure AD 전용 인증을](../database/authentication-azure-ad-only-authentication.md)참조하세요. | 
...
| **Machine Learning Services GA** | Azure SQL Managed Instance Machine Learning Services는 이제 GA(일반 제공)입니다. 자세한 내용은 [SQL Managed Instance Machine Learning Services를](machine-learning-services-overview.md)참조하세요.| 
...
| **SSRS 카탈로그 데이터베이스 호스팅** | 이제 SQL Managed Instance 지원되는 모든 버전의 SQL Server Reporting Services(SSRS)에 대한 카탈로그 데이터베이스를 호스트할 수 있습니다. | 
...
| **기계 학습 지원** | R 및 Python 언어를 지원하는 Machine Learning Services에는 이제 Azure SQL Managed Instance(미리 보기)에 대한 미리 보기 지원이 포함됩니다. 자세한 내용은 [SQL Managed Instance 사용하여 기계 학습을](machine-learning-services-overview.md)참조하세요. | 
