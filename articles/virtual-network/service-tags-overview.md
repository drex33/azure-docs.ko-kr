...
| **ApiManagement** | Azure API Management 전용 배포에 대한 관리 트래픽입니다. <br/><br/>**참고**:이 태그는 지역별 제어 평면에 대 한 Azure API Management 서비스 끝점을 나타냅니다. 이렇게 하면 고객이 API Management 서비스에 구성된 API, 작업, 정책, 명명된 값에 대해 관리 작업을 수행할 수 있습니다.  | 인바운드 | 예 | 예 |
...
| **AzureArcInfrastructure** | Azure Arc 사용 서버, Azure Arc 사용 Kubernetes, 게스트 구성 트래픽<br/><br/>**참고**:이 태그는 **AzureActiveDirectory**,**AzureTrafficManager** 및 **AzureResourceManager** 태그에 종속 되어 있습니다. 이 태그는 현재 Azure Portal을 통해 구성할 수 없습니다.| 아웃바운드 | 예 | 예 |
| **AzureAttestation** | Azure Attestation.<br/><br/>**참고**:이 태그는 현재 Azure Portal를 통해 구성할 수 없습니다. | 아웃바운드 | 예 | 예 | 
| **AzureBackup** |Azure Backup<br/><br/>**참고**:이 태그는 **Storage** 및 **AzureActiveDirectory** 태그에 종속 됩니다. | 아웃바운드 | 예 | 예 |
...
### <a name="use-the-service-tag-discovery-api"></a>서비스 태그 검색 API 사용
...
>
> - 서비스 태그에 새 IP 주소가 추가되면 적어도 1주일 동안 Azure에서 사용되지 않습니다. 이렇게 하면 서비스 태그와 연결된 IP 주소를 추적해야 하는 시스템을 업데이트하는 데 시간이 걸릴 수 있습니다.
