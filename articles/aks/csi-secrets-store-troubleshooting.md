...
### <a name="keyvaultbaseclientgetsecret-failure-sending-request-statuscode0--original-error-context-canceled"></a>keyvault. BaseClient # GetSecret: 요청을 보내는 동안 오류 발생: StatusCode = 0 – 원래 오류: 컨텍스트가 취소 되었습니다. " 
...
다음 이유로 인해 공급자 pod가 AKV 인스턴스에 액세스할 수 없음을 의미 합니다.
- 공급자에서 송신 트래픽을 차단 하는 방화벽 규칙이 있습니다.
- 송신 트래픽을 차단 하는 클러스터에서 구성 된 네트워크 정책입니다.
- Pod 공급자는 hostNetwork에서 실행 됩니다. 따라서이 트래픽을 차단 하는 정책이 있거나 노드에 network jitter가 있는 경우 위의 오류가 발생할 수 있습니다. 트래픽을 차단 하 고 공급자 pod을 allowlist 구성 된 정책을 확인 합니다. 또한 노드에서 AAD 및 keyvault에 연결 되어 있는지 확인 합니다.

호스트 네트워크에서 실행 되는 pod에서 다음과 같이 Azure Key Vault 연결을 테스트할 수 있습니다.
- Pod 만들기
