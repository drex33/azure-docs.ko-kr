...
* SSMS(SQL Server Management Studio)가 설치되었습니다. SSMS를 다운로드하고 설치하려면 [SSMS(SQL Server Management Studio) 다운로드](/sql/ssms/download-sql-server-management-studio-ssms)를 참조하세요.
...
1. **데이터베이스** 를 확장하고 _contosoconcerthall_ 을 마우스 오른쪽 단추로 클릭하고 **새 쿼리** 를 선택합니다.
...
1. **개체 탐색기** 에서 **contosoconcethall** > **보기** 를 확장합니다.
   ...
   ![4가지 유형의 dbo.Venue를 포함하는 뷰 노드의 내용을 보여 주는 스크린샷](./media/saas-tenancy-cross-tenant-reporting/views.png)
...
2. SSMS에서 \학습 모듈\운영 분석\임시 보고\ _Initialize-AdhocReportingDB.sql_ 을 엽니다.
