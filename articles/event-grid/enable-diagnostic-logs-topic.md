---
title: Azure Event Grid-Event Grid 리소스에 대 한 진단 로그 사용
description: 이 문서에서는 Event Grid 리소스에 대해 진단 로그를 사용 하도록 설정 하는 방법에 대 한 단계별 지침을 제공 합니다.
ms.topic: how-to
ms.date: 11/11/2021
...
ms.locfileid: "132556288"
---
# <a name="enable-diagnostic-logs-for-event-grid-resources"></a>Event Grid 리소스에 대 한 진단 로그 사용

이 문서에서는 Event Grid 리소스에 대 한 진단 설정을 사용 하도록 설정 하는 단계별 지침을 제공 합니다.

이러한 설정을 사용 하 여 정보를 캡처하고 볼 수 있으며, 다음 표에서는 Event Grid 리소스 간 설정의 호환성을 보여 줍니다.

| 진단 설정 | Description | Event Grid 항목 | Event Grid 시스템 항목 | Event Grid 도메인 |
| ------------- | ------------- | --------- | ----------- | ----------- |
| DeliveryFailures | 배달 실패 | 예 | 예 | 예 |
| PublishFailures | 게시 실패 | Yes | 예 | 예 |
| DataPlaneRequests| 데이터 평면 요청 | Yes | 예 | 예 |
...
- 프로 비전 된 Event Grid 리소스
- 진단 로그를 캡처하기 위해 프로 비전 된 대상:
  - Log Analytics 작업 영역
  - 스토리지 계정
  - 이벤트 허브
  - 파트너 솔루션

## <a name="enable-diagnostic-logs-for-event-grid-topics-and-event-grid-domains"></a>Event Grid 토픽 및 Event Grid 도메인에 대 한 진단 로그 사용

> [!NOTE]
> 다음 절차에서는 Event Grid 항목에 대 한 진단 로그를 사용 하도록 설정 하는 단계별 지침을 제공 합니다. 도메인에 대한 진단 로그를 사용하도록 설정하는 단계는 매우 유사합니다. 2 단계에서 Azure Portal의 Event Grid **도메인** 으로 이동 합니다.  
...
## <a name="enable-diagnostic-logs-for-event-grid-system-topics"></a>Event Grid 시스템 항목에 대 한 진단 로그 사용
...
범주 그룹 **감사** 를 선택 하거나 **로그** 에서 **DataPlaneRequests** 범주를 선택 하는 경우 데이터 평면 작업에 대 한 감사 추적을 게시할 수 있습니다. 감사 추적을 사용 하 여 데이터 액세스가 인증 된 목적 으로만 허용 되는지 확인 하 고 리소스 이름, 작업 유형, 네트워크 액세스와 같은 보안 제어에 대 한 정보를 수집할 수 있습니다. Event Grid  수준, 지역 등 진단 설정을 사용 하도록 설정 하는 방법에 대 한 자세한 내용은 [Event Grid 항목 및 Event Grid 도메인의 진단 로그](enable-diagnostic-logs-topic.md#enable-diagnostic-logs-for-event-grid-topics-and-event-grid-domains)를 참조 하세요.
![감사 추적 선택](./media/enable-diagnostic-logs-topic/enable-audit-logs.png)

> [!IMPORTANT]
> 스키마에 대 한 자세한 내용은 `DataPlaneRequests` [진단 로그](diagnostic-logs.md)를 참조 하십시오.
