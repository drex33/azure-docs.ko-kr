...
이 섹션은 엔터프라이즈 관리자가 작업을 수행할 수 없는 경우에만 정보를 제공하기 위한 것입니다. 등록이 [자동 등록 전송](#auto-enrollment-transfer)에 해당되지 않는 한 엔터프라이즈 등록을 새 항목으로 전송하려면 지원 요청이 필요합니다.
...
-    RI 구매가 양도하는 등록계약에 있는 경우 RI 구매 요금은 원본 등록계약에 그대로 유지되지만, 모든 RI 혜택은 새 등록계약에서 사용할 수 있도록 양도됩니다.
-    이전 등록에서 이미 발생한 마켓플레이스 일회성 구매 요금 및 월간 고정 요금은 새 등록으로 이전되지 않습니다. 소비 기반 마켓플레이스 요금은 양도됩니다.

### <a name="effective-transfer-date"></a>발효 양도 날짜

발효 양도 날짜는 대상 등록계약의 시작 날짜 또는 이후일 수 있습니다.

원본 등록 사용량에 대한 요금은 Azure 선불 또는 초과분으로 청구됩니다. 유효 양도 날짜 이후에 발생하는 사용량은 새 등록으로 이전되어 요금이 청구됩니다.
...
등록 이전을 요청하는 지원 티켓을 제출하지 않은 경우에도 등록에 **이전됨** 상태가 표시될 수 있습니다. **이전됨** 상태는 자동 등록 이전 프로세스의 결과입니다. 갱신 문구 중에 자동 등록 이전 프로세스가 수행되려면 다음과 같은 몇 가지 항목을 새 계약에 포함해야 합니다.

- 이전 등록 번호(EA 포털에 있어야 함)
- 이전 등록 번호의 만료일은 새 계약의 유효 시작일 하루 전입니다.
- 새 계약에는 현재 날짜가 있거나 소급되어 청구된 Azure 선불 주문이 있습니다.
- EA 포털에 새 등록이 생성됩니다.

이전 등록과 새 등록 사이에 EA 포털에 누락된 사용 데이터가 없는 경우 전송 지원 티켓을 만들 필요가 없습니다.
