...
ms.openlocfilehash: 9dedc570a12b4233e69a77209bc8f602a0545c5f
ms.sourcegitcommit: e9e332a512ed615a3c8ad5a11baa21649f14116d
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/24/2021
ms.locfileid: "133097629"
---
...
3. 등록 된 데이터 원본에 대 한 검색 설정
    ...
    - **범위 검색**
        - 검색 범위를 설정 하는 동안 세분화 된 수준 또는 부모 수준과 관련 된 자산을 선택 합니다. 이렇게 하면 검색 비용이 최적이 고 성능이 효율적으로 유지 됩니다. 부모를 완전히 확인 하거나 부분적으로 선택 하는 경우 특정 부모에서 이후의 모든 자산이 자동으로 선택 됩니다. 예를 들면 다음과 같습니다.
            ...
            - Power BI 테넌트에서는 개인 작업 영역을 포함할지 아니면 제외할지 여부만 지정할 수 있습니다.
            - 일반적으로 와일드카드(예: 데이터 레이크의 경우)를 기반으로 '패턴 무시'(지원되는 경우)를 사용하여 임시, 구성 파일, RDMS 시스템 테이블 또는 백업/STG 테이블을 제외하는 것이 좋습니다.
            - 문서/비정형 데이터를 검색할 때는 검사에서 이러한 문서의 처음 20MB를 처리하고 검색 기간이 길어질 수 있기 때문에 이러한 문서를 대량으로 검사하지 않도록 합니다.

    - **검사 규칙 집합**
        - 검사 규칙 집합을 설정하는 동안 이전에 만든 관련 시스템/사용자 지정 검사 규칙 집합을 구성해야 합니다.
        - 사용자 지정 파일 형식을 만드는 옵션이 있으며 이에 따라 세부 정보를 채울 수 있습니다. 현재 Azure Purview는 사용자 지정 구분 기호에서 하나의 문자만 지원합니다. 실제 데이터에서 ~ 와 같은 사용자 지정 구분 기호를 사용하는 경우 새 검사 규칙 집합을 만들어야 합니다.

    - **검사 유형 및 일정**
        - 전체 또는 증분 검사를 실행하도록 검사 프로세스를 구성할 수 있습니다.
        - 검사 프로세스는 즉시 실행하도록 트리거되거나 메타데이터를 최신 상태로 유지하기 위해 주기적으로(매주 또는 매월) 실행하도록 예약할 수 있습니다. 또한 리소스 사용량은 검사되는 데이터의 양에 따라 크게 달라집니다.
        - 원본에서 처리 오버로드를 방지하려면 업무 외 시간 또는 사용량이 많은 시간 동안 검사를 실행하는 것이 좋습니다.
        - 초기 검색은 전체 검사이며 모든 후속 검사는 증분 검사입니다. 후속 검사는 주기적인 증분 검사로 예약될 수 있습니다.
        - 검사 빈도는 데이터 원본 및/또는 비즈니스 요구 사항의 변경 관리 일정과 일치해야 합니다. 예를 들면 다음과 같습니다.
            - 원본 구조가 잠재적으로 변경될 수 있는 경우(자산 내의 새 자산 또는 필드가 추가, 수정 또는 삭제됨) 검사 빈도는 동일해야 합니다.
            - 분류/민감도 레이블이 매주 최신 상태로 설정되어야 하는 경우(규정상의 이유 때문일 수 있음) 검사 빈도는 매주여야 합니다.
            예를 들어 파티션 파일이 매주 원본 데이터 레이크에 추가되는 경우 메타데이터에 변경이 없으면(새 분류 시나리오가 없다고 가정) 주간 검색 대신 월별 검색을 예약할 수 있습니다.
            - 검색을 만든 날에 실행하도록 예약하는 경우 시작 시간은 검색 시간 1분 이상 이전이어야 합니다.
            - 검색이 실행될 수 있는 최대 기간은 7일입니다(메모리 문제로 인해 발생할 수 있음). 이 경우 검색 프로세스는 제외됩니다. 7일 후에 진행이 업데이트되지 않은 경우 검색이 실패한 것으로 표시됩니다. 카탈로그로의 수집 프로세스에는 현재 이러한 제한이 없습니다.

    - **검사 취소**
        - 현재 검색을 트리거한 후 검색 상태가 "대기 중"에서 "진행 중" 상태로 전환된 경우에만 검사를 취소하거나 일시 중지할 수 있습니다.
        - 개별 자식 검색 취소는 지원되지 않습니다.
        <!--
    ...
    - **시나리오 삭제 및 업데이트**
        - 필드/열, 테이블 또는 파일이 검색이 실행된 후 원본 시스템에서 제거되는 경우 다음 예약된 전체/증분 검사 후에 Purview에만 반영(제거)됩니다.
        - 자산 이름 아래에 있는 삭제 아이콘을 사용하여 Azure Purview 카탈로그에서 자산을 **삭제할** 수 있습니다(원본의 개체는 제거되지 않음). 그러나 동일한 원본에서 전체 검색을 실행하면 카탈로그에서 다시 수집됩니다. 대신 주별/월별 검사를 예약한 경우(증분) 원본에서 개체를 수정하지 않는 한 삭제된 자산이 선택되지 않습니다(예: 테이블에서 열이 추가/제거됨).
        - Purview Studio를 통해 데이터 자산 또는 기본 스키마를 *수동으로* 편집한 후 후속 검색의 동작을 이해하려면 [카탈로그 자산 세부 정보](./catalog-asset-details.md#scans-on-edited-assets)를 참조하세요.

        자세한 내용은 [자산을 보고 편집하고 삭제하는 방법에 대한 자습서를 참조하세요.](./catalog-asset-details.md)
