---
title: Azure Security Center의 적응형 네트워크 강화 | Microsoft Docs
...
ms.locfileid: "122536061"
---
# <a name="improve-your-network-security-posture-with-adaptive-network-hardening"></a>적응형 네트워크 강화를 통해 네트워크 보안 상태 개선

적응형 네트워크 강화는 Azure Security Center의 에이전트 없는 기능입니다. 이 네트워크 강화 도구를 활용하기 위해 머신에 아무것도 설치할 필요가 없습니다.

이 페이지에서는 Security Center에서 적응형 네트워크 강화를 구성하고 관리하는 방법을 설명합니다.
...
1. Security Center 메뉴에서 **Azure Defender** 대시보드를 열고 적응형 네트워크 강화 타일(1) 또는 적응형 네트워크 강화와 관련된 인사이트 패널 항목(2)을 선택합니다. 

    :::image type="content" source="./media/security-center-adaptive-network-hardening/traffic-hardening.png" alt-text="적응형 네트워크 강화 도구 액세스" lightbox="./media/security-center-adaptive-network-hardening/traffic-hardening.png":::

    > [!TIP]
    > 인사이트 패널에는 현재 적응형 네트워크 강화로 보호되는 VM의 백분율이 표시됩니다. 

1. **인터넷 연결 가상 머신에 적응형 네트워크 강화 권장 사항을 적용해야 함** 권장 사항의 세부 정보 페이지가 열리고, 네트워크 VM이 다음 세 개의 탭으로 그룹화되어 있습니다.
   ...
   * **검색되지 않은 리소스**: 다음 이유 중 하나로 인해 적응형 네트워크 강화 알고리즘을 실행할 수 없는 VM입니다.
      ...
      * **충분한 데이터를 사용할 수 없음**: Security Center에서 정확한 트래픽 강화 권장 사항을 생성하려면 최소 30일간의 트래픽 데이터가 필요합니다.
      * **VM이 Azure Defender로 보호되지 않음**: [서버용 Azure Defender](defender-for-servers-introduction.md)로 보호되는 VM에서만 이 기능을 사용할 수 있습니다.

    :::image type="content" source="./media/security-center-adaptive-network-hardening/recommendation-details-page.png" alt-text="인터넷 연결 가상 머신에 적응형 네트워크 강화 권장 사항을 적용해야 함 권장 사항의 세부 정보 페이지":::
...
3. NSG에 적용할 규칙을 선택하고 **적용** 을 선택합니다.
    ...
    :::image type="content" source="./media/security-center-adaptive-network-hardening/hardening-alerts.png" alt-text="적응형 네트워크 강화 규칙 관리":::
...
Security Center에서 권장되지 않은 “허용” 규칙을 추가할 수 있습니다.
...
알고리즘을 실행한 결과, Security Center에서 기존 NSG 구성에 따라 허용해야 하는 트래픽을 식별하지 않는 경우 **모든 트래픽 거부** 규칙이 권장됩니다. 따라서 권장 규칙은 지정된 포트에 대한 모든 트래픽을 거부하는 것입니다. 이 유형의 규칙 이름은 “시스템 생성”으로 표시됩니다. 이 규칙을 적용한 후 NSG의 실제 이름은 프로토콜, 트래픽 방향, “DENY”, 난수로 구성된 문자열이 됩니다.