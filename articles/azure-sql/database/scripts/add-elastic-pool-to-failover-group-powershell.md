---
title: 'PowerShell: 자동 장애 조치(failover) 그룹에 탄력적 풀 추가'
description: Azure SQL Database 탄력적 풀을 만들어서 자동 장애 조치(failover) 그룹에 추가하고 장애 조치(failover)를 테스트하는 Azure PowerShell 예제 스크립트입니다.
...
이 Azure PowerShell 스크립트 예제에서는 Azure SQL Database의 데이터베이스를 만들어 탄력적 풀에 추가하고, 장애 조치(failover) 그룹을 만들고, 장애 조치(failover)를 테스트합니다.
...
| [New-AzSqlElasticPool](/powershell/module/az.sql/new-azsqlelasticpool) | 탄력적 데이터베이스 풀을 만듭니다.|
...
| [Get-AzSqlDatabaseFailoverGroup](/powershell/module/az.sql/get-azsqldatabasefailovergroup) | 데이터베이스 장애 조치(failover) 그룹을 가져오거나 나열합니다. |
| [Switch-AzSqlDatabaseFailoverGroup](/powershell/module/az.sql/switch-azsqldatabasefailovergroup)| 데이터베이스 장애 조치(failover) 그룹의 장애 조치(failover)를 실행합니다. |
