---
title: VNet 서비스 엔드포인트 - Azure Database for MariaDB
...
가상 네트워크 규칙을 만들려면 먼저 참조할 규칙에 대한 VNet([가상 네트워크 서비스 끝점][vm-virtual-network-overview]) 및 [가상 네트워크 서비스 엔드포인트][vm-virtual-network-service-endpoints-overview-649d]가 있어야 합니다. 다음 그림에서는 Virtual Network 서비스 엔드포인트가 Azure Database for MariaDB에서 작동하는 방법을 보여 줍니다.

![VNet 서비스 엔드포인트 작동 방식 예제](media/concepts-data-access-security-vnet/vnet-concept.png)

> [!NOTE]
> 이 기능은 Azure Database for MariaDB가 범용 및 메모리 최적화 서버용으로 배포된 모든 Azure 지역에서 사용할 수 있습니다.

연결에 [프라이빗 링크](concepts-data-access-security-private-link.md)를 사용하는 것을 고려할 수도 있습니다. 프라이빗 링크는 Azure Database for MariaDB 서버에 대한 VNet의 개인 IP 주소를 제공합니다.
...
가상 네트워크 규칙은 서브넷에 있는 모든 노드의 통신을 허용하도록 Azure Database for MariaDB 서버에 지시합니다.
...
각 가상 네트워크 규칙은 서버에 있는 하나의 특정 데이터베이스가 아니라 Azure Database for MariaDB 서버 전체에 적용됩니다. 즉, 가상 네트워크 규칙은 데이터베이스 수준이 아니라 서버 수준에서 적용됩니다.
...
> - 반드시 두 구독 모두에 **Microsoft.Sql** 및 **Microsoft.DBforMariaDB** 리소스 공급자를 등록해야 합니다. 자세한 내용은 [resource-manager-registration][resource-manager-portal]을 참조하세요.
...
- **Microsoft.Sql** 서비스 태그를 사용하여 Azure Database for MariaDB에 가상 네트워크 서비스 엔드포인트를 설정하면, 모든 Azure Database 서비스(Azure Database for MariaDB, Azure Database for MySQL, Azure Database for PostgreSQL, Azure SQL Database 및 Azure Synapse Analytics)에 대한 엔드포인트도 사용하도록 설정할 수 있습니다.
...
회로에서 Azure Database for MariaDB로의 통신을 허용하려면 회로의 공용 IP 주소에 대한 IP 네트워크 규칙을 만들어야 합니다. ExpressRoute 회로의 공용 IP 주소를 찾기 위해 Azure Portal을 사용하여 ExpressRoute에서 지원 티켓을 엽니다.
