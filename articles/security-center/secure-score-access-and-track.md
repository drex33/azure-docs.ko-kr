---
title: Azure Security Center의 보안 점수 추적
description: Azure Security Center의 보안 점수에 액세스하고 추적하는 여러 가지 방법을 알아봅니다.
author: memildin
ms.author: memildin
manager: rkarlin
...
Security Center 포털에서 점수가 눈에 띄게 표시됩니다. Security Center 개요 페이지의 첫 번째 기본 타일입니다. 이 타일을 선택하면 전용 보안 점수 페이지로 이동하고, 구독별로 분석된 점수를 볼 수 있습니다. 단일 구독을 선택하여 우선 순위가 지정된 권장 사항의 세부 목록과 이러한 권장 사항에 따라 수정할 때 구독의 점수에 미치는 영향을 확인할 수 있습니다. 

요약하자면 보안 점수는 Security Center 포털 페이지의 다음 위치에 표시됩니다.

- Security Center의 **개요** 탭(기본 대시보드):
...
- 전용 **보안 점수** 페이지에서 구독 및 관리 그룹에 대한 보안 점수를 볼 수 있습니다.
    ...
    :::image type="content" source="./media/secure-score-security-controls/score-on-dedicated-dashboard.png" alt-text="Security Center의 보안 점수 페이지의 구독 관련 보안 점수":::

    :::image type="content" source="./media/secure-score-security-controls/secure-score-management-groups.png" alt-text="Security Center의 보안 점수 페이지의 관리 그룹 관련 보안 점수":::
...
Security Center 통합 문서 페이지에는 구독, 보안 제어 등의 점수를 시각적으로 추적하기 위해 준비된 보고서가 포함되어 있습니다. [Security Center 데이터에 대한 풍부한 대화형 보고서 만들기](custom-dashboards-azure-workbooks.md)에서 자세히 알아보세요.

:::image type="content" source="media/custom-dashboards-azure-workbooks/secure-score-over-time-snip.png" alt-text="Azure Security Center의 통합 문서 갤러리에서 시간 경과에 따른 보안 점수 보고서의 섹션":::
...
> [!TIP]
> GitHub의 Azure Security Center 커뮤니티 전용 영역에서 이 대시보드와 보안 점수를 사용하여 프로그래밍 방식으로 작업하기 위한 다른 도구를 찾을 수 있습니다. https://github.com/Azure/Azure-Security-Center/tree/master/Secure%20Score
