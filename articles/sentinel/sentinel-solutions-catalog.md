---
title: Azure 센티널 콘텐츠 허브 카탈로그 | Microsoft Docs
description: 이 문서에서는 현재 사용할 수 있는 Azure 센티널 콘텐츠 허브 패키지를 표시 하 고 자세히 설명 합니다.
services: sentinel
cloud: na
...
> [!IMPORTANT]
>
> Azure 센티널 콘텐츠 허브 환경은 모든 개별 솔루션 패키지와 마찬가지로 현재 **미리 보기** 상태입니다. 베타 또는 미리 보기로 제공되거나 아직 일반 공급으로 릴리스되지 않은 Azure 기능에 적용되는 추가 약관은 [Microsoft Azure 미리 보기에 대한 추가 사용 약관](https://azure.microsoft.com/support/legal/preview-supplemental-terms/)을 참조하세요.
...
이 문서에는 작업 영역에서 배포할 수 있는 기본 제공 (기본 제공), 주문형 Azure 센티널 데이터 커넥터 및 솔루션이 나열 되어 있습니다. 솔루션을 배포 하면 데이터 커넥터, 플레이 북, 통합 문서, 규칙 등의 포함 된 보안 콘텐츠가 Azure 센티널의 관련 영역에 포함 됩니다. 

자세한 내용은 [중앙에서 Azure 센티널 기본 콘텐츠와 솔루션 검색 및 배포](sentinel-solutions-deploy.md)를 참조 하세요.
...
|**SQL PaaS용 Azure Sentinel**     |  [데이터 커넥터](data-connectors-reference.md#azure-sql-databases), 통합 문서, 분석 규칙, 플레이 북, 구하기 쿼리     | 애플리케이션        |      커뮤니티   |
|**Azure 센티널 학습 랩** |통합 문서, 분석 규칙, 플레이 북, 구하기 쿼리 | 교육 및 자습서 |Microsoft |
...
|Name    |Includes  |범주 |지원 요소  |
|---------|---------|---------|---------|
|**Vectra Stream 솔루션** |데이터 커넥터, 헌팅 쿼리, 파서 |보안 - 네트워크 |Microsoft |
| | | |
