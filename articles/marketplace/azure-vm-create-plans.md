...
이 옵션을 사용하면 동일한 제안 내의 계획에서 동일한 기술 구성 설정을 사용할 수 있으므로 동일한 이미지 집합을 활용할 수 있습니다. 기술 구성 재사용 옵션을 사용하도록 설정하면 계획이 선택한 기본 계획과 동일한 기술 구성 설정을 상속합니다.  기본 계획을 변경하면 구성을 다시 사용할 계획에 변경 내용이 반영됩니다.
...
[Azure IMDS(Instance Metadata Service)를](/azure/virtual-machines/windows/instance-metadata-service) 활용하여 라이선스의 유효성을 검사하거나 적절한 기능을 사용하도록 설정하기 위해 솔루션이 배포되는 계획을 식별합니다.

나중에 계획 간에 다른 변경 내용을 게시하기로 결정한 경우 분리할 수 있습니다. 계획에서 이 옵션을 선택 취소하여 기술 구성을 다시 사용하는 계획을 분리합니다. 분리되면 계획이 마지막 설정 대신 동일한 기술 구성 설정을 전달하며 계획이 구성에서 달라질 수 있습니다. 과거에 독립적으로 게시된 계획은 나중에 기술 구성을 다시 사용할 수 없습니다. 
...
> [!NOTE]
> 게시된 세대에는 고객이 사용할 수 있는 상태로 유지되는 이미지 버전이 하나 이상 필요합니다. 전체 계획(모든 세대 및 이미지와 함께)을 제거하려면 계획 **개요** 페이지에서 **계획 사용 해제를** 선택합니다(이 문서의 첫 번째 섹션 참조).
