...
Azure Event Grid와 Azure Key Vault를 통합하면 키 자격 증명 모음에 저장된 비밀의 상태가 변경될 때 사용자에게 알릴 수 있습니다. 기능에 대한 개요는 [Event Grid를 사용하여 Key Vault 모니터링](event-grid-overview.md)을 참조하세요.
...
![Runbook UI 만들기](../media/event-grid-tutorial-3.png)
...
![Runbook UI 게시](../media/event-grid-tutorial-4.png)
...
1.  키 자격 증명 모음으로 이동하여 **이벤트** 탭을 선택합니다.

    ![Azure Portal의 이벤트 탭](../media/event-grid-tutorial-7.png)
...
![Event Grid 구독의 구성 테스트](../media/event-grid-tutorial-9.png)

![비밀 만들기 창](../media/event-grid-tutorial-10.png)
...
1.  **메트릭** 아래에서 이벤트가 캡처되었는지 확인합니다. 두 가지 이벤트가 필요합니다. SecretNewVersion 및 SecretNearExpiry 이벤트이며, 이러한 이벤트는 Event Grid가 키 자격 증명 모음에서 비밀의 상태 변경을 캡처했는지 확인합니다.
    ...
    ![메트릭 창: 캡처한 이벤트를 확인합니다.](../media/event-grid-tutorial-11.png)
