...
이제 Azure Compute 갤러리에 기존 공유 이미지 갤러리 서비스와 새 [VM 응용 프로그램] (applications.md) 기능 및 기능이 포함 되어 있습니다.  
...
| 리소스 | Description|
|----------|------------|
| **이미지 원본** | 갤러리에서 **이미지 버전** 을 만드는 데 사용할 수 있는 리소스입니다. 이미지 원본은 [일반화 되거나 특수화](#generalized-and-specialized-images)된 기존 Azure VM이 될 수 있습니다. 즉, 다른 갤러리의 관리 되는 이미지, 스냅숏, VHD 또는 이미지 버전을 사용할 수 있습니다. |
...
Azure Compute 갤러리를 사용하면 이제 가상 머신 확장 집합에 최대 1,000개의 VM 인스턴스를 배포할 수 있습니다(관리형 이미지를 사용하여 최대 600개). 이미지 복제본은 보다 나은 배포 성능, 안정성 및 일관성을 제공합니다.   지역에 필요한 규모에 따라 각 대상 지역에 서로 다른 복제본 수를 설정할 수 있습니다. 각 복제본은 이미지의 전체 복사본이기 때문에 이렇게 하면 복제본을 추가할 때마다 선형적으로 배포를 확장할 수 있습니다. 두 개의 이미지 또는 지역이 똑같지는 않지만, 지역에서 복제본을 사용하는 방법에 대한 일반적인 지침은 다음과 같습니다.
...
| 공유한 항목     | Azure Compute 갤러리 | 이미지 정의 | 이미지 버전 |
|----------------------|----------------------|--------------|----------------------|
| Azure Compute 갤러리 | 예                  | 예          | 예                  |
| 이미지 정의     | 예                   | 예          | 예                  |
...
- [갤러리에서 이미지 버전 만들기](https://azure.microsoft.com/resources/templates/sig-image-version-create/)
...
예, [특수 이미지](windows/create-vm-specialized.md)에서 VM을 만들 수 있습니다. 
