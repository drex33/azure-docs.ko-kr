...
영향이 거의 없는 유지 관리는 VM을 10초 미만으로 일시 중지합니다. 경우에 따라 Azure는 메모리 보존 유지 관리 메커니즘을 사용합니다. 이러한 메커니즘은 일반적으로 30 초 동안 VM을 일시 중지 하 고 RAM에 메모리를 유지 합니다. 그런 다음, VM이 다시 시작되고 클록이 자동으로 동기화됩니다. 
...
**유연한** 오케스트레이션 모드의 가상 머신 확장 집합은 Azure 계산 리소스를 사용 하 여 가상 머신 확장 집합의 확장성을 가용성 집합에 대 한 지역별 가용성 보장을 갖춘 단일 오케스트레이션 모드와 결합할 수 있습니다.

유연한 오케스트레이션을 사용 하면 인스턴스가 여러 영역에 걸쳐 분산 되는지, 단일 지역 내에서 여러 장애 도메인에 걸쳐 분산 되어 있는지를 선택할 수 있습니다. 

#### <a name="availability-sets-and-uniform-scale-sets"></a>가용성 집합 및 균일 한 확장 집합
...
**균일** 한 오케스트레이션 모드의 가상 머신 *확장 집합* 은 단일 리소스로 동일한 vm 집합을 배포 하 고 관리 하는 데 사용할 수 있는 Azure compute 리소스입니다. 확장 집합은 가용성 집합의 VM과 같은 UD에 걸쳐 자동으로 배포됩니다. 가용성 집합과 마찬가지로, 단일 확장 집합을 사용 하는 경우 예약 된 유지 관리 중에 지정 된 시간에 한 UD만 업데이트 됩니다.
