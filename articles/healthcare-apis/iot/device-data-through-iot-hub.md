---
title: Azure IoT Hub를 통해 디바이스 데이터 받기 - Azure Healthcare API
...
IoT 커넥터는 다양한 IoT 상태 관련 또는 의료 디바이스에서 FHIR(전자 의료 기록 교환®) 서비스로 상태 데이터를 수집하고 수집하는 기능을 제공합니다. IoT 커넥터는 향상된 워크플로와 사용 편의성을 위해 Azure IoT Hub를 통해 만들고 관리하는 디바이스와 상호 운용 가능하고 응답성이 뛰어납니다. 이 자습서에서는 디바이스 데이터를 Azure IoT Hub에서 IoT 커넥터로 연결하고 라우팅하는 절차를 제공합니다.
...
Azure IoT Hub는 IoT 커넥터와 안전하게 연결하기 위해 연결 문자열이 필요합니다. 연결 문자열 생성에 설명된 대로 IoT 커넥터에 대한 새 [연결 문자열을](../azure-api-for-fhir/iot-fhir-portal-quickstart.md#generate-a-connection-string)만듭니다. 다음 단계에서 사용할 이 연결 문자열을 금고.
...
```
> [!IMPORTANT]
> IoT 커넥터로 구성된 [매핑 템플릿을](how-to-use-fhir-mappings.md) 준수하는 디바이스 메시지를 보내야 합니다.

## <a name="view-device-data-in-fhir-service"></a>FHIR 서비스에서 디바이스 데이터 보기

Postman을 사용하여 FHIR 서비스에서 IoT 커넥터가 만든 FHIR 관찰 리소스를 볼 수 있습니다. 자세한 내용은 [Postman을 사용하여 FHIR 서비스에 액세스하고](./../use-postman.md) `GET` 위의 샘플 `https://your-fhir-server-url/Observation?code=http://loinc.org|8867-4` 메시지에서 제출된 심박수 값이 있는 관찰 FHIR 리소스를 보도록 요청합니다.
