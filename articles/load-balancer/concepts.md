...
부하 분산 장치는 계층 4에서 작동하며 애플리케이션 계층 게이트웨이 기능을 제공하지 않습니다. 프로토콜 핸드셰이크는 항상 클라이언트와 백 엔드 풀 인스턴스 사이에서 직접 발생합니다. 
