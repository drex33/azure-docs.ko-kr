---
title: 인증서 배포
description: Defender for IoT에 대한 인증서를 설정하고 배포하는 방법을 알아봅니다.
ms.date: 11/09/2021
ms.topic: how-to
...
ms.locfileid: "132343708"
---
# <a name="about-certificates"></a>인증서 정보

이 문서에서는 Microsoft Defender for IoT용 인증서를 만들고 배포할 때 필요한 정보를 제공합니다. 보안, PKI 또는 기타 정규화된 인증서 리더는 인증서 만들기 및 배포를 처리해야 합니다.

Defender for IoT는 SSL/TLS 인증서를 사용하여 다음 시스템 구성 요소 간의 통신을 보호합니다. 

- 사용자와 어플라이언스의 웹 콘솔 간. 
- 센서와 온-프레미스 관리 콘솔 간. 
- 관리 콘솔과 고가용성 관리 콘솔 간.
- 센서 및 온-프레미스 관리 콘솔의 REST API. 

Defender for IoT 관리자 사용자는 SSL/TLS 인증서 대화 상자에서 센서 콘솔 및 해당 온-프레미스 관리 콘솔에 인증서를 업로드할 수 있습니다.
...
모든 인증서 생성 방법은 다음을 사용하여 지원됩니다.  

- 프라이빗 및 Enterprise 키 인프라(프라이빗 PKI) 
- 공개 키 인프라(공용 PKI) 
- 어플라이언스에 로컬로 생성된 인증서(로컬로 자체 서명됨) 

> [!Important]
> 로컬로 자체 서명된 인증서를 사용하지 않는 것이 좋습니다. 이 유형의 연결은 안전하지 않으며 테스트 환경에만 사용해야 합니다. 인증서 소유자의 유효성을 검사할 수 없고 시스템 보안을 유지할 수 없으므로 자체 서명된 인증서를 프로덕션 네트워크에 사용하면 안 됩니다.

## <a name="about-certificate-validation"></a>인증서 유효성 검사 정보

시스템 구성 요소 간의 통신 보안 외에도 사용자는 인증서 유효성 검사를 수행할 수 있습니다.  

유효성 검사는 다음을 통해 평가됩니다.
...
1. 센서 및 온-프레미스 관리 콘솔에 인증서를 업로드하는 경우 유효성 검사에 실패하면 인증서를 업로드할 수 없습니다.
1. 다음 간에 암호화된 통신을 시작하는 경우:

    - Defender for IoT 시스템 구성 요소(예: 센서 및 온-프레미스 관리 콘솔).

    - Defender for IoT 및 전달 규칙에 정의된 특정 타사 서버  자세한 내용은 [전달된 경고 정보](how-to-forward-alert-information-to-partners.md#about-forwarded-alert-information) 정보를 참조하세요.  

유효성 검사에 실패하면 관련 구성 요소 간의 통신이 중지되고 유효성 검사 오류가 콘솔에 표시됩니다.

## <a name="about-certificate-upload-to-defender-for-iot"></a>Defender for IoT에 인증서 업로드 정보

센서 및 온-프레미스 관리 콘솔을 설치한 후 로컬 자체 서명된 인증서가 생성되고 센서 및 온-프레미스 관리 콘솔 웹 애플리케이션에 액세스하는 데 사용됩니다.

센서 및 온-프레미스 관리 콘솔에 처음으로 로그인하면 관리자에게 SSL/TLS 인증서를 업로드하라는 메시지가 표시됩니다. SSL/TLS 인증서를 사용하는 것이 좋습니다.

인증서 잠재 고객에 의해 인증서가 제대로 만들어지지 않았거나 연결 문제가 있는 경우 인증서를 업로드할 수 없으며 사용자는 로컬로 서명된 인증서로 작업해야 합니다.  

업로드된 인증서 및 타사 인증서의 유효성을 검사하는 옵션은 자동으로 사용하도록 설정되지만 사용하지 않도록 설정할 수 있습니다. 사용하지 않도록 설정하면 인증서가 유효하지 않더라도 구성 요소 간의 암호화된 통신이 계속됩니다.

## <a name="certificate-deployment-tasks"></a>인증서 배포 작업

이 섹션에서는 인증서 배포가 원활하게 실행되도록 하기 위해 수행해야 하는 단계를 설명합니다.

**인증서를 배포하려면 다음을 확인합니다.**

- 보안, PKI 또는 인증서 전문가가 인증서 만들기를 만들거나 감독하고 있습니다. 
- 각 센서, 관리 콘솔 및 HA 머신에 대해 고유한 인증서를 만듭니다.
- 인증서 만들기 요구 사항을 충족합니다.  [인증서 만들기 요구 사항을 참조하세요.](#certificate-creation-requirements)
- 각 Defender for IoT 센서에 로그인하는 관리자와 온-프레미스 관리 콘솔 및 HA 머신은 인증서에 액세스할 수 있습니다.

## <a name="certificate-creation-requirements"></a>인증서 만들기 요구 사항

이 섹션에서는 다음을 포함하여 인증서 만들기 요구 사항에 대해 설명합니다.
...
- [인증서 체인 파일 요구 사항(.pem을 사용하는 경우)](#certificate-chain-file-requirements-if-pem-is-used)

### <a name="port-access-requirements-for-certificate-validation"></a>인증서 유효성 검사를 위한 포트 액세스 요구 사항

인증서 유효성 검사를 사용 중인 경우 포트 80에 대한 액세스를 사용할 수 있는지 확인합니다.

인증서 유효성 검사는 인증서 해지 목록 및 인증서 만료 날짜에 대해 평가됩니다. 즉, 어플라이언스 인증서에 의해 정의 된 CRL 서버에 대 한 연결을 설정할 수 있어야 합니다. 기본적으로 인증서는 HTTP 포트 80에서 CRL URL을 참조합니다. 

일부 조직의 보안 정책은 이 포트에 대한 액세스를 차단할 수 있습니다. 조직에 포트 80에 대한 액세스 권한이 없는 경우 다음을 수행할 수 있습니다. 

1. 인증서에 다른 URL과 특정 포트를 정의합니다.

    - URL은 https: // 대신 http: //로 정의되어야 합니다.
...
Defender for IoT를 사용하려면 CA 서명된 각 인증서에 .key 파일과 .crt 파일이 포함되어야 합니다. 이러한 파일은 로그인 후 센서 및 온-프레미스 관리 콘솔에 업로드됩니다. 일부 조직에서는 .pem 파일이 필요할 수 있습니다. Defender for IoT에는 이 파일 형식이 필요하지 않습니다.

**.crt – 인증서 컨테이너 파일**

확장이 다른 .pem 또는 .der 형식의 파일입니다. 해당 파일은 Windows 탐색기에서 인증서로 인식됩니다. .pem 파일은 Windows 탐색기에서 인식되지 않습니다.

**.key – 프라이빗 키 파일**

키 파일은 PEM 파일과 동일한 형식이지만 확장명이 다릅니다.

**.pem – 인증서 컨테이너 파일(선택 사항)**

PEM은 인증서 텍스트의 Base64 인코딩을 포함하는 텍스트 파일로, 인증서의 시작과 끝을 표시하는 바닥글을 & 일반 텍스트 헤더입니다.

기존 파일 형식을 지원되는 형식으로 변환해야 할 수 있습니다. 자세한 내용은 [기존 파일을 지원되는 파일로 변환을](#convert-existing-files-to-supported-files) 참조하세요.

### <a name="certificate-file-parameter-requirements"></a>인증서 파일 매개 변수 요구 사항

인증서를 만들기 전에 다음 매개 변수 요구 사항을 충족했는지 확인합니다.
...
- 주체 CN(일반 이름) = 어플라이언스의 도메인 이름입니다. 예를 들어 Sensor.contoso.com 또는 *.contoso.com.  
