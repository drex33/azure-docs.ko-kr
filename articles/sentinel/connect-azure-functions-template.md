---
title: Azure Functions 사용하여 Microsoft Sentinel을 데이터 원본 | 연결 Microsoft Docs
description: Azure Functions 사용하여 데이터 원본에서 Microsoft Sentinel로 데이터를 얻는 데이터 커넥터를 구성하는 방법을 알아봅니다.
services: sentinel
...
ms.locfileid: "132722471"
---
# <a name="use-azure-functions-to-connect-microsoft-sentinel-to-your-data-source"></a>Azure Functions 사용하여 Microsoft Sentinel을 데이터 원본에 연결

[!INCLUDE [Banner for top of topics](./includes/banner.md)]

[PowerShell](../azure-functions/functions-reference-powershell.md) 또는 Python과 같은 다양한 코딩 언어와 함께 [Azure Functions](../azure-functions/functions-overview.md)를 사용하여 호환되는 데이터 원본의 REST API 엔드포인트에 대한 서버리스 커넥터를 만들 수 있습니다. 그러면 Azure Function Apps를 사용하여 Microsoft Sentinel을 데이터 원본의 REST API 연결하여 로그를 끌어올 수 있습니다.

이 문서에서는 Azure Function Apps를 사용하기 위해 Microsoft Sentinel을 구성하는 방법을 설명합니다. 원본 시스템을 구성해야 할 수도 있으며, 포털의 각 데이터 커넥터 페이지 또는 [Microsoft Sentinel](data-connectors-reference.md) 데이터 커넥터 참조 페이지에서 서비스에 대한 섹션에서 공급업체 및 제품별 정보 링크를 찾을 수 있습니다.




> [!NOTE]
> - Microsoft Sentinel에 수집되면 데이터는 Microsoft Sentinel을 실행하는 작업 영역의 지리적 위치에 저장됩니다.
>
...
>
> - Azure Functions 사용하여 Microsoft Sentinel로 데이터를 수집하면 추가 데이터 수집 비용이 발생할 수 있습니다. 자세한 내용은 [Azure Functions 가격 책정](https://azure.microsoft.com/pricing/details/functions/) 페이지를 참조하세요.

## <a name="prerequisites"></a>필수 조건

Azure Functions 사용하여 Microsoft Sentinel을 데이터 원본에 연결하고 해당 로그를 Microsoft Sentinel로 끌어오기 전에 다음 권한 및 자격 증명이 있는지 확인합니다.
...
- 또한 제품 API에 액세스하기 위한 자격 증명(사용자 이름 및 암호, 토큰, 키 또는 기타 조합)이 필요합니다. 엔드포인트 URI와 같은 다른 API 정보가 필요할 수도 있습니다.
    ...
    자세한 내용은 연결 중인 서비스에 대한 설명서 및 Microsoft Sentinel 데이터 커넥터 참조 페이지에서 서비스에 대한 [섹션을 참조하세요.](data-connectors-reference.md)
...
>
> - 일부 데이터 커넥터는 정상적으로 작동하기 위해 [Kusto Function](/azure/data-explorer/kusto/query/functions/user-defined-functions)을 기반으로 하는 파서를 사용합니다. Kusto 함수 및 별칭을 만드는 지침에 대한 링크는 [Microsoft Sentinel 데이터 커넥터 참조](data-connectors-reference.md) 페이지의 서비스에 대한 섹션을 참조하세요.
...
필요한 정확한 자격 증명에 대한 세부 정보 및 해당 자격 증명을 찾거나 만들기 위한 제품 지침에 대한 링크는 포털의 데이터 커넥터 페이지 및 [Microsoft Sentinel 데이터 커넥터 참조](data-connectors-reference.md) 페이지의 서비스에 대한 섹션에서 찾을 수 있습니다.
...
1. Microsoft Sentinel 포털에서 **데이터 커넥터를 선택합니다.** 목록에서 Azure Functions 기반 커넥터를 선택한 다음, **커넥터 페이지 열기** 를 선택합니다.

1. **구성에서** Microsoft Sentinel **작업 영역 ID** 및 **기본 키를** 복사하여 따로 붙여넣습니다.
...
1. **사용자 지정 배포** 화면이 나타납니다.
    ...
    - 복사한 Microsoft Sentinel **작업 영역 ID** 및 작업 영역 **키(기본** 키)를 입력하고 따로 보관합니다.
    ...
    - **사용자 지정 배포** 화면에서 양식의 다른 필드를 작성합니다. 포털의 데이터 커넥터 페이지 또는 [Microsoft Sentinel 데이터 커넥터 참조](data-connectors-reference.md) 페이지의 서비스에 대한 섹션을 참조하세요.
...
1. **함수 앱 코드 가져오기**
    ...
    1. 원본 시스템의 공급업체에서 제공하는 함수 앱 코드를 다운로드하고 복사하여 **함수 앱** *run.ps1* 편집기에 붙여 넣어 편집기에 기본값으로 있는 내용을 대체합니다. Microsoft [Sentinel 데이터](data-connectors-reference.md) 커넥터 참조 페이지의 커넥터 페이지 또는 서비스에 대한 섹션에서 다운로드 링크를 찾을 수 있습니다.
...
1. **함수 앱 구성**
    ...
    1. 제품에 대해 지정된 애플리케이션 설정을 대/소문자 구분 문자열 값과 함께 개별적으로 추가합니다. [Microsoft Sentinel](data-connectors-reference.md) 데이터 커넥터 참조 페이지에서 서비스에 대한 섹션의 데이터 커넥터 페이지 또는 제품 섹션을 참조하세요.
...
1. **함수 앱 배포**
    ...
    1. 데이터 커넥터 페이지 및 [Microsoft Sentinel](data-connectors-reference.md) 데이터 커넥터 참조 페이지의 서비스 섹션에 제공된 링크를 사용하여 Azure 함수 앱 파일을 다운로드합니다. 로컬 개발 컴퓨터로 보관 파일을 추출합니다.
