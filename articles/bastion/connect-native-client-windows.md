...
ms.openlocfilehash: 5a4a6ae81e78113cef5c3108c2f53957a2a11aad
ms.sourcegitcommit: 93c7420c00141af83ed3294923b4826dd4dc6ff2
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/02/2021
ms.locfileid: "133438000"
---
...
* 네이티브 클라이언트 지원은 아직 로컬 Linux 워크스테이션에서 사용할 수 없습니다. Linux 워크스테이션에서 대상 VM에 연결하는 경우 Azure Portal 환경을 사용합니다.
...
* Azure AD 자격 증명을 사용하여 가상 머신에 로그인하려는 경우 다음 방법 중 하나를 사용하여 가상 머신이 설정되었는지 확인합니다.
  * Windows VM 또는 [Linux](../active-directory/devices/howto-vm-sign-in-azure-ad-linux.md) [VM에](../active-directory/devices/howto-vm-sign-in-azure-ad-windows.md) 대해 Azure AD 로그인을 사용하도록 설정합니다.
...
* Azure AD 로그인 방법을 사용하는 경우 Virtual Machine 관리자 로그인 또는 Virtual Machine 사용자 로그인 역할 Azure Windows VM 및 Azure AD 또는 Azure [Linux VM](../active-directory/devices/howto-vm-sign-in-azure-ad-linux.md)및 [Azure AD](../active-directory/devices/howto-vm-sign-in-azure-ad-windows.md) 문서에 설명된 프로세스를 사용하여 Azure AD 로그인을 사용하도록 설정하는 경우에만 이 작업을 수행해야 합니다.
...
1. 다음 옵션 중 하나를 사용하여 대상 Linux VM에 로그인합니다.
   ...
   > [!NOTE]
   > 사용자 지정 포트 값을 지정하려면 로그인 명령에 **--resource-port** 필드도 포함해야 합니다.
   >
   ...
   * 로컬 사용자 이름 및 암호를 사용하여 로그인하는 경우 다음 명령을 사용합니다. 그러면 대상 VM에 대한 암호를 묻는 메시지가 표시됩니다.
...
1. 다음 옵션 중 하나를 사용하여 대상 Windows VM에 로그인합니다.

   > [!NOTE]
   > 사용자 지정 포트 값을 지정하려면 로그인 명령에 **--resource-port** 필드도 포함해야 합니다.
   >

   * RDP를 통해 연결하려면 다음 명령을 사용합니다. 그러면 자격 증명을 입력하라는 메시지가 표시됩니다. 로컬 사용자 이름 및 암호 또는 Azure AD 자격 증명을 사용할 수 있습니다. Azure AD를 사용하여 Azure Windows VM에 로그인하는 방법에 대한 자세한 내용은 [Azure Windows VM 및 Azure AD를](../active-directory/devices/howto-vm-sign-in-azure-ad-windows.md)참조하세요.

      ```azurecli-interactive
      az network bastion rdp --name "<BastionName>" --resource-group "<ResourceGroupName>" --target-resource-id "<VMResourceId>"
      ```

   * SSH 키 쌍을 사용하여 로그인하려면 다음 명령을 사용합니다. SSH CLI 확장은 현재 미리 보기로 제공됩니다. "az extension add --name ssh"를 실행하여 확장을 설치할 수 있습니다.

      ```azurecli-interactive
      az network bastion ssh --name "<BastionName>" --resource-group "<ResourceGroupName>" --target-resource-id "<VMResourceId>" --auth-type "ssh-key" --username "<Username>" --ssh-key "<Filepath>"
      ```

1. 대상 VM에 로그인하면 워크스테이션의 네이티브 클라이언트가 VM 세션과 함께 열립니다. RDP 세션용 MSTSC 및 SSH 세션용 SSH CLI 확장.
