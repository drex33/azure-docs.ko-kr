...
지원되는 버전 목록은 Azure PaaS에서 Postgres 관리형 서비스와의 패리티 보장을 진행하면서 시간이 지남에 따라 진화합니다. 현재 지원되는 주 버전은 다음과 같습니다.
- Postgres 12(기본값)
- Postgres 11
...
지원하는 버전에 관계없이 Kubernetes 클러스터에는 Postgres CRD(사용자 지정 리소스 정의)가 하나만 있습니다.
예를 들어 다음 명령을 실행합니다.
```console
kubectl get crds
```
...
이 예제에서 이 출력은 Postgres와 관련된 CRD가 하나 있음을 나타냅니다. postgresqls.arcdata.microsoft.com, shortname postgresqls. CRD는 Postgres 인스턴스 또는 Postgres 서버 그룹이 아닙니다. CRD가 있다는 것은 서버 그룹이 생성되었음을 나타내는 것이 아닙니다. CRD는 Kubernetes 클러스터에서 만들 수 있는 리소스 종류를 나타냅니다.
