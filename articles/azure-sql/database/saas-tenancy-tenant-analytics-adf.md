---
title: 테넌트 데이터베이스에 대한 분석 쿼리 실행
description: Azure SQL Database, Azure Synapse Analytics, Azure Data Factory 또는 Power BI에서 추출된 데이터를 사용하는 교차 테넌트 분석 쿼리입니다.
...
![이 자습서에서 사용되는 별모양 스키마를 보여주는 다이어그램.](./media/saas-tenancy-tenant-analytics-adf/starschematables.JPG)
...
[SSMS(SQL Server Management Studio)](/sql/ssms/download-sql-server-management-studio-ssms)를 사용하여 **tenants1-dpt-&lt;user&gt;** 및 **catalog-dpt-&lt;user&gt;** 서버에 연결합니다. &lt;user&gt;를 앱을 배포할 때 사용한 값으로 바꿉니다. 로그인 = *developer* 및 암호 = *P\@ssword1* 을 사용합니다. 자세한 내용은 [입문용 자습서](./saas-dbpertenant-wingtip-app-overview.md)를 참조하세요.

![SSMS에서 SQL Database에 연결](./media/saas-tenancy-tenant-analytics-adf/ssmsSignIn.JPG)
...
1. **wingtipstaging\<user\>** 스토리지 계정을 클릭하여 존재하는 개체를 살펴봅니다.
1. **Blob** 타일 클릭
...
리소스 그룹의 [Azure Portal](https://ms.portal.azure.com)에서 _dbtodwload-\<user\>_ 라는 Azure Data Factory가 추가되었는지 확인합니다.

 ![adf_portal](./media/saas-tenancy-tenant-analytics-adf/adf-data-factory-portal.png)

이 섹션에서는 만든 데이터 팩터리를 탐색합니다.
아래 단계를 따라 데이터 팩터리를 시작합니다.

1. 포털에서 **dbtodwload-\<user\>** 라는 데이터 팩터리를 클릭합니다.
...
개요 페이지에서 왼쪽 패널의 **작성자** 탭으로 전환하고 세 [파이프라인](../../data-factory/concepts-pipelines-activities.md) 및 세 [데이터 세트](../../data-factory/concepts-datasets-linked-services.md)가 있는지 확인합니다.
...
1. **트리거** 를 클릭하고 끌어온 메뉴에서 **트리거 시작** 을 클릭합니다. 이 작업은 파이프라인을 즉시 실행합니다. 프로덕션 시나리오에서 일정에 따라 데이터를 새로 고치려면 파이프라인을 실행하기 위한 시간표를 정의합니다.
  ![Factory 리소스에서 SQLDBToDW라는 파이프라인의 트리거 옵션을 확장하고 지금 트리거를 선택한 상태를 보여 주는 스크린샷](./media/saas-tenancy-tenant-analytics-adf/adf_trigger.JPG)
...
1. SQLDBToDW 파이프라인의 상태가 **성공** 할 때까지 **새로 고침** 을 클릭합니다.
  ![성공 상태의 SQLDBToDW라는 파이프라인을 보여 주는 스크린샷](./media/saas-tenancy-tenant-analytics-adf/adf_monitoring.JPG)
...
이 자습서에서는 소프트웨어 공급 업체의 해당 테넌트에 대한 이해를 개선하는 데 사용된 테넌트 간 분석에 중점을 뒀습니다. 분석은 _테넌트_ 에 인사이트를 제공하여 비즈니스 자체를 보다 효율적으로 관리할 수 있도록 도와줍니다.
