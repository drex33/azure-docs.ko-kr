...
- Azure Sentinel CMK 기능은 ‘Azure Sentinel에 아직 온보딩되지 않은 Log Analytics 전용 클러스터의 작업 영역’에만 제공됩니다. 

- 다음 CMK 관련 변경 사항은 효과가 없기 때문에 ‘지원되지 않습니다’(Azure Sentinel 데이터는 CMK가 아닌 Microsoft 관리 키를 통해서만 계속 암호화됨).

  - Azure Sentinel에 ‘이미 온보딩된’ 작업 영역에서 CMK를 사용으로 설정합니다.
...
- 다음 CMK 관련 변경 사항은 정의되지 않고 문제가 있는 동작을 유발할 수 있으므로 ‘지원되지 않습니다’.
  ...
  - Azure Sentinel에 이미 온보딩된 작업 영역에서 CMK를 사용 안 함으로 설정합니다.
...
- Azure Sentinel은 CMK 구성에서 시스템 할당 ID를 지원합니다. 따라서 전용 Log Analytics 클러스터의 ID는 **시스템 할당** 유형이어야 합니다. 생성 시 Log Analytics 클러스터에 자동으로 할당되는 ID를 사용하는 것이 좋습니다.
...
- 프로덕션 작업 영역 또는 Log Analytics 클러스터에 CMK를 변경하기 전에 [Azure Sentinel 제품 그룹](mailto:azuresentinelCMK@microsoft.com)에 문의하세요.

## <a name="how-cmk-works"></a>CMK 작동 방법 

Azure Sentinel 솔루션은 Log Analytics 전용 클러스터를 포함하여 로그 수집 및 기능에 여러 스토리지 리소스를 사용합니다. Azure Sentinel CMK 구성의 일부로 관련 Log Analytics 전용 클러스터에서도 CMK 설정을 구성해야 합니다. Log Analytics 이외의 스토리지 리소스에 Azure Sentinel이 저장한 데이터도 전용 Log Analytics 클러스터에 맞게 구성된 고객 관리형 키를 사용하여 암호화됩니다.
...
5.  [온보딩 API](https://github.com/Azure/Azure-Sentinel/raw/master/docs/Azure%20Sentinel%20management.docx)를 통해 Azure Sentinel에 작업 영역을 온보딩합니다.
...
다음 단계에서 Azure Sentinel 작업 영역으로 사용할 CMK 작업 영역을 만들기 위해 [Azure Monitor 고객 관리형 키 구성](../azure-monitor/logs/customer-managed-keys.md)의 지침을 따릅니다.

### <a name="step-3-register-to-the-cosmos-db-resource-provider"></a>3단계: Cosmos DB 리소스 공급자에 등록

Azure Sentinel은 추가 스토리지 리소스인 Cosmos DB와 함께 작동합니다. Cosmos DB 리소스 공급자에 등록하세요.
...
### <a name="step-5-onboard-the-workspace-to-azure-sentinel-via-the-onboarding-api"></a>5단계: 온보딩 API를 통해 Azure Sentinel에 작업 영역을 온보딩합니다.

[온보딩 API](https://github.com/Azure/Azure-Sentinel/raw/master/docs/Azure%20Sentinel%20management.docx)를 통해 Azure Sentinel에 작업 영역을 온보딩합니다.

## <a name="key-encryption-key-revocation-or-deletion"></a>키 암호화 키 해지 또는 삭제

사용자가 키 암호화 키(CMK)를 삭제하거나 전용 클러스터와 Cosmos DB 리소스 공급자의 액세스 권한을 제거하여 해당 키를 철회하는 경우, Azure Sentinel은 1시간 이내에 변경 내용을 적용하며 데이터를 더 이상 사용할 수 없는 것처럼 동작합니다. 이 시점에서는 데이터 수집, 영구 구성 변경, 인시던트 생성과 같은 영구 스토리지 리소스를 사용하는 작업은 금지됩니다. 이전에 저장된 데이터는 삭제되지 않지만 액세스할 수 없는 상태가 됩니다. 액세스할 수 없는 데이터는 데이터 보존 정책에 의해 제어되며 해당 정책에 따라 제거됩니다.
...
Azure Sentinel에서는 고객 관리형 키를 대체하는 기능을 지원하지 않습니다. 대신 [키 순환 기능](#customer-managed-key-rotation)을 사용해야 합니다.
