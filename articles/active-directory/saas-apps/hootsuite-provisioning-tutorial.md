...
> [!div class="checklist"]
> * Hootsuite에서 사용자 만들기
> * Hootsuite에서 더 이상 액세스할 필요가 없는 사용자 제거
> * Azure AD와 Hootsuite 간에 사용자 특성을 동기화된 상태로 유지
> * Hootsuite에서 그룹 및 그룹 구성원 프로비저닝
> * Hootsuite에 대한 [Single Sign-On](./hootsuite-tutorial.md)(권장)
...
3. [Azure AD와 Hootsuite 간에 매핑](../app-provisioning/customize-application-attributes.md)할 데이터를 결정합니다. 
...
1. [Azure Portal](https://portal.azure.com)에 로그인합니다. **엔터프라이즈 애플리케이션**, **모든 애플리케이션** 을 차례로 선택합니다.
    ...
    ![엔터프라이즈 애플리케이션 블레이드](./media/hootsuite-provisioning-tutorial/enterprise-applications.png)

    ![모든 애플리케이션 블레이드](./media/hootsuite-provisioning-tutorial/all-applications.png)

2. 애플리케이션 목록에서 **Hootsuite** 를 선택합니다.

    ![애플리케이션 목록의 Hootsuite 링크](common/all-applications.png)
...
5. **관리자 자격 증명** 섹션 아래의 테넌트 URL에 `https://platform.hootsuite.com/scim/v2`를 입력합니다. **2단계** 에 이전에 검색된 오래 지속되는 비밀 토큰 값을 입력합니다. **연결 테스트** 를 클릭하여 Azure AD가 Hootsuite에 연결할 수 있는지 확인합니다. 연결에 실패하면 Hootsuite 계정에 관리자 권한이 있는지 확인하고 다시 시도합니다.
    ...
    ![스크린샷은 테넌트 URL 및 비밀 토큰을 입력할 수 있는 관리자 자격 증명 대화 상자를 보여 줍니다.](./media/hootsuite-provisioning-tutorial/provisioning.png)
...
10. **매핑** 섹션 아래에서 **Azure Active Directory 그룹 동기화** 를 선택합니다.

11. **특성 매핑** 섹션에서 Azure AD에서 Hootsuite로 동기화되는 그룹 특성을 검토합니다. **일치** 속성으로 선택한 특성은 업데이트 작업을 위해 Hootsuite의 그룹을 일치시키는 데 사용됩니다. **저장** 단추를 선택하여 변경 내용을 커밋합니다.
