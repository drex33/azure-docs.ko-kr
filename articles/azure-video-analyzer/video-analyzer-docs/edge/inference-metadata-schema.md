...
|특성 목록|선택적 특성의 목록입니다.|
|직사각형|이미지 왼쪽 위 모서리를 기준으로 하는 사각형 영역입니다. 필요한 속성은 `left` `width` 원본에서 , 및 `height` 입니다." `top` <br/>경계 상자의 왼쪽, 위쪽, 너비 및 높이는 0.0에서 1.0까지의 배율로 정규화됩니다. `left`(x 좌표) 및 `top` (y 좌표)는 경계 상자의 위쪽과 왼쪽을 나타내는 좌표입니다. `left`반환되는 및 `top` 값은 전체 비디오 차원의 비율입니다. 예를 들어 비디오 크기가 1920 x 1080픽셀이고 경계 상자의 왼쪽 위 좌표가 (350,50)인 경우 경계 `left` 상자의 값은 0.1822(350/1920)이고 `top` 값은 0.0462(50/1080)입니다. <br/>`width`및 `height` 값은 경계 상자의 크기를 전체 비디오 차원의 비율로 나타냅니다. 차원이 1920x1080인 비디오의 경우 경계 상자 너비가 70픽셀이면 반환되는 너비는 0.0364(70/1920)입니다.|
