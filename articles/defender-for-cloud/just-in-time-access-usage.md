...
1. **워크로드 보호 대시보드를** 열고 고급 보호 영역에서 **Just-In-Time VM 액세스를** 선택합니다.
    ...
    - **지원되지 않음** - JIT를 사용하도록 설정하지 않았으며 해당 기능을 지원하지 않는 VM입니다. 다음과 같은 이유로 VM이 이 탭에 포함될 수 있습니다.
      - NSG(네트워크 보안 그룹) 또는 Azure Firewall 누락 - JIT를 사용하려면 NSG를 구성하거나 방화벽 구성(또는 둘 다)해야 합니다.
...
1. **구성됨** 탭에서 포트를 추가하려는 VM을 마우스 오른쪽 단추로 클릭하고 편집을 선택합니다. 
    ...
    ![Microsoft Defender for Cloud에서 JIT VM 액세스 구성 편집](./media/just-in-time-access-usage/jit-policy-edit-security-center.png)
...
1. 이러한 값을 편집하거나 JIT 구성에 포트를 더 추가하려면 Microsoft Defender for Cloud의 Just-In-Time 페이지를 사용합니다.

    1. Defender for Cloud 메뉴에서 **Just-In-Time VM 액세스를** 선택합니다.
...
PowerShell에서 Just-In-Time VM 액세스를 사용하도록 설정하려면 공식 Microsoft Defender for Cloud PowerShell cmdlet을 `Set-AzJitNetworkAccessPolicy` 사용합니다.
...
Just-In-Time VM 액세스 기능은 Microsoft Defender for Cloud API를 통해 사용할 수 있습니다. 이 API를 사용하여 구성된 VM에 대한 정보를 가져오고 새로 추가하고 VM에 대한 액세스를 요청하는 등을 할 수 있습니다. 
...
Azure Portal(Defender for Cloud 또는 Azure Virtual Machines에서) 또는 프로그래밍 방식으로 JIT 사용 VM에 대한 액세스를 요청할 수 있습니다.

이러한 각 옵션은 아래 별도의 탭에 설명되어 있습니다.

### <a name="microsoft-defender-for-cloud"></a>[**Microsoft Defender for Cloud**](#tab/jit-request-asc)

### <a name="request-access-to-a-jit-enabled-vm-from-microsoft-defender-for-cloud"></a>Microsoft Defender for Cloud에서 JIT 사용 VM에 대한 액세스 요청 

VM에서 JIT를 사용하도록 설정한 경우 연결에 대한 액세스를 요청해야 합니다. JIT를 사용하도록 설정하는 방법에 관계 없이 지원되는 방법 중 하나를 사용하여 액세스를 요청할 수 있습니다.

:::image type="content" source="./media/just-in-time-access-usage/jit-request-security-center.gif" alt-text="Defender for Cloud에서 JIT 액세스 요청":::
...
VM에서 JIT를 사용하도록 설정한 경우 연결에 대한 액세스를 요청해야 합니다. JIT를 사용하도록 설정하는 방법에 관계 없이 지원되는 방법 중 하나를 사용하여 액세스를 요청할 수 있습니다.
  ...
  >![JIT(Just-In-Time) 요청](./media/just-in-time-access-usage/jit-request-vm.png)
...
> [!NOTE]
> Azure Firewall 의해 보호되는 VM에 대한 요청이 승인되면 Defender for Cloud는 VM에 연결하는 데 사용할 적절한 연결 세부 정보(DNAT 테이블의 포트 매핑)를 사용자에게 제공합니다.
...
## <a name="audit-jit-access-activity-in-defender-for-cloud"></a>Defender for Cloud에서 JIT 액세스 활동 감사
...
1. 메뉴에서 **활동 로그** 를 선택합니다.
   ...
   ![JIT(Just-in-time) 활동 로그 선택](./media/just-in-time-access-usage/jit-select-activity-log.png)
...
> [JIT 설명이 포함된 문서](just-in-time-access-overview.md)
