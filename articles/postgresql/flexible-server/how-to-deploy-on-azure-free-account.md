...
ms.openlocfilehash: b543098fcc9a14946dd4abcf007c021858ac7dbc
ms.sourcegitcommit: dcf3424d7149fceaea0340eb0657baa2c27882a5
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/30/2021
ms.locfileid: "133268822"
---
...
Azure Database for PostgreSQL - 유연한 서버는 클라우드에서 고가용성 PostgreSQL 데이터베이스를 실행, 관리 및 확장하는 데 사용하는 관리형 서비스입니다. Azure 체험 계정을 사용하면 유연한 서버를 **12개월**  동안 사용할 수 있으며 최대 **월간 한도** 는 다음과 같습니다.
...
1. Azure Database for PostgreSQL **배포 옵션 선택** 페이지에서 **유연한 서버**를 선택합니다.
...
1.  **네트워킹** 탭을 선택하여 서버에 도달하는 방법을 구성합니다.
    ...
    프라이빗 액세스(VNet 통합)를 사용하는 경우 서버에 대한 액세스는 가상 네트워크로 제한됩니다. 연결 방법에 대한 자세한 내용은 [**네트워킹 개요를 참조하세요.**](./concepts-networking.md)
...
1. **검토 + 만들기** 를 선택하여 유연한 서버 구성을 검토합니다.
    ...
    >[!IMPORTANT]
    >Azure 체험 계정에서 유연한 서버 인스턴스를 만드는 동안 **컴퓨팅 + Storage : 비용 요약** 블레이드 및 검토 + **만들기** 탭에 월별 예상 **비용이** 표시됩니다. 그러나 Azure 체험 계정을 사용하고 무료 서비스 사용량이 월별 한도 내에 있는 한(사용량 정보를 보려면 아래 [**무료 서비스 사용량 모니터링 및 추적**](#monitor-and-track-free-services-usage) 섹션 참조) 서비스에 대한 요금이 청구되지 않습니다. 현재 체험 서비스에 대한 **비용 요약** 환경을 개선하기 위해 노력하고 있습니다. 
