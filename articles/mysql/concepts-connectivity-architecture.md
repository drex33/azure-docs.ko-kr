...
* 게이트웨이 해제는 서버에 대한 연결에 영향을 미칠 수 있습니다. 이 방법은 **사용하지 않는 것이 좋습니다**. `<servername>.mysql.database.azure.com`응용 프로그램에 대 한 연결 문자열에서 서버의 FQDN (정규화 된 도메인 이름)을 형식으로 사용 해야 합니다. 
...
|  오스트레일리아 동남부  | 13.73.109.251, 13.77.49.32, 13.77.48.10     |        |            |
...
|  한국 남부           |  52.231.145.3, 52.231.151.97                                              |  52.231.200.86     |                                               |
...
|  영국 남부            |  51.140.144.32, 51.105.64.0                 |  51.140.184.11                                |                                               |
|  영국 서부             |  51.141.8.11                                               |                                 |                                               |
...
|  미국 서부2               |  13.66.136.195, 13.66.136.192, 13.66.226.202                              |       |                                   |
|  미국 서부3              |  20.150.184.2                |                               |                                 |
...
서버의 FQDN(예: )을  ``ping xxx.mysql.database.azure.com`` Ping합니다. 반환된 IP 주소가 위 문서에서 게이트웨이 IP 주소(해제 중) 아래에 나열된 IP 중 하나라면 연결이 이전 게이트웨이를 통과하고 있다는 의미입니다. 반면, 반환된 IP 주소가 게이트웨이 IP 주소 아래에 나열된 IP 중 하나라면 연결이 새 게이트웨이를 통과하고 있다는 의미입니다.
