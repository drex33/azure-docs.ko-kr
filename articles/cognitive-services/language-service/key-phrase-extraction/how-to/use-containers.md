---
title: 핵심 구 추출 온-프레미스에 Docker 컨테이너 사용
titleSuffix: Azure Cognitive Services
description: 핵심 구 추출 온-프레미스에 Docker 컨테이너를 사용하는 방법을 알아봅니다.
...
keywords: 온-프레미스, Docker, 컨테이너, 자연어 처리
ms.openlocfilehash: 51d7c53beaeabe57df5fe817cdb5c16c6d1d4252
ms.sourcegitcommit: 106f5c9fa5c6d3498dd1cfe63181a7ed4125ae6d
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/02/2021
ms.locfileid: "131103713"
---
# <a name="install-and-run-key-phrase-extraction-containers"></a>핵심 구 추출 컨테이너 설치 및 실행


컨테이너를 사용하면 자체 인프라에서 핵심 구 추출 API를 호스트할 수 있습니다. 원격으로 핵심 구 추출 호출하여 충족할 수 없는 보안 또는 데이터 거버넌스 요구 사항이 있는 경우 컨테이너가 좋은 옵션일 수 있습니다.


> [!NOTE]
> * 체험 계정은 매월 5,000개의 텍스트 레코드로 제한되며 **무료** 및 **표준** [가격 책정 계층](https://azure.microsoft.com/pricing/details/cognitive-services/text-analytics)만 컨테이너에 유효합니다. 트랜잭션 요청 속도에 대한 자세한 내용은 [데이터 제한](call-api.md#data-limits)을 참조하세요.

컨테이너를 사용하면 사용자 환경에서 핵심 구 추출 API를 실행할 수 있으며 특정 보안 및 데이터 거버넌스 요구 사항에 적합합니다. 핵심 구 추출 컨테이너는 원시 텍스트에 대한 고급 자연어 처리를 제공하며 감정 분석, 핵심 구 추출 및 언어 감지의 세 가지 주요 기능을 포함합니다.
...
다음 표에서는 사용 가능한 핵심 구 추출 컨테이너에 대한 최소 및 권장 사양을 설명합니다. 각 CPU 코어는 속도가 2.6GHz 이상이어야 합니다. 허용되는 TPS(초당 트랜잭션 수)도 나열됩니다.
...
| 자리 표시자 | 값 | 형식 또는 예 |
|-------------|-------|---|
| **{API_KEY}** | 핵심 구 추출 리소스의 키입니다. Azure Portal의 리소스 **키 및 엔드포인트** 페이지에서 이 값을 찾을 수 있습니다. |`xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`|
| **{ENDPOINT_URI}** | 핵심 구 추출 API에 액세스하기 위한 엔드포인트입니다. Azure Portal의 리소스 **키 및 엔드포인트** 페이지에서 이 값을 찾을 수 있습니다. | `https://<your-custom-subdomain>.cognitiveservices.azure.com` |
...
핵심 구 추출 컨테이너는 Azure 계정의 _핵심 구 추출_ 리소스를 사용하여 청구 정보를 Azure로 보냅니다. 

[!INCLUDE [Container's Billing Settings](../../../../../includes/cognitive-services-containers-how-to-billing-info.md)]
\
## <a name="summary"></a>요약

이 문서에서는 핵심 구 추출 컨테이너를 다운로드, 설치 및 실행하기 위한 개념과 워크플로를 배웠습니다. 요약하면 다음과 같습니다.

* 핵심 구 추출 Docker용 Linux 컨테이너를 제공합니다.
* 컨테이너 이미지는 Microsoft 컨테이너 레지스트리(MCR)에서 다운로드됩니다.
* 컨테이너 이미지는 Docker에서 실행됩니다.
* REST API 또는 SDK를 사용하여 컨테이너의 호스트 URI를 지정하여 핵심 구 추출 컨테이너에서 작업을 호출할 수 있습니다.
