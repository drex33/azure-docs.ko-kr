...
1. 정책 키의 **이름** 을 입력합니다. 예들 들어 `GoogleSecret`입니다. `B2C_1A_` 접두사가 키의 이름에 자동으로 추가됩니다.
