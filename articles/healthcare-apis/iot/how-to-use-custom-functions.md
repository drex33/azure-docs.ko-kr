---
title: IoT 커넥터의 사용자 지정 함수-Azure 의료 Api
description: 이 문서에서는 IoT Connector 장치 매핑 템플릿과 함께 사용자 지정 기능을 사용 하는 방법을 설명 합니다.
author: msjasteppe
...
ms.locfileid: "132733530"
---
# <a name="how-to-use-custom-functions"></a>사용자 지정 함수를 사용 하는 방법
...
이 문서에서는 IoT connector Customer 함수를 사용 하는 방법을 설명 합니다.

**JmesPath** 를 식 언어로 사용 하는 경우 많은 함수를 사용할 수 있습니다. JmesPath 사양의 일부로 사용할 수 있는 함수 외에도 많은 사용자 지정 함수를 사용할 수 있습니다. 이 문서에서는 정규화 과정에서 장치 매핑 템플릿과 함께 사용 하기 위한 IoT 커넥터 관련 사용자 지정 기능을 설명 합니다.

> [!TIP]
> JmesPath 함수에 대 한 자세한 내용은 JmesPath [사양](https://jmespath.org/specification.html#built-in-functions)을 참조 하십시오.

## <a name="function-signature"></a>함수 시그니처

각 함수에는 JmesPath 사양 뒤에 오는 시그니처가 있습니다. 이 서명은 다음과 같이 나타낼 수 있습니다.

```jmespath
return_type function_name(type $argname)
```

시그니처는 인수에 대 한 유효한 형식을 나타냅니다. 인수에 대해 잘못 된 형식이 전달 되 면 오류가 발생 합니다.

> [!NOTE]
> 수학 관련 함수가 완료 되 면 최종 결과는 c # [long](/dotnet/csharp/language-reference/builtin-types/integral-numeric-types#characteristics-of-the-integral-types) 값에 맞출 수 **있어야** 합니다. 최종 결과가 c # Long 값에 맞지 않을 경우 수학적 오류가 발생 합니다.

## <a name="exception-handling"></a>예외 처리

이벤트 처리 수명 주기 내의 다양 한 지점에서 예외가 발생할 수 있습니다. 발생할 수 있는 다양 한 사항은 다음과 같습니다.

|작업|When|템플릿을 구문 분석 하는 동안 발생할 수 있는 예외|결과|
|------|----|-------------------------------------------------|-------|
|**템플릿 구문 분석**|새 메시지 일괄 처리를 받을 때마다 장치 매핑 템플릿이 로드 되 고 구문 분석 됩니다.|템플릿을 구문 분석 하지 못했습니다.|구문 분석이 성공할 때까지 시스템에서 최신 장치 매핑 템플릿 다시 로드 및 구문 분석을 시도 합니다. 구문 분석이 성공할 때까지 새 메시지가 처리 되지 않습니다.|
|**템플릿 구문 분석**|새 메시지 일괄 처리를 받을 때마다 장치 매핑 템플릿이 로드 되 고 구문 분석 됩니다.|식을 구문 분석 하지 못했습니다.|구문 분석이 성공할 때까지 시스템에서 최신 장치 매핑 템플릿 다시 로드 및 구문 분석을 시도 합니다. 구문 분석이 성공할 때까지 새 메시지가 처리 되지 않습니다.|
|**함수 실행**|메시지 내의 데이터에 대해 함수가 실행 될 때마다|입력 데이터가 함수 시그니처의 일치 하지 않습니다.|시스템에서 해당 메시지의 처리를 중지 합니다. 메시지를 다시 시도 하지 않습니다.|
|**함수 실행**|메시지 내의 데이터에 대해 함수가 실행 될 때마다|함수 설명에 나열 된 다른 모든 예외|시스템에서 해당 메시지의 처리를 중지 합니다. 메시지를 다시 시도 하지 않습니다.|
...
왼쪽 인수를 오른쪽에 추가 하 여 생성 된 결과를 반환 합니다.
...
|-----------------------------|------------------|--------|
| 해당 없음                         | 추가 (10, 10)      | 20     |
| {"left": 40, "right": 50}   | 추가 (왼쪽, 오른쪽) | 90     |
| {"left": 0, "right": 50}    | 추가 (왼쪽, 오른쪽) | 50     |
...
|-----------------------------|---------------------|----------------------------------|
| 해당 없음                         | 나누기 (10, 10)      | 1                                |
| {"left": 40, "right": 50}   | 나누기 (왼쪽, 오른쪽) | 0.8                              |
| {"left": 0, "right": 50}    | 나누기 (왼쪽, 오른쪽) | 0                                |
| {"left": 50, "right": 0}    | 나누기 (왼쪽, 오른쪽) | 수학 error: 0으로 나누었습니다. |
...
|-----------------------------|-----------------------|--------|
| 해당 없음                         | 곱하기 (10, 10)      | 100    |
| {"left": 40, "right": 50}   | 곱하기 (왼쪽, 오른쪽) | 2000   |
| {"left": 0, "right": 50}    | 곱하기 (왼쪽, 오른쪽) | 0      |
...
왼쪽 인수를 오른쪽으로 거듭제곱 한 결과를 반환 합니다.
...
>[디바이스 매핑을 사용하는 방법](how-to-use-device-mappings.md)
