...
이 시나리오를 구성하려면 이 자습서의 각 섹션에 제공되는 단계를 사용하여 다음 작업을 완료합니다.
...
- 원하는 임계값으로 Azure 예산을 만들어서 작업 그룹에 연결합니다.
...
2. Azure의 왼쪽 위 모서리에서 **리소스 만들기** 단추를 선택합니다.
3. **관리 도구** > **자동화** 를 선택합니다.
   > [!NOTE]
...
5. 완료되면 **만들기** 를 선택하여 Automation 계정 배포를 시작합니다.
...
1. **프로세스 자동화** 섹션에서 **Runbook 갤러리** 를 선택합니다.
1. **갤러리 원본** 을 **스크립트 센터** 로 설정하고 **확인** 을 선택합니다.
1. Azure Portal 내에서 [Azure V2 VM 중지](https://gallery.technet.microsoft.com/scriptcenter/Stop-Azure-ARM-VMs-1ba96d5b) 갤러리 항목을 찾아 선택합니다.
1. **가져오기** 를 선택하여 **가져오기** 영역을 표시하고 **확인** 을 선택합니다. Runbook 개요 영역이 표시됩니다.
...
[Azure V2 VM 중지](https://gallery.technet.microsoft.com/scriptcenter/Stop-Azure-ARM-VMs-1ba96d5b) 그래픽 Runbook을 사용하여 단일 HTTP 요청을 통해 Azure Automation에서 Runbook을 시작하는 webhook를 두 개 만듭니다. 첫 번째 webhook는 선택적 VM을 중지할 수 있도록 리소스 그룹 이름을 매개 변수로 사용하여 80% 예산 임계값에서 Runbook을 호출합니다. 그런 다음, 두 번째 webhook는 매개 변수 없이(100%) Runbook을 호출하여 남은 VM 인스턴스가 모두 중지됩니다.

1. [Azure Portal](https://portal.azure.com/)의 **Runbook** 페이지에서 Runbook의 개요 영역을 표시하는 **StopAzureV2Vm** Runbook을 선택합니다.
1. 페이지 맨 위에서 **Webhook** 를 선택하여 **Webhook 추가** 영역을 엽니다.
1. **새 Webhook 만들기** 를 선택하여 **새 Webhook 만들기** 영역을 엽니다.
1. Webhook의 **이름** 을 **Optional** 로 설정합니다. **사용** 속성은 **예** 이어야 합니다. **만료** 값은 변경할 필요가 없습니다. Webhook 속성에 대한 자세한 내용은 [Webhook 속성](../../automation/automation-webhooks.md#webhook-properties)을 참조하세요.
1. URL 값 옆에 있는 복사 아이콘을 선택하여 webhook의 URL을 복사합니다.
   > [!IMPORTANT]
   > **Optional** 로 명명된 Webhook의 URL을 안전한 곳에 저장합니다. 이 URL은 자습서의 뒷부분에서 사용합니다. 보안상의 이유로 Webhook를 만들고 나면 URL을 다시 보거나 검색할 수 없습니다.
1. **확인** 을 선택하여 새 webhook를 만듭니다.
1. **매개 변수 및 실행 설정 구성** 을 선택하여 Runbook에 대한 매개 변수 값을 봅니다.
   > [!NOTE]
   > runbook에 필수 매개 변수가 있으면 값을 제공 하지 않는 한 webhook를 만들 수 없습니다.
1. **확인** 을 선택하여 webhook 매개 변수 값을 적용합니다.
1. **만들기** 를 선택하여 webhook를 만듭니다.
1. 다음으로, 위의 단계에 따라 **Complete** 로 명명된 두 번째 Webhook를 만듭니다.
    > [!IMPORTANT]
...
1. **논리 앱 만들기** 영역에서 논리 앱을 만들기 위해 필요한 세부 정보를 제공하고 **대시보드에 고정** 을 선택한 다음, **만들기** 를 선택합니다.  
    ![Azure - 논리 앱 만들기](./media/cost-management-budget-scenario/billing-cost-management-budget-scenario-03a.png)

Azure가 논리 앱을 배포하면 **Logic Apps 디자이너** 가 열리고 소개 비디오 및 일반적으로 사용되는 트리거가 포함된 영역을 보여줍니다.
...
1. **메서드** 값에 **Post** 를 선택합니다.
1. 이 자습서 초반에 만든 **Optional** 로 명명된 Webhook의 URL을 **Uri** 값으로 입력합니다.  
    ![Azure - 논리 앱 - HTTP 작업 URI](./media/cost-management-budget-scenario/billing-cost-management-budget-scenario-16.png)
1. **If true** 상자에서 **작업 추가** 를 선택합니다. 받는 사람에게 선택적 VM이 종료되었음을 알리는 이메일을 보내는 이메일 작업을 추가합니다.
...
1. 받는 사람에게 선택적 VM이 종료되었음을 알리는 전자 메일에 대해 **받는 사람**, **제목** 및 **본문** 텍스트를 추가합니다. **BudgetName** 및 **NotificationThresholdAmount** 동적 콘텐츠를 사용하여 제목과 본문 필드를 채웁니다. 
    ![Azure - 논리 앱 - 이메일 세부 정보](./media/cost-management-budget-scenario/billing-cost-management-budget-scenario-19.png)
...
1. "이메일 보내기"를 검색하고 사용하는 이메일 서비스에 따라 *이메일 보내기* 작업을 선택합니다.
1. 받는 사람에게 선택적 VM이 종료되었음을 알리는 전자 메일에 대해 **받는 사람**, **제목** 및 **본문** 텍스트를 추가합니다. **BudgetName** 및 **NotificationThresholdAmount** 동적 콘텐츠를 사용하여 제목과 본문 필드를 채웁니다.  
    ![Azure - 논리 앱 - 이메일 보내기 세부 정보](./media/cost-management-budget-scenario/billing-cost-management-budget-scenario-24.png)
1. **Logic App 디자이너** 영역의 맨 위에서 **저장** 을 선택합니다.
...
논리 앱을 저장하면 호출할 수 있는 URL이 생성됩니다. 이 URL을 이 자습서의 다음 섹션에서 사용할 것입니다.
...
작업 그룹은 예산과 통합할 유일한 엔드포인트입니다. 여러 채널에서 알림을 설정할 수 있지만, 이 시나리오의 경우 이 자습서 초반에 만든 논리 앱에 중점을 둡니다.

### <a name="create-an-action-group-in-azure-monitor"></a>Azure Monitor에서 작업 그룹 만들기

작업 그룹을 만들면 이 자습서 초반에 만든 논리 앱을 가리킵니다.

1. [Azure Portal](https://portal.azure.com/)에 아직 로그인하지 않은 경우 로그인하고 **모든 서비스** > **모니터** 를 선택합니다.
1. **경고** 를 선택한 다음, **작업 관리** 를 선택합니다.
1. **작업 그룹** 영역에서 **작업 그룹 추가** 를 선택합니다.
1. 다음 사항을 추가하고 확인합니다.
    - 작업 그룹 이름
    - 짧은 이름
    - Subscription
...
1. **작업 그룹 추가** 창에서 LogicApp 작업을 추가합니다. 작업 이름을 **Budget-BudgetLA** 로 지정합니다. **논리 앱** 창에서 **구독** 과 **리소스 이름** 을 선택합니다. 그런 다음, 이 자습서 초반에 만든 **논리 앱** 을 선택합니다.
1. **확인** 을 선택하여 논리 앱을 설정합니다. 그런 다음, **작업 그룹 추가** 창에서 **확인** 을 선택하여 작업 그룹을 만듭니다.
...
1. 리포지토리를 복제하여 로컬 복사본을 얻습니다.
1. Visual Studio에서 프로젝트를 열고 빌드합니다.
1. 빌드가 성공하면 실행 파일은 *\bin\debug* 폴더에 있어야 합니다.
1. ARMClient를 실행합니다. 명령 프롬프트를 열고 프로젝트 루트에서 *\bin\debug* 폴더로 이동합니다.
1. 로그인하여 인증하려면 명령 프롬프트에 다음 명령을 입력합니다.<br>
    `ARMClient login prod`
1. 출력에서 **구독 guid** 를 복사합니다.
1. 권한 부여 토큰을 클립보드에 복사하려면 명령 프롬프트에 다음 명령을 입력합니다. 단, 반드시 위의 단계에서 복사한 구독 ID를 사용하세요. <br>
    `ARMClient token <subscription GUID from previous step>`

    위 단계를 완료하면 다음 메시지가 표시됩니다.<br>
    **토큰을 클립보드에 복사했습니다.**
1. 이 자습서의 다음 섹션의 단계에 사용할 토큰을 저장합니다.
...
1. 새 요청에 아무 것도 없도록 새 요청을 컬렉션으로 저장합니다.  
    ![Postman - 새 요청 저장](./media/cost-management-budget-scenario/billing-cost-management-budget-scenario-28.png)
1. `Get`에서 `Put` 작업으로 요청을 변경합니다.
1. `{subscriptionId}`를 이 자습서의 이전 섹션에서 사용한 **구독 ID** 로 바꿔 다음 URL을 수정합니다. 또한, `{budgetName}`에 대한 값으로 "SampleBudget"을 포함하도록 URL을 수정합니다. `https://management.azure.com/subscriptions/{subscriptionId}/providers/Microsoft.Consumption/budgets/{budgetName}?api-version=2018-03-31`
1. Postman에서 **헤더** 탭을 선택합니다.
1. "권한 부여"로 명명된 새 **키** 를 추가합니다.
