...
Service Bus에서 FIFO 보장을 실현하려면 세션을 사용합니다. Service Bus는 메시지 간의 관계 특성을 규정 하지 않으며 메시지 시퀀스의 시작 또는 종료 위치를 결정 하기 위한 특정 모델을 정의 하지 않습니다.

모든 발신자는 세션 **ID** 속성을 세션에 고유한 응용 프로그램 정의 식별자로 설정 하 여 토픽 또는 큐로 메시지를 전송할 때 세션을 만들 수 있습니다. **Amqp 1.0** 프로토콜 수준에서이 값은 **그룹 id** 속성에 매핑됩니다.
...
> [!IMPORTANT]
> 큐 또는 구독에서 세션을 사용 하는 경우 클라이언트 응용 프로그램은 ***더 이상*** 일반 메시지를 보내고 받을 수 없습니다. 모든 메시지는 세션 ID를 설정하여 세션의 일부로 전송되고 세션을 수락하여 수신되어야 합니다.
...
수신기에서 close 메서드를 호출 하거나 잠금이 만료 되 면 잠금이 해제 됩니다. 수신기가 잠금을 갱신하는 메서드도 있습니다. 대신 자동 잠금 갱신 기능을 사용하여 잠금을 계속 갱신할 기간을 지정할 수 있습니다. 세션 잠금은 파일에 대한 배타적인 잠금으로 처리되어야 합니다. 즉, 애플리케이션은 세션이 더 이상 필요하지 않거나 더 이상 메시지를 기대하지 않는 경우 세션을 닫아야 합니다.
