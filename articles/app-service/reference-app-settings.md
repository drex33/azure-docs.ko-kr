...
| `WEBSITE_AUTH_ENCRYPTION_KEY` | 기본적으로, 자동으로 생성된 키는 암호화 키로 사용됩니다. 재정의하려면 원하는 키로 설정합니다. 이는 여러 앱에서 토큰 또는 세션을 공유하려는 경우에 권장됩니다. ||
| `WEBSITE_AUTH_SIGNING_KEY` | 기본적으로 자동으로 생성된 키는 서명 키로 사용됩니다. 재정의하려면 원하는 키로 설정합니다. 이는 여러 앱에서 토큰 또는 세션을 공유하려는 경우에 권장됩니다. ||
...
| `AZUREFILESSTORAGE_` | 컨테이너 앱에 대한 사용자 지정 Azure File 스토리지에 대한 연결 문자열. |
...
| `WEBSITES_PORT` | 사용자 지정 컨테이너의 경우, 요청을 라우팅할 컨테이너의 사용자 지정 포트 번호. 기본적으로 App Service는 포트 80 및 8080의 자동 포트 검색을 시도합니다. ||
...
| `MACHINEKEY_Decryption` | Windows 컨테이너의 경우, 이 변수는 ASP.NET 암호화 루틴을 사용하도록 컨테이너에 삽입됩니다([machineKey 요소](/previous-versions/dotnet/netframework-4.0/w8h3skw9(v=vs.100)) 참조). 기본 `decryption`값을 재정의하려면, 앱 설정으로 설정합니다. ||
| `MACHINEKEY_DecryptionKey` | Windows 컨테이너의 경우, 이 변수는 ASP.NET 암호화 루틴을 사용하도록 컨테이너에 삽입됩니다([machineKey 요소](/previous-versions/dotnet/netframework-4.0/w8h3skw9(v=vs.100)) 참조). 자동으로 생성된 `decryptionKey` 값을 재정의하려면, 앱 설정으로 설정합니다. ||
| `MACHINEKEY_Validation` | Windows 컨테이너의 경우, 이 변수는 ASP.NET 암호화 루틴을 사용하도록 컨테이너에 삽입됩니다([machineKey 요소](/previous-versions/dotnet/netframework-4.0/w8h3skw9(v=vs.100)) 참조). 기본 `validation`값을 재정의하려면, 앱 설정으로 설정합니다. ||
| `MACHINEKEY_ValidationKey` | Windows 컨테이너의 경우, 이 변수는 ASP.NET 암호화 루틴을 사용하도록 컨테이너에 삽입됩니다([machineKey 요소](/previous-versions/dotnet/netframework-4.0/w8h3skw9(v=vs.100)) 참조). 자동으로 생성된 `validationKey` 값을 재정의하려면, 앱 설정으로 설정합니다. ||
