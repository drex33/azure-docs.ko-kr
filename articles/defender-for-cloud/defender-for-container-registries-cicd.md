---
title: CI/CD 워크플로의 컨테이너 이미지에 대 한 클라우드의 취약성 검색을 위한 Defender
description: 컨테이너 레지스트리 용 Microsoft Defender를 사용 하 여 CI/CD 워크플로에서 컨테이너 이미지를 검색 하는 방법을 알아봅니다.
author: memildin
ms.author: memildin
ms.date: 11/09/2021
ms.topic: how-to
...
스캐너를 설정 하려면 컨테이너 레지스트리 및 CI/CD 통합을 **위해 Microsoft Defender** 를 사용 하도록 설정 해야 합니다. CI/CD 워크플로가 레지스트리에 이미지를 푸시하면 레지스트리 검사 결과와 CI/CD 검사 결과의 요약을 볼 수 있습니다.

CI/CD 검사의 결과는 Qualys의 기존 레지스트리 검사 결과를 보강한 결과입니다. 클라우드의 CI/CD 검색을 위한 Defender는 [바다색 Trivy](https://github.com/aquasecurity/trivy)에서 제공 합니다.
...
CI/CD 워크플로를 사용 하 여 레지스트리에 푸시할 때 이미지를 검색 하려면 구독에서 사용 하도록 설정 된 **컨테이너 레지스트리 용 Microsoft Defender** 가 있어야 합니다.
...
[1 단계. Defender for Cloud에서 CI/CD 통합 사용](#step-1-enable-the-cicd-integration-in-defender-for-cloud)
...
1. GitHub 워크플로에서 다음과 같이 CI/CD 검사를 사용하도록 설정합니다.
    ...
    > [!IMPORTANT]
    >  레지스트리에 푸시하는 결과가 게시 되기 전에 발생 해야 합니다.
...
1. 선택한 컨테이너 레지스트리에 이미지를 푸시할 워크플로를 실행합니다. 이미지가 레지스트리에 푸시되 면 레지스트리 검사가 실행 되며, Microsoft Defender for Cloud 내에서 레지스트리 검색 결과와 함께 CI/CD 검색 결과를 볼 수 있습니다.
...
[Microsoft Defender For Cloud의 고급 보호 계획](defender-for-cloud-introduction.md)에 대해 자세히 알아보세요.
