---
title: Microsoft 센티널에서 플레이 북을 사용 하 여 위협 대응 자동화 | Microsoft Docs
description: 이 문서에서는 Microsoft 센티널의 자동화에 대해 설명 하 고, 플레이 북을 사용 하 여 위협 방지 및 응답을 자동화 하는 방법을 보여 줍니다.
services: sentinel
cloud: na
...
ms.locfileid: "132522140"
---
# <a name="automate-threat-response-with-playbooks-in-microsoft-sentinel"></a>Microsoft 센티널에서 플레이 북을 사용 하 여 위협 대응 자동화

[!INCLUDE [Banner for top of topics](./includes/banner.md)]

이 문서에서는 Microsoft 센티널 플레이 북의 정의와이를 사용 하 여 보안 오케스트레이션, Automation and Response (대화 충성도) 작업을 구현 하는 데 시간과 리소스를 절약 하는 동시에 더 나은 결과를 얻는 방법에 대해 설명 합니다.
...
플레이 북은 Microsoft 센티널에서 루틴으로 실행할 수 있는 이러한 수정 작업의 컬렉션입니다. 플레이북은 [**위협 대응을 자동화하고 오케스트레이션**](tutorial-respond-threats-playbook.md)하는 데 도움이 될 수 있습니다. 각각 분석 규칙 또는 자동화 규칙에 따라 트리거되는 경우 특정 경고나 인시던트에 대응하여 플레이북을 자동으로 실행되도록 설정하거나 수동으로 실행할 수 있습니다.

예를 들어 계정과 머신이 손상된 경우 플레이북은 SOC 팀이 인시던트 알림을 받을 때까지 네트워크에서 머신을 격리하고 계정을 차단할 수 있습니다.

플레이 북은 속한 구독 내에서 사용할 수 있지만, 플레이 **북 탭 (** **Automation** 블레이드에서)은 선택한 구독에서 사용할 수 있는 모든 플레이 북을 표시 합니다.

### <a name="playbook-templates"></a>플레이 북 템플릿
...
- **플레이 북 템플릿** 탭 ( **Automation** 아래)은 Microsoft 센티널 커뮤니티에서 제공 하는 주요 시나리오를 제공 합니다. 동일한 템플릿에서 여러 활성 플레이 북을 만들 수 있습니다.

    새 버전의 템플릿이 게시 되 면 해당 템플릿에서 만든 활성 플레이 북 (플레이 **북** 탭)에 업데이트를 사용할 수 있다는 알림이 표시 됩니다.

- 플레이 북 템플릿은 특정 제품의 컨텍스트에서 [Microsoft 센티널 솔루션](sentinel-solutions.md) 의 일부로 가져올 수도 있습니다. 솔루션을 배포 하면 활성 플레이 북이 생성 됩니다.

- [Microsoft 센티널 GitHub 리포지토리에](https://github.com/Azure/Azure-Sentinel/tree/master/Playbooks) 는 많은 플레이 북 템플릿이 포함 되어 있습니다. Azure **에 배포** 단추를 선택 하 여 azure 구독에 배포할 수 있습니다.

기술적으로 플레이 북 템플릿은 관련 된 각 연결에 대 한 Azure Logic Apps 워크플로 및 API 연결의 여러 리소스로 구성 된 [ARM 템플릿입니다](../azure-resource-manager/templates/index.yml) .

### <a name="azure-logic-apps-basic-concepts"></a>Azure Logic Apps 기본 개념

Microsoft 센티널의 플레이 북은 기업 전체 시스템에서 작업 및 워크플로를 예약, 자동화 및 오케스트레이션 하는 데 도움이 되는 클라우드 서비스인 [Azure Logic Apps](../logic-apps/logic-apps-overview.md)에서 빌드된 워크플로를 기반으로 합니다. 즉, 플레이북은 Logic Apps 기본 제공 템플릿을 모두 활용하고 사용자 지정할 수 있습니다.
...
- **Microsoft 센티널 커넥터:** Microsoft 센티널과 상호 작용 하는 플레이 북을 만들려면 Microsoft 센티널 커넥터를 사용 합니다.
  - [Microsoft 센티널 커넥터 설명서](/connectors/azuresentinel/)

- **트리거:** 플레이북을 시작하는 커넥터 구성 요소입니다. 플레이북이 트리거될 때 수신하는 데 필요한 스키마를 정의합니다. Microsoft 센티널 커넥터에는 현재 두 개의 트리거가 있습니다.
...
### <a name="permissions-required"></a>필요한 사용 권한
 ...
 SecOps 팀에 대화 충성도 (보안 오케스트레이션, 자동화 및 응답) 작업에 대 한 Logic Apps를 사용 하는 기능을 제공 하는 것입니다. 즉, Microsoft 센티널에서 플레이 북을 만들고 실행 하려면 보안 운영 팀의 특정 멤버나 전체 팀에 Azure 역할을 할당할 수 있습니다. 다음은 사용 가능한 여러 역할과 해당 역할에 할당해야 하는 작업에 대한 설명입니다.
...
- **Microsoft 센티널 기여자** 역할을 사용 하면 플레이 북를 분석 규칙에 연결할 수 있습니다.
- **Microsoft 센티널 응답기** 역할을 통해 플레이 북를 수동으로 실행할 수 있습니다.
- **Microsoft 센티널 Automation 기여자** 를 사용 하면 자동화 규칙에서 플레이 북을 실행할 수 있습니다. 다른 용도로는 사용되지 않습니다.
...
- [Microsoft 센티널의 Azure 역할에 대해 자세히 알아보세요](roles.md).
...
Azure Logic Apps 플랫폼은 수백 개의 작업과 트리거를 제공하므로 거의 모든 자동화 시나리오를 만들 수 있습니다. Microsoft 센티널은 다음 SOC 시나리오부터 시작 하는 것이 좋습니다.
...
Microsoft 센티널 인시던트는 IP 주소 엔터티를 생성 하는 분석 규칙에 의해 경고에서 만들어졌습니다.

인시던트는 다음 단계에 따라 플레이북을 실행하는 자동화 규칙을 트리거합니다.

- [새 Microsoft 센티널 인시던트가 생성](/connectors/azuresentinel/#triggers)될 때 시작 합니다. 인시던트에 표시되는 엔터티는 인시던트 트리거의 동적 필드에 저장됩니다.
...
**플레이 북은 Microsoft 센티널 인시던트를 다른 티켓 시스템과 동기화 하는 데 사용할 수 있습니다.**

예를 들면 다음과 같습니다.

모든 인시던트 생성에 대한 자동화 규칙을 만들고 ServiceNow에서 티켓을 여는 플레이북을 연결합니다.

- [새 Microsoft 센티널 인시던트가 생성](/connectors/azuresentinel/#triggers)될 때 시작 합니다.

- [ServiceNow](/connectors/service-now/)에서 새 티켓을 만듭니다.

- 간편한 피벗을 위해 인시던트 이름, 중요 필드 및 Microsoft 센티널 인시던트에 대 한 URL을 티켓에 포함 합니다.
...
Microsoft 센티널 인시던트는 사용자 이름 및 IP 주소 엔터티를 생성 하는 분석 규칙에 의해 경고에서 만들어졌습니다.
...
**예 2:** [엔드포인트용 Microsoft Defender](/windows/security/threat-protection/)에서 검색한 손상된 머신을 나타내는 분석 규칙에 응답합니다.
   ...
   - Microsoft 센티널에서 **엔터티-호스트 가져오기** 작업을 사용 하 여 인시던트 엔터티에 포함 된 의심 스러운 컴퓨터를 구문 분석 합니다.
...
경고 생성에 의해 트리거되고 경고를 입력으로 수신 하는 플레이 북의 경우 (첫 번째 단계는 "Microsoft 센티널 경고가 트리거될 때") 플레이 북를 분석 규칙에 연결 합니다.
...
인시던트 생성 및 수신 인시던트를 입력으로 트리거한 플레이 북의 경우 (첫 번째 단계는 "Microsoft 센티널 인시던트가 트리거될 때") automation 규칙을 만들고 여기에서 **플레이 북 실행** 작업을 정의 합니다. 이 작업은 두 가지 방법으로 수행할 수 있습니다.
...
- **자동화** 블레이드의 **자동화 규칙** 탭에서 새 자동화 규칙을 만들고 적절한 조건 및 원하는 작업을 지정합니다. 이 자동화 규칙은 지정된 조건을 충족하는 모든 분석 규칙에 적용됩니다.
    ...
    > [!NOTE]
    > **Microsoft 센티널 automation 규칙은 플레이 북을 실행할 수 있는 권한이 필요 합니다.**
    >
    > Automation 규칙에서 플레이 북를 실행 하기 위해 Microsoft 센티널은 특별히 권한이 부여 된 서비스 계정을 사용 합니다. 사용자 계정과 달리 이 계정을 사용하면 서비스의 보안 수준을 높이고 자동화 규칙 API를 사용하여 CI/CD 사용 사례를 지원할 수 있습니다.
    >
    > 이 계정에는 플레이 북 있는 리소스 그룹에 대 한 명시적 사용 권한 ( **Microsoft 센티널 Automation 참가자** 역할의 형식 사용)이 있어야 합니다. 이때 모든 자동화 규칙은 해당 리소스 그룹에서 모든 플레이북을 실행할 수 있습니다.
    >
    > **플레이북 실행** 작업을 자동화 규칙에 추가하면 선택한 플레이북의 드롭다운 목록이 표시됩니다. Microsoft 센티널에 권한이 없는 플레이 북은 사용할 수 없는 것으로 표시 됩니다 ("회색으로 표시 됨"). **플레이 북 권한 관리** 링크를 선택 하 여 해당 지점에서 Microsoft 센티널에 권한을 부여할 수 있습니다.
    >
    ...
    >
    > **MSSP(관리형 보안 서비스 공급자)** 가 있는 고유한 시나리오가 있습니다. 여기서 서비스 공급자는 자체 테넌트에 로그인한 상태에서 [Azure Lighthouse](../lighthouse/index.yml)를 사용하여 고객의 작업 영역에 자동화 규칙을 만듭니다. 그러면 이 자동화 규칙이 고객의 테넌트에 속한 플레이북을 호출합니다. 이 경우 Microsoft 센티널은 **_두 테 넌 트_ _에 대 한 권한을 부여 받아야 합니다 *. 고객 테 넌 트에서*** 일반 다중 테 넌 트 시나리오와 마찬가지로 _ 플레이 북 권한 관리 패널에서 부여 합니다. 서비스 공급자 테 넌 트에서 관련 사용 권한을 부여 하려면 플레이 북가 있는 리소스 그룹에서 **Microsoft 센티널 Automation 참가자** 역할을 사용 하 여 **azure 보안 Insights** 앱에 대 한 액세스 권한을 부여 하는 추가 azure Lighthouse 위임을 추가 해야 합니다. [이 위임을 추가하는 방법을 알아봅니다](tutorial-respond-threats-playbook.md#permissions-to-run-playbooks).
...
다음 블레이드의 Microsoft 센티널 포털에서 수동 트리거를 사용할 수 있습니다.

- **인시던트** 보기에서 특정 인시던트를 선택하고, **경고** 탭을 연 다음 경고를 선택합니다.

- **조사** 에서 특정 경고를 선택합니다.

1. 선택한 경고에 대해 **플레이북 보기** 를 클릭합니다. **Microsoft 센티널 경고가 트리거되고** 액세스할 수 있는 경우로 시작 하는 모든 플레이 북의 목록이 표시 됩니다.
...
|-|-|
| **Microsoft 센티널 인시던트/경고** | 플레이북이 Sentinel 트리거 중 하나(경고, 인시던트)로 시작됩니다. |
| **Microsoft 센티널 작업 사용** | 플레이 북는 비 센티널 트리거로 시작 되지만 Microsoft 센티널 작업을 사용 합니다. |
...
[Microsoft 센티널 GitHub 리포지토리에서](https://github.com/Azure/Azure-Sentinel/tree/master/Playbooks)다음과 같은 권장 플레이 북 및 기타 유사한 플레이 북을 사용할 수 있습니다.
...
- **플레이북을 만들거나 업데이트하거나 닫으면** Microsoft Sentinel, Microsoft 365 보안 서비스 또는 기타 티켓 시스템에서 인시던트 만들기, 업데이트 또는 종결이 가능합니다.
...
- [자습서: 플레이북을 사용하여 Microsoft Sentinel에서 위협 대응 자동화](tutorial-respond-threats-playbook.md)
