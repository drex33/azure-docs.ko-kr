...
Azure PowerShell Azure에 연결해야 합니다.

- **로컬 컴퓨터에 Azure PowerShell cmdlet을 설치합니다.** Bicep 파일을 배포하려면 [Azure PowerShell](/powershell/azure/install-az-ps) 버전 **5.6.0 이상이** 필요합니다. 자세한 내용은 [Azure PowerShell 시작](/powershell/azure/get-started-azureps)을 참조하세요.
...
- **구독에** 배포하려면 cmdlet의 별칭인 [New-AzSubscriptionDeployment](/powershell/module/az.resources/new-azdeployment)를 사용합니다. `New-AzDeployment`
...
현재 Azure PowerShell은 원격 Bicep 파일 배포를 지원하지 않습니다. [Bicep CLI](./install.md#vs-code-and-bicep-extension)를 사용하여 Bicep 파일을 JSON 템플릿으로 컴파일한 다음 JSON 파일을 원격 위치로 로드합니다.
...
인라인 매개 변수를 전달하려면 `New-AzResourceGroupDeployment` 명령을 사용하여 매개 변수 이름을 제공합니다. 예를 들어, 문자열 및 배열을 Bicep 파일에 전달하려면 다음을 사용합니다.
