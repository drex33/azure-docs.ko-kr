...
* Velpic SAML에서 **SP** 시작 SSO를 지원합니다.
* Velpic SAML에서 [자동화된 사용자 프로비저닝](velpic-provisioning-tutorial.md)을 지원합니다.
...
## <a name="configure-and-test-azure-ad-sso-for-velpic-saml"></a>Velpic SAML에 대한 Azure AD SSO 구성 및 테스트
...
Velpic SAML에서 Azure AD SSO를 구성하고 테스트하려면 다음 단계를 수행합니다.
...
1. Azure Portal의 **Velpic SAML** 애플리케이션 통합 페이지에서 **관리** 섹션을 찾고 **Single Sign-On** 을 선택합니다.
...
4. **관리** 탭을 클릭하고 **플러그 인** 단추를 클릭해야 하는 **통합** 섹션으로 이동하여 로그인을 위한 새 플러그 인을 만듭니다.
    ...
    ![스크린샷은 플러그 인을 선택할 수 있는 통합 페이지를 보여줍니다.](./media/velpicsaml-tutorial/plugin.png)

5. **플러그 인 추가** 단추를 클릭합니다.
    
    ![스크린샷은 선택된 플러그 인 추가 단추를 보여줍니다.](./media/velpicsaml-tutorial/add-button.png)

6. 플러그 인 추가 페이지에서 **SAML** 타일을 클릭합니다.
    
    ![스크린샷은 플러그 인 추가 페이지에서 선택한 SAML을 보여줍니다.](./media/velpicsaml-tutorial/integration.png)

7. 새 SAML 플러그 인 이름을 입력하고 **추가** 단추를 클릭합니다.

    ![스크린샷은 Azure AD가 입력된 새 SAML 플러그 인 추가 대화 상자를 보여줍니다.](./media/velpicsaml-tutorial/new-plugin.png)

8. 세부 정보를 다음과 같이 입력합니다.

    ![스크린샷은 설명된 값을 입력할 수 있는 Azure AD 페이지를 보여줍니다.](./media/velpicsaml-tutorial/details.png)
    ...
    d. **새 사용자 자동 만들기** 확인란을 사용하여 SAML JIT(just in time) 프로비전을 사용할 수도 있습니다. 사용자자 Velpic에 없는 경우 이 플래그는 활성화되지 않으며 Azure에서 로그인에 실패합니다. 플래그가 활성화된 경우 사용자는 로그인 시 자동으로 Velpic으로 프로비전됩니다. 
...
1. 관리 탭을 클릭하고 사용자 섹션으로 이동한 다음 새로 만들기 단추를 클릭하여 사용자를 추가합니다.
    ...
    ![사용자 추가](./media/velpicsaml-tutorial/new-user.png)

2. **“새 사용자 만들기”** 대화 상자 페이지에서 다음 단계를 수행합니다.

    ![사용자](./media/velpicsaml-tutorial/create-user.png)
...
> [!NOTE]
> Velpic SAML에서 자동화된 사용자 프로비저닝도 지원합니다. 자동화된 사용자 프로비저닝 구성 방법에 대한 자세한 내용은 [여기](./velpic-provisioning-tutorial.md)를 참조하세요.

## <a name="test-sso"></a>SSO 테스트 

이 섹션에서는 [내 앱]을 사용하여 Azure AD Single Sign-On 구성을 테스트합니다.

1. 내 앱에서 Velpic SAML 타일을 클릭하면 Velpic SAML 애플리케이션의 로그인 페이지를 가져와야 합니다. 로그인 페이지에 **Azure AD를 사용하여 로그인** 단추가 표시되어야 합니다.

    ![스크린샷은 Azure AD를 사용하여 로그인이 선택된 학습 포털을 보여줍니다.](./media/velpicsaml-tutorial/login.png)

1. **Azure AD를 사용하여 로그인** 단추를 클릭하여 Azure AD 계정을 사용하여 Velpic에 로그인합니다.

## <a name="next-steps"></a>다음 단계

Velpic SAML을 구성하면 세션 제어를 적용하여 조직의 중요한 데이터 반출과 반입을 실시간으로 보호할 수 있습니다. 세션 제어는 조건부 액세스에서 확장됩니다. [Microsoft Defender for Cloud Apps를 사용하여 세션 제어를 적용하는 방법을 알아봅니다](/cloud-app-security/proxy-deployment-aad).
