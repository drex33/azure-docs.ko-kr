---
title: 관리되는 실행 명령을 사용하여 Azure의 Linux VM에서 스크립트 실행(미리 보기)
description: 이 항목에서는 업데이트된 관리되는 실행 명령 기능을 사용하여 Azure Linux 가상 머신 내에서 스크립트를 실행하는 방법에 대해 설명합니다.
services: automation
...
ms.locfileid: "131554814"
---
# <a name="preview-run-scripts-in-your-linux-vm-by-using-managed-run-commands"></a>미리 보기: 관리형 실행 명령을 사용하여 Linux VM에서 스크립트 실행
...
실행 명령 기능은 VM(가상 머신) 에이전트를 사용하여 Azure Linux VM 내에서 스크립트를 작성합니다. 이러한 스크립트는 일반 머신 또는 애플리케이션 관리에 사용할 수 있습니다. VM 액세스 및 네트워크 문제를 신속하게 진단 및 수정하고 VM을 다시 양호한 상태로 되돌릴 수 있습니다.
...
다음 예제에서는 [az vm run-command를](/cli/azure/vm/run-command) 사용하여 Azure Linux VM에서 셸 스크립트를 실행합니다.
