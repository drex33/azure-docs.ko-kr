---
title: Azure Sentinel 작업 영역 아키텍처 모범 사례
description: Azure Sentinel 작업 영역을 디자인하기 위한 모범 사례에 대해 알아봅니다.
services: sentinel
author: batamig
ms.author: bagol
...
사용할 테넌트 및 작업 영역 수를 결정하는 경우 대부분의 Azure Sentinel 기능에서 단일 작업 영역 또는 Azure Sentinel 인스턴스를 사용하여 작동하고, Azure Sentinel에서 작업 영역 내에 있는 모든 로그를 수집한다는 것을 고려합니다.

> [!IMPORTANT]
> 비용은 Azure Sentinel 아키텍처를 결정할 때 고려해야 할 주요 사항 중 하나입니다. 자세한 내용은 [Azure Sentinel 비용 및 청구](azure-sentinel-billing.md)를 참조하세요.
>
...
데이터가 수집, 저장 및 처리되면 규정 준수는 Azure Sentinel 아키텍처에 상당한 영향을 주는 중요한 디자인 요구 사항이 될 수 있습니다. 모든 조건에서 누가 어떤 데이터에 액세스할 수 있는지 확인하고 증명할 수 있는 능력은 많은 국가와 지역에서 중요한 데이터 주권 요구 사항이며, Azure Sentinel 워크플로에서 위험을 평가하고 인사이트를 얻는 것이 많은 고객의 우선 순위입니다.

Azure Sentinel에서 데이터는 Microsoft의 기계 학습을 활용하는 검색 규칙을 사용하는 경우와 같이 일부 예외를 제외하고는 대부분 동일한 지리적 위치 또는 지역에 저장되고 처리됩니다. 이러한 경우 데이터는 작업 영역 외부에 복사하여 처리할 수 있습니다.
...
>
> 데이터를 Azure Sentinel 작업 영역과 다른 지리적 위치 또는 지역에 보내는 경우 보내는 리소스가 Azure에 있는지 여부에 관계없이 동일한 지리적 위치 또는 지역의 작업 영역을 사용하는 것이 좋습니다.
>

## <a name="region-considerations"></a>지역 고려 사항

각 지역에 대해 별도의 Azure Sentinel 인스턴스를 사용합니다. Azure Sentinel은 여러 지역에서 사용할 수 있지만, 데이터를 팀, 지역 또는 사이트별로 구분해야 하는 요구 사항 또는 다중 지역 모델을 불가능하게 만들거나 필요한 것 이상으로 복잡하게 만드는 규정과 제어가 있을 수 있습니다. 각 지역에 대해 별도의 인스턴스와 작업 영역을 사용하면 데이터를 지역 간에 이동하기 위한 대역폭/송신 비용을 방지할 수 있습니다.
...
예를 들어 미국 동부의 Virtual Machines에서 로그를 수집하여 미국 서부의 Azure Sentinel 작업 영역에 보내도록 결정하면 데이터 전송에 대한 수신 비용이 청구됩니다. Log Analytics 에이전트는 전송 중 데이터를 압축하므로 대역폭에 대해 청구되는 크기는 Azure Sentinel의 로그 크기보다 작을 수 있습니다.

전 세계의 여러 원본에서 Syslog 및 CEF 로그를 수집하는 경우 규정 준수가 문제가 되는 경우를 제외하고는 대역폭 비용을 방지하기 위해 Syslog 수집기를 Azure Sentinel 작업 영역과 동일한 지역에 설정할 수 있습니다.

대역폭 비용이 별도의 Azure Sentinel 작업 영역을 정당화하는지 여부를 이해하는 것은 지역 간에 전송해야 하는 데이터의 양에 따라 달라집니다. [Azure 가격 계산기](https://azure.microsoft.com/pricing/details/bandwidth/)를 사용하여 비용을 예측합니다.
...
여러 팀에서 동일한 데이터에 액세스해야 하는 상황이 계획되어 있을 수 있습니다. 예를 들어 SOC 팀에서 모든 Azure Sentinel 데이터에 액세스할 수 있어야 하며, 운영 및 애플리케이션 팀에서는 특정 부분에만 액세스해야 합니다. 또한 독립적인 보안 팀은 다양한 데이터 세트를 사용하여 Azure Sentinel 기능에 액세스해야 할 수 있습니다.
...
이 이미지에서 Azure Sentinel 작업 영역은 권한을 더 효율적으로 격리하기 위해 별도의 구독에 배치됩니다.

> [!NOTE]
> 또 다른 옵션은 Azure Sentinel을 별도의 보안 전용 관리 그룹에 배치하여 최소한의 권한 할당만 상속하도록 하는 것입니다. 보안 팀 내에서 권한은 해당 기능에 따라 여러 그룹에 할당됩니다. 이러한 팀은 전체 작업 영역에 액세스할 수 있으므로 할당된 Azure Sentinel 역할에서만 제한되는 전체 Azure Sentinel 환경에 액세스할 수 있습니다. 자세한 내용은 [Azure Sentinel의 권한](roles.md)을 참조하세요.
>

보안 구독 외에도 별도의 구독이 애플리케이션 팀에서 워크로드를 호스트하는 데 사용됩니다. 애플리케이션 팀에는 리소스를 관리할 수 있는 해당 리소스 그룹에 대한 액세스 권한이 부여됩니다. 로그가 직접 액세스할 수 *없는* 작업 영역에 저장되는 경우에도 이러한 팀은 이 별도의 구독 및 리소스 컨텍스트 RBAC를 통해 액세스 권한이 있는 모든 리소스에서 생성된 로그를 볼 수 있습니다. 애플리케이션 팀은 Azure Portal의 **로그** 영역을 통해 로그에 액세스하여 특정 리소스에 대한 로그를 표시하거나 Azure Monitor를 통해 동시에 액세스할 수 있는 모든 로그를 표시할 수 있습니다.
...
조직 내에 다양한 엔터티, 자회사 또는 지역이 있고 각각 Azure Sentinel에 액세스해야 하는 자체 보안 팀이 있는 경우 각 엔터티 또는 자회사에 대해 별도의 작업 영역을 사용합니다. 단일 Azure AD 테넌트 내에서 또는 Azure Lighthouse를 사용하여 여러 테넌트에 간에 별도의 작업 영역을 구현합니다. 

또한 중앙 SOC 팀은 선택적인 추가 Azure Sentinel 작업 영역 사용하여 분석 규칙 또는 통합 문서와 같은 중앙 집중식 아티팩트를 관리할 수 있습니다.
...
Azure Sentinel에 사용할 Log Analytics 작업 영역을 만드는 경우 다음과 같은 모범 사례 지침을 사용합니다.

- **작업 영역 이름을 지정할 때** 다른 작업 영역에서 쉽게 식별할 수 있도록 *Azure Sentinel* 또는 다른 표시기를 이름에 포함합니다.

- Azure Security Center에서 수집한 모든 로그를 Azure Sentinel에서도 수집하고 사용할 수 있도록 **동일한 작업 영역을 Azure Sentinel과 Azure Security Center 모두에 사용합니다**. Azure Security Center에서 만든 기본 작업 영역은 Azure Sentinel에 사용할 수 있는 작업 영역으로 표시되지 않습니다.

- **예상 데이터 수집이 1TB/일 이상이면 전용 작업 영역 클러스터를 사용합니다**. [전용 클러스터](../azure-monitor/logs/logs-dedicated-clusters.md)를 사용하면 Azure Sentinel 데이터에 대한 리소스를 보호할 수 있으므로 대규모 데이터 세트에 대한 쿼리 성능이 향상됩니다. 또한 전용 클러스터는 조직의 키를 더 많이 암호화하고 제어할 수 있는 옵션을 제공합니다.

## <a name="simplify-working-with-multiple-workspaces"></a>여러 작업 영역 작업 간소화

여러 작업 영역을 사용해야 하는 경우 [각 Azure Sentinel 인스턴스의 모든 인시던트를 단일 위치에 압축하고 나열](multiple-workspace-view.md)하여 인시던트 관리 및 조사를 간소화합니다.

[작업 영역 간 통합 문서](extend-sentinel-across-workspaces-tenants.md#cross-workspace-workbooks)에서와 같이 다른 Azure Sentinel 작업 영역에 저장된 데이터를 참조하려면 [작업 영역 간 쿼리](extend-sentinel-across-workspaces-tenants.md)를 사용합니다.
...
자세한 내용은 [작업 영역 및 테넌트에 걸쳐 Azure Sentinel 확장](extend-sentinel-across-workspaces-tenants.md)을 참조하세요.
