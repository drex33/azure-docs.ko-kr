...
ms.openlocfilehash: a692fb39a374919db4c9762d5db901f0dc6328c7
ms.sourcegitcommit: 81a1d2f927cf78e82557a85c7efdf17bf07aa642
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/19/2021
ms.locfileid: "132807657"
---
...
관리형 온라인 엔드포인트는 턴키 방식으로 ML 모델을 배포하는 데 도움이 됩니다. 관리형 온라인 엔드포인트는 확장 가능하고 완전히 관리되는 방식으로 Azure의 강력한 CPU 및 GPU 머신에서 작동합니다. 관리형 온라인 엔드포인트는 모델 제공, 크기 조정, 보안 및 모니터링을 처리하여 기본 인프라를 설정하고 관리하는 오버헤드로부터 벗어날 수 있습니다. 이 문서의 주요 예제에서는 배포에 관리되는 온라인 엔드포인트를 사용합니다. 대신 Kubernetes를 사용하려면 관리되는 온라인 엔드포인트 토론과 함께 이 문서의 참고를 인라인으로 참조하세요. 자세한 내용은 [Azure Machine Learning 엔드포인트(미리 보기)란?](concept-endpoints.md)을 참조하세요.
...
> [!TIP]
> 온라인 끝점의 점수 매기기 스크립트 형식은 이전 버전의 CLI 및 Python SDK에서 사용 되는 것과 동일한 형식입니다.
...
디버깅 시간을 절약하려면 엔드포인트를 로컬에서 테스트 실행하는 것이 *매우 좋습니다*. 자세한 내용은 [Visual Studio Code에서 로컬로 관리 되는 온라인 끝점 디버그](how-to-debug-managed-online-endpoints-visual-studio-code.md)를 참조 하세요.
...
> [!TIP]
> Visual Studio Code를 사용 하 여 끝점을 로컬로 테스트 하 고 디버그할 수 있습니다. 자세한 내용은 [Visual Studio Code에서 로컬로 관리 되는 온라인 끝점 디버그](how-to-debug-managed-online-endpoints-visual-studio-code.md)를 참조 하세요.

### <a name="verify-the-local-deployment-succeeded"></a>로컬 배포에 성공했는지 확인

상태를 확인 하 여 모델이 오류 없이 배포 되었는지 여부를 확인 합니다.

:::code language="azurecli" source="~/azureml-examples-main/cli/deploy-local-endpoint.sh" ID="get_status":::

출력은 다음 JSON과 유사 하 게 표시 됩니다. 여기서 `provisioning_state`는 `Succeeded`입니다.
...
REST 클라이언트(예: curl)를 사용하려면 채점 URI가 있어야 합니다. 채점 URI를 얻으려면 `az ml online-endpoint show --local -n $ENDPOINT_NAME`을 실행합니다. 반환된 데이터에서 `scoring_uri` 특성을 찾습니다. Sample 말아 based 명령은이 문서의 뒷부분에서 사용할 수 있습니다.
...
> [!Important]
> 위의 모든 트래픽 플래그는 `az ml online-deployment create` 끝점에 대 한 트래픽의 100%를 새로 만든 배포에 할당 합니다. 이 방법은 개발 및 테스트 목적으로 유용 하지만 프로덕션 환경에서는 명시적인 명령을 통해 새 배포에 대 한 트래픽을 열 수 있습니다. 예를 들어 `az ml online-endpoint update -n $ENDPOINT_NAME --traffic "blue=100"` 
...
다음 예제에서는 끝점을 인증 하는 데 사용 되는 키를 가져오는 방법을 보여 줍니다.
...
다음으로, 말아 넘기기를 사용 하 여 데이터 점수를 계산 합니다.

::: code language="azurecli" source="~/azureml-examples-main/cli/deploy-managed-online-endpoint.sh" ID="test_endpoint_using_curl" :::

`show`및 `get-credentials` 명령을 사용 하 여 인증 자격 증명을 가져옵니다. 또한 플래그를 사용 하 여 `--query` 특성을 필요한 항목 으로만 필터링 합니다. `--query`에 대한 자세한 내용은 [Azure CLI 명령 출력 쿼리](/cli/azure/query-azure-cli)를 참조하세요.

호출 로그를 보려면 `get-logs`를 다시 실행합니다.

### <a name="optional-update-the-deployment"></a>(선택 사항) 배포 업데이트

코드, 모델 또는 환경을 업데이트 하려면 YAML 파일을 업데이트 한 후 `az ml online-endpoint update` 명령을 실행 합니다. 

> [!Note]
> 단일 명령에서 인스턴스 개수와 기타 모델 설정 (코드, 모델 또는 환경)을 함께 업데이트 하는 경우에 `update` 는 먼저 크기 조정 작업이 수행 되 고 다른 업데이트가 적용 됩니다. 프로덕션 환경에서는 이러한 작업을 개별적으로 수행 하는 것이 좋습니다.
...
이 `update` 명령은 로컬 배포 에서도 작동 합니다. 동일한 `az ml online-deployment update` 명령에 `--local` 플래그를 사용합니다.
