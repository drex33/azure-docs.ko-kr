...
이 마이그레이션 가이드는 데이터베이스를 MongoDB에서 Azure CosmosDB API for MongoDB로 마이그레이션하는 시리즈의 일부입니다. 중요한 마이그레이션 단계는 아래와 같이 [마이그레이션 사전 작업](mongodb-pre-migration.md), 마이그레이션 및 [마이그레이션 사후 작업](mongodb-post-migration.md)입니다.

:::image type="content" source="./media/mongodb-pre-migration/overall-migration-steps.png" alt-text="마이그레이션 단계 다이어그램":::
...
- 처리량 추정 및 분할된 데이터베이스 키 선택과 같은 [사전 마이그레이션 단계를 완료](mongodb-pre-migration.md)합니다.
...
:::image type="content" source="./media/mongodb-migrate-databricks/databricks-cluster-creation.png" alt-text="Databrick 새 클러스터 만들기 다이어그램.":::
...
:::image type="content" source="./media/mongodb-migrate-databricks/databricks-cluster-dependencies.png" alt-text="Databrick 클러스터 종속성을 추가하는 다이어그램.":::
...
- **maxBatchSize**: `maxBatchSize` 값은 데이터가 대상 Azure Cosmos DB 컬렉션에 저장되는 속도를 제어합니다. 그러나 maxBatchSize가 수집 처리량에 비해 너무 높으면 [속도 제한](prevent-rate-limiting-errors.md) 오류가 발생할 수 있습니다.

  Spark 클러스터의 실행자 수, 잠재적으로 작성되는 각 문서의 크기(RU 비용) 및 대상 컬렉션 처리량 제한에 따라 작업자 수와 maxBatchSize를 조정해야 합니다.

  >[!TIP]
  >maxBatchSize = 수집 처리량/(문서 1개의 RU 비용 \* Spark 작업자 수 \* 작업자당 CPU 코어 수)
...
- **큰 데이터 볼륨으로 인한 과도한 데이터 기울이기**. 특정 테이블로 마이그레이션할 데이터가 많지만 데이터에 상당한 기울이기가 있는 경우 테이블에 프로비저닝된 여러 [요청 단위](request-units.md)가 있더라도 여전히 비율 제한이 발생할 수 있습니다. 요청 단위는 실제 파티션 간에 균등하게 분할되며 과도한 데이터 기울이기로 인해 단일 분할된 데이터베이스에 대한 요청 병목 현상이 발생할 수 있습니다. 데이터 기울이기는 동일한 분할 키 값에 대한 많은 수의 레코드를 의미합니다.
...
데이터를 마이그레이션한 후 Azure Cosmos DB에 연결하여 데이터를 관리할 수 있습니다. 인덱싱 정책 최적화와 같은 다른 마이그레이션 후 단계를 따르거나, 기본 일관성 수준을 업데이트하거나, Azure Cosmos DB 계정에 대한 글로벌 배포를 구성할 수도 있습니다. 자세한 내용은 [마이그레이션 후 최적화](mongodb-post-migration.md) 문서를 참조하세요.
