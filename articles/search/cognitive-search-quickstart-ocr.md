---
title: '빠른 시작: OCR 및 이미지 분석'
titleSuffix: Azure Cognitive Search
description: 데이터 가져오기 마법사 및 AI 인식 기술을 사용해 OCR 및 이미지 분석을 적용하여 이미지 파일에서 검색 가능한 텍스트를 식별하고 추출합니다.
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
Azure Cognitive Search의 AI 보강이 OCR(광학 문자 인식) 및 이미지 분석을 추가하여 이미지 파일에서 검색 가능한 콘텐츠를 만드는 방법을 알아봅니다. 

이 빠른 시작에서는 **데이터 가져오기** 마법사를 실행하여 JPG 파일의 시각적 콘텐츠를 분석합니다. 콘텐츠는 기호 사진으로 구성됩니다. 출력은 OCR을 통해 식별된 캡션, 태그 및 텍스트를 포함하는 검색 가능한 인덱스이며, 모두 [검색 탐색기](search-explorer.md)를 사용하여 포털에서 쿼리할 수 있습니다. 
...
1. GitHub에서 [샘플 데이터를 다운로드](https://github.com/Azure-Samples/azure-search-sample-data)합니다. 여러 데이터 세트가 있습니다. 이 빠른 시작의 경우 **unsplash-images\jpg-signs** 폴더의 파일을 사용합니다.
...
1. **데이터에 연결** 에서 **Azure Blob 스토리지** 를 선택합니다. 사용자가 만든 스토리지 계정 및 컨테이너에 대한 기존 연결을 선택합니다. 데이터 원본의 이름을 지정하고, 나머지는 기본값을 사용합니다. 
   ...
   :::image type="content" source="media/cognitive-search-quickstart-blob/connect-to-signs.png" alt-text="Azure Blob 구성" border="true":::

### <a name="step-2---add-cognitive-skills"></a>2단계: 인식 기술 추가

다음으로, OCR 및 이미지 분석을 호출하도록 AI 보강을 구성합니다. 

1. 이 빠른 시작에서는 **체험** Cognitive Services 리소스를 사용합니다. 샘플 데이터는 19개의 파일로 구성되어 있으므로 이 빠른 시작에서는 Cognitive Services에서 매일 인덱서당 20개의 트랜잭션을 무료로 할당하는 것으로 충분합니다. 
...
1. 같은 페이지에서 **보강 추가** 를 확장하고 트리 항목을 추가합니다.

   OCR을 활성화하고 모든 텍스트를 merged_content 필드에 병합합니다.

   "이미지에서 태그 생성" 및 "이미지에서 캡션 생성"을 선택합니다.

   :::image type="content" source="media/cognitive-search-quickstart-blob/select-ocr-image-enrichments.png" alt-text="기술 세트에 대한 Cognitive Services 선택 서비스 연결" border="true":::

   이미지 분석을 위해 문서 크래킹 중에 이미지가 텍스트에서 분할됩니다. "merged_content" 필드는 AI 보강 파이프라인의 텍스트와 이미지를 다시 연결합니다.
...
+ 기본 필드는 기존 Blob의 속성과 보강 출력을 포함하는 새 필드(예: `text`, `layoutText`, `imageCaption`)를 기반으로 합니다. 데이터 형식은 메타데이터 및 데이터 샘플링에서 유추됩니다.
...
+ 기본 특성은 **조회 가능** 하고 **검색 가능** 합니다. **검색 가능** 은 필드의 전체 텍스트 검색을 허용합니다. **조회 가능** 은 필드 값을 결과에 반환할 수 있다는 뜻입니다. 마법사는 사용자가 기술 세트를 통해 필드를 생성했기 때문에 이 필드를 조회 및 검색 가능하도록 하려고 한다고 가정합니다.
  ...
  :::image type="content" source="media/cognitive-search-quickstart-blob/index-fields-ocr-images.png" alt-text="인덱스 필드" border="true":::
...
1. **인덱서** 페이지에서 기본 이름을 적용하고, **한 번** 일정 옵션을 클릭하여 즉시 실행할 수 있습니다. 
   ...
   :::image type="content" source="media/cognitive-search-quickstart-blob/indexer-signs.png" alt-text="인덱서 정의" border="true":::
...
인지 기술 인덱싱은 일반적인 텍스트 기반 인덱싱보다 완료하는 데 더 오래 걸립니다. 진행 상황을 모니터링하려면 [개요] 페이지로 이동하여 페이지 중간에 있는 **인덱서** 탭을 선택합니다.
  ...
  :::image type="content" source="media/cognitive-search-quickstart-blob/indexer-status-signs.png" alt-text="인덱서 상태" border="true":::
...
1. 쿼리 문자열에 `search=sign&searchFields=imageTags&$select=text,imageCaption,imageTags&$count=true`와 같은 인덱스를 쿼리할 검색 문자열을 입력한 다음, **검색** 을 선택합니다.

   :::image type="content" source="media/cognitive-search-quickstart-blob/search-explorer-query-string-signs.png" alt-text="검색 탐색기의 쿼리 문자열" border="true":::
...
+ 전체 텍스트 검색 범위를 특정 필드로 지정하려면 `searchField`를 추가합니다.
+ JSON 내에서 특정 속성 또는 용어를 검색하려면 CTRL-F를 사용합니다.

  :::image type="content" source="media/cognitive-search-quickstart-blob/search-explorer-results-signs.png" alt-text="Search 탐색기 예제" border="true":::
...
Cognitive Search에는 데이터 가져오기 마법사에서 연습할 수 있는 다른 기본 제공 기술이 있습니다. 다음 빠른 시작에서는 엔터티 인식, 언어 감지 및 텍스트 번역을 사용합니다. 

> [!div class="nextstepaction"]
> [빠른 시작: 데이터 가져오기 마법사를 사용하여 텍스트 번역 및 엔터티 인식](cognitive-search-quickstart-blob.md)