...
관리형 엔드포인트 권한이 시스템 할당 관리 ID를 통해 스토리지에 액세스하도록 허용하거나 이전 섹션에서 만든 스토리지 계정에 액세스할 수 있는 권한을 사용자 할당 관리 ID에 부여할 수 있습니다.
...
특정 데이터만 반환하도록 위의 쿼리를 구체화하려면 [쿼리 Azure CLI 명령 출력을 참조하세요.](/cli/azure/query-azure-cli)
