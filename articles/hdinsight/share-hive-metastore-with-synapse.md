---
title: Azure Synapse Spark 풀과 Hive Metastore 공유
description: Azure Synapse Spark 풀과 기존 Azure HDInsight 외부 Hive Metastore를 공유하는 방법을 알아봅니다.
...
ms.locfileid: "124736846"
---
# <a name="share-hive-metastore-with-synapse-spark-pool-preview"></a>Synapse Spark 풀과 Hive Metastore 공유(미리 보기)

Azure Synapse Analytics 사용하면 동일한 작업 영역의 Apache Spark 풀이 관리되는 HMS(Hive Metastore 서비스) 호환 메타스토어를 카탈로그로 공유할 수 있습니다. 고객이 HDInsight Hive 및 Spark 클러스터와 같은 작업 영역 외부의 다른 계산 엔진과 카탈로그 개체를 공유하려는 경우 외부 Hive Metastore에 연결하는 것을 선호합니다. 이 문서에서는 외부 Apache Hive Metastore에 Synapse Spark를 연결하는 방법에 대해 알아봅니다. 
...
> [!NOTE]
> HDInsight 클러스터의 기존 외부 Hive 메타스토어(3.6 및 4.0 클러스터)를 사용할 수 있습니다. [Azure HDInsight 외부 메타데이터 저장소 사용을](./hdinsight-use-external-metadata-stores.md)참조하세요.
아래 단계에 따라 Synapse 작업 영역에서 외부 Hive 메타스토어 및 기본 카탈로그 스토리지에 연결된 서비스를 설정하고 연결된 외부 Hive 메타스토어를 사용하도록 Spark 풀을 구성합니다.
...
1.  Synapse Studio 열고 왼쪽에서 **> 연결된 서비스 관리로** 이동하고 **새로** 만들기를 클릭하여 새 연결된 서비스를 만듭니다.
    ...
    :::image type="content" source="./media/share-hive-metastore-with-synapse/set-up-hive-metastore-linked-service.png" alt-text="Hive Metastore 연결된 서비스 설정" border="true":::
...
Spark 풀을 만들 때 **추가 설정** 탭에서 아래 구성을 텍스트 파일에 배치하고 **Apache Spark 구성** 섹션에 업로드합니다. 기존 Spark 풀에 대한 상황에 맞는 메뉴를 사용하고 Apache Spark 구성을 선택하여 이러한 구성을 추가할 수도 있습니다.
   ...
   :::image type="content" source="./media/share-hive-metastore-with-synapse/config-spark-pool.png" alt-text="Spark 풀 구성":::
...
1.  Synapse Studio 열고 데이터 > **연결된 탭 > 추가** 단추 > 외부 데이터에 **커넥트 으로 이동합니다.**
    ...
    :::image type="content" source="./media/share-hive-metastore-with-synapse/connect-to-storage-account.png" alt-text="스토리지 계정에 연결" border="true":::
...
6.  Blob Storage 계정에 연결된 서비스를 만든 후 Spark 쿼리를 실행할 때 Notebook에서 아래의 Spark 코드를 실행하여 Spark 세션의 Blob Storage 계정에 액세스해야 합니다. [여기에서](../synapse-analytics/spark/apache-spark-secure-credentials-with-tokenlibrary.md)이 작업을 수행해야 하는 이유에 대해 자세히 알아보세요.
...
- [외부 HMS를](../synapse-analytics/sql/develop-storage-files-spark-tables.md) 사용하는 경우 SQL < > Spark 동기화가 작동하지 않습니다.  
- Azure SQL Database 외부 Hive Metastore 데이터베이스로만 지원됩니다. SQL 권한 부여만 지원 됩니다.
- 현재 Spark는 외부 Hive 테이블 및 비트랜잭션/비 ACID 관리 Hive 테이블 에서만 작동 합니다. 현재 Hive ACID/트랜잭션 테이블을 지원 하지 않습니다.
...
HMS 버전을 마이그레이션해야 하는 경우 [hive 스키마 도구](https://cwiki.apache.org/confluence/display/Hive/Hive+Schema+Tool)를 사용 하는 것이 좋습니다. 그리고 HMS가 HDInsight 클러스터에서 사용 되는 경우 [Hms 제공 버전](./interactive-query/apache-hive-migrate-workloads.md)을 사용 하는 것이 좋습니다. 
