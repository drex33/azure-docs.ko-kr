...
IDPS 서명 규칙(미리 보기)을 사용하면 다음을 수행할 수 있습니다.

- 하나 이상의 서명을 사용자 지정하고 해당 모드를 *사용 안 함,* *경고* 또는 *경고 및 거부로* 변경합니다. 

   예를 들어 잘못된 서명으로 인해 Azure Firewall 의해 합법적인 요청이 차단되는 가양성을 수신하는 경우 애플리케이션 규칙 로그의 서명 ID를 사용하고 해당 IDPS 모드를 off로 설정할 수 있습니다. 이로 인해 "잘못된" 서명이 무시되고 가양성 문제가 해결됩니다.
- 너무 낮은 우선 순위의 경고를 만들어 우선 순위가 높은 경고에 대한 가시성을 방해하는 서명에 대해 동일한 미세 조정 절차를 적용할 수 있습니다.
- 전체 55,000개의 서명에 대한 전체적인 보기 얻기
- 스마트 검색

   모든 유형의 특성으로 전체 서명 데이터베이스를 검색할 수 있습니다. 예를 들어 검색 표시줄에 ID를 입력하기만 하면 특정 CVE ID를 검색하여 이 CVE를 처리할 서명을 검색할 수 있습니다.
...
|IDPS 바이패스 목록|IDPS 바이패스 목록에서 IP 그룹을 지원 하지 않습니다.|수정 사항을 조사하고 있습니다.|
