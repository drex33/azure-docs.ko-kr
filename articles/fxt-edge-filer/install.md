...
* Azure FXT Edge Filer의 요구 사항을 충족하는 AC 전원 공급 장치 및 냉각 시스템. (설치 계획 및 크기 조정에 대한 도움말은 [전원 및 열 사양](specs.md#power-and-thermal-specifications)을 참조하세요.)  

  > [!NOTE]
  > 두 중복 PSU(전원 공급 장치)를 최대로 활용하려면 AC 전원에 연결할 때 서로 다른 두 분기 회로에서 전력 분배 장치를 사용하세요. 자세한 내용은 [전원 케이블 연결](network-power.md#connect-power-cables)을 참조하세요.  
...
> [네트워크 포트 및 전원 공급 장치 연결](network-power.md)
