...
ms.topic: conceptual
ms.date: 10/07/2021
ms.openlocfilehash: 2cd95c5da8bb122bdca36e82712b898ce89f479b
ms.sourcegitcommit: 1e9139680ca51f55ac965c4dd6dd82bf2fd43675
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/04/2021
ms.locfileid: "133539309"
---
...
| [StartupExpression](data-flow-alter-row.md) | 행 한정자 | 각 행에 대 한 어설션 규칙을 설정 합니다. |
| [조건부 분할](data-flow-conditional-split.md) | 다중 입력/출력 | 일치 조건에 따라 데이터 행을 서로 다른 스트림으로 라우팅합니다. |
| [파생 열](data-flow-derived-column.md) | 스키마 한정자 | 데이터 흐름 식 언어를 사용 하 여 새 열을 생성 하거나 기존 필드를 수정 합니다. | 
| [외부 호출](data-flow-external-call.md) | 스키마 한정자 | 외부 끝점을 행 단위로 인라인으로 호출 합니다. | 
| [Exists](data-flow-exists.md) | 다중 입력/출력 | 데이터가 다른 원본 또는 스트림에 있는지 확인합니다. | 
| [Filter](data-flow-filter.md) | 행 한정자 | 조건에 따라 행을 필터링합니다. |
| [평면화(flatten)](data-flow-flatten.md) | 포맷터 |  JSON과 같은 계층 구조 내부의 배열 값을 가져와서 개별 행으로 언롤합니다. |
...
| [Parse](data-flow-parse.md) | 포맷터 |  데이터 스트림에서 JSON, 구분된 텍스트, 또는 XML 형식 텍스트의 문자열인 텍스트 열을 구문 분석합니다. |
...
| [문자열 변환](data-flow-stringify.md) | 포맷터 | 복합 형식을 일반 문자열로 변환 |
