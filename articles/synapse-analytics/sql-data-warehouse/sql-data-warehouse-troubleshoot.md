...
ms.locfileid: "131504059"
---
# <a name="troubleshoot-dedicated-sql-pool-formerly-sql-dw-in-azure-synapse-analytics"></a>Azure Synapse Analytics 전용 SQL 풀(이전의 SQL DW) 문제 해결
...
| 문제                                                        | 해결 방법                                                   |
| :----------------------------------------------------------- | :----------------------------------------------------------- |
| 'NT AUTHORITY\ANONYMOUS LOGON' 사용자에 대해 로그인 실패 (Microsoft SQL Server, 오류: 18456) | 이 오류는 Azure AD 사용자가 데이터베이스에 연결하려고 하지만 에 사용자가 없는 경우에 `master` 발생합니다. `master`  이 문제를 해결하려면 연결 시 연결할 전용 SQL 풀(이전의 SQL DW)을 지정하거나 데이터베이스에 사용자를 `master` 추가합니다.  자세한 내용은 [보안 개요](sql-data-warehouse-overview-manage-security.md) 를 참조하세요. |
| 서버 보안 주체 "MyUserName"이 현재 보안 컨텍스트에서 데이터베이스에 액세스할 수 `master` 없습니다. 사용자 기본 데이터베이스를 열 수 없습니다. 로그인이 실패했습니다. 사용자 'MyUserName'에 대한 로그인이 실패했습니다. (Microsoft SQL Server, 오류: 916) | 이 오류는 Azure AD 사용자가 데이터베이스에 연결하려고 하지만 에 사용자가 없는 경우에 `master` 발생합니다. `master`  이 문제를 해결하려면 연결 시 연결할 전용 SQL 풀(이전의 SQL DW)을 지정하거나 데이터베이스에 사용자를 `master` 추가합니다.  자세한 내용은 [보안 개요](sql-data-warehouse-overview-manage-security.md) 를 참조하세요. |
| CTAIP 오류                                                  | 이 오류는 특정 SQL 데이터베이스가 아닌 SQL Database 데이터베이스에 로그인이 만들어진 경우에 발생할 수 `master` 있습니다.  이 오류가 발생하는 경우 [보안 개요](sql-data-warehouse-overview-manage-security.md) 문서를 살펴보세요.  이 문서에서는 데이터베이스에서 로그인 및 사용자를 만드는 `master` 방법과 SQL 데이터베이스에서 사용자를 만드는 방법을 설명합니다. |
...
| 도구 또는 드라이버에 연결할 수 없음                           | 전용 SQL 풀(이전의 SQL DW)에서는 [SSMS(SQL Server Management Studio), Visual Studio](/sql/ssms/download-sql-server-management-studio-ssms?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest&preserve-view=true) [SSDT](sql-data-warehouse-install-visual-studio.md)또는 [sqlcmd를](sql-data-warehouse-get-started-connect-sqlcmd.md) 사용하여 데이터를 쿼리하는 것이 좋습니다. 드라이버와 Azure Synapse에 연결하는 방법에 대한 자세한 내용은 [Azure Synapse용 드라이버](sql-data-warehouse-connection-strings.md) 및 [Azure Synapse에 연결](sql-data-warehouse-connect-overview.md) 문서를 참조하세요. |
...
| 수동 스크립팅, 스크립팅 마법사 사용 또는 SSMS를 통한 연결이 느려지거나 응답하지 않거나 오류를 생성함 | 데이터베이스에서 사용자를 만들 수 있는지 `master` 확인합니다. 스크립팅 옵션에서 엔진 버전이 "Microsoft Azure Synapse Analytics Edition"으로 설정되고 엔진 유형이 "Microsoft Azure SQL Database"로 설정되어 있는지도 확인합니다. |
...
| 인덱스 품질 저하로 인한 쿼리 성능 저하     | 경우에 따라 [columnstore 인덱스 품질 저하](sql-data-warehouse-tables-index.md#causes-of-poor-columnstore-index-quality)로 인해 쿼리가 느려질 수 있습니다.  자세한 내용 및 [세그먼트 품질을 개선하기 위해 인덱스 다시 작성](sql-data-warehouse-tables-index.md#rebuild-indexes-to-improve-segment-quality)방법에 대해서는 이 문서를 참조하세요. |
...
| `sp_rename` (미리 보기)는 다음의 외부 스키마에서는 작동 하지 않습니다. `dbo` | 이는 현재 Synapse [열에 대한 sp_rename(미리 보기)](/sql/relational-databases/system-stored-procedures/sp-rename-transact-sql?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest&preserve-view=true)의 제한 사항입니다.  스키마의 일부가 아닌 개체의 열은 `dbo` CTAS를 통해 새 테이블로 이름을 바꿀 수 있습니다. |
