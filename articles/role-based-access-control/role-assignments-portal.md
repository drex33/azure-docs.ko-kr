...
1. **세부 정보** 열에서 **보기** 를 클릭하여 역할에 대한 자세한 정보를 확인합니다.
   ...
   ![사용 권한 탭이 있는 역할 세부 정보 보기 창의 스크린샷.](./media/role-assignments-portal/select-role-permissions.png)
...
1. 관리 ID를 찾아서 선택합니다.
   ...
   ![관리 ID 선택 창의 스크린샷.](./media/role-assignments-portal/select-managed-identity.png)

1. **선택을 클릭하여** 관리 ID를 멤버 목록에 추가합니다.
...
조건을 지원하는 역할을 선택한 경우 **조건(선택 사항)** 탭이 나타나고 역할 할당에 조건을 추가할 수 있는 옵션이 있습니다. [조건](conditions-overview.md)은 필요에 따라 역할 할당에 추가하여 더 세분화된 액세스 제어를 제공할 수 있는 추가 검사입니다.
...
1. 스토리지 Blob 특성에 따라 역할 할당을 더 구체화하려면 **조건 추가** 를 클릭합니다. 자세한 내용은 [Azure 역할 할당 조건 추가 또는 편집](conditions-role-assignments-portal.md)을 참조하세요.
   ...
   ![조건 추가 탭이 있는 역할 할당 추가 페이지의 스크린샷.](./media/shared/condition.png)

1. **다음** 을 클릭합니다.

## <a name="step-6-assign-role"></a>6단계: 역할 할당

1. **검토 + 할당** 탭에서 역할 할당 설정을 검토합니다.

   ![검토 + 할당 탭이 있는 역할 할당 페이지의 스크린샷.](./media/role-assignments-portal/review-assign.png)
...
#### <a name="classic"></a>[클래식](#tab/classic/)

## <a name="step-1-identify-the-needed-scope-classic"></a>1단계: 필요한 범위 식별(클래식)

[!INCLUDE [Scope for Azure RBAC introduction](../../includes/role-based-access-control/scope-intro.md)] 자세한 내용은 [범위 이해](scope-overview.md)를 참조하세요.

![클래식 환경의 Azure RBAC 범위 수준을 보여 주는 다이어그램](../../includes/role-based-access-control/media/scope-levels.png)
...
1. 해당 범위에 대한 특정 리소스를 클릭합니다.
    ...
    ![클래식 환경의 리소스 그룹 개요 페이지 스크린샷.](./media/shared/rg-overview.png)

## <a name="step-2-open-the-add-role-assignment-pane-classic"></a>2단계: 역할 할당 추가 창 열기(클래식)
...
1. **액세스 제어(IAM)** 를 클릭합니다.
    ...
    ![클래식 환경의 리소스 그룹에 대한 액세스 제어(IAM) 페이지의 스크린샷.](./media/shared/rg-access-control.png)
...
1. **추가** > **역할 할당 추가** 를 클릭합니다.
   ...
   ![클래식 환경의 역할 할당 추가 > 추가 메뉴의 스크린샷.](./media/shared/add-role-assignment-menu.png)

1. 역할 할당 추가 페이지에서 **클래식 환경 사용을** 클릭합니다.

   ![클래식 환경용 클래식 환경 사용 링크가 있는 역할 할당 추가 페이지의 스크린샷.](./media/role-assignments-portal/add-role-assignment-page-use-classic.png)

    역할 할당 추가 창이 열립니다.

   ![역할, 액세스 할당 및 클래식 환경의 옵션 선택과 함께 역할 할당 추가 페이지의 스크린샷.](./media/role-assignments-portal/add-role-assignment-page.png)

## <a name="step-3-select-the-appropriate-role-classic"></a>3단계: 적절한 역할 선택(클래식)
...
1. **역할** 목록에서 검색하거나 스크롤하여 할당할 역할을 찾습니다.
   ...
   ![클래식 환경의 역할 할당 추가에서 역할 목록 선택 스크린샷.](./media/role-assignments-portal/add-role-assignment-role.png)

1. 역할을 클릭하여 선택합니다.

## <a name="step-4-select-who-needs-access-classic"></a>4단계: 액세스가 필요한 사람 선택(클래식)
...
1. **액세스 할당 대상** 목록에서 액세스 권한을 할당할 보안 주체 유형을 선택합니다.
   ...
   ![클래식 환경의 역할 할당 추가에서 보안 주체를 선택하는 스크린샷.](./media/role-assignments-portal/add-role-assignment-type.png)

1. 사용자 할당 관리 ID 또는 시스템 할당 관리 ID를 선택한 경우 관리 ID가 있는 **구독** 을 선택합니다.

1. **선택** 섹션에서 문자열을 입력하거나 목록을 스크롤하여 보안 주체를 검색합니다.

   ![클래식 환경의 역할 할당 추가에서 사용자를 선택하는 스크린샷.](./media/role-assignments-portal/add-role-assignment-user.png)

1. 보안 주체를 찾았으면 클릭하여 선택합니다.

## <a name="step-5-assign-role-classic"></a>5단계: 역할 할당(클래식)
...
1. **역할 할당** 탭에서 목록에 역할 할당이 표시되는지 확인합니다.
    ...
    ![클래식 환경의 역할을 할당한 후의 역할 할당 목록 스크린샷.](./media/role-assignments-portal/rg-role-assignments.png)
