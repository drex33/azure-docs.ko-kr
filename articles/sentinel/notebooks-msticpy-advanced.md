---
title: Microsoft Sentinel | Jupyter Notebook 및 MSTICPy에 대한 고급 구성 Microsoft Docs
description: Microsoft Sentinel에서 작업할 때 MSTICPy를 사용하여 Jupyter Notebook에 사용할 수 있는 고급 구성에 대해 알아봅니다.
author: batamig
ms.author: bagol
ms.topic: how-to
ms.date: 11/09/2021
ms.custom: ignite-fall-2021
ms.openlocfilehash: 1624dcbcea968b0b5973fe2699b13ea0f0ab036f
ms.sourcegitcommit: 1244a72dbec39ac8cf16bb1799d8c46bde749d47
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/18/2021
ms.locfileid: "132758501"
---
# <a name="advanced-configurations-for-jupyter-notebooks-and-msticpy-in-microsoft-sentinel"></a>Microsoft Sentinel의 Jupyter Notebook 및 MSTICPy에 대한 고급 구성

[!INCLUDE [Banner for top of topics](./includes/banner.md)]

이 문서에서는 Microsoft Sentinel에서 Jupyter Notebook 및 MSTICPy를 사용하는 고급 구성에 대해 설명합니다.

자세한 내용은 [Jupyter Notebook을 사용하여 보안 위협 헌트](notebooks.md) 및 [자습서: Microsoft Sentinel에서 Jupyter Notebook 및 MSTICPy 시작을 참조하세요.](notebook-get-started.md)

## <a name="prerequisites"></a>사전 요구 사항

이 문서는 [자습서: Microsoft Sentinel에서 Jupyter Notebook 및 MSTICPy 시작의](notebook-get-started.md)연속입니다. 아래 설명된 고급 절차를 계속하기 전에 자습서를 수행하는 것이 좋습니다.

## <a name="specify-authentication-parameters-for-azure-and-microsoft-sentinel-apis"></a>Azure 및 Microsoft Sentinel API에 대한 인증 매개 변수 지정

이 절차에서는 **msticpyconfig.yaml** 파일에서 Microsoft Sentinel 및 기타 Azure API 리소스에 대한 인증 매개 변수를 구성하는 방법을 설명합니다.

**MSTICPy 설정 편집기에서 Azure 인증 및 Microsoft Sentinel API 설정을 추가하려면:**
...
1. **AzureData:** [Azure 리소스에](#specify-authentication-parameters-for-azure-and-microsoft-sentinel-apis) 대한 세부 정보를 쿼리하는 데 사용하는 모듈
1. **AzureSentinelAPI:** [Microsoft Sentinel API를](#specify-authentication-parameters-for-azure-and-microsoft-sentinel-apis) 쿼리하는 데 사용하는 모듈
...
1. 구성 **요소 자동 로드** 탭에서 필요에 따라 매개 변수 값을 정의합니다. 다음은 그 예입니다.
   ...
   - **AzureData 및 AzureSentinelAPI 구성 요소 .**  다음 값을 정의합니다.
      ...
      자세한 내용은 [Azure 및 Microsoft Sentinel API에 대한 인증 매개 변수 지정을 참조하세요.](#specify-authentication-parameters-for-azure-and-microsoft-sentinel-apis)

   - **Notebooklet.** **Notebooklets** 구성 요소에는 단일 매개 변수 **블록인 AzureSentinel이** 있습니다.

      구문을 사용하여 Microsoft Sentinel 작업 영역을 `workspace:\<workspace name>` 지정합니다. 작업 영역 이름은 **Microsoft Sentinel** 탭에 정의된 작업 영역 중 하나여야 합니다.

      함수에 보낼 매개 변수를 더 추가하려면 `notebooklets init` 키:값 쌍으로 지정하고 줄임표로 구분합니다. 다음은 그 예입니다.
...
다른 Notebook에 대해 서로 다른 설정을 사용하려는 경우 여러 환경 변수와 함께 여러 구성 파일을 사용합니다.
...
1. 시스템 변수 영역에서 새로 **설정...** **을** 선택하고 다음과 같이 값을 정의합니다.
    ...
    - **변수 값:** **msticpyconfig.yaml** 파일의 경로를 입력합니다.

# <a name="linux"></a>[Linux](#tab/linux)

이 절차에서는 **.bashrc** 파일을 업데이트하여 Linux 시스템에서 **MSTICPYCONFIG** 환경 변수를 설정하는 방법을 설명합니다.

1. 필요에 따라 **msticpyconfig.yaml** 파일을 Compute 인스턴스로 이동합니다.

1. Microsoft Sentinel Notebooks 페이지에서와 같은 Azure ML **터미널을** 엽니다.

1. **msticpyconfig.yaml** 파일에 액세스할 수 있는지 확인합니다.

   Azure ML 터미널에서 현재 디렉터리 는 Compute Linux 시스템에 탑재된 Azure ML 파일 저장소 홈 디렉터리여야 합니다. 프롬프트는 다음 예제와 유사합니다.

   ```python
   azureuser@alicecontoso-azml7:~/cloudfiles/code/Users/alicecontoso$
   ```

   를 입력하여 **msticpyconfig.yaml** 파일을 포함하여 홈 디렉터리에 있는 모든 파일을 `ls` 나열합니다.

1. **msticpyconfig.yaml** 파일을 Compute 파일 저장소로 이동하려면 다음을 입력합니다.

   ```python
   mv msticpyconfig.yaml ~
   ```

1. 다음 프로세스 중 하나를 사용하여 환경 변수에 대한 **.bashrc** 파일을 편집합니다.

    |명령  |단계  |
    |---------|---------|
    |**vim**     |     1. 다음을 실행합니다. `vim ~/.bashrc` <br>2. **Shift+G**  >  **End** 를 눌러 파일 끝으로 이동합니다. 3. 를 **입력한** 다음 **Enter** 키를 눌러 새 줄을 만듭니다. <br>4. 환경 변수를 추가한 **다음, ESC를** 눌러 명령 모드로 돌아갑니다. <br>5. **:wq** 를 입력하여 파일을 저장합니다.    |
    |**나노**     |           1. 다음을 실행합니다. `nano ~/.bashrc`<br>        1. ALT+/ 또는 **OPTION+/를** 눌러 파일 끝으로 이동합니다. <br>        1. 환경 변수를 추가한 다음 파일을 저장합니다. **Ctrl+X를** 누른 다음 **Y를** 누릅니다.      |
    |     |         |

    다음 환경 변수 중 하나를 추가합니다.

    - **msticpyconfig.yaml** 파일을 이동한 경우 를 `export MSTICPYCONFIG=~/msticpyconfig.yaml` 실행합니다.
    - **msticpyconfig.yaml** 파일을 이동하지 않은 경우 를 `export MSTICPYCONFIG=~/cloudfiles/code/Users/<YOURNAME>/msticpyconfig.yaml` 실행합니다.

# <a name="azure-ml-options"></a>[Azure ML 옵션](#tab/azure-ml)

**Msticpyconfig.yaml** 파일을 Azure ML 사용자 폴더 이외의 다른 곳에 저장해야 하는 경우 다음 옵션 중 하나를 사용합니다.

- **사용자 폴더의 루트에 있는 *nbuser_settings.py* 파일입니다.**  이 프로세스는 **kernel.json** 파일을 편집하는 것보다 더 간단하고 덜 방해가 되지만 Notebook 코드 시작 시 함수를 실행할 때만 `init_notebook` 지원됩니다. 이것이 기본 동작이지만 를 먼저 실행하지 않고 Notebook 코드를 실행하면 `init_notebook` MSTICPy가 구성 파일을 찾지 못할 수 있습니다.

    1. Azure ML 터미널에서 사용자 이름이 있는 폴더인 사용자 폴더의 루트에 **nbuser_settings.py** 파일을 만듭니다.
    1. **nbuser_settings.py** 파일에서 다음 줄을 추가합니다.
    ...
    이 파일은 함수에서 자동으로 `init_notebook` 가져오고 `MSTICPYCONFIG` 현재 Notebook에 대한 환경 변수를 설정합니다.

- **Python 커널 에 대한 *kernel.json* 파일입니다.** Notebook을 수동으로 실행하려는 경우 그리고 시작 시 함수를 호출하지 않고 이 절차를 `init_notebook` 사용합니다.

  Python 3.6 및 Python 3.8에 대한 커널이 있습니다. 두 커널을 모두 사용하는 경우 두 파일을 모두 편집합니다.
  ...
  **kernel.json** 파일에서 환경 변수를 설정하려면 다음을 수행합니다.

  1. **kernel.json** 파일의 복사본을 만들고 편집기에서 원본을 엽니다. 를 사용하여 `sudo` **kernel.json** 파일을 덮어써야 할 수 있으며 파일 내용은 다음 예제와 유사합니다.
  ...
  1. 항목 다음에 `"language"` 다음 줄을 추가합니다. `"env": { "MSTICPYCONFIG": "~/msticpyconfig.yaml" }`

      줄 끝에 쉼표 를 추가해야 `"language": "python"` 합니다. 다음은 그 예입니다.
