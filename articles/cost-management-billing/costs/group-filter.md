...
description: 이 문서에서는 Cost Management에서 그룹 및 필터 옵션을 사용 하는 방법을 설명 합니다.
author: bandersmsft
ms.author: banders
ms.date: 10/11/2021
ms.topic: conceptual
...
| **청구 기간** | PAYG 비용을 청구서가 발부되었거나 발부될 월별로 분류합니다. | **청구 기간** 을 사용 하 여 송장 발부 된 PAYG 요금을 정확 하 게 표현할 수 있습니다. 사용자 지정 날짜 범위를 기준으로 필터링 하는 경우 청구 기간 앞뒤에 2 일을 더 포함 합니다. 정확한 청구 기간 날짜로 제한하면 청구서와 일치하지 않습니다. 청구 기간의 모든 청구서에 대한 비용을 표시합니다. **청구서 ID** 를 사용하여 특정 청구서로 필터링합니다. EA 및 MCA는 월별로 청구되므로 PAYG 구독에만 적용됩니다. EA/MCA 계정은 동일한 목표를 달성하기 위해 날짜 선택 또는 월간 세분성에서 달력 월을 사용할 수 있습니다. |
...
| **공급자** | Azure, Microsoft 365, Dynamics 365, AWS 등의 공급자 유형별로 비용을 나눕니다. | 제품 및 lob (기간 업무)의 식별자입니다. |
| **게시자 유형** | Microsoft, Azure, AWS 및 Marketplace 비용을 나눕니다. | 값은 MCA 계정에 대 한 **Microsoft** 이며 EA 및 종 량 제 계정으로 **Azure** 입니다. |
...
## <a name="changes-to-publisher-type-values"></a>Publisher 형식 값에 대 한 변경 내용

MCA 규약을 사용 하는 고객의 `Publisher type` 경우 **Azure** 값 **은 Microsoft** 에서 Cost Management + 청구로 추적 되는 모든 요금에 적용 됨을 반영 하는 값으로 대체 됩니다. 이 변경은 2021 년 10 월 14 일에 적용 됩니다.

다음 사항에 유의하세요.

- 비용 분석의 저장 된 보기-Azure 필터를 사용 하는 저장 된 모든 보기는 `PublisherType`  =   **Microsoft** 의 새 값을 반영 하도록 업데이트 됩니다. 
    - 필요한 작업: 없음.
- 예산을 사용한 예산 `PublisherType` -Azure 필터를 사용 하 여 만든 모든 예산이 `PublisherType`  =   **Microsoft** 의 새로운 가치를 반영 하도록 업데이트 됩니다.
    - 작업 필요: 없음.
- 내보내기 - 2021년 10월 14일 이전에 생성된 모든 내보내기 파일의 값은 에서 **Azure이고** `PublisherType field` 2021년 10월 14일 이후에는 **Microsoft** 가 됩니다.
    - 필요한 작업: *새 파일을 2021년 10월 14일 이전에 다운로드한 파일과 병합할 때 변경된 값을 고려해야* 합니다.
- 다운로드한 데이터 - 비용 분석에서 다운로드한 파일에서 2021년 10월 14일 이전에 다운로드한 사용량 세부 정보 파일에는 필드에 이전 **값인 Azure가** 계속 `PublisherType` 있습니다.
    - 필요한 작업: *새 파일을 2021년 10월 14일 이전에 다운로드한 파일과 병합할 때 변경된 값을 고려해야* 합니다.
- REST API 호출 - PublisherType 필드를 **Azure** 값으로 필터링하는 Cost Management + Billing REST API 호출을 사용하는 경우 2021년 10월 14일 이후에 **Microsoft에서** 필터링하도록 업데이트합니다.
    - 필요한 작업: *`Publisher type`  =  REST API 호출에서 **Microsoft를** 사용하여 모든 제1자 요금을 받습니다. `Publisher type`  =  **Azure를** 호출하면 데이터가 반환되지 않습니다.*

> [!NOTE]
> EA(기업계약) 또는 PAYG(종량제) 계정이 있는 경우 이 변경 내용은 영향을 받지 않습니다.
