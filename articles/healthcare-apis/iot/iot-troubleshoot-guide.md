...
## <a name="device-and-fhir-destination-mappings-validations"></a>디바이스 및 FHIR 대상 매핑 유효성 검사

이 섹션에서는 IoT 커넥터가 수행하는 유효성 검사 프로세스에 대해 설명합니다. 유효성 검사 프로세스는 디바이스 및 FHIR 대상 매핑의 유효성을 검사한 후 사용할 수 있도록 저장합니다. 이러한 요소는 Device 및 FHIR 대상 매핑에 필요합니다.
...
|잠재적 이슈|수정 프로그램|
|----------------|-----|
|데이터는 여전히 처리되고 있습니다.|데이터는 egressed (15 분 마다)에 배치 됩니다.  데이터가 계속 처리 되 고 있으며 데이터를 FHIR 서비스에 유지 하기 위해 추가 시간이 필요할 수 있습니다.|
...
> [!TIP]
> IoT 커넥터 [장치 매핑](how-to-use-device-mapping-iot.md) 및 [fhir 대상 매핑에](how-to-use-fhir-mapping-iot.md) 대 한 자세한 정보
...
4. 붙여넣기 작업을 수행 합니다 (예: **ctrl + V** 를 눌러 Microsoft Visual Studio 코드 또는 메모장 같은 편집기 내의 새 파일로). 확장명을 사용 하 여 파일을 저장 해야 **합니다.**

> [!TIP]
> IoT 커넥터에 대 한 [Azure 기술 지원](https://azure.microsoft.com/support/create-ticket/) 티켓을 열 경우 문제 해결 프로세스에 도움이 되도록 장치 복사본과 FHIR 대상 매핑을 포함 해야 합니다.
