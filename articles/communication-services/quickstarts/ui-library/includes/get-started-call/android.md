...
- read:packages 범위가 선택된 개인용 액세스 토큰을 제공해야 합니다.
- 해당 PAT에 대해 SSO를 사용하도록 설정해야 할 수도 있습니다.
...
-----
### <a name="create-communicationtokencredential"></a>`CommunicationTokenCredential`을 만듭니다.
...
Graph API를 사용하여 Microsoft Teams 미팅 링크를 검색할 수 있습니다. 이 프로세스는 [Graph 설명서](https://docs.microsoft.com/graph/api/onlinemeeting-createorget?tabs=http&view=graph-rest-beta&preserve-view=true)에 자세히 설명되어 있습니다.

Communication Services 통화 SDK는 전체 Microsoft Teams 미팅 링크를 수락합니다. 이 링크는 [`joinWebUrl` 속성](https://docs.microsoft.com/graph/api/resources/onlinemeeting?view=graph-rest-beta&preserve-view=true)에서 액세스할 수 있는 `onlineMeeting` 리소스의 일부로 반환됩니다. Teams 미팅 초대 자체의 **미팅 조인** URL에서 필요한 미팅 정보를 가져올 수도 있습니다.
...
## <a name="subscribe-on-events-from-callcomposite"></a>`CallComposite`에서 이벤트 구독
