---
title: SAML 2.0 토큰 클레임 참조 | Azure
titleSuffix: Microsoft identity platform
description: 해당 JWT를 포함하여 Microsoft ID 플랫폼에서 발급한 SAML 2.0 토큰에 포함된 클레임에 관한 세부 정보가 포함된 클레임 참조입니다.
author: kenwith
...
ms.locfileid: "123219984"
---
# <a name="saml-token-claims-reference"></a>SAML 토큰 클레임 참조

Microsoft ID 플랫폼은 각 인증 흐름의 처리 과정에서 여러 형식의 보안 토큰을 내보냅니다. 이 문서에서는 SAML 2.0 토큰의 형식, 보안 특성 및 내용을 설명합니다.
...
> |그룹 | `groups` |주체의 그룹 멤버 자격을 나타내는 개체 ID를 제공합니다. 이러한 값은 고유하며(개체 ID 참조) 리소스 액세스 시 강제로 인증하게 하는 경우처럼 액세스 관리에 안전하게 사용할 수 있습니다. 그룹 클레임에 포함된 그룹은 애플리케이션 매니페스트의 "groupMembershipClaims" 속성을 통해 애플리케이션별로 구성됩니다. Null 값은 모든 그룹을 제외하고, "SecurityGroup" 값은 Active Directory 보안 그룹 멤버 자격만 포함하고, "All" 값은 보안 그룹과 Microsoft 365 메일 그룹을 모두 포함합니다. <br><br> **참고**: <br> 그룹 수가 한도(SAML은 150, JWT는 200)를 넘어설 경우 초과 클레임은 해당 사용자에 대한 그룹 목록을 포함하는 Graph 엔드포인트를 가리키는 클레임 원본에 추가됩니다. | `<Attribute Name="http://schemas.microsoft.com/ws/2008/06/identity/claims/groups">`<br>`<AttributeValue>07dd8a60-bf6d-4e17-8844-230b77145381</AttributeValue>` |
> | 그룹 초과 지표 | `groups:src1` | 길이는 제한되지 않으나 토큰에 너무 큰 토큰 요청의 경우 사용자의 전체 그룹 목록에 대한 링크가 포함됩니다. SAML의 경우 `groups` 클레임 대신 새 클레임으로 추가됩니다. <br><br> **참고**: <br> Azure AD Graph API는 Microsoft Graph API로 바뀝니다. 해당 엔드포인트에 관한 자세한 내용은 [user: getMemberObjects](/graph/api/user-getmemberobjects)를 참조하세요. | `<Attribute Name=" http://schemas.microsoft.com/claims/groups.link">`<br>`<AttributeValue>https://graph.windows.net/{tenantID}/users/{userID}/getMemberObjects<AttributeValue>` |
...
* Microsoft Graph API를 사용한 토큰 수명 정책 관리에 관한 자세한 내용은 [Azure AD 정책 리소스 개요](/graph/api/resources/policy-overview)를 참조하세요.
