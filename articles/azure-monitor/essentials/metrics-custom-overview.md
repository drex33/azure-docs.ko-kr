...
애플리케이션이 로컬로 사전 집계할 수 없고 수집 시 각 불연속 샘플 또는 이벤트를 즉시 내보내야 하는 경우 원시 측정값을 내보낸다. 예를 들어, 앱에서 로그인 트랜잭션이 발생할 때마다 단일 측정값만 사용하여 메트릭을 Azure Monitor에 게시합니다. 따라서 12밀리초가 걸린 로그인 트랜잭션의 경우 메트릭 게시는 다음과 같습니다.
* 최소: 12
* 최대: 12
* 합계: 12
* 개수: 1

이 프로세스를 사용하면 지정된 1분 동안 동일한 메트릭/차원 조합에 대해 여러 값을 내보낸다. 그런 다음, Azure Monitor 지정된 1분 동안 내보내는 모든 원시 값을 가져와서 집계합니다.

### <a name="sample-custom-metric-publication"></a>샘플 사용자 지정 메트릭 게시
다음 예제에서는 가상 머신에 대한 **메모리 프로필** 메트릭 네임스페이스에 **사용 중인 메모리 바이트** 라는 사용자 지정 메트릭을 만듭니다. 이 메트릭에는 **프로세스** 라는 단일 차원이 있습니다. 타임스탬프의 경우 두 프로세스에 대해 메트릭 값이 내보내됩니다.
...
내보내기 전에 사용자 지정 메트릭을 Azure Monitor에서 미리 정의할 필요가 없습니다. 게시된 각 메트릭 데이터 요소에는 네임스페이스, 이름 및 차원 정보가 포함되어 있습니다. 따라서 사용자 지정 메트릭을 Azure Monitor에 처음 내보낼 때 메트릭 정의가 자동으로 생성됩니다. 이 메트릭 정의는 메트릭 정의를 통해 메트릭이 내보내지는 모든 리소스에서 검색할 수 있습니다.

> [!NOTE]  
> Azure Monitor는 사용자 지정 메트릭에 대한 **단위** 정의를 아직 지원하지 않습니다.

## <a name="using-custom-metrics"></a>사용자 지정 메트릭 사용
사용자 지정 메트릭이 Azure Monitor 제출되면 Azure Portal 통해 검색하고 Azure Monitor REST API를 통해 쿼리할 수 있습니다. 특정 조건이 충족되면 알리도록 메트릭에 대한 경고를 만들 수도 있습니다.

> [!NOTE]
> 사용자 지정 메트릭을 보려면 읽기 또는 기여자 역할이 있어야 합니다. [Monitoring Reader](../../role-based-access-control/built-in-roles.md#monitoring-reader)를 참조합니다. 

### <a name="browse-your-custom-metrics-via-the-azure-portal"></a>Azure Portal을 통해 사용자 지정 메트릭 찾아보기
1. [Azure Portal](https://portal.azure.com)로 이동합니다.
2. **모니터** 창을 선택합니다.
3. **메트릭** 을 선택합니다.
4. 사용자 지정 메트릭을 내어본 리소스를 선택합니다.
5. 사용자 지정 메트릭에 대한 메트릭 네임스페이스를 선택합니다.
6. 사용자 지정 메트릭을 선택합니다.

Azure Portal 메트릭 보기에 대한 자세한 내용은 Azure [메트릭 탐색기 시작을 참조하세요.](./metrics-getting-started.md)

## <a name="supported-regions"></a>지원되는 지역
공개 미리 보기 중 사용자 지정 메트릭을 게시할 수 있는 기능은 일부 Azure 지역에서만 사용할 수 있습니다. 이 제한 사항은 지원되는 지역 중 하나의 리소스에 대해서만 메트릭을 게시할 수 있음을 의미합니다. Azure 지역에 대한 자세한 내용은 [Azure 지역을 참조하세요.](https://azure.microsoft.com/global-infrastructure/geographies/) 

다음 표에서는 사용자 지정 메트릭에 대해 지원되는 Azure 지역을 나열합니다. 또한 해당 지역의 리소스에 대한 메트릭을 게시해야 하는 해당 엔드포인트를 나열합니다. 엔드포인트 접두사에 사용되는 Azure 지역 코드는 공백이 제거된 지역의 이름일 뿐입니다. 
...
메트릭에 새로 추가된 메트릭 또는 새로 추가된 차원이 나타나려면 최대 3분이 걸릴 수 있습니다. 데이터가 시스템에 있는 경우 30초 이내에 99% 이내에 표시됩니다. 
...
|범주|제한|
|---|---|
|배포한 모든 지역에서 구독의 총 활성 시간 시리즈|50,000|
|메트릭당 차원 키|10|
|메트릭 네임스페이스, 메트릭 이름, 차원 키 및 차원 값의 문자열 길이|256자|

활성 시계열은 지난 12시간 동안 메트릭 값이 게시된 메트릭, 차원 키 또는 차원 값의 고유한 조합으로 정의됩니다.

Time Series에서 50,000의 제한을 이해하려면 다음 메트릭을 고려하세요.

> 차원이 있는 서버 응답 시간: 지역, 부서, *CustomerID*

이 메트릭을 사용하면 10개 지역, 20개 부서 및 100명의 고객이 있는 경우 10 x 20 x 100 = 2,000개의 Time Series를 제공합니다. 

100개 지역, 200개 부서 및 2,000명의 고객이 있는 경우 100 x 200 x 2,000 = 4,000, 4,000, 이 메트릭에 대한 제한을 초과합니다. 

다시 말하지만, 이 제한은 개별 메트릭에 해당하지 않습니다. 구독 및 지역에서 이러한 모든 메트릭의 합계를 위한 것입니다.  

## <a name="design-limitations-and-considerations"></a>디자인 제한 사항 및 고려 사항

**감사 목적으로 애플리케이션 Insights 사용** Application Insights 원격 분석 파이프라인은 성능 영향을 최소화하고 네트워크 트래픽이 애플리케이션을 모니터링하지 못하도록 제한하기 위해 최적화되어 있습니다. 따라서 초기 데이터 세트가 너무 커지면 이를 제한하거나 샘플링합니다(원격 분석의 일부만 사용하고 나머지는 무시함). 이 동작으로 인해 일부 레코드가 삭제될 가능성이 있으므로 감사 목적으로 사용할 수 없습니다. 

**이름에 변수가 있는 메트릭입니다.** 메트릭 이름의 일부로 변수를 사용하지 마세요. 대신 상수를 사용합니다. 변수가 해당 값을 변경할 때마다 Azure Monitor 새 메트릭을 생성합니다. Azure Monitor 메트릭 수에 대한 제한에 빠르게 도달합니다. 일반적으로 개발자는 메트릭 이름에 변수를 포함하려는 경우 하나의 메트릭 내에서 여러 개의 Time Series를 추적하려고 하며 변수 메트릭 이름 대신 차원을 사용해야 합니다. 

**카디널리티 메트릭 차원이 높습니다.** 차원에 유효한 값이 너무 많은 *메트릭(높은 카디널리티)은* 50,000개 제한에 도달할 가능성이 훨씬 높습니다. 일반적으로 차원에는 지속적으로 변화하는 값을 사용해서는 안됩니다. 예를 들어 타임스탬프는 차원이면 안 됩니다. 서버, 고객 또는 제품 ID를 사용할 수 있지만 각 유형이 더 적은 경우에만 사용할 수 있습니다. 

시험 삼아 이러한 데이터를 그래프에 모두 표시하는지 자문해 보세요. 서버 10대 또는 100대가 있는 경우 비교를 위해 그래프에 모두 표시하는 것이 유용할 수 있습니다. 그러나 1,000이 있는 경우 결과 그래프는 읽기 어렵거나 불가능할 수 있습니다. 100개 미만의 유효한 값으로 유지하는 것이 가장 좋습니다. 300개까지는 애매합니다. 300개 이상을 사용해야 하는 경우 Azure Monitor 사용자 지정 로그를 사용하세요.   

이름 또는 카디널리티가 높은 차원에 변수가 있는 경우 다음이 발생할 수 있습니다.
- 제한으로 인해 메트릭이 불안정해집니다.
- 메트릭 탐색기 작동하지 않습니다.
- 경고 및 알림을 예측할 수 없게 됩니다.
- 비용이 예기치 않게 증가할 수 있습니다. Microsoft는 이 기능이 공개 미리 보기에 있는 동안 차원이 있는 사용자 지정 메트릭에 대한 요금을 청구하지 않습니다. 그러나 나중에 요금이 시작된 후에는 예기치 않은 요금이 발생합니다. 모니터링되는 계열 수 및 API 호출 수에 따라 메트릭 사용량에 대한 요금을 청구할 계획입니다.

메트릭 이름 또는 차원 값이 실수로 식별자 또는 높은 카디널리티 차원으로 채워진 경우 변수 부분을 제거하여 쉽게 수정할 수 있습니다.

그러나 높은 카디널리티가 시나리오에 필수적인 경우 집계된 메트릭이 적합한 선택이 아닐 수 있습니다. 사용자 지정 로그(즉, [trackEvent를](/azure/azure-monitor/app/api-custom-events-metrics#trackevent)사용한 trackMetric API 호출)를 사용하여 전환합니다. 그러나 로그는 값을 집계하지 않으므로 모든 단일 항목이 저장됩니다. 따라서 작은 기간(예: 초당 1백만 개)에 많은 양의 로그가 있는 경우 제한 및 검색 지연이 발생할 수 있습니다. 

## <a name="next-steps"></a>다음 단계
다양한 서비스의 사용자 지정 메트릭을 사용합니다. 
