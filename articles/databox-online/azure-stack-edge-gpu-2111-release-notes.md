---
title: Azure Stack Edge 2111 릴리스 정보
description: 2111 릴리스를 실행 하는 Azure Stack Edge에 대 한 중요 한 미해결 문제 및 해결 방법을 설명 합니다.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.date: 11/18/2021
ms.author: alkohli
ms.openlocfilehash: 940c5d837ca6088ff4bea902aa41a0e9534b75de
ms.sourcegitcommit: b00a2d931b0d6f1d4ea5d4127f74fc831fb0bca9
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/20/2021
ms.locfileid: "132871766"
---
# <a name="azure-stack-edge-2111-release-notes"></a>Azure Stack Edge 2111 릴리스 정보

[!INCLUDE [applies-to-GPU-and-pro-r-and-mini-r-skus](../../includes/azure-stack-edge-applies-to-gpu-pro-r-mini-r-sku.md)]

다음 릴리스 정보는 Azure Stack Edge 장치의 2111 릴리스에 대 한 중요 한 미해결 문제 및 해결 된 문제를 식별 합니다. 이러한 릴리스 정보는 Azure Stack Edge Pro GPU, Azure Stack Edge Pro R 및 Azure Stack Edge Mini R 디바이스에만 해당됩니다. 특정 모델에 해당하는 기능 및 문제가 설명됩니다(해당하는 경우).

릴리스 정보는 계속 업데이트되며, 해결 방법이 필요한 중대한 문제가 발견되면 해당 내용이 추가됩니다. 디바이스를 배포하기 전에 릴리스 정보에 포함된 정보를 주의 깊게 검토하세요.

이 문서는 소프트웨어 버전 번호 **2.2.1777.4088** 에 매핑되는 **Azure Stack Edge 2111** 릴리스에 적용 됩니다. 2.2.1636.3457 (Azure Stack Edge 2106) 소프트웨어를 하나 이상 실행 하는 경우이 소프트웨어를 장치에 적용할 수 있습니다.

## <a name="whats-new"></a>새로운 기능

Azure Stack Edge 2111 릴리스에는 MEC (다중 액세스에 지 계산) 배포에 대 한 버그 수정이 있습니다.


## <a name="issues-fixed-in-2111-release"></a>2111 릴리스에서 해결 된 문제

다음 테이블에는 이전 릴리스의 릴리스 정보에 언급되고 현재 릴리스에서 수정된 이슈가 나열되어 있습니다.

| 아니요. | 기능 | 문제 | 
| --- | --- | --- |
|**1.**|다중 액세스에 지 계산 | 이전 릴리스에서는 Azure Stack Edge 장치가 다시 설정 중인 MEC 작업 관리자 (MEC 에이전트의 구성 요소)에 업데이트로 인해 하 여 Azure 네트워크 함수 관리자에 게 다시 VNF 작업 결과를 전송 하지 않았습니다.  |
|**2.**|다중 액세스에 지 계산 |Mellanox Connectx-3 Lx 이더넷 어댑터에서 최대 가상 함수 수는 이전 릴리스에서 8로 설정 되었습니다. 2111부터 포트 당 기본 가상 함수 수가 32로 증가 합니다. |
|**3.**|다중 액세스에 지 계산 |Azure Stack Edge가 2106를 실행 하 고 네트워크 기능 장치 리소스가 생성 된 경우에는 장치가 2110로 업데이트 됩니다. 네트워크 기능을 배포 하는 경우 배포에 실패 합니다. 장치를 2110으로 업데이트 한 후에는 가상 네트워크가 생성 되지 않습니다. Azure Stack Edge에 기존 네트워크 기능을 배포 하는 경우에는 오류가 발생 하지 않습니다. |
|**4.**|다중 액세스에 지 계산 |네트워크 기능 Vm이 실행 되는 동안 Azure Stack Edge가 버전 2110로 업데이트 되었습니다. 이러한 경우, 이벤트 로그에서 다음 오류가 발생 하 여 업데이트가 실패할 수 있습니다. *"Mellanox connectx-3-4 Lx 이더넷 어댑터" 네트워크 인터페이스가 다시 설정 되기 시작 했습니다. 하드웨어를 다시 설정 하는 동안 네트워크 연결이 일시적으로 중단 됩니다. 이유: 네트워크 드라이버가 적절 한 시간 내에 OID 요청에 응답 하지 않았습니다. 이 네트워크 인터페이스가 마지막으로 초기화 된 후 3 번 다시 설정 되었습니다.* | 


## <a name="known-issues-in-2111-release"></a>2111 릴리스의 알려진 문제

다음 표에서는 2111 릴리스의 알려진 문제에 대 한 요약을 제공 합니다.
