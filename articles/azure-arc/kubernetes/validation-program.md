...
ms.locfileid: "122966250"
---
# <a name="azure-arc-enabled-kubernetes-validation"></a>Azure Arc 지원 Kubernetes 유효성 검사

Azure Arc 지원 Kubernetes는 모든 CNCF(Cloud Native Computing Foundation) 인증 Kubernetes 클러스터에서 작동합니다. Azure Arc 팀은 주요 업계 Kubernetes 제품 공급자와 협력하여 Kubernetes 배포에서 Azure Arc 지원 Kubernetes의 유효성을 검사했습니다. 향후 이러한 공급자가 릴리스한 Kubernetes 배포판의 주 버전과 부 버전은 Azure Arc 지원 Kubernetes와의 호환성을 위해 유효성을 검사합니다.

## <a name="validated-distributions"></a>유효성을 검사한 배포

Microsoft에서 제공한 다음 Kubernetes 배포 및 인프라 공급자는 Azure Arc 지원 Kubernetes에 대한 적합성 테스트를 성공적으로 통과했습니다.
...
| VMware       | [Tanzu Kubernetes Grid](https://tanzu.vmware.com/kubernetes-grid) | Kubernetes 버전: v1.17.5 |
...
또한 Azure Arc 팀은 다음 퍼블릭 클라우드 공급자에서 적합성 테스트를 실행하고 Azure Arc 지원 Kubernetes 시나리오의 유효성을 검사했습니다.
...
다음 시나리오에서는 Azure Arc 지원 Kubernetes 유효성 검사의 일부로 적합성 테스트가 실행되었습니다.
...
2. 구성: 
    * Azure Arc 지원 Kubernetes 리소스 위에 구성을 만듭니다.
