...
description: 이 문서에서는 상태 프로브를 사용하여 Azure Load Balancer 뒤의 인스턴스를 모니터링하는 방법을 알아봅니다.
...
ms.date: 09/17/2019
ms.author: allensu
ms.openlocfilehash: 0682565a8e55d4f6db54a2e90e0ed3e5904bfcfe
ms.sourcegitcommit: 56235f8694cc5f88db3afcc8c27ce769ecf455b0
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/24/2021
ms.locfileid: "133074548"
---
...
| **[프로브 다운 동작](#probedown)** | 모든 프로브가 다운되고, 모든 TCP 흐름이 계속됩니다. | 모든 프로브가 다운되고, 모든 TCP 흐름이 만료됩니다. | 


>[!IMPORTANT]
>Load Balancer 상태 프로브는 168.63.129.16 IP 주소에서 시작되며, 프로브에서 인스턴스를 표시하기 위해 차단되지 않아야 합니다.  자세한 내용은 [원본 IP 주소 프로브](#probesource)를 검토하세요. 백 엔드 인스턴스 내에서 이 프로브 트래픽을 보려면 [이 FAQ](./load-balancer-faqs.yml)를 검토하세요.
...
## <a name="understanding-application-signal-detection-of-the-signal-and-reaction-of-the-platform"></a>애플리케이션 신호 이해, 신호 검색 및 플랫폼 반응 이해

간격 값은 상태 프로브가 백 엔드 풀 인스턴스의 응답을 검색 하는 빈도를 결정 합니다. 상태 프로브가 실패 하는 경우 백 엔드 풀 인스턴스를 비정상으로 즉시 표시 합니다. 마찬가지로, 다음에 정상적인 프로브에서 상태 프로브는 백 엔드 풀 인스턴스를 즉시 정상으로 표시 합니다.

상태 프로브 간격이 5 초로 설정 된 예제를 사용 하 여 더 많은 동작을 설명할 수 있습니다. 프로브가 전송 된 시간은 응용 프로그램의 상태를 변경할 때와 동기화 되지 않기 때문에 응용 프로그램 상태를 반영 하는 상태 프로브에 소요 되는 총 시간은 다음 두 가지 시나리오 중 하나에 해당 될 수 있습니다.

1. 응용 프로그램이 다음 프로브 도착 직전에 시간 제한 프로브 응답을 생성 하기 시작 하는 경우 이러한 이벤트의 검색에는 5 초와 응용 프로그램이 프로브를 도착할 때까지 시간 초과 신호를 보내기 시작 하는 기간이 소요 됩니다.  이 검색은 5 초 이상 소요 된다고 가정할 수 있습니다.
2. 응용 프로그램이 다음 프로브 도착 후에도 시간 제한 프로브 응답을 생성 하기 시작 하면 프로브 도착 (및 시간 초과) 및 다른 5 초까지 이러한 이벤트의 검색이 시작 되지 않습니다.  10 초 미만으로이 검색을 수행할 수 있습니다.

이 예에서는 검색이 발생하면 플랫폼이 이 변경에 대응하는 데 약간의 시간이 소요됩니다.  즉, 다음에 따라 

1. 애플리케이션이 상태 변경을 시작하는 경우
2. 이 변경 내용이 검색 되 면 (다음 상태 프로브가 전송 될 때)
3. 플랫폼 간에 검색이 전달된 경우 

시간 제한 프로브 응답에 대 한 반응은 응용 프로그램에서 신호의 변경에 반응 하는 데 최소 5 초 이상, 최대 10 초 이상까지 소요 될 수 있습니다.  이 예는 발생하는 상황을 설명하기 위해 제공되지만, 이 예에 설명된 위의 대략적인 지침 이상으로 정확한 기간을 예측할 수는 없습니다.
...
Load Balancer 상태 프로브 외에도 [이 IP 주소는 다음 작업에서 사용합니다](../virtual-network/what-is-ip-address-168-63-129-16.md).
...
애플리케이션에 대한 상태 모델을 디자인할 경우, 해당 인스턴스의 상태 __및__ 사용자가 제공하는 애플리케이션 서비스를 반영하는 백 엔드 엔드포인트에서 포트를 프로브해야 합니다.  애플리케이션 포트와 프로브 포트는 동일하지 않아도 됩니다.  일부 시나리오에서는 프로브 포트가 애플리케이션이 서비스를 제공하는 포트와 달라야 할 수 있습니다.  
...
[TCP 타임스탬프](https://tools.ietf.org/html/rfc1323)를 사용하지 않도록 설정합니다.  TCP 타임스탬프를 사용하도록 설정하면 VM의 게스트 OS TCP 스택에서 TCP 패킷이 삭제되어 상태 프로브가 실패할 수 있으며, 이로 인해 Load Balancer가 각 엔드포인트를 표시합니다.  TCP 타임스탬프는 기본적으로 보안이 강화된 VM 이미지에서 주기적으로 사용하도록 설정되며 사용하지 않도록 설정해야 합니다.

## <a name="monitoring"></a>모니터링

공용 및 내부 [표준 Load Balancer](./load-balancer-overview.md) 은 끝점 당 끝점 및 백 엔드 끝점 상태 프로브 상태를 [Azure Monitor](./monitor-load-balancer.md)를 통해 다중 차원 메트릭으로 노출 합니다. 이러한 메트릭은 다른 Azure 서비스나 파트너 애플리케이션에서 사용할 수 있습니다. 

Azure Monitor 로그는 공용 및 내부 기본 부하 분산 장치에 사용할 수 없습니다.

## <a name="limitations"></a>제한 사항

- HTTPS 프로브는 클라이언트 인증서를 사용한 상호 인증을 지원하지 않습니다.
- TCP 타임스탬프를 사용하도록 설정하면 상태 프로브가 실패한다고 가정해야 합니다.
- 기본 SKU 부하 분산 장치 상태 프로브는 가상 머신 확장 집합에서 지원 되지 않습니다.
