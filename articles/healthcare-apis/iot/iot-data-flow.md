...
이 문서에서는 데이터 흐름의 각 단계에 대해 자세히 설명 합니다. 다음 단계는 장치 매핑 (정규화 단계) 및 FHIR 대상 매핑 (변환 단계)을 사용 하 여 [IoT 커넥터를 배포](deploy-iot-connector-in-azure.md) 하는 방법입니다.

다음 섹션에서는 이벤트 허브에서 IoT 커넥터로 IoMT (의료 사물 인터넷) 데이터가 전달 되는 단계에 대해 설명 합니다.

:::image type="content" source="media/iot-data-flow/iot-data-flow.png" alt-text="IoT 장치에서 이벤트 허브로 데이터 흐름을 IoMT 합니다. IoMT 데이터는 FHIR 서비스에서 표준화, 그룹화, 변환 및 지속 될 때 IoT 커넥터에 의해 수집 됩니다." lightbox="media/iot-data-flow/iot-data-flow.png":::

## <a name="ingest"></a>수집
수집은 장치 데이터가 IoT 커넥터로 수신 되는 첫 번째 단계입니다. 장치 데이터에 대 한 수집 끝점은 [Azure Event Hubs](../../event-hubs/index.yml)에서 호스팅됩니다. Azure Event Hubs platform은 초당 수백만 개의 메시지를 수신 하 고 처리할 수 있는 기능을 통해 높은 확장성 및 처리량을 지원 합니다. 또한 IoT 커넥터에서 메시지를 비동기적으로 사용 하 여 장치 데이터를 처리 하는 동안 장치가 대기할 필요가 없습니다.

> [!NOTE]
> 이번에는 장치 데이터에 대해 JSON이 유일 하 게 지원 되는 형식입니다.

## <a name="normalize"></a>Normalize
정규화는 위의 이벤트 허브에서 장치 데이터를 검색 하 고 장치 매핑을 사용 하 여 처리 하는 다음 단계입니다. 이 매핑 프로세스는 장치 데이터를 정규화 된 스키마로 변환 합니다. 
...
변환 단계에서 관찰 FHIR 리소스가 생성 되 면 리소스가 FHIR 서비스에 저장 됩니다. 관찰 FHIR 리소스가 새로운 리소스인 경우 FHIR 서비스에 생성 됩니다. 관찰 FHIR 리소스가 이미 있으면 업데이트 됩니다.
