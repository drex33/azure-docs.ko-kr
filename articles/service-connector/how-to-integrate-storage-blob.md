---
title: 서비스 커넥터와 Azure Blob Storage 통합
description: 서비스 커넥터를 사용 하 여 응용 프로그램에 Azure Blob Storage 통합
author: shizn
ms.author: xshi
...
ms.locfileid: "131052885"
---
# <a name="integrate-azure-blob-storage-with-service-connector"></a>서비스 커넥터와 Azure Blob Storage 통합

이 페이지에는 서비스 커넥터를 사용 하는 Azure Blob Storage의 지원 되는 인증 유형 및 클라이언트 유형이 나와 있습니다. 서비스 커넥터를 사용 하지 않고 다른 프로그래밍 언어의 Azure Blob Storage에 계속 연결할 수 있습니다. 이 페이지에는 서비스 연결을 만들 때 제공 되는 기본 환경 변수 이름 및 값 (또는 스프링 부팅 구성)도 표시 됩니다. [서비스 커넥터 환경 변수 명명 규칙](concept-service-connector-internals.md)에 대해 자세히 알아볼 수 있습니다.
...
| 기본 환경 변수 이름 | 설명 | 예제 값 |
| --- | --- | --- |
| AZURE_STORAGEBLOB_CONNECTIONSTRING | Blob storage 연결 문자열 | `DefaultEndpointsProtocol=https;AccountName={accountName};AccountKey={****};EndpointSuffix=core.windows.net` |

**시스템 할당 관리 ID**

| 기본 환경 변수 이름 | 설명 | 예제 값 |
| --- | --- | --- |
| AZURE_STORAGEBLOB_RESOURCEENDPOINT | Blob storage 끝점 | `https://{storageAccountName}.blob.core.windows.net/` |
...
| --- | --- | --- |
| AZURE_STORAGEBLOB_RESOURCEENDPOINT | Blob storage 끝점 | `https://{storageAccountName}.blob.core.windows.net/` |
| AZURE_STORAGEBLOB_CLIENTID | 클라이언트 ID | `{yourClientID}` |
...
| AZURE_STORAGEBLOB_RESOURCEENDPOINT | Blob storage 끝점 | `https://{storageAccountName}.blob.core.windows.net/` |
| AZURE_STORAGEBLOB_CLIENTID | 클라이언트 ID | `{yourClientID}` |
| AZURE_STORAGEBLOB_CLIENTSECRET | 클라이언트 암호 | `{yourClientSecret}` |
| AZURE_STORAGEBLOB_TENANTID | 테넌트 ID | `{yourTenantID}` |
...
| 애플리케이션 속성 | 설명 | 예제 값 |
| --- | --- | --- |
| azure. 계정-이름 | Blob storage 계정 이름 | `{storageAccountName}` |
| azure. 계정-키 | Blob 저장소 계정 키 | `{yourSecret}` |
| azure.storage.blob-endpoint | Blob storage 끝점 | `https://{storageAccountName}.blob.core.windows.net/` |
