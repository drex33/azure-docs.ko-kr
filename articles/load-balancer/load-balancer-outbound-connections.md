...
ms.date: 07/01/2021
ms.author: allensu
ms.openlocfilehash: 3c7f99e20110ea610a41096152c2d70e1523044c
ms.sourcegitcommit: 702df701fff4ec6cc39134aa607d023c766adec3
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/03/2021
ms.locfileid: "131472906"
---
...
| 5 | [기본 아웃 바운드 액세스](../virtual-network/ip-services/default-outbound-access.md) 사용 | 암시적 | 아니요 | 최하위 |
...
다음이 구성 되지 않은 부하 분산 장치의 백 엔드 리소스는 부하 분산 장치의 프런트 엔드 IP를 통해 아웃 바운드 연결을 만듭니다. 리소스는 기본 SNAT (기본 아웃 바운드 액세스 라고도 함)를 사용 합니다.

* 아웃바운드 규칙
* 인스턴스 수준 공용 IP 주소
* NAT 게이트웨이가 구성 되어 있습니다.

## <a name="default-outbound-access"></a>기본 아웃바운드 액세스

다음이 구성 되지 않은 리소스는 기본 SNAT를 통해 아웃 바운드 연결을 만듭니다. 
...
이 액세스를 기본 아웃 바운드 액세스 라고 합니다. 기본 SNAT를 사용 하는 시나리오의 또 다른 예는 Azure의 가상 머신 (위에서 언급 한 연결 제외)입니다. 이 경우 아웃바운드 연결은 기본 아웃바운드 액세스 IP에서 제공됩니다. 이 IP는 제어할 수 없는 Azure에서 할당 된 동적 IP입니다. 기본 SNAT는 프로덕션 워크 로드에 권장 되지 않습니다.
...
정의에 따르면 모든 IP 주소에는 65,535개의 포트가 있습니다. 각 포트는 TCP(Transmission Control Protocol) 및 UDP(User Datagram Protocol)의 인바운드 또는 아웃바운드 연결에 사용할 수 있습니다. 공용 IP 주소를 부하 분산 장치에 프런트 엔드 IP로 추가하면 64,000개의 포트가 SNAT에 적합합니다. 프런트 엔드 IP로 추가된 모든 공용 IP를 할당할 수 있지만 프런트 엔드 IP는 한 번에 하나씩 소비됩니다. 예를 들어 두 개의 백 엔드 인스턴스에 각각 64,000개의 포트가 할당되고 2개의 프런트 엔드 IP에 액세스할 수 있는 경우 두 백 엔드 인스턴스는 모두 64,000개의 포트가 모두 소진될 때까지 첫 번째 프런트 엔드 IP의 포트를 소비합니다.  
...
아웃바운드 연결이 폭주할 수 있습니다. 백 엔드 인스턴스에 충분한 포트가 할당되지 못할 수 있습니다. 애플리케이션 내에서 **연결 재사용** 기능을 사용합니다. **연결을 다시 사용할** 수 없으면 SNAT 포트 **소모** 위험이 증가합니다. Azure App Service 연결 풀링에 대한 자세한 내용은 Azure App Service [간헐적인 아웃바운드 연결 오류 문제 해결을 참조하세요.](../app-service/troubleshoot-intermittent-outbound-connection-errors.md#avoiding-the-problem)
...
"인스턴스당 포트" 또는 "최대 백 엔드 인스턴스 수"로 SNAT 포트를 수동으로 할당할 수 있습니다. 백 엔드에 Virtual Machines 경우 최대 SNAT 포트 사용량을 얻기 위해 "인스턴스당 포트"로 포트를 할당하는 것이 좋습니다. 

인스턴스당 포트는 다음과 같이 계산되어야 합니다. 

**프런트 엔드 IP 수 * 64K/백 엔드 인스턴스 수** 

백 엔드에 Virtual Machine Scale Sets 경우 "백 엔드 인스턴스의 최대 수"로 포트를 할당하는 것이 좋습니다. 허용되는 나머지 SNAT 포트보다 더 많은 VM이 백 엔드에 추가되면 가상 머신 확장 집합 강화가 차단되거나 새 VM이 충분한 SNAT 포트를 받지 못할 수 있습니다. 
