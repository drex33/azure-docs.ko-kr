...
author: arifgani
ms.author: argani
ms.openlocfilehash: d731b7e637e29d7446f058d157ece0f06a9a4a4b
ms.sourcegitcommit: 3d04177023a3136832adb561da831ccc8e9910c7
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/23/2021
ms.locfileid: "132940281"
---
...
Azure Marketplace SaaS 서비스에서 최종 사용자에게 사용하는 인증 방법에 제약 조건을 적용하지 않습니다. 아래 흐름은 Azure Marketplace에서 SaaS 서비스를 인증하는 경우에만 필요합니다.
...
2. 계정에서 두 개 이상의 액세스 권한을 제공하는 경우 오른쪽 위 모서리에서 계정을 선택합니다. 그런 다음 포털 세션을 원하는 Azure AD 테넌트로 설정합니다.
3. 왼쪽 탐색 창에서 Azure Active Directory 서비스를 **선택하고, 앱 등록** 선택한 다음, 새 애플리케이션 **등록을** 선택합니다.
...
6. 클라이언트 암호를 만들려면 **인증서 & 비밀 페이지로** 이동하고 **+새 클라이언트 암호** 를 선택합니다.  코드에서 사용하려면 암호 값을 복사해야 합니다.
...
애플리케이션을 등록한 후에는 게시자의 권한 부여 토큰(Azure AD V1 엔드포인트를 사용하여 Azure AD 액세스 토큰)을 프로그래밍 방식으로 요청할 수 있습니다. 게시자는 다양한 SaaS 처리 API를 호출할 때 이 토큰을 사용해야 합니다. 이 토큰은 1시간 동안만 유효합니다.

이러한 토큰에 대한 자세한 내용은 [Azure Active Directory 액세스 토큰](../../active-directory/develop/access-tokens.md)을 참조하세요.  아래 흐름에서 V1 엔드포인트 토큰이 사용됩니다.
...
| 요소 | Description |
| ------- | ----------- |
| `access_token` | 이 요소는 `<access_token>` 모든 SaaS 처리 및 Marketplace 계량 API를 호출할 때 권한 부여 매개 변수로 전달할 입니다. 보안이 설정된 REST API를 호출할 때 토큰은 호출자를 인증하는 API를 허용하는 "전달자" 토큰으로 `Authorization` 요청 헤더 필드에 포함됩니다. |
...
이제 Azure AD 보안 앱에서 [SaaS 처리 구독 API 버전 2](pc-saas-fulfillment-subscription-api.md) 및 [SaaS 처리 작업 API 버전 2를](pc-saas-fulfillment-operations-api.md)사용할 수 있습니다.
