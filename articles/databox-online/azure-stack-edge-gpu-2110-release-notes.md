...
description: 2110 릴리스를 실행하는 Azure Stack Edge 대한 중요한 미해결 문제 및 해결에 대해 설명합니다.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.date: 10/26/2021
ms.author: alkohli
ms.openlocfilehash: 42823eed591430641f6097bdd811de47efbe3fcc
ms.sourcegitcommit: b00a2d931b0d6f1d4ea5d4127f74fc831fb0bca9
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/20/2021
ms.locfileid: "132863439"
---
# <a name="azure-stack-edge-2110-release-notes"></a>Azure Stack Edge 2110 릴리스 정보

[!INCLUDE [applies-to-GPU-and-pro-r-and-mini-r-skus](../../includes/azure-stack-edge-applies-to-gpu-pro-r-mini-r-sku.md)]

다음 릴리스 정보에서는 Azure Stack Edge 디바이스에 대한 2110 릴리스의 중요한 미해결 문제 및 해결된 문제를 식별합니다. 이러한 릴리스 정보는 Azure Stack Edge Pro GPU, Azure Stack Edge Pro R 및 Azure Stack Edge Mini R 디바이스에만 해당됩니다. 특정 모델에 해당하는 기능 및 문제가 설명됩니다(해당하는 경우).

릴리스 정보는 계속 업데이트되며, 해결 방법이 필요한 중대한 문제가 발견되면 해당 내용이 추가됩니다. 디바이스를 배포하기 전에 릴리스 정보에 포함된 정보를 주의 깊게 검토하세요.

이 문서는 소프트웨어 버전 번호 **2.2.1758.4034에 매핑되는 Azure Stack Edge 2110** 릴리스에 적용됩니다.  2106(2.2.1636.3457) 소프트웨어를 Azure Stack Edge 실행하는 경우 이 소프트웨어를 디바이스에 적용할 수 있습니다.

## <a name="whats-new"></a>새로운 기능

Azure Stack Edge 2110 릴리스에서 사용할 수 있는 새로운 기능은 다음과 같습니다. 

- **Windows 업데이트 및 보안 수정** - [Windows 및 9월 보안 수정에 대한 최신 LCU(누적 업데이트)가](https://support.microsoft.com/topic/september-14-2021-kb5005623-monthly-rollup-bcdb6598-517e-4d53-aa7c-dd7fcfdca204) Azure Stack Edge 업데이트 패키지로 롤업되었습니다.
- **원격 지원** - 이 릴리스에서는 Microsoft 지원 디바이스에 원격으로 액세스하여 문제를 진단하고 수정할 수 있도록 Azure Stack Edge 디바이스에서 원격 지원을 사용하도록 설정할 수 있습니다. 이 기능을 사용하도록 설정하면 액세스 수준 및 액세스 기간에 대한 동의를 제공합니다. 자세한 내용은 [Azure Stack Edge 원격 지원 및 진단 사용을](azure-stack-edge-gpu-remote-support-diagnostics-repair.md)참조하세요.
- **고성능 네트워크 가상 머신** - 이 릴리스부터 고성능 네트워크 가상 머신을 Azure Stack Edge 디바이스에 배포할 수 있습니다. 자세한 내용은 [Azure Stack Edge 고성능 네트워크 가상 머신 배포를 참조하세요.](azure-stack-edge-gpu-deploy-virtual-machine-high-performance-network.md)
- **Edge 컨테이너 레지스트리 및 Kubernetes 대시보드에 대한 인증서** - 이제 Edge 컨테이너 레지스트리 및 Kubernetes 대시보드에 대한 인증서가 지원됩니다. 로컬 UI를 통해 인증서를 만들고 업로드할 수 있습니다. 자세한 내용은 [Kubernetes 인증서 및 Kubernetes 인증서](azure-stack-edge-gpu-certificates-overview.md#kubernetes-certificates) [업로드 참조하세요.](azure-stack-edge-gpu-manage-certificates.md#upload-kubernetes-certificates)
- **BGP 모드의 Metallb** - 이 릴리스부터는 BGP(Border Gateway Protocol)를 통해 MetalLB를 사용하여 Azure Stack Edge 디바이스에서 부하 분산을 구성할 수 있습니다. 구성은 디바이스의 PowerShell 인터페이스에 연결한 다음 특정 cmdlet을 실행하여 수행됩니다. 자세한 내용은 [Azure Stack Edge 디바이스에서 MetalLB로 부하 분산 구성을 참조하세요.](azure-stack-edge-gpu-configure-metallb-bgp-mode.md)



## <a name="issues-fixed-in-2110-release"></a>2110 릴리스에서 해결된 문제
...
|**6.**|VM |VNF를 배포하는 동안 VM 관리 NIC에 대한 고정 IP 중복 검사가 추가됩니다. 명시적 오류 메시지가 반환됩니다. | 
|**7.**|VM |주소 공간에서 처음 4개의 IP 주소에 대한 IP 예약 검사가 제거되었습니다.  | 
|**8.**|다중 액세스 Edge 컴퓨팅 |VNF를 배포하는 동안 로컬 Azure Resource Manager 토큰 만료 문제가 해결되었습니다. 이전 릴리스에서는 VHD 다운로드가 오래 걸리면 Azure Resource Manager 토큰이 만료되기 때문에 VNF 배포가 실패합니다.  | 
|**9.**|다중 액세스 Edge 컴퓨팅 |VNF를 배포하는 동안 Azure Resource Manager 호출에 대한 시간 초과가 추가되었습니다. 이전 릴리스에서는 Azure Resource Manager 호출에 성공하지 못한 경우 VNF 배포에 오랜 시간이 걸렸습니다.   | 
|**10.**|다중 액세스 Edge 컴퓨팅 |다중 액세스 Edge 컴퓨팅은 VHD 다운로드가 완료된 후 Azure Resource Manager 템플릿 배포를 정리합니다. 이전 릴리스에서는 많은 VNF 배포 후 사용자가 배포 할당량 초과 오류에 도달했습니다. 기본 할당량은 리소스 그룹당 800개의 배포였습니다.  | 



## <a name="known-issues-in-2110-release"></a>2110 릴리스의 알려진 문제

다음 표에서는 2110 릴리스의 알려진 문제에 대한 요약을 제공합니다.

| 아니요. | 기능 | 문제 | 해결 방법/설명 |
| --- | --- | --- | --- |
|**1.**|미리 보기 기능 |이 릴리스의 경우 로컬 Azure Resource Manager, VM, VM의 클라우드 관리, Kubernetes 클라우드 관리, Azure Stack Edge Pro R 및 Azure Stack Edge Mini R VPN, MPS(다중 프로세스 서비스) 및 Azure Stack Edge Pro GPU용 MEC(Multi-Access Edge Computing) 기능은 모두 미리 보기로 제공됩니다.  |이러한 기능은 이후 릴리스에서 일반적으로 제공될 예정입니다. |
...
|**11.**|Kubernetes 클러스터|기존 Azure IoT Edge 마켓플레이스 모듈은 Azure Stack Edge 디바이스에서 IoT Edge를 실행하기 위해 수정이 필요할 수 있습니다.|자세한 내용은 [Azure Stack Edge Pro GPU 디바이스의 Azure Stack Edge Pro FPGA 디바이스에서 기존 IoT Edge 모듈 실행을 참조하세요.](azure-stack-edge-gpu-modify-fpga-modules-gpu.md)|
...
|**25.**|MEC(다중 액세스 Edge 컴퓨팅) |Azure Stack Edge 2106을 실행하고 네트워크 함수 디바이스 리소스가 만들어지고 Azure Stack Edge 2110으로 업데이트되면 네트워크 함수를 배포할 때 배포가 실패합니다. 디바이스가 2110으로 업데이트된 후 가상 네트워크가 만들어지지 않습니다. Azure Stack Edge 기존 네트워크 함수 배포가 있는 경우 오류가 발생하지 않습니다. | 이 문제를 해결하려면 Azure Stack Edge cmdlet을 사용하여 동일한 디바이스 리소스를 다시 `Invoke-MecRegister` 등록하고 Azure Stack Edge 리소스의 활성화 키를 사용합니다. 또는 다음 명령을 통해 가상 스위치를 만들 수 있습니다. <ul><li> `Add-HcsExternalVirtualSwitch -InterfaceAlias  Port5 -WaitForSwitchCreation $true -switchName mec-vswitch-LAN  -SupportsAcceleratedNetworking $true` </li><li> `Add-HcsExternalVirtualSwitch -InterfaceAlias  Port6 -WaitForSwitchCreation $true -switchName mec-vswitch-WAN -SupportsAcceleratedNetworking $true` </li></ul>  |
|**26.**|MEC(다중 액세스 Edge 컴퓨팅) |Azure Stack Edge 네트워크 함수 VM이 실행되는 동안 버전 2110으로 업데이트되었습니다. 이러한 경우 Mellanox 이더넷 어댑터에 연결된 가상 머신을 중지하려고 할 때 업데이트가 실패할 수 있습니다. 다음 오류가 이벤트 로그에 *표시됩니다. 네트워크 인터페이스 "Mellanox ConnectX-4 Lx 이더넷 어댑터"가 다시 설정되기 시작했습니다. 하드웨어가 다시 설정되는 동안 네트워크 연결이 잠시 중단됩니다. 이유: 네트워크 드라이버가 적시에 OID 요청에 응답하지 않았습니다. 이 네트워크 인터페이스는 마지막으로 초기화된 이후 3번 다시 설정되었습니다.*  |이 문제를 해결하려면 Azure Stack Edge 다시 부팅하고 디바이스 업데이트를 다시 시도합니다. |
