...
description: Azure Data Factory 또는 Synapse Analytics 파이프라인의 복사 활동을 사용하여 Amazon Marketplace 웹 서비스에서 지원되는 싱크 데이터 저장소로 데이터를 복사하는 방법을 알아봅니다.
...
ms.locfileid: "124828305"
---
# <a name="copy-data-from-amazon-marketplace-web-service-using-azure-data-factory-or-synapse-analytics"></a>Azure Data Factory 또는 Synapse Analytics 사용하여 Amazon Marketplace 웹 서비스에서 데이터 복사

[!INCLUDE[appliesto-adf-asa-md](includes/appliesto-adf-asa-md.md)]

이 문서에서는 Azure Data Factory 또는 Synapse Analytics 파이프라인의 복사 작업을 사용하여 Amazon Marketplace 웹 서비스에서 데이터를 복사하는 방법을 설명합니다. 이 문서는 복사 작업에 대한 일반적인 개요를 제공하는 [복사 작업 개요](copy-activity-overview.md) 문서를 기반으로 합니다.

## <a name="supported-capabilities"></a>지원되는 기능

이 Amazon Marketplace Web Service 커넥터는 다음과 같은 작업에 지원됩니다.
...
## <a name="create-a-linked-service-to-amazon-marketplace-web-service-using-ui"></a>UI를 사용하여 Amazon Marketplace 웹 서비스에 연결된 서비스 만들기

다음 단계를 사용하여 Azure Portal UI에서 Amazon Marketplace Web Service에 연결된 서비스를 생성합니다.
...
2. Amazon을 검색하고 Amazon Marketplace Web Service 커넥터를 선택합니다.

   :::image type="content" source="media/connector-amazon-marketplace-web-service/amazon-marketplace-web-service-connector.png" alt-text="Amazon Marketplace Web Service 커넥터의 스크린샷":::    


1. 서비스 세부 정보를 구성하고, 연결을 테스트하고, 새로운 연결된 서비스를 만듭니다.

   :::image type="content" source="media/connector-amazon-marketplace-web-service/configure-amazon-marketplace-web-service-linked-service.png" alt-text="Amazon Marketplace 웹 서비스에 대한 연결된 서비스 구성의 스크린샷":::
...
| mwsAuthToken | Amazon MWS 인증 토큰입니다. 이 필드를 SecureString으로 표시하여 안전하게 저장하거나, [Azure Key Vault에 저장된 비밀을 참조](store-credentials-in-key-vault.md)합니다. | 예 |
| accessKeyId | 데이터 액세스에 사용되는 액세스 키 ID입니다.  | 예 |
| secretKey | 데이터 액세스에 사용되는 비밀 키입니다. 이 필드를 SecureString으로 표시하여 안전하게 저장하거나, [Azure Key Vault에 저장된 비밀을 참조](store-credentials-in-key-vault.md)합니다. | 예 |
...
| 속성 | 설명 | 필수 |
|:--- |:--- |:--- |
| type | 복사 작업 원본의 type 속성은 **AmazonMWSSource** 로 설정해야 합니다. | 예 |
| Query | 사용자 지정 SQL 쿼리를 사용하여 데이터를 읽습니다. 예: `"SELECT * FROM Orders where  Amazon_Order_Id = 'xx'"` | 아니요(데이터 세트의 "tableName"이 지정된 경우) |
