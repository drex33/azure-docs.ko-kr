...
단일 구독, 여러 구독 (공유 범위) 및 관리 그룹에 대해 예약 된 용량을 사용할 수 Azure Storage. 단일 구독으로 범위가 지정되면 예약 할인이 선택한 구독에만 적용됩니다. 여러 구독으로 범위가 지정되면 고객의 청구 컨텍스트 내에서 해당 구독 간에 예약 할인이 공유됩니다. 범위가 관리 그룹으로 지정 된 경우 예약 할인은 관리 그룹 및 청구 범위의 일부인 구독에서 공유 됩니다.
...
모든 액세스 계층(핫, 쿨 및 보관)은 예약에 대해 지원됩니다. 액세스 계층에 대 한 자세한 내용은 [blob 데이터에 대 한 핫, 쿨 및 보관 액세스 계층](access-tiers-overview.md)을 참조 하세요.
...
1. 아래 표에서 설명한 대로 필요한 필드를 채웁니다.
   ...
   | **지역** | 예약이 적용되는 지역입니다. |
   | **액세스 계층** | 예약이 적용되는 액세스 계층입니다. *핫*, *쿨* 또는 *보관* 옵션이 있습니다. 액세스 계층에 대 한 자세한 내용은 [blob 데이터에 대 한 핫, 쿨 및 보관 액세스 계층](access-tiers-overview.md)을 참조 하세요. |
...
언제든지 Azure Storage 예약을 취소할 수 있습니다. 취소 하면 예약의 남은 기간을 기준으로 비례 상환 하는 환불을 받게 됩니다. 연간 최대 환불액은 5만 달러입니다.
