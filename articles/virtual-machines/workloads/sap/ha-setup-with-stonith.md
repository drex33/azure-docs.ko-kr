...
ms.locfileid: "112455251"
---
# <a name="high-availability-set-up-in-suse-using-the-stonith-device"></a>STONITH 디바이스를 사용하여 SUSE에서 고가용성 설정

이 문서는 STONITH 디바이스를 사용하여 SUSE 운영 체제의 HANA Large Instance에서 고가용성을 설정하기 위한 자세한 단계별 지침을 제공합니다.
...
SUSE 클러스터링을 사용하여 고가용성(HA)을 설정하려면 다음 필수 구성 요소를 충족해야 합니다.
...
HANA 시스템 복제(HSR)를 사용하여 HANA Large Instances를 설정하는 경우, Microsoft 서비스 관리 팀에 STONITH 디바이스 설정을 요청해야 합니다. 프로비전 시 이 작업을 바로 수행합니다. 
...
이 섹션에서는 Microsoft 서비스 관리 팀이 STONITH 디바이스를 구성한 후 설정에 맞는 SBD 디바이스를 결정하는 방법을 설명합니다. **이 섹션은 기존 고객에게만 적용됩니다**. 새 고객인 경우, Microsoft 서비스 관리 팀에서 SBD 디바이스 이름을 제공하므로 이 섹션을 건너뜁니다.
...
1.3 검색 명령을 실행하여 4개 세션을 표시합니다. 두 노드에서 모두 실행합니다.
...
1.4 iSCSI 디바이스에 로그인하는 명령을 실행하고 4개 세션을 표시합니다. **두** 노드에서 모두 실행합니다.
...
1.5 다시 검사 스크립트인 *rescan-scsi-bus.sh* 를 실행합니다. 이 스크립트는 생성된 새 디스크를 표시합니다.  두 노드에서 모두 실행합니다. 0보다 더 큰 LUN 번호(예: 1, 2 등)가 표시됩니다.
...
1.6 디바이스 이름을 가져오려면 *fdisk –l* 명령을 실행합니다. 두 노드에서 모두 실행합니다. **178MiB** 크기를 가진 디바이스를 선택합니다.
...
2.2 디바이스에 기록된 내용을 확인합니다. 두 노드에서 **모두** 수행합니다.
...
리소스 STONITH를 추가합니다. 파일을 생성하고 다음 텍스트를 추가합니다.
