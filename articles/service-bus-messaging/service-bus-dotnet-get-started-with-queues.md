...
ms.openlocfilehash: e55ae7a0bb0a39346d9bff54f2ebecf890880319
ms.sourcegitcommit: 66b6e640e2a294a7fbbdb3309b4829df526d863d
ms.translationtype: HT
ms.contentlocale: ko-KR
ms.lasthandoff: 12/01/2021
ms.locfileid: "133366132"
---
...
> [!NOTE]
> 이 빠른 시작에서는 Service Bus 큐에 메시지 일괄 처리를 보내고 받는 간단한 시나리오를 구현 하는 간단한 시나리오를 구현 하는 단계별 지침을 제공 합니다. 다른 시나리오 및 고급 시나리오에 대 한 추가 예제는 [GitHub에서 Service Bus .net 샘플](https://github.com/Azure/azure-sdk-for-net/tree/master/sdk/servicebus/Azure.Messaging.ServiceBus/samples)을 참조 하세요. 
...
1. **Program.cs** 의 네임스페이스 정의 위쪽에서 다음 `using` 문을 클래스 선언 앞에 추가합니다.
    ...
    ```
2. `Program` 클래스 내에서 `Main` 메서드 바로 앞에 다음 속성을 선언합니다. 

    `<NAMESPACE CONNECTION STRING>`을 Service Bus 네임스페이스에 대한 기본 연결 문자열로 바꿉니다. 그런 후 `<QUEUE NAME>`을 큐의 이름으로 바꿉니다.
    ...
    ```
3. `Main` 메서드의 코드를 다음 코드로 바꿉니다. 코드에 대한 자세한 내용은 코드 주석을 참조하세요. 다음은 코드의 중요한 단계입니다.  
...
3. 수신된 메시지 및 오류를 처리하려면 다음 메서드를 `Program` 클래스에 추가합니다. 
    ...
    ```
4. `Main` 메서드의 코드를 다음 코드로 바꿉니다. 코드에 대한 자세한 내용은 코드 주석을 참조하세요. 다음은 코드의 중요한 단계입니다. 
    1. 네임스페이스에 대한 기본 연결 문자열을 사용하여 [ServiceBusClient](/dotnet/api/azure.messaging.servicebus.servicebusclient) 개체를 만듭니다. 
    ...
    1. 사용자가 키를 눌러 처리를 종료하면 [ServiceBusProcessor](/dotnet/api/azure.messaging.servicebus.servicebusprocessor) 개체에서 [StopProcessingAsync](/dotnet/api/azure.messaging.servicebus.servicebusprocessor.stopprocessingasync)를 호출합니다. 
