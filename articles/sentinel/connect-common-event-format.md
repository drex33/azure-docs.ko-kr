---
title: 디바이스 또는 어플라이언스에서 Microsoft Sentinel | CEF 형식 로그를 얻습니다. Microsoft Docs
description: Linux 기반 로그 전달자에서 설치된 Log Analytics 에이전트를 사용하여 Syslog를 통해 CEF(Common Event Format)로 전송된 로그를 Microsoft Sentinel 작업 영역으로 가져옵니다.
author: yelevin
ms.topic: conceptual
ms.date: 11/09/2021
ms.author: yelevin
ms.custom: ignite-fall-2021
ms.openlocfilehash: 9828e8bd7bd21a7748cfa78bd521240e2fda2ba4
ms.sourcegitcommit: 1244a72dbec39ac8cf16bb1799d8c46bde749d47
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/18/2021
ms.locfileid: "132763053"
---
# <a name="get-cef-formatted-logs-from-your-device-or-appliance-into-microsoft-sentinel"></a>디바이스 또는 어플라이언스에서 Microsoft Sentinel로 CEF 형식 로그를 얻습니다.

[!INCLUDE [Banner for top of topics](./includes/banner.md)]

[!INCLUDE [reference-to-feature-availability](includes/reference-to-feature-availability.md)]

많은 네트워킹 및 보안 디바이스와 어플라이언스는 Syslog 프로토콜을 통해 CEF(Common Event Format)라는 특수 형식으로 시스템 로그를 보냅니다. 이 형식에는 표준 Syslog 형식보다 더 많은 정보가 포함되며 구문 분석된 키-값 배열로 정보를 표시합니다. Log Analytics 에이전트는 CEF 로그를 수락하고 Microsoft Sentinel 작업 영역에 전달하기 전에 Microsoft Sentinel에서 사용하기 위해 특히 형식을 지정합니다.

이 문서에서는 CEF 형식 로그를 사용하여 데이터 원본을 연결하는 프로세스를 설명합니다. 이 방법을 사용하는 데이터 커넥터에 대한 자세한 내용은 [Microsoft Sentinel 데이터 커넥터 참조를 참조하세요.](data-connectors-reference.md)

이 연결을 만드는 주요 2단계는 아래에서 자세히 설명합니다.

- Linux 머신 또는 VM을 전용 로그 전달자로 지정하고, Log Analytics 에이전트를 설치하고, 로그를 Microsoft Sentinel 작업 영역으로 전달하도록 에이전트를 구성합니다. 에이전트의 설치 및 구성은 배포 스크립트에 의해 처리됩니다.
...
Log Analytics에 CEF 데이터를 수집하려면 Microsoft Sentinel 작업 영역이 필요합니다.
...
이 섹션에서는 디바이스에서 Microsoft Sentinel 작업 영역으로 로그를 전달할 Linux 머신을 지정하고 구성하는 방법을 설명합니다.
...
자세한 내용은 [Microsoft Sentinel에 Syslog 및 CEF 로그를 기록하는 로그 전달자 배포를 참조하세요.](connect-log-forwarder.md)
...
제품이 데이터 커넥터 갤러리에 표시되는 경우 [Microsoft Sentinel 데이터 커넥터 참조를 참조하여](data-connectors-reference.md) 지원을 받을 수 있습니다. 여기서 구성 지침에는 아래 목록에 있는 설정이 포함되어야 합니다.
...
데이터 커넥터 갤러리에 나오는 일부 제품은 추가 파서가 있어야 최상의 결과를 얻을 수 있습니다. 이러한 파서는 Kusto 함수를 사용하여 구현됩니다. 자세한 내용은 Microsoft Sentinel 데이터 커넥터 참조 페이지에서 제품에 대한 [섹션을 참조하세요.](data-connectors-reference.md)

이러한 제품에 대한 CEF 이벤트를 찾으려면 ‘CommonSecurityLog’ 대신 Kusto 함수의 이름을 쿼리 제목으로 입력하세요.

Microsoft Sentinel 포털에 있는 제품 데이터 커넥터 페이지의 **다음 단계** 탭에서 제품에 대해 특별히 만들어진 유용한 샘플 쿼리, 통합 문서 및 분석 규칙 템플릿을 찾을 수 있습니다.
...
기본적으로 Log Analytics 에이전트는 스키마의 *TimeGenerated* 필드를 에이전트가 Syslog 디먼에서 이벤트를 받은 시간으로 채웁니다. 따라서 원본 시스템에서 이벤트가 생성된 시간은 Microsoft Sentinel에 기록되지 않습니다.
...
이 문서에서는 Microsoft Sentinel이 디바이스 및 어플라이언스에서 CEF 로그를 수집하는 방법을 배웠습니다. Microsoft Sentinel에 제품을 연결하는 방법에 대한 자세한 내용은 다음 문서를 참조하세요.

- [Syslog/CEF 전달자 배포](connect-log-forwarder.md)
- [Microsoft Sentinel 데이터 커넥터 참조](data-connectors-reference.md)
- [로그 전달자 연결 문제 해결](troubleshooting-cef-syslog.md#validate-cef-connectivity)

Microsoft Sentinel에서 수집한 데이터로 수행할 작업을 자세히 알아보려면 다음 문서를 참조하세요.
