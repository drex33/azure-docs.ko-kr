...
- Marketplace, 사용자 지정 이미지 및 [Azure Compute 갤러리](./shared-image-galleries.md) (이전의 공유 이미지 갤러리)에서 지원 됩니다.
...
| **재배포** | OS 디스크 데이터가 유지됩니다. | Os 디스크의 데이터가 삭제 되 고 OS가 다시 프로 비전 됩니다. | 
| **VM 중지/시작** | OS 디스크 데이터가 유지됩니다. | Os 디스크의 데이터가 삭제 되 고 OS가 다시 프로 비전 됩니다. | 
...
A: 플랫폼, Shared Image Gallery 및 사용자 지정 이미지를 지원합니다. OS 캐시 배치가 있는 VM 캐시 크기 및 임시 디스크 배치를 통한 임시 디스크 크기까지 지원합니다. 여기서 OS 디스크에 대한 모든 읽기/쓰기는 Virtual Machine과 동일한 노드에서 로컬로 설정됩니다. 
