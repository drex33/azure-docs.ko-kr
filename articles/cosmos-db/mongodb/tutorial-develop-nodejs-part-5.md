---
title: Mongoose를 사용하여 Angular 앱을 Azure Cosmos DB의 API for MongoDB에 연결
...
여러 파트로 구성된 이 자습서에서는 Express 및 Angular를 사용하여 Node.js 앱을 만들고, [Cosmos DB의 API for MongoDB를 사용하여 구성된 Cosmos 계정](mongodb-introduction.md)에 연결하는 방법을 보여줍니다. 이 문서는 자습서의 5부를 설명하며 [4부](tutorial-develop-nodejs-part-4.md)를 기반으로 합니다.
...
* 이 자습서를 시작하기 전에 [4부](tutorial-develop-nodejs-part-4.md)의 단계를 완료해야 합니다.
...
1. **environment.js** 파일에서 `accountName` 값을 이 자습서의 [4부](tutorial-develop-nodejs-part-4.md)에서 만든 Azure Cosmos DB 계정 이름으로 변경합니다. 
...
1. 터미널 창에서 다음 CLI 명령을 사용하여 Azure Cosmos DB 계정에 대한 기본 키를 검색합니다. 
    ...
    \<cosmosdb-name>은 이 자습서의 [4부](tutorial-develop-nodejs-part-4.md)에서 만든 Azure Cosmos DB 계정의 이름입니다.
...
1. Visual Studio Code에서 모든 변경 내용을 저장합니다. 왼쪽에서 **디버그** 단추:::image type="icon" source="./media/tutorial-develop-nodejs-part-5/debug-button.png":::를 선택한 다음 **디버깅 시작** 단추:::image type="icon" source="./media/tutorial-develop-nodejs-part-5/start-debugging-button.png":::를 선택합니다.

1. 이제 브라우저로 전환합니다. **개발자 도구** 를 열고 **네트워크 탭** 을 엽니다. `http://localhost:3000`으로 이동하면 애플리케이션을 볼 수 있습니다.

    :::image type="content" source="./media/tutorial-develop-nodejs-part-5/azure-cosmos-db-heroes-app.png" alt-text="Azure Portal의 새 Azure Cosmos DB 계정":::
...
> [6부: 앱에 게시, 배치 및 삭제 함수 추가](tutorial-develop-nodejs-part-6.md)
