...
ms.locfileid: "131033041"
---
# <a name="multitenant-applications-in-azure"></a>Azure의 다중 테넌트 애플리케이션
다중 테넌트 애플리케이션은 "개별 테넌트의 사용자"가 자신의 고유 항목처럼 애플리케이션을 볼 수 있게 하는 공유 리소스입니다. 다중 테넌트 애플리케이션이 적합한 일반적인 시나리오는 다른 테넌트의 모든 애플리케이션 사용자가 사용자 환경을 사용자 지정하려고 하지만 기본적인 비즈니스 요구 사항을 동일하게 준수해야 하는 상황입니다. 대규모 다중 테넌트 애플리케이션의 예로는 Microsoft 365, Outlook.com 및 visualstudio.com이 있습니다.
...
간단히 말해서 확장성이 뛰어난 서비스를 제공 하기 위해 고려해 야 할 여러 가지 고려 사항이 있지만 많은 다중 테 넌 트 응용 프로그램에 공통 되는 여러 목표 및 요구 사항도 있습니다. 일부는 특정 시나리오에서 적절하지 않을 수 있으며, 개별 목표와 요구 사항의 중요성은 시나리오별로 다릅니다. 다중 테 넌 트 응용 프로그램의 공급자는 테 넌 트의 요구 사항, 수익성, 요금 청구, 여러 서비스 수준, 프로 비전, 유지 관리 모니터링 및 자동화와 같은 목표와 요구 사항도 있습니다.
...
* 호스트 헤더로 웹 사이트 테넌트 분할(TLS 통신을 사용하거나 사용하지 않음)
* 쿼리 매개 변수로 웹 사이트 테넌트 분할
* 작업자 역할의 웹 서비스
  * 일반적으로 애플리케이션 백 엔드에서 데이터를 처리하는 작업자 역할
...
* SQL Database 테넌트별 SQL Server 로그인에서 다중 테넌트 데이터 보호.
* 컨테이너 수준 액세스 정책을 지정하여 애플리케이션 리소스용 Azure 테이블 사용. 공유 액세스 서명으로 보호되는 리소스에 대해 새 URL을 발급하지 않고도 권한을 조정할 수 있습니다.
* 애플리케이션 리소스의 Azure 큐 - Azure 큐는 일반적으로 테넌트를 대신하여 처리를 구동하는 데 사용되지만 프로비전 또는 관리에 필요한 작업을 분산하는 데에도 사용됩니다.
* Service Bus 공유 서비스에 작업을 푸시하는 응용 프로그램 리소스에 대 한 큐는 단일 큐를 사용할 수 있습니다 .이 큐에서 각 테 넌 트 발신자는 ACS에서 발급 된 클레임에서 파생 된 권한만 보유 하 여 해당 큐에 푸시할 수 있는 반면, 서비스의 수신자는 여러 테 넌 트의 데이터를 큐에서 가져올 수 있는 권한이 있습니다.
...
* Azure Service Bus는 분산 및 하이브리드 애플리케이션에 대해 복잡한 방화벽 및 보안 인프라 없이도 Azure 호스트된 애플리케이션과 온-프레미스 애플리케이션 및 서비스 사이의 통신과 같은 보안 메시징 및 데이터 흐름 기능을 제공합니다. 애플리케이션 리소스의 Service Bus Relay를 사용합니다. 엔드포인트로 노출되는 서비스는 테넌트에 속할 수 있으며(예: 온-프레미스와 같이 시스템 외부에서 호스트됨) 또는 테넌트용으로 특별히 프로비저닝된(중요한 테넌트 특정 데이터가 전송되므로) 서비스일 수 있습니다.

**리소스 프로비전**

Azure에는 애플리케이션의 새 테넌트를 프로비저닝하는 방법이 많습니다. 테넌트가 많은 다중 테넌트 애플리케이션의 경우 셀프 서비스 프로비전을 사용하여 이 프로세스를 자동화해야 하는 경우가 많습니다.
