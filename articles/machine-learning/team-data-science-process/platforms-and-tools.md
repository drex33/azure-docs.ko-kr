...
Azure Machine Learning은 데이터 과학 개발을 위한 기본 권장 플랫폼입니다.  이 PaaS(Platform as a Service)는 독립 실행형 작업 또는 이 웹 페이지에 언급된 다른 플랫폼 및 도구와의 통합을 모두 제공합니다. AzureML([Azure Machine Learning](../overview-what-is-azure-ml.md))은 다음을 포함하는 엔드투엔드 플랫폼입니다.
...
Microsoft에서 Windows 및 Linux 모두에 제공하는 데이터 과학 가상 머신에는 데이터 과학 모델링 및 개발 작업을 위한 인기 있는 도구가 포함되어 있습니다. 
...
Data Science Virtual Machine에는 가장 유용한 데이터 과학 도구가 미리 설치되어 제공됩니다.  최신 도구 및 버전 목록은 [Data Science Virtual Machine에 포함된 도구](/azure/machine-learning/data-science-virtual-machine/tools-included)를 참조하세요.

현재 DSVM은 **Windows** 및 **Linux Ubuntu** 운영 체제에서 사용할 수 있습니다. 실행하려는 데이터 과학 프로젝트의 요구 사항에 따라 DSVM의 크기(CPU 코어 수 및 메모리 양)를 선택합니다. 

DSVM의 Windows 버전에 대한 자세한 내용은 Azure Marketplace의 [Microsoft 데이터 과학 Virtual Machine](https://azuremarketplace.microsoft.com/marketplace/apps/microsoft-dsvm.dsvm-win-2019)을 참조하세요. Linux DSVM 버전의 경우 [Linux 데이터 과학 Virtual Machine](https://azuremarketplace.microsoft.com/marketplace/apps/microsoft-dsvm.ubuntu-1804)을 참조하세요.
...
Azure Synapse Analytics를 사용하면 과도하게 프로비저닝하거나 요금을 과도하게 지급하지 않고 컴퓨팅 리소스를 몇 초 만에 쉽게 스케일링할 수 있습니다. 또한 컴퓨팅 리소스의 사용을 일시 중지하는 고유한 옵션을 제공하므로 클라우드 비용을 자유롭게 더 효율적으로 관리할 수 있습니다. 스케일링 가능한 컴퓨팅 리소스를 배포하는 기능을 사용하면 모든 데이터를 Azure Synapse Analytics로 가져올 수 있습니다. 스토리지 비용은 최소화되며, 분석하려는 데이터 세트 부분에 대해서만 컴퓨팅을 실행할 수 있습니다. 

Azure Synapse Analytics에 대한 자세한 내용은 [Azure Synapse Analytics](https://azure.microsoft.com/services/sql-data-warehouse) 웹 사이트를 참고하세요. Azure Synapse Analytics를 사용하여 엔드투엔드 고급 분석 솔루션을 빌드하는 방법은 [실행 중인 Team Data Science Process: Azure Synapse Analytics 사용](sqldw-walkthrough.md)을 참고하세요.
...
R Services(In-Database)는 포괄적인 SQL Server 도구 및 기술 세트를 통해 오픈 소스 R 언어를 지원합니다. 탁월한 성능, 보안, 안정성 및 관리 효율성을 제공합니다. 편리하고 친숙한 도구를 사용하여 R 솔루션을 배포할 수 있습니다. 프로덕션 애플리케이션에서 R 런타임을 호출하고 Transact-SQL을 사용하여 예측 및 시각적 개체를 검색할 수 있습니다. 또한 ScaleR 라이브러리를 사용하여 R 솔루션의 규모와 성능을 향상시킵니다. 자세한 내용은 [SQL Server R Services](/sql/advanced-analytics/r/sql-server-r-services)를 참조하세요.

Microsoft의 TDSP 팀은 SQL Server 2016 R Services에서 R 프로그래머용 및 SQL 개발자용 데이터 과학 솔루션을 빌드하는 방법을 보여 주는 두 가지 엔드투엔드 연습을 게시했습니다. **R 프로그래머** 의 경우 [데이터 과학 엔드투엔드 연습](/sql/advanced-analytics/tutorials/walkthrough-data-science-end-to-end-walkthrough)을 참조하세요. **SQL 개발자** 의 경우 [SQL 개발자용 In-Database 고급 분석(자습서)](/sql/advanced-analytics/tutorials/sqldev-in-database-r-for-sql-developers)을 참조하세요.
...
**Windows** 에서 TDSP를 수행하는 경우 Git 리포지토리와 통신하기 위해 **GCM(Git Credential Manager)** 을 설치해야 합니다. GCM을 설치하려면 먼저 **Chocolatey** 를 설치해야 합니다. Chocolatey 및 GCM을 설치하려면 Windows PowerShell에서 **관리자** 권한으로 다음 명령을 실행합니다.  
...
1. 페이지의 오른쪽 위 모서리에 있는 **<사용자 이름\>** 을 클릭하고 **보안** 을 클릭합니다. 
...
1. 텍스트 상자에서 복사한 ssh 키를 붙여넣고 저장합니다.
...
Azure Machine Learning Studio(클래식)를 사용하여 Team Data Science Process의 단계를 실행하는 방법을 보여주는 예제는 [Azure ML 사용](./index.yml) 학습 경로를 참조하세요.
