...
- 지원되는 원본 환경.
- [Oracle용 SSMA(SQL Server Migration Assistant)](https://www.microsoft.com/download/details.aspx?id=54258)
...
1. **인벤토리 데이터베이스 만들기** 를 선택합니다. 새 인벤토리 데이터베이스의 이름과 간략한 설명을 입력한 다음, **확인** 을 선택합니다.
...
데이터 원본을 식별한 다음 [Oracle용 SQL Server Migration Assistant](https://www.microsoft.com/download/details.aspx?id=54258)를 사용하여 SQL Server VM으로 마이그레이션할 Oracle 인스턴스를 평가합니다. 도우미를 사용하면 원본과 대상 데이터베이스 간의 차이를 파악할 수 있습니다. 데이터베이스 개체와 데이터를 검토하고, 마이그레이션할 데이터베이스를 평가하며, 데이터베이스 개체를 SQL Server로 마이그레이션한 다음, 데이터를 SQL Server로 마이그레이션할 수 있습니다.

평가를 만들려면 다음 단계를 따릅니다.


1. [Oracle용 SQL Server Migration Assistant](https://www.microsoft.com/download/details.aspx?id=54258)를 엽니다.
...
1. 데이터 마이그레이션: **Oracle 메타데이터 탐색기** 에서 마이그레이션할 데이터베이스 또는 개체를 마우스 오른쪽 단추로 클릭하고 **데이터 마이그레이션** 을 선택합니다. 또는 **데이터 마이그레이션** 탭을 선택할 수 있습니다. 전체 데이터베이스에 대한 데이터를 마이그레이션하려면 데이터베이스 이름 옆의 확인란을 선택합니다. 개별 테이블에서 데이터를 마이그레이션하려면 데이터베이스를 확장하고 **테이블** 을 확장한 다음, 테이블 옆의 확인란을 선택합니다. 개별 테이블에서 데이터를 생략하려면 확인란의 선택을 취소합니다.
...
1. SSMA for Oracle을 열고 **Tester** 를 선택한 다음, **새 테스트 사례** 를 선택합니다.
   ...
   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/ssma-tester-new.png" alt-text="새 테스트 사례를 보여 주는 스크린샷":::

1. 테스트 사례 마법사에서 다음 정보를 제공합니다.
   ...
   **설명**: 테스트 사례의 목적을 식별하기 위한 추가 정보를 입력합니다.
   ...
   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/tester-init-test-case.png" alt-text="테스트 사례를 초기화하는 단계를 보여 주는 스크린샷":::

1. 왼쪽에 있는 Oracle 개체 트리에서 테스트 사례의 일부인 개체를 선택합니다.

   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/tester-select-configure-objects.png" alt-text="개체 선택 및 구성 단계를 보여 주는 스크린샷":::
...
1. 그런 다음, 왼쪽 창의 Oracle 개체 트리에서 테이블, 외래 키 및 기타 종속 개체를 선택합니다.
   ...
   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/tester-select-configure-affected.png" alt-text="영향을 받는 개체를 선택하고 구성하는 단계를 보여 주는 스크린샷":::
...
1. 개체의 평가 순서를 검토합니다. 그리드의 단추를 클릭하여 순서를 변경합니다.
   ...
   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/test-call-ordering.png" alt-text="테스트 개체 실행 순서를 보여 주는 스크린샷":::

1. 이전 단계에서 제공된 정보를 검토하여 테스트 사례를 완료합니다. 테스트 시나리오를 기준으로 테스트 실행 옵션을 구성합니다.

   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/tester-finalize-case.png" alt-text="개체를 완성하는 단계를 보여 주는 스크린샷":::
...
1. 마침을 클릭하여 테스트 사례를 만듭니다.
   ...
   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/tester-test-repo.png" alt-text="리포지토리 테스트 단계를 보여 주는 스크린샷":::
...
1. 테스트 리포지토리에서 테스트 사례를 선택하고 실행을 클릭합니다.
   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/tester-repo-run.png" alt-text="테스트 리포지토리를 검토하기 위해 표시되는 스크린샷":::

1. 시작 테스트 사례를 검토하고 실행을 클릭합니다.

   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/tester-run-test-case.png" alt-text="테스트 사례를 시작하는 단계를 보여 주는 스크린샷":::

1. 다음으로 Oracle 원본 자격 증명을 제공합니다. 자격 증명을 입력한 후 연결을 클릭합니다.

   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/tester-oracle-connect.png" alt-text="Oracle 원본에 연결하는 단계를 보여 주는 스크린샷":::

1. 대상 SQL Server 자격 증명을 제공하고 연결을 클릭합니다.

   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/tester-sqlservervm-connect.png" alt-text="SQL 대상에 연결하는 단계를 보여 주는 스크린샷":::

   성공하면 테스트 사례가 초기화 단계로 이동합니다.

1. 실시간 진행률 표시줄은 테스트 실행의 실행 상태를 보여 줍니다.

   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/tester-run-status.png" alt-text="테스터 테스트 진행 상황을 보여 주는 스크린샷":::

1. 테스트가 완료된 후 보고서를 검토합니다. 보고서는 통계, 테스트 실행 중 오류 및 세부 보고서를 제공합니다.

   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/tester-test-result.png" alt-text="샘플 테스터 테스트 보고서를 보여 주는 스크린샷":::

7. 자세한 내용을 보려면 세부 정보를 클릭합니다.

   긍정적인 데이터 유효성 검사의 예
   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/tester-test-success.png" alt-text="샘플 테스터 성공 보고서를 보여 주는 스크린샷":::

   실패한 데이터 유효성 검사의 예
   :::image type="content" source="./media/oracle-to-sql-on-azure-vm-guide/tester-test-failed.png" alt-text="테스터 실패 보고서를 보여 주는 스크린샷":::
...
| **제목/링크**                                                                                                                                          | **설명**                                                                                                                                                                                                                                                                                                   |
| ------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Data Workload Assessment Model and Tool](https://www.microsoft.com/download/details.aspx?id=103130)(데이터 워크로드 평가 모델 및 도구) | 이 도구는 지정된 워크로드에 권장되는 최적의 대상 플랫폼, 클라우드 준비 상태 및 애플리케이션/데이터베이스 수정 수준을 제공합니다. 또한 자동화되고 균일한 대상 플랫폼 결정 프로세스를 제공하여 대규모 자산 평가를 가속화하는 데 도움이 되는 간단한 원클릭 계산 및 보고서 생성 기능을 제공합니다.                                                          |
| [Oracle Inventory Script Artifacts](https://www.microsoft.com/download/details.aspx?id=103121)(Oracle 인벤토리 스크립트 아티팩트)                 | 이 자산에는 Oracle 시스템 테이블을 대상으로 하고 스키마 형식, 개체 형식 및 상태별로 개체 수를 제공하는 PL/SQL 쿼리가 포함됩니다. 또한 각 스키마의 대략적인 ‘원시 데이터’ 예측 및 각 스키마의 테이블 크기 조정을 CSV 형식으로 저장된 결과와 함께 제공합니다.                                                                                                               |
