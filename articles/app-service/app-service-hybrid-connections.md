...
- Windows 앱 및 Linux 앱에 대한 GA에서 지원합니다. Windows 컨테이너 앱에 대해 지원되지 않습니다.
...
1. Azure 계정으로 로그인하여 구독에 제공되는 하이브리드 연결을 가져옵니다. HCM은 그 이상 Azure 계정을 계속 사용하지 않습니다. 
1. 구독을 선택합니다.
...
기본 Azure Service Bus Relay에 대한 충분한 권한이 있는 사용자가 기존 하이브리드 연결을 다른 App Service Web Apps에 추가할 수 있습니다. 즉, 다른 사용자가 동일한 하이브리드 연결을 다시 사용하지 못하도록 해야 하는 경우(예: 대상 리소스가 무단 액세스를 방지하기 위한 추가 보안 조치가 없는 서비스인 경우) Azure Service Bus Relay에 대한 액세스를 잠가야 합니다.

릴레이에 액세스할 수 있는 사용자는 `Reader` Azure Portal 웹앱에 추가하려고 할 때 하이브리드 연결을 _볼_ 수 있지만 릴레이 연결을 설정하는 데 사용되는 연결 문자열을 검색할 수 있는 권한이 없기 때문에 하이브리드 연결을 _추가할_ 수 없습니다. 하이브리드 연결을 추가하려면 `listKeys` 권한(`Microsoft.Relay/namespaces/hybridConnections/authorizationRules/listKeys/action`)이 있어야 합니다. `Contributor`역할 또는 릴레이에 대한 이 권한을 포함하는 다른 역할을 통해 사용자는 하이브리드 연결을 사용하여 자신의 Web Apps 추가할 수 있습니다.

## <a name="manage-your-hybrid-connections"></a>하이브리드 연결 관리 ##

하이브리드 연결에 대한 엔드포인트 호스트 또는 포트를 변경해야 하는 경우 다음 단계를 수행합니다.

1. 연결을 선택하고 하이브리드 연결 세부 정보 창의 왼쪽 위에서 **제거를** 선택하여 로컬 컴퓨터의 하이브리드 연결 관리자 하이브리드 연결을 제거합니다.
1. App Service **네트워킹** 페이지에서 하이브리드 연결로 이동하여 App Service **하이브리드 연결을** 끊습니다.
1. 업데이트해야 하는 엔드포인트에 대한 릴레이로 이동하고 왼쪽 탐색 **메뉴의** **엔터티** 아래에서 하이브리드 연결을 선택합니다.
1. 업데이트하려는 하이브리드 연결을 선택하고 왼쪽 탐색 메뉴의 **설정** 아래에서 **속성을** 선택합니다.
1. 변경하고 맨 위에 **있는 변경 내용 저장을** 누릅니다.
1. App Service **대한 하이브리드 연결** 설정으로 돌아가서 하이브리드 연결을 다시 추가합니다. 엔드포인트가 의도한 대로 업데이트되었는지 확인합니다. 목록에 하이브리드 연결이 표시되지 않으면 5-10분 안에 새로 고칩니다.
1. 로컬 컴퓨터의 하이브리드 연결 관리자 돌아가서 연결을 다시 추가합니다.

## <a name="troubleshooting"></a>문제 해결 ##

"연결됨" 상태는 하나 이상의 HCM이 해당 하이브리드 연결로 구성되어 Azure에 연결할 수 있음을 의미합니다. 하이브리드 연결 상태가 **연결됨으로** 표시되어 있지 않으면 Azure에 액세스할 수 있는 HCM에서 하이브리드 연결이 구성되지 않습니다. HCM이 **연결되지 않음** 으로 표시될 때 몇 가지 확인할 사항이 있습니다.
...
* App Service on Linux 사용하는 경우 엔드포인트 호스트로 "localhost"를 사용하지 않는지 확인합니다. 대신 로컬 컴퓨터에서 리소스와의 연결을 만들려는 경우 컴퓨터 이름을 사용합니다.

App Service에서 **tcpping** 명령줄 도구는 고급 도구(Kudu) 콘솔에서 호출할 수 있습니다. 이 도구는 TCP 엔드포인트에 대한 액세스 권한이 있는지 알 수 있지만 하이브리드 연결 엔드포인트에 대한 액세스 권한이 있는지는 알려주지 않습니다. 하이브리드 연결 엔드포인트에 대해 콘솔에서 도구를 사용하는 경우 호스트:포트 조합을 사용하는지 확인합니다.  
