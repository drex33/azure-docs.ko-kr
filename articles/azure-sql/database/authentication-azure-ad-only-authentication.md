...
- [CDC(변경 데이터 캡처)](/sql/relational-databases/track-changes/about-change-data-capture-sql-server) - Azure SQL Database Azure AD 사용자로 데이터베이스를 만들고 변경 데이터 캡처를 사용하도록 설정하는 경우 SQL 사용자는 CDC 아티팩트에서 사용하지 않도록 설정하거나 변경할 수 없습니다. 그러나 다른 Azure AD 사용자는 동일한 데이터베이스에서 CDC를 사용하거나 사용하지 않도록 설정할 수 있습니다. 마찬가지로 SQL 사용자로 Azure SQL Database 만드는 경우 Azure AD 사용자로 CDC를 사용하거나 사용하지 않도록 설정하면 작동하지 않습니다.
- [트랜잭션 복제](../managed-instance/replication-transactional-overview.md) - 복제 참가자 간의 연결에 SQL 인증이 필요하므로 Azure AD 전용 인증을 사용하는 경우 트랜잭션 복제를 사용하여 Azure SQL Managed Instance 온-프레미스 SQL Server 변경 내용을 푸시하는 시나리오에서는 트랜잭션 복제가 SQL Database 지원되지 않습니다. 또는 Azure VM이 Azure SQL Database 데이터베이스에 인스턴스를 SQL Server.
- [SQL 인사이트](../../azure-monitor/insights/sql-insights-overview.md)
...
자세한 제한은 [SQL Server & Azure SQL Managed Instance 간의 T SQL 차이점을](../managed-instance/transact-sql-tsql-differences-sql-server.md#logins-and-users)참조하세요.
