...
이러한 오류에 대 한 자세한 내용을 보려면 다음을 실행 합니다.

```azurecli
az ml online-deployment get-logs -n <endpoint-name> --deployment <deployment-name> --l 100
``` 

#### <a name="unable-to-download-user-container-image"></a>사용자 컨테이너 이미지를 다운로드할 수 없습니다.

사용자 컨테이너를 찾지 못할 수 있습니다.
...
#### <a name="unable-to-download-user-model-or-code-artifacts"></a>사용자 모델 또는 코드 아티팩트를 다운로드할 수 없음

사용자 모델 또는 코드 아티팩트를 찾을 수 없는 경우가 있습니다.
...
- 예를 들어 blob이 인 경우 `https://foobar.blob.core.windows.net/210212154504-1517266419/WebUpload/210212154504-1517266419/GaussianNB.pkl` 이 명령을 사용 하 여 존재 하는지 확인할 수 있습니다.

  `az storage blob exists --account-name foobar --container-name 210212154504-1517266419 --name WebUpload/210212154504-1517266419/GaussianNB.pkl --subscription <sub-name>`

### <a name="error-resourcenotready"></a>오류: ResourceNotReady

배포의 일부로 제공된 `score.py`를 실행하기 위해 Azure는 `score.py`에 필요한 모든 리소스를 포함하는 컨테이너를 만들고 해당 컨테이너에서 채점 스크립트를 실행합니다. 이 시나리오의 오류는 실행 시이 컨테이너가 충돌 하는 것을 의미 합니다. 즉, 점수 매기기가 발생 하지 않습니다. 이 오류는 다음과 같은 경우에 발생합니다.
...
이 오류는 Azure Resource Manager 필요한 리소스를 찾을 수 없을 때 발생 합니다. 예를 들어 저장소 계정을 참조 했지만 지정 된 경로에서 찾을 수 없는 경우이 오류가 표시 됩니다. 정확한 경로 또는 이름 철자에서 제공 했을 수 있는 리소스를 두 번 확인 해야 합니다.

이 오류 코드에 대한 자세한 내용은 [리소스를 찾을 수 없음 오류 해결](../azure-resource-manager/troubleshooting/error-not-found.md)을 참조하세요. 

### <a name="error-operationcancelled"></a>오류: OperationCancelled 되었습니다.

Azure 작업은 우선 순위 수준이 높으며 최고에서 낮은 수준으로 실행 됩니다. 이 오류는 우선 순위가 높은 다른 작업에 의해 작업이 재정의 된 경우에 발생 합니다. 작업을 다시 시도 하면 취소 하지 않고 작업을 수행할 수 있습니다.

### <a name="error-internalservererror"></a>오류: InternalServerError

안정적이 고 신뢰할 수 있는 서비스를 제공 하는 것이 가장 좋지만 계획에 따라 진행 되지 않는 경우가 있습니다. 이 오류가 발생 하는 경우에는 어느 쪽에도 문제가 없다는 것을 의미 하 고 해결 해야 합니다. 모든 관련 정보와 함께 [고객 지원 티켓](https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest)을 제출하면 문제를 해결하겠습니다. 
