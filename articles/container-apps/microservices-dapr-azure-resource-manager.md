...
### <a name="create-azure-resource-manager-arm-templates"></a>ARM (Azure Resource Manager) 템플릿 만들기

두 개의 Azure Resource Manager (ARM) 템플릿을 만듭니다.

ARM 템플릿에는 컨테이너 앱 정의와 4 개의 구성 요소 정의가 있습니다.

다음 예제에서는 Azure Blob Storage 계정에 대해 구성 된 경우 ARM 템플릿이 어떻게 표시 되는지 보여 줍니다.

다음 파일을 *serviceapp. json* 으로 저장 합니다.
...
ARM 템플릿 파일을 저장 한 디렉터리로 이동 하 고 아래 명령을 실행 하 여 서비스 컨테이너 앱을 배포 합니다.
...
이 명령은 `targetPort: 3000` `"appId": "nodeapp",` `"appPort": 3000,` 서비스 검색 및 호출에 대해 및 d 4를 사용 하 여 구성 된 함께 제공 되는 사이드카와 함께 (앱의 포트)에 서비스 (노드) 앱 서버를 배포 합니다. 상태 저장소는 `components` `"type": "state.azure.blobstorage"` 사이드카가 상태를 유지할 수 있도록 하는의 개체를 사용 하 여 구성 됩니다.
...
이 명령은 `pythonapp` 에 대 한 사이드카를 조회 하 고 안전 하 게 호출 하는 데 사용 되는 사이드카와 함께 실행 되는 배포 됩니다 `nodeapp` . 이 응용 프로그램은 헤드리스 응용 프로그램 이므로 `targetPort` 서버를 시작 하지 않으며 수신을 사용 하도록 설정 하지 않아도 됩니다.
