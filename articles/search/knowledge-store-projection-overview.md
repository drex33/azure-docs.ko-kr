...
:::image type="content" source="media/knowledge-store-concept-intro/kstore-in-storage-explorer.png" alt-text="Azure Storage 표현된 프로젝션" border="true":::
...
+ [Storage Browser를](knowledge-store-view-storage-explorer.md) 사용하여 개체 만들기 및 콘텐츠를 확인합니다.
...
1. Cognitive Search 인덱서에서 [보강 캐싱(미리 보기)을 사용하도록 설정한](search-howto-incremental-index.md) 다음 [인덱서](search-howto-run-reset-indexers.md) 를 실행하여 기술 도구를 실행하고 캐시를 채웁다. 미리 보기 기능이므로 인덱서 요청에 미리 보기 REST API(api-version=2020-06-30-preview 이상)을 사용해야 합니다. 캐시가 채워지면 기술 자체가 수정되지 않는 한 지식 저장소의 프로젝션 정의를 무료로 수정할 수 있습니다.
