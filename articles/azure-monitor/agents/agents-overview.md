...
- 프로덕션의 모든 기능에서 아직 지원되지 않습니다. [지원되는 서비스 및 기능](./azure-monitor-agent-overview.md#supported-services-and-features)을 참조하세요.
...
- Dependency Agent Windows 및 Linux 버전 모두에서 사용자 공간 서비스와 커널 드라이버를 사용하여 데이터 수집이 수행됩니다. 
...
하이브리드 머신에서 [Azure Arc 지원 서버를](../../azure-arc/servers/manage-vm-extensions.md) 사용하여 Azure Monitor 에이전트, Log Analytics 및 Azure Monitor 종속성 VM 확장을 배포합니다.
...
종속성 에이전트는 커널 수준에서 작동하므로 지원은 커널 버전에도 좌우됩니다. 종속성 에이전트 버전 9.10. * 에이전트는 * 커널을 지원 합니다.  다음 표에는 종속성 에이전트에 대한 주 및 부 Linux OS 릴리스와 지원되는 커널 버전이 나와 있습니다.
