...
description: Easv5 및 Eadsv5 시리즈 VM에 대한 사양입니다.
author: brbell
ms.author: brbell
...
Easv5 시리즈 및 Eadsv5 시리즈는 최대 256MB L3 캐시가 있는 다중 스레드 구성에서 AMD의 3세대 EPYC<sup>TM</sup> 7763v 프로세서를 활용하여 대부분의 메모리 최적화 워크로드를 실행하기 위한 고객 옵션을 증가합니다. 이러한 가상 머신은 관계형 데이터베이스 서버 및 메모리 내 분석 워크로드와 같은 대부분의 메모리 집약적 엔터프라이즈 애플리케이션과 관련된 요구 사항을 충족하기 위해 vC CPU와 메모리의 조합을 제공합니다.

## <a name="easv5-series"></a>Easv5 시리즈

Easv5 시리즈는 최대 3.5GHz의 상승된 빈도를 달성할 수 있는 AMD의 3세대 EPYC<sup>TM</sup> 7763v 프로세서를 활용합니다. Easv5 시리즈 크기는 메모리 집약적 엔터프라이즈 애플리케이션에 이상적인 vCPU와 메모리의 조합을 제공합니다. 로컬 디스크가 없는 새 VM은 로컬 임시 디스크가 필요하지 않은 워크로드에 더 나은 가치 제안을 제공합니다.

> [!NOTE]
> 질문과 대답은 [로컬 임시 디스크가 없는 Azure VM 크기](azure-vms-no-temp-disk.yml)를 참조하세요.

Easv5 시리즈 가상 머신은 표준 SSD, 표준 HDD 및 Premium SSD 디스크 유형을 지원합니다. 지역별 가용성에 따라 Ultra Disk 스토리지를 연결할 수도 있습니다. 디스크 스토리지는 가상 머신과 별도로 청구됩니다. [디스크에 대한 가격 책정을 참조하세요.](https://azure.microsoft.com/pricing/details/managed-disks/)
...
<sup>1</sup> Easv5 시리즈 VM은 디스크 성능을 [버스트하고](disk-bursting.md) 한 번에 최대 30분 동안 최대 버스트 최대값을 얻을 수 있습니다.<br>
<sup>2</sup> [사용 가능한 제한된 코어 크기](constrained-vcpu.md)



## <a name="eadsv5-series"></a>Eadsv5 시리즈

Eadsv5 시리즈는 최대 3.5GHz의 상승된 빈도를 달성할 수 있는 AMD의 3세대 EPYC<sup>TM</sup> 7763v 프로세서를 활용합니다. Eadsv5 시리즈 크기는 메모리 집약적 엔터프라이즈 애플리케이션에 이상적인 vCPU, 메모리 및 임시 스토리지의 조합을 제공합니다. 새 VM에는 50% 더 큰 로컬 스토리지가 있으며, [Gen2](generation-2.md) VM이 있는 [Eav4/Easv4](eav4-easv4-series.md) 크기에 비해 읽기 및 쓰기 모두에 더 나은 로컬 디스크 IOPS가 있습니다.

Eadsv5 시리즈 가상 머신은 표준 SSD, 표준 HDD 및 Premium SSD 디스크 유형을 지원합니다. 지역별 가용성에 따라 Ultra Disk 스토리지를 연결할 수도 있습니다. 디스크 스토리지는 가상 머신과 별도로 청구됩니다. [디스크에 대한 가격 책정을 참조하세요.](https://azure.microsoft.com/pricing/details/managed-disks/)
...
| Standard_E64ads_v5<sup>2</sup> | 64 | 512 | 2400 | 32 | 30만/4000 | 80000/1200   | 80000/2000 | 8 | 32000 |
| Standard_E96ads_v5<sup>2</sup> | 96 | 672 | 3600 | 32 | 45만/4000 | 80000/1600   | 80000/2000 | 8 | 40,000 |

* 이러한 IOPs 값은 Gen2 Vm을 사용 하 여 달성할 수 있습니다.<br>
<sup>1</sup> Eadsv5 시리즈 vm은 디스크 성능을 [버스트](disk-bursting.md) 하 고 한 번에 최대 30 분 동안 최대 버스트를 얻을 수 있습니다.<br>
