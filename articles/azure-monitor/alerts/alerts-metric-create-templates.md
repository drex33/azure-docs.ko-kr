---
title: Azure Resource Manager 템플릿을 사용하여 새로운 메트릭 경고 만들기
...
ms.locfileid: "122536745"
---
# <a name="create-a-metric-alert-with-a-resource-manager-template"></a>Resource Manager 템플릿을 사용하여 메트릭 경고 만들기

[!INCLUDE [updated-for-az](../../../includes/updated-for-az.md)]

이 문서에서는 [Azure Resource Manager 템플릿](../../azure-resource-manager/templates/syntax.md)을 사용하여 Azure Monitor에서 [최신 메트릭 경고](./alerts-metric-near-real-time.md)를 구성하는 방법을 설명합니다. Resource Manager 템플릿을 사용하면 환경 전체에서 일관되고 재현 가능한 방법으로 경보를 프로그래밍 방식으로 설정할 수 있습니다. 최신 메트릭 경고는 현재 [이 리소스 유형 집합](./alerts-metric-near-real-time.md#metrics-and-dimensions-supported)에 대해 사용할 수 있습니다.
...
3. `metricName` 매개 변수에 대해서는 [Azure Monitor에서 지원되는 메트릭](../essentials/metrics-supported.md)에서 사용 가능한 메트릭을 참조하세요.
...
이 연습의 목적을 위해 아래의 json을 simplestaticmetricalert.json으로 저장합니다.
...
아래의 json을 simplestaticmetricalert.parameters.json으로 저장하고 필요에 따라 수정합니다.
...
## <a name="template-for-a-simple-dynamic-thresholds-metric-alert"></a>간단한 동적 임계값 메트릭 경고에 대한 템플릿

Resource Manager 템플릿을 사용하여 경고를 만들려면 `Microsoft.Insights/metricAlerts` 형식의 리소스를 만들어 관련 속성을 기입합니다. 다음은 메트릭 경고 규칙을 만드는 샘플 템플릿입니다.

이 연습의 목적을 위해 아래의 json을 simpledynamicmetricalert.json으로 저장합니다.
...
아래의 json을 simpledynamicmetricalert.parameters.json으로 저장하고 필요에 따라 수정합니다.
...
## <a name="template-for-a-static-threshold-metric-alert-that-monitors-multiple-criteria"></a>여러 기준을 모니터링하는 정적 임계값 메트릭 경고에 대한 템플릿

최신 메트릭 경고는 다차원 메트릭에 대한 경고 및 여러 조건의 정의를 지원합니다(경고 규칙당 최대 5개 조건까지 정의할 수 있음). 다음 템플릿을 사용하여 차원 메트릭에 대한 추가 고급 메트릭 경고 규칙을 작성하고 여러 조건을 지정할 수 있습니다.

여러 조건이 포함된 경고 규칙에서 차원을 사용하는 경우 다음 제약 조건을 참조하세요.
- 각 조건 내에서 차원당 하나의 값만 선택할 수 있습니다.
- "\*"는 차원 값으로 사용할 수 없습니다.
- 다른 조건으로 구성된 메트릭에서 동일한 차원을 지원하는 경우 구성된 차원 값은 모든 메트릭(관련 조건의)에 대해 동일한 방식으로 명시적으로 설정해야 합니다.
...
이 연습의 목적을 위해 아래의 json을 advancedstaticmetricalert.parameters.json으로 저장하고 수정합니다.
...
## <a name="template-for-a-static-metric-alert-that-monitors-multiple-dimensions"></a>여러 차원을 모니터링하는 정적 메트릭 경고에 대한 템플릿

다음 템플릿을 사용하여 차원 메트릭에 대한 정적 메트릭 경고 규칙을 만들 수 있습니다.

단일 경고 규칙은 여러 메트릭 시계열을 한 번에 모니터링할 수 있으므로 관리할 경고 규칙이 줄어듭니다.

아래 예제에서 경고 규칙은 **Transactions** 메트릭에 대한 **ResponseType** 및 **ApiName** 차원의 차원 값 조합을 모니터링합니다.
...
이 연습의 목적을 위해 아래 json을 multidimensionalstaticmetricalert.json으로 저장합니다.
...
이 연습의 목적을 위해 아래 json을 multidimensionalstaticmetricalert.parameters.json으로 저장하고 수정합니다.
...
> [!NOTE]
>
> "모두"를 차원 값으로 사용하는 것은 "\*"(모든 현재 및 미래 값)를 선택하는 것과 같습니다.


## <a name="template-for-a-dynamic-thresholds-metric-alert-that-monitors-multiple-dimensions"></a>여러 차원을 모니터링하는 동적 임계값 메트릭 경고에 대한 템플릿

다음 템플릿을 사용하여 차원 메트릭에 대한 추가 고급 동적 임계값 메트릭 경고 규칙을 만들 수 있습니다.

단일 동적 임계값 경고 규칙은 수백 가지 메트릭 시계열(다양한 형식의 경우에도)에 대한 맞춤형 임계값을 한 번에 만들 수 있으므로 관리할 경고 규칙이 줄어듭니다.
...
이 연습의 목적을 위해 아래의 json을 advanceddynamicmetricalert.parameters.json으로 저장하고 수정합니다.
...
## <a name="template-for-a-static-threshold-metric-alert-that-monitors-a-custom-metric"></a>사용자 지정 메트릭을 모니터링하는 정적 임계값 메트릭 경고에 대한 템플릿
...
이 연습의 목적을 위해 아래 json을 customstaticmetricalert.json으로 저장합니다.
...
이 연습의 목적을 위해 아래의 json을 customstaticmetricalert.parameters.json으로 저장하고 수정합니다.
...
## <a name="template-for-a-metric-alert-that-monitors-multiple-resources"></a>여러 리소스를 모니터링하는 메트릭 경고에 대한 템플릿

이전 섹션에서는 단일 리소스를 모니터링하는 메트릭 경고를 만드는 Azure Resource Manager 템플릿 샘플에 대해 설명했습니다. 이제 Azure Monitor는 동일한 Azure 지역에 있는 리소스에 대해 단일 메트릭 경고 규칙을 사용하여 여러 리소스(동일한 종류)를 모니터링할 수 있도록 지원합니다. 이 기능은 현재 Azure 퍼블릭 클라우드에서만 지원되고 가상 머신, SQL Server 데이터베이스, SQL Server 탄력적 풀, Data Box Edge 디바이스에 대해서만 지원됩니다. 또한 이 기능은 플랫폼 메트릭에만 사용할 수 있으며, 사용자 지정 메트릭에는 지원되지 않습니다.
...
> [!NOTE]
>
> 여러 리소스를 모니터링하는 메트릭 경고 규칙에서는 다음과 같은 제한 사항이 적용됩니다.
> - 경고 규칙 범위에는 선택한 리소스 종류에 해당하는 리소스가 하나 이상 포함되어야 합니다.
...
위의 템플릿을 아래에 제공된 매개 변수 파일과 함께 사용할 수 있습니다.
이 연습의 목적을 위해 아래의 json을 all-vms-in-resource-group-static.json으로 저장하고 수정합니다.
...
현재 작업 디렉터리의 PowerShell 또는 Azure CLI를 사용한 템플릿 및 매개 변수 파일을 사용하여 정적 메트릭 경고를 만들 수 있습니다.
...
### <a name="dynamic-thresholds-alert-on-all-virtual-machines-in-one-or-more-resource-groups"></a>하나 이상의 리소스 그룹에 있는 모든 가상 머신에 대한 동적 임계값 경고

이 템플릿은 하나 이상의 리소스 그룹에 있는 모든 가상 머신(한 Azure 지역에 있는)의 CPU 백분율을 모니터링하는 동적 임계값 메트릭 경고 규칙을 만듭니다.

이 연습의 목적을 위해 아래의 json을 all-vms-in-resource-group-dynamic.json으로 저장합니다.
...
위의 템플릿을 아래에 제공된 매개 변수 파일과 함께 사용할 수 있습니다.
이 연습의 목적을 위해 아래의 json을 all-vms-in-resource-group-dynamic.parameters.json으로 저장하고 수정합니다.
...
### <a name="static-threshold-alert-on-all-virtual-machines-in-a-subscription"></a>구독에 있는 모든 가상 머신에 대한 정적 임계값 경고

이 템플릿은 구독에 있는 모든 가상 머신(한 Azure 지역에 있는)의 CPU 백분율을 모니터링하는 정적 임계값 메트릭 경고 규칙을 만듭니다.

이 연습의 목적 위해 아래의 json을 all-vms-in-subscription-static.json으로 저장합니다.
...
위의 템플릿을 아래에 제공된 매개 변수 파일과 함께 사용할 수 있습니다.
이 연습의 목적을 위해 아래의 json을 all-vms-in-subscription-static.parameters.json으로 저장하고 수정합니다.
...
### <a name="dynamic-thresholds-alert-on-all-virtual-machines-in-a-subscription"></a>구독에 있는 모든 가상 머신에 대한 동적 임계값 경고

이 템플릿은 구독에 있는 모든 가상 머신(한 Azure 지역에 있는)의 CPU 백분율을 모니터링하는 동적 임계값 메트릭 경고 규칙을 만듭니다.

이 연습의 목적을 위해 아래의 json을 all-vms-in-subscription-dynamic.json으로 저장합니다.
...
위의 템플릿을 아래에 제공된 매개 변수 파일과 함께 사용할 수 있습니다.
이 연습의 목적을 위해 아래의 json을 all-vms-in-subscription-dynamic.parameters.json으로 저장하고 수정합니다.
...
### <a name="static-threshold-alert-on-a-list-of-virtual-machines"></a>가상 머신의 목록에서 정적 임계값 경고

이 템플릿은 구독에 있는 가상 머신 목록(한 Azure 지역에 있는)의 CPU 백분율을 모니터링하는 정적 임계값 메트릭 경고 규칙을 만듭니다.

이 연습의 목적을 위해 아래의 json을 list-of-vms-static.json으로 저장합니다.
...
위의 템플릿을 아래에 제공된 매개 변수 파일과 함께 사용할 수 있습니다.
이 연습의 목적을 위해 아래의 json을 list-of-vms-static.parameters.json으로 저장하고 수정합니다.
...
### <a name="dynamic-thresholds-alert-on-a-list-of-virtual-machines"></a>가상 머신의 목록에 대한 동적 임계값 경고

이 템플릿은 구독에 있는 가상 머신 목록(한 Azure 지역에 있는)의 CPU 백분율을 모니터링하는 동적 임계값 메트릭 경고 규칙을 만듭니다.

이 연습의 목적을 위해 아래의 json을 list-of-vms-dynamic.json으로 저장합니다.
...
위의 템플릿을 아래에 제공된 매개 변수 파일과 함께 사용할 수 있습니다.
이 연습의 목적을 위해 아래의 json을 list-of-vms-dynamic.parameters.json으로 저장하고 수정합니다.
...
## <a name="template-for-an-availability-test-along-with-a-metric-alert"></a>메트릭 경고와 함께 가용성 테스트에 대한 템플릿
...
메트릭 경고(Microsoft.Insights/metricAlerts)와 동일한 리소스 종류의 가용성 테스트 경고입니다. 다음 샘플 Azure Resource Manager 템플릿을 사용하여 간단한 가용성 테스트 및 관련 경고를 설정할 수 있습니다.

이 연습의 목적을 위해 아래 json을 availabilityalert.json으로 저장합니다.
...
아래의 json을 availabilityalert.parameters.json으로 저장하고 필요에 따라 수정합니다.
...
PowerShell 또는 Azure CLI를 사용한 템플릿 및 매개 변수 파일을 사용하여 가용성 테스트 및 관련 경고를 만들 수 있습니다.
