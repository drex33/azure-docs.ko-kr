...
SaaS는 중앙에서 호스트되고 관리되는 소프트웨어입니다. 일반적으로 다중 테넌트 아키텍처를 기반으로 하며, 단일 버전의 애플리케이션이 모든 고객에게 사용됩니다. 모든 위치에서 최상의 성능을 보장하기 위해 여러 인스턴스로 확장할 수 있습니다. SaaS 소프트웨어는 일반적으로 월간 또는 연간 구독을 통해 사용이 허가됩니다. SaaS 소프트웨어 공급업체가 소프트웨어 스택의 모든 구성 요소를 담당하므로 사용자는 제공되는 서비스만 관리하면 됩니다.
...
Azure를 사용할 때 얻을 수 있는 이점 중 하나는 전 세계의 다양한 데이터 센터에 애플리케이션을 배포할 수 있다는 것입니다. 선택한 지역은 애플리케이션의 성능에 영향을 줄 수 있습니다. 네트워크 요청 대기 시간을 줄이기 위해 고객에게 가장 가까운 지역을 선택하는 것이 좋습니다. 특정 국가/지역에서 앱을 배포하기 위한 법적 요건을 충족하는 지역을 선택할 수도 있습니다.
...
- [Azure 소비 API 개요](../../cost-management-billing/manage/consumption-api-overview.md)를 사용하여 메타데이터와 함께 사용 가능한 모든 리소스를 나열할 수 있습니다. 가격에 대한 자세한 내용은 [Azure 소매 가격 개요](/rest/api/cost-management/retail-prices/azure-retail-prices)를 참조하세요.
...
[Azure의 기본 제공 역할](../../role-based-access-control/built-in-roles.md)에 대한 몇 가지 예는 다음과 같습니다.

- **소유자**: 이 역할의 사용자는 액세스를 비롯한 모든 사항을 관리할 수 있습니다.

- **읽기 권한자**: 이 역할을 가진 사용자는 모든 유형의 리소스(비밀 제외)를 읽을 수는 있지만 변경할 수 없습니다.
