...
Microsoft 센티널 노트북의 커널은 Azure VM (가상 머신)에서 실행 됩니다. Notebook에 복잡한 기계 학습 모델이 포함되어 있는 경우 더 강력한 가상 머신을 사용할 수 있는 몇 가지 라이선스 옵션이 있습니다.
...
1. **네트워킹** 탭에서 작업 영역을 연결하는 데 퍼블릭 엔드포인트를 사용할지, 아니면 프라이빗 엔드포인트를 사용할지 선택합니다. Microsoft Sentinel 작업 영역에 공용 엔드포인트가 있는 경우 네트워크 통신의 잠재적인 문제를 방지하려면 Azure ML 작업 영역에 공용 엔드포인트를 사용하는 것이 좋습니다. 완료되면 **검토 + 만들기** 를 선택합니다.
...
1. 배포가 완료되면 Microsoft Sentinel **Notebooks로** 돌아가서 새 Azure ML 작업 영역에서 Notebook을 시작할 수 있습니다.
...
AML 작업 영역을 만든 후 Microsoft Sentinel에서 Azure ML 작업 영역에서 Notebook을 시작합니다.
