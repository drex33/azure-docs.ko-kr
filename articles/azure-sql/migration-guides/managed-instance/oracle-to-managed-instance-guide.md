...
ms.locfileid: "122536627"
---
# <a name="migration-guide-oracle-to-azure-sql-managed-instance"></a>마이그레이션 가이드: Oracle에서 Azure SQL Managed Instance로

[!INCLUDE[appliesto-sqldb-sqlmi](../../includes/appliesto-sqlmi.md)]

 이 가이드에서는 SQL Server Migration Assistant for Oracle을 사용하여 Oracle 스키마를 Azure SQL Managed Instance로 마이그레이션하는 방법을 설명합니다.
...
1. **Oracle 메타데이터 탐색기** 에서 마이그레이션할 스키마 또는 개체를 마우스 오른쪽 단추로 클릭하고 **데이터 마이그레이션** 을 선택하여 데이터를 마이그레이션합니다. 또는 **데이터 마이그레이션** 탭을 선택할 수 있습니다. 전체 데이터베이스의 데이터를 마이그레이션하려면 데이터베이스 이름 옆의 확인란을 선택합니다. 개별 테이블의 데이터를 마이그레이션하려면 데이터베이스를 펼치고 **테이블** 을 펼친 다음, 테이블 옆의 확인란을 선택합니다. 개별 테이블의 데이터를 생략하려면 확인란의 선택을 취소합니다.
...
1. SSMA for Oracle을 열고 **Tester** 를 선택한 다음, **새 테스트 사례** 를 선택합니다.
   ...
   :::image type="content" source="./media/oracle-to-managed-instance-guide/ssma-tester-new.png" alt-text="새 테스트 사례 스크린샷":::

1. 테스트 사례 마법사에서 다음 정보를 제공합니다.

   **이름:** 테스트 사례를 식별하는 이름을 입력합니다.

   **만든 날짜:** 자동으로 정의되는 오늘 현재 날짜입니다.

   **최종 수정 날짜:** 자동으로 입력되며 변경하면 안 됩니다.

   **설명:** 테스트 사례의 목적을 식별하기 위한 추가 정보를 입력합니다.

   :::image type="content" source="./media/oracle-to-managed-instance-guide/tester-init-test-case.png" alt-text="테스트 사례를 초기화하는 단계 스크린샷":::

1. 왼쪽에 있는 Oracle 개체 트리에서 테스트 사례에 속하는 개체를 선택합니다.

   :::image type="content" source="./media/oracle-to-managed-instance-guide/tester-select-configure-objects.png" alt-text="개체를 선택하고 구성하는 단계 스크린샷":::

   이 예제에서는 `ADD_REGION` 저장 프로시저와 `REGION` 테이블이 선택되었습니다.

   자세한 내용은 [테스트할 개체 선택 및 구성](/sql/ssma/oracle/selecting-and-configuring-objects-to-test-oracletosql)을 참조하세요.

1. 그런 다음, 왼쪽 창의 Oracle 개체 트리에서 테이블, 외래 키, 기타 종속 개체를 선택합니다.

   :::image type="content" source="./media/oracle-to-managed-instance-guide/tester-select-configure-affected.png" alt-text="영향을 받는 개체를 선택하고 구성하는 단계 스크린샷":::
...
1. 개체의 평가 순서를 검토합니다. 그리드의 단추를 클릭하여 순서를 변경합니다.
   ...
   :::image type="content" source="./media/oracle-to-managed-instance-guide/test-call-ordering.png" alt-text="테스트 개체 실행 순서를 지정하는 단계 스크린샷":::

1. 이전 단계에서 제공한 정보를 검토하여 테스트 사례를 완료합니다. 테스트 시나리오에 따라 테스트 실행 옵션을 구성합니다.

   :::image type="content" source="./media//oracle-to-managed-instance-guide/tester-finalize-case.png" alt-text="개체를 완료하는 단계 스크린샷":::
...
1. 마침을 클릭하여 테스트 사례를 만듭니다.
   ...
   :::image type="content" source="./media/oracle-to-managed-instance-guide/tester-test-repo.png" alt-text="리포지토리를 테스트하는 단계 스크린샷":::
...
1. 테스트 리포지토리에서 테스트 사례를 선택하고 실행을 클릭합니다.
   ...
   :::image type="content" source="./media/oracle-to-managed-instance-guide/tester-repo-run.png" alt-text="테스트 리포지토리를 검토하는 단계 스크린샷":::

1. 테스트 사례 시작을 검토하고 실행을 클릭합니다.

   :::image type="content" source="./media/oracle-to-managed-instance-guide/tester-run-test-case.png" alt-text="테스트 사례를 시작하는 단계 스크린샷":::

1. 다음으로, Oracle 원본 자격 증명을 제공합니다. 자격 증명을 입력한 후 연결을 클릭합니다.

   :::image type="content" source="./media/oracle-to-managed-instance-guide/tester-oracle-connect.png" alt-text="Oracle 원본에 연결하는 단계 스크린샷":::

1. 대상 SQL Server 자격 증명을 제공하고 연결을 클릭합니다.

   :::image type="content" source="./media/oracle-to-managed-instance-guide/tester-sqlmi-connect.png" alt-text="SQL 대상에 연결하는 단계 스크린샷":::

   성공하면 테스트 사례가 초기화 스테이지로 이동합니다.

1. 실시간 진행률 표시줄은 테스트 실행의 실행 상태를 보여 줍니다.

   :::image type="content" source="./media/oracle-to-managed-instance-guide/tester-run-status.png" alt-text="Tester 테스트 진행 상황 스크린샷":::

1. 테스트가 완료된 후 보고서를 검토합니다. 보고서는 통계, 테스트 실행 중 발생한 오류, 세부 정보 보고서를 제공합니다.

   :::image type="content" source="./media/oracle-to-managed-instance-guide/tester-test-result.png" alt-text="샘플 Tester 테스트 보고서 스크린샷":::

1. 세부 정보를 클릭하여 자세한 정보를 얻습니다.

   성공적인 데이터 유효성 검사의 예

   :::image type="content" source="./media/oracle-to-managed-instance-guide/tester-test-success.png" alt-text="샘플 Tester 성공 보고서 스크린샷":::

   실패한 데이터 유효성 검사의 예

   :::image type="content" source="./media/oracle-to-managed-instance-guide/tester-test-failed.png" alt-text="Tester 실패 보고서 스크린샷":::
