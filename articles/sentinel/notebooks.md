...
ms.date: 11/14/2021
ms.openlocfilehash: bc122d5d0e1c0c83fb561a5d4e6f78ad6b4922d0
ms.sourcegitcommit: 845eb7b0ed05ef2f0dfa6f054eaf5f32c780567c
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 11/30/2021
ms.locfileid: "133302641"
---
...
1. **네트워킹** 탭에서 공용 **엔드포인트(모든 네트워크)** 를 선택합니다.

    **고급** 또는 **태그** 탭에서 관련 설정을 정의한 **다음, 검토 + 만들기를** 선택합니다.

1. **검토 + 만들기** 탭에서 정보를 검토하여 올바른지 확인한 다음, **만들기** 를 선택하여 작업 영역 배포를 시작합니다. 예를 들면 다음과 같습니다.

    :::image type="content" source="media/notebooks/machine-learning-create-last-step.png" alt-text="Microsoft Sentinel에서 Machine Learning 작업 영역을 검토 + 만듭니다.":::
...
이 절차의 단계에서는 관련된 경우 Azure Machine Learning 설명서의 특정 문서를 참조합니다. 자세한 내용은 [보안 Azure ML 작업 영역을 만드는 방법을 참조하세요.](/azure/machine-learning/tutorial-create-secure-workspace)

1.  VNet 내에서 VM 점프 상자를 만듭니다. VNet은 퍼블릭 인터넷의 액세스를 제한하므로 VNet 뒤에 있는 리소스에 연결하는 방법으로 점프 상자가 사용됩니다.

1.  점프 상자에 액세스한 다음, Microsoft Sentinel 작업 영역으로 이동합니다. [Azure Bastion](/azure/bastion/bastion-overview) 사용하여 VM에 액세스하는 것이 좋습니다.

1. Microsoft Sentinel에서 **위협 관리**  >  **Notebook을** 선택한 다음, **새 AML 작업 영역 만들기를** 선택합니다.
...
1. **네트워킹** 탭에서 프라이빗 **엔드포인트** 를 선택합니다. VM 점프 상자에서와 동일한 VNet을 사용해야 합니다. 예:

    :::image type="content" source="media/notebooks/create-private-endpoint.png" alt-text="Microsoft Sentinel에서 프라이빗 엔드포인트 만들기 페이지의 스크린샷." lightbox="media/notebooks/create-private-endpoint.png":::

1. **고급** 또는 **태그** 탭에서 관련 설정을 정의한 **다음, 검토 + 만들기를** 선택합니다.
...
1.  Azure Machine Learning Studio의 **컴퓨팅** 페이지에서 새 컴퓨팅을 만듭니다. 고급 **설정** 탭에서 VM 점프 상자에 사용한 것과 동일한 VNet을 선택해야 합니다. 자세한 내용은 [Azure Machine Learning 컴퓨팅 인스턴스 만들기 및 관리를 참조하세요.](/azure/machine-learning/how-to-create-manage-compute-instance?tabs=python)

1.  방화벽 뒤에서 Azure ML 액세스하도록 네트워크 트래픽을 구성합니다. 자세한 내용은 [인바운드 및 아웃바운드 네트워크 트래픽 구성을 참조하세요.](/azure/machine-learning/how-to-access-azureml-behind-firewall?tabs=ipaddress%2Cpublic)

다음 단계 집합 중 하나를 계속합니다.

- **프라이빗 링크가 하나만 있는 경우:** 이제 다음 방법 중 하나를 통해 Notebook에 액세스할 수 있습니다.

    - Microsoft Sentinel에서 notebook을 복제하고 시작하여 Azure Machine Learning
    - 수동으로 Azure Machine Learning Notebook 업로드
    - Azure Machine Learning 터미널에서 [Microsoft Sentinel Notebook GitHub 리포지토리](https://github.com/Azure/Azure-Sentinel-Notebooks) 복제

- **다른 VNET을 사용하는 다른 프라이빗 링크가 있는 경우** 다음을 수행합니다.

    1. Azure Portal Azure Machine Learning 작업 영역의 리소스 그룹으로 이동한 다음 privatelink.api.azureml.ms 라는 **프라이빗 DNS 영역** 리소스를 검색하고 **를** **privatelink.notebooks.azure.ms.**  예:

        :::image type="content" source="media/notebooks/select-private-dns-zone.png" alt-text="선택된 프라이빗 DNS 영역 리소스의 스크린샷." lightbox="media/notebooks/select-private-dns-zone.png":::

    1. privatelink.api.azureml.ms **및** **privatelink.notebooks.azure.ms** 모두 포함하여 각 리소스에 대해 가상 네트워크 링크를 추가합니다.

        **가상 네트워크 링크** 추가 > 리소스를  >  선택합니다. 자세한 내용은 [가상 네트워크 연결](/azure/dns/private-dns-getstarted-portal)을 참조하세요.

자세한 내용은 다음을 참조하세요.

- [보안 작업 영역을 사용하는 경우 네트워크 트래픽 흐름](/azure/machine-learning/concept-secure-network-traffic-flow)
- [VNet(가상 네트워크)을 사용하여 Azure Machine Learning 작업 영역 리소스 보호](/azure/machine-learning/how-to-network-security-overview)

---

배포가 완료되면 Microsoft Sentinel **Notebooks로** 돌아가서 새 Azure ML 작업 영역에서 Notebook을 시작할 수 있습니다.
...
AML 작업 영역을 만든 후 Microsoft Sentinel에서 Azure ML 작업 영역에서 Notebook을 시작하기 시작합니다.
...
**Microsoft Sentinel에서 Notebook을 시작하려면:**

1. Azure Portal **Microsoft Sentinel**  >  **위협 관리**  >  **Notebook으로** 이동합니다. 여기서 Microsoft Sentinel에서 제공하는 Notebook을 볼 수 있습니다.
...
1. 페이지 맨 위에서 Notebook 서버에 사용할 **컴퓨팅** 인스턴스를 선택합니다.
    ...
    >
    > 커널은 Azure ML 창의 오른쪽 위에도 표시 됩니다. 필요한 커널을 선택 하지 않은 경우 드롭다운 목록에서 다른 버전을 선택 합니다.
    >
...
1. 노트북의 작동이 중단 되거나 다시 시작 하려는 경우 커널을 다시 시작 하 고 처음부터 노트북 셀을 다시 실행할 수 있습니다. **커널 작업**  >  **다시 시작 커널을** 선택 합니다. 예를 들면 다음과 같습니다.

    :::image type="content" source="media/notebooks/sentinel-notebooks-restart-kernel.png" alt-text="Notebook 커널을 다시 시작합니다.":::

    > [!IMPORTANT]
    > 커널을 다시 시작 하면 모든 변수와 기타 상태를 초기화 합니다. 다시 시작한 후에 초기화 및 인증 셀을 다시 실행 해야 합니다.
    >

## <a name="run-code-in-your-notebook"></a>노트북에서 코드 실행

노트북에서:

- **Markdown** 셀에는 HTML, 정적 이미지 등의 텍스트가 있습니다.
- **코드** 셀에는 코드가 포함 되어 있습니다. 코드 셀을 선택한 후 셀 왼쪽의 **재생** 아이콘을 선택 하거나 **SHIFT + ENTER** 를 눌러 셀에서 코드를 실행 합니다.

> [!IMPORTANT]
> 항상 노트북 코드 셀을 순서 대로 실행 합니다. 셀을 건너뛰면 오류가 발생할 수 있습니다.
>

예를 들어 노트북에서 다음 코드 셀을 실행 합니다.
...
위에 표시 된 샘플 코드는 다음과 같은 출력을 생성 합니다.
...
노트북 코드 셀 내에 설정 된 변수는 셀 사이에서 유지 되므로 셀을 함께 연결할 수 있습니다. 예를 들어 다음 코드 셀은 이전 셀의 값을 사용 합니다 `y` .
...
## <a name="download-all-microsoft-sentinel-notebooks"></a>모든 Microsoft 센티널 노트북 다운로드

이 섹션에서는 Git를 사용 하 여 microsoft 센티널 노트북 내에서 Azure ML 작업 영역으로 직접 [microsoft 센티널 GitHub 리포지토리에서](https://github.com/Azure/Azure-Sentinel-Notebooks/)사용 가능한 모든 노트북을 다운로드 하는 방법을 설명 합니다.

Microsoft 센티널 노트북을 Azure ML 작업 영역에 저장 하면 쉽게 업데이트 된 상태로 유지할 수 있습니다.

1. Microsoft 센티널 노트북에서 빈 셀에 다음 코드를 입력 하 고 셀을 실행 합니다.

    ```python
    !git clone https://github.com/Azure/Azure-Sentinel-Notebooks.git azure-sentinel-nb
    ```

    azure ML 작업 영역에서 사용자 폴더의 **azure-센티널-nb** 디렉터리에 GitHub 리포지토리 콘텐츠의 복사본이 생성 됩니다.

1. 이 폴더에서 원하는 노트북을 작업 디렉터리로 복사 합니다.

1. GitHub의 최근 변경 내용을 사용 하 여 전자 필기장을 업데이트 하려면 다음을 실행 합니다.
...
일반적으로 노트북은 커널을 원활 하 게 만들거나 연결 하며 수동으로 변경할 필요가 없습니다. 오류가 발생 하거나 노트북이 실행 되 고 있지 않은 경우 커널 버전 및 상태를 확인 해야 할 수 있습니다.
...
### <a name="force-caching-for-user-accounts-and-credentials-between-notebook-runs"></a>노트북 실행 간 사용자 계정 및 자격 증명에 대 한 캐싱 강제 적용

기본적으로 사용자 계정 및 자격 증명은 동일한 세션에 대해서도 노트북 실행 간에 캐시 되지 않습니다.

**세션 기간 동안 캐싱을 강제 적용 하려면 다음을 수행 합니다**.

1. Azure CLI를 사용 하 여 인증 합니다. 빈 전자 필기장 셀에 다음 코드를 입력 하 고 실행 합니다.
...
1. 출력에서 9 자 토큰을 선택 하 고 복사 하 고 URL을 선택 `devicelogin` 하 여 표시 된 페이지로 이동 합니다. 

1. 토큰을 대화 상자에 붙여넣고 메시지가 표시 되 면 계속 로그인 합니다.

    로그인이 성공적으로 완료 되 면 다음과 같은 출력이 표시 됩니다.
...
1. Proceed to the cell with the following code and run it:
    ...
    누락 된 Python 종속성 ()을 나타내는 다음 메시지와 유사한 경고가 표시 됩니다 `pygobject` .
...
1. Microsoft 센티널 노트북 GitHub 리포지토리에 있는 [aml-compute-setup.sh](https://github.com/Azure/Azure-Sentinel-Notebooks/blob/master/HowTos/aml-compute-setup.sh) 스크립트를 사용 하 여 `pygobject` 계산 인스턴스의 모든 노트북 및 Anaconda 환경에를 자동으로 설치할 수 있습니다.

> [!TIP]
> 노트북에서 다음 코드를 실행 하 여이 경고를 해결할 수도 있습니다.
>
> ```python
...
Azure Synapse의 빅 데이터 분석과 노트북 환경을 통합 하세요. 자세한 내용은 [Azure Synapse와 노트북 통합 (공개 미리 보기)](notebooks-with-synapse.md)을 참조 하세요.

[Microsoft 센티널 GitHub 리포지토리](https://github.com/Azure/Azure-Sentinel-Notebooks) 에서 공유 되는 다른 노트북은 고유한 노트북을 개발할 때 사용할 수 있는 유용한 도구, 일러스트레이션 및 코드 샘플로 제공 됩니다.

여러분의 피드백을 환영합니다. 제안, 기능 요청, Notebook 기여, 버그 보고, 기존 Notebook에 대한 개선 및 추가 사항 등 무엇이든 괜찮습니다. [Microsoft 센티널 GitHub 리포지토리로](https://github.com/Azure/Azure-Sentinel) 이동 하 여 문제를 만들거나 참여를 업로드 합니다.

- [**Azure Log Analytics의 자격 증명 검색과**](https://www.youtube.com/watch?v=OWjXee8o04M) 단계별 **조사 프로세스 경고** 와 같은 일부 노트북 템플릿을 탐색 하 여 위협 검색 및 조사에서 노트북을 사용 하는 방법에 **대해 자세히 알아보세요** .

    Microsoft 센티널 > **노트북**  >  **템플릿** 탭에서 더 많은 노트북 템플릿을 찾습니다.

- [Microsoft 센티널 GitHub 리포지토리에서](https://github.com/Azure/Azure-Sentinel-Notebooks) **더 많은 노트북을 찾습니다** .

  - 디렉터리에는 [`Sample-Notebooks`](https://github.com/Azure/Azure-Sentinel-Notebooks/tree/master/Sample-Notebooks) 원하는 출력을 표시 하는 데 사용할 수 있는 데이터와 함께 저장 되는 샘플 노트북이 포함 되어 있습니다.

  - 디렉터리에는 [`HowTos`](https://github.com/Azure/Azure-Sentinel-Notebooks/tree/master/HowTos) 기본 Python 버전 설정, 노트북에서 Microsoft 센티널 책갈피 만들기 등의 개념을 설명 하는 노트북이 포함 되어 있습니다.

자세한 내용은 다음을 참조하세요.

- [첫 번째 Microsoft 센티널 노트북 만들기](https://techcommunity.microsoft.com/t5/microsoft-sentinel-blog/creating-your-first-microsoft-sentinel-notebook/ba-p/2977745) (블로그 시리즈)
- [자습서: Microsoft 센티널에서 Jupyter 노트북 및 MSTICPy 시작 하기](notebook-get-started.md)
- [자습서: Microsoft 센티널 노트북-시작](https://www.youtube.com/results?search_query=azazure+sentinel+notebooks) (비디오)
- [자습서: Azure ML studio를 종료 하지 않고 jupyter 노트북 편집 및 실행](https://www.youtube.com/watch?v=AAj-Fz0uCNk) (비디오)
- [웹 세미나: Microsoft 센티널 노트북 기본 사항](https://www.youtube.com/watch?v=rewdNeX6H94)
- [위협 요소를 사전에 헌팅하기](hunting.md)
- [헌팅 시 책갈피를 사용하여 흥미로운 정보 저장하기](bookmarks.md)
- [Jupyter, msticpy 및 Microsoft 센티널](https://msticpy.readthedocs.io/en/latest/getting_started/JupyterAndAzureSentinel.html)
