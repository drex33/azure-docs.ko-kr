...
| `early_stopping_patience` | 을(를) 통해 최소 Epoch 수 또는 유효성 검사 평가<br>실행이 중지되기 전에 기본 메트릭이 개선되지 않습니다.<br> 양의 정수여야 합니다. | 5 |
...
| `momentum` | 최적화 도구가 일 때의 추진력 `sgd` 값입니다. <br> [0, 1] 범위의 float여야 합니다. | 0.9 |
...
|`checkpoint_frequency`| 모델 검사점을 저장하는 빈도입니다. <br> 양의 정수여야 합니다. | 유효성 검사에 가장 적합한 기본 메트릭이 있는 Epoch의 검사점입니다.|
...
| `valid_resize_size` | 유효성 검사 데이터 세트를 자르기 전에 크기를 조정하는 이미지 크기입니다. <br> 양의 정수여야 합니다. <br> <br> *노트: <li> `seresnext` 은 임의의 크기를 받지 않습니다. <li>크기가 너무 크면 학습 실행이 CUDA OOM에 포함될 수 있습니다.*  | 256  |
| `valid_crop_size` | 유효성 검사 데이터 세트를 위해 신경망에 입력되는 이미지 자르기 크기입니다.  <br> 양의 정수여야 합니다. <br> <br> *노트: <li> `seresnext` 은 임의의 크기를 받지 않습니다. <li> *ViT 변형은* 및 가 동일해야 `valid_crop_size` `train_crop_size` 합니다. <li> 크기가 너무 크면 학습 실행이 CUDA OOM에 포함될 수 있습니다.* | 224 |
| `train_crop_size` | 학습 데이터 세트를 위해 신경망에 입력되는 이미지 자르기 크기입니다.  <br> 양의 정수여야 합니다. <br> <br> *노트: <li> `seresnext` 은 임의의 크기를 받지 않습니다. <li> *ViT 변형은* 및 가 동일해야 `valid_crop_size` `train_crop_size` 합니다. <li> 크기가 너무 크면 학습 실행이 CUDA OOM에 포함될 수 있습니다.* | 224 |
...
### <a name="model-specific-hyperparameters"></a>모델별 하이퍼parameters
...
| `multi_scale` | 이미지 크기를 +/- 50%까지 변경하여 다중 크기 이미지를 사용하도록 설정 <br> 0 또는 1이어야 합니다. <br> <br> *참고: 충분한 GPU 메모리가 없으면 학습 실행이 CUDA OOM에 포함될 수 있습니다.* | 0 |
| `box_score_thresh` | 유추 중에는 보다 큰 점수의 제안만 `box_score_thresh` 반환합니다. 점수는 개체성 점수와 분류 확률을 곱한 것입니다. <br> [0, 1] 범위의 float여야 합니다. | 0.1 |
